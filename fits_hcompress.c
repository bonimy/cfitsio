...
The following modifications have been made to the original code:
  ...
  - changed the first parameter in encode (and in lower level routines from a file
stream to a char array
...
static void qtree_onebit64(LONGLONG a[], int n, int nx, int ny, unsigned char b[],
                           int bit);
...
static void write_bdirect64(char* outfile, LONGLONG a[], int n, int nqx, int nqy,
                            unsigned char scratch[], int bit);
...
/* ---------------------------------------------------------------------- */
int fits_hcompress(int* a, int ny, int nx, int scale, char* output, long* nbytes,
                   int* status) {
    /*
   ...
   compression scale = 0 does lossless compression output - pre-allocated array to hold
   the output compressed stream of bytes nbyts  - input value = size of the output
   buffer; returned value = size of the compressed byte stream, in bytes
...
/* ---------------------------------------------------------------------- */
int fits_hcompress64(LONGLONG* a, int ny, int nx, int scale, char* output, long* nbytes,
                     int* status) {
    /*
       compress the input image using the H-compress algorithm
     ...
     scale - quantization scale factor. Larger values results in more (lossy)
   compression scale = 0 does lossless compression output - pre-allocated array to hold
   the output compressed stream of bytes nbyts  - size of the compressed byte stream, in
   bytes
...
static int encode(char* outfile, long* nlength, int a[], int nx, int ny, int scale) {
    ...
    /*
      ...
      int a[];								 input H-transform array
      (nx,ny)
      int nx,ny;								 size of H-transform
      array
      int scale;								 scale factor for
      digitization
    */
...
static int encode64(char* outfile, long* nlength, LONGLONG a[], int nx, int ny,
    ...
    /*
      long * nlength    returned length (in bytes) of the encoded array)
      LONGLONG a[];								 input H-transform array
      (nx,ny)
      int nx,ny;								 size of H-transform
      array
      int scale;								 scale factor for
...
static int doencode(char* outfile, int a[], int nx, int ny,
    ...
    int a[];							 Array of values to
    encode
    int nx,ny;							 Array dimensions
    [nx][ny] unsigned char nbitplanes[3];		 Number of bit planes in
    quadrants
    */
    ...
    if (!stat) stat = qtree_encode(outfile, &a[ny2], ny, nx2, ny / 2, nbitplanes[1]);
...
static int doencode64(char* outfile, LONGLONG a[], int nx, int ny,
    ...
    */
    ...
    if (!stat) stat = qtree_encode64(outfile, &a[ny2], ny, nx2, ny / 2, nbitplanes[1]);
 ...
 *					quadtree coding for each bit plane.  Assumes array
 *is positive.
 *
...
static int qtree_encode(char* outfile, int a[], int n, int nqx, int nqy,
    ...
    /*
    int a[];
    int n;								 physical dimension of row in
    a int nqx;							 length of row int nqy;
    length of column (<=n)
    int nbitplanes;						 number of bit planes to
    output
    */
...
static void qtree_onebit64(LONGLONG a[], int n, int nx, int ny, unsigned char b[],
    ...
    for (i = 0; i < nx - 1; i += 2) {
        ...
        s10 = s00 + n; /* s10 is index of a[i+1,j]	*/
        for (j = 0; j < ny - 1; j += 2) {
            b[k] = (unsigned char)(((a[s10 + 1] & b0) | ((a[s10] << 1) & b1) |
                                    ((a[s00 + 1] << 2) & b2) | ((a[s00] << 3) & b3)) >>
                                   bit);
            k += 1;
            s00 += 2;
        ...
        if (j < ny) {
             ...
             * s00+1,s10+1 are off edge
             */
            b[k] = (unsigned char)((((a[s10] << 1) & b1) | ((a[s00] << 3) & b3)) >>
                                   bit);
            k += 1;
        }
    }
    if (i < nx) {
        ...
        s00 = n * i;
        for (j = 0; j < ny - 1; j += 2) {
            b[k] = (unsigned char)((((a[s00 + 1] << 2) & b2) | ((a[s00] << 3) & b3)) >>
                                   bit);
            k += 1;
            s00 += 2;
        }
