...
/******************************************************/
{
    ...
    exposure = 1500;
    if (fits_update_key(fptr, TLONG, "EXPOSURE", &exposure, "Total Exposure Time",
                        &status))
        printerror(status);
...
/*******************************************************************/
{
    ...
    if (fits_create_tbl(fptr, ASCII_TBL, nrows, tfields, ttype, tform, tunit, extname,
                        &status))
        printerror(status);
...
/*******************************************************************/
{
    ...
    if (fits_create_tbl(fptr, BINARY_TBL, nrows, tfields, ttype, tform, tunit, extname,
                        &status))
        printerror(status);
...
void copyhdu(void) {
    ...
    if (fits_open_file(&infptr, infilename, READONLY, &status)) printerror(status);
    ...
    if (fits_copy_hdu(infptr, outfptr, morekeys, &status)) printerror(status);

    /* move to the 3rd HDU in the input file */
    if (fits_movabs_hdu(infptr, 3, &hdutype, &status)) printerror(status);
...
/*********************************************************************/
{
    ...
    if (fits_movabs_hdu(outfptr, 2, &hdutype, &status)) printerror(status);

    /* create new extension in the output file */
    if (fits_create_hdu(outfptr, &status)) printerror(status);

    /* get number of keywords */
    if (fits_get_hdrpos(infptr, &nkeys, &keypos, &status)) printerror(status);
    ...
    felem = 1;
    nullval = -99.;
    if (fits_read_col(infptr, TFLOAT, colnum, frow, felem, naxes[1], &nullval, density,
                      &anynulls, &status))
        printerror(status);
...
/************************************************************/
{
    ...
    for (hdunum = 2; hdunum <= 3; hdunum++) /*read ASCII, then binary table */
    {
        ...
        /*  read the columns */
        fits_read_col(fptr, TSTRING, 1, frow, felem, nelem, strnull, name, &anynull,
                      &status);
        fits_read_col(fptr, TLONG, 2, frow, felem, nelem, &longnull, dia, &anynull,
                      &status);
        fits_read_col(fptr, TFLOAT, 3, frow, felem, nelem, &floatnull, den, &anynull,
                      &status);
