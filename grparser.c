...
NGP_RAW_LINE ngp_curline = {NULL, NULL,          NULL, NGP_TTYPE_UNKNOWN,
                            NULL, NGP_FORMAT_OK, 0};
NGP_RAW_LINE ngp_prevline = {NULL, NULL,          NULL, NGP_TTYPE_UNKNOWN,
                             NULL, NGP_FORMAT_OK, 0};
...
NGP_TKDEF ngp_tkdef[] = /* tokens recognized by parser */
        {{"\\INCLUDE", NGP_TOKEN_INCLUDE}, {"\\GROUP", NGP_TOKEN_GROUP},
         {"\\END", NGP_TOKEN_END},         {"XTENSION", NGP_TOKEN_XTENSION},
         {"SIMPLE", NGP_TOKEN_SIMPLE},     {NULL, NGP_TOKEN_UNKNOWN}};
...
int ngp_get_extver(char* extname, int* version) {
    ...
    if (NULL == ngp_extver_tab) {
        ...
        p = (NGP_EXTVER_TAB*)ngp_realloc(
                ngp_extver_tab, (ngp_extver_tab_size + 1) * sizeof(NGP_EXTVER_TAB));
    }
    ...
    if ('\'' == *p) /* we have found string within quotes */
    {
        ...
        for (;;) /* analyze it */
        {
            ...
            if ('\'' == *p) /* we have found doublequote */
            {
                ...
                if (('\t' == p[1]) ||
                    (' ' == p[1])) /* duoblequote was string terminator */
                {
                    *s = 0;
                    p++;
                    break;
            ...
            *(s++) =
                    *(p++); /* compact string in place, necess. by "" -> " conversion */
        }
    } else /* regular token */
    {
...
int ngp_include_file(char* fname) /* try to open include file */
{
    ...
    if (NULL ==
        (ngp_fp[ngp_inclevel] = fopen(fname, "r"))) { /* if simple open failed .. */
        envar = getenv(
                "CFITSIO_INCLUDE_FILES"); /* scan env. variable, and retry to open */

        if (NULL != envar) /* is env. variable defined ? */
        {
            strncpy(envfiles, envar, NGP_MAX_ENVFILES - 1);
            envfiles[NGP_MAX_ENVFILES - 1] =
                    0; /* copy search path to local variable, env. is fragile */

            for (p2 = ffstrtok(envfiles, ":", &saveptr); NULL != p2;
                 p2 = ffstrtok(NULL, ":", &saveptr)) {
        ...
        if (NULL ==
            ngp_fp[ngp_inclevel]) /* finally try to open relative to top level */
        {
#ifdef MSDOS
            if ('\\' == fname[0])
                return (NGP_ERR_FOPEN); /* abs. pathname for MSDOS, does not support
                                           C:\\PATH */
#else
...
int ngp_read_line(int ignore_blank_lines) {
    ...
    for (;;) {
        ...
        switch (ngp_curline.line[0]) {
            case 0:
                if (0 == ignore_blank_lines) break; /* ignore empty lines if told so */
            case '#':
                continue; /* ignore comment lines */
        }

        r = ngp_extract_tokens(
                &ngp_curline); /* analyse line, extract tokens and comment */
        ...
        ngp_keyidx =
                ngp_tkdef[k].code; /* save this index, grammar parser will need this */

        if (NGP_TOKEN_INCLUDE ==
        ...
        ngp_linkey.type = NGP_TTYPE_UNKNOWN; /* now, get the keyword type, it's a long
                                                story ... */
        ...
        if (NULL != ngp_curline.value) /* if no value given signal it */
        {
            ...
            {
                if ((!fits_strcasecmp("T", ngp_curline.value)) ||
                    (!fits_strcasecmp("F", ngp_curline.value))) {
                    ngp_linkey.type = NGP_TTYPE_BOOL;
                    ngp_linkey.value.b =
                            (fits_strcasecmp("T", ngp_curline.value) ? 0 : 1);
                }
            }
            if (NGP_TTYPE_UNKNOWN == ngp_linkey.type) /* complex type test */
            {
                if (2 == sscanf(ngp_curline.value, "(%lg,%lg)%n",
                                &(ngp_linkey.value.c.re), &(ngp_linkey.value.c.im),
                                &nc)) {
                    if ((' ' == ngp_curline.value[nc]) ||
                        ('\t' == ngp_curline.value[nc]) ||
                        ('\n' == ngp_curline.value[nc]) ||
                        (0 == ngp_curline.value[nc])) {
            ...
            {
                if (strchr(ngp_curline.value, '.') &&
                    (1 ==
                     sscanf(ngp_curline.value, "%lg%n", &(ngp_linkey.value.d), &nc))) {
                    if ('D' == ngp_curline.value[nc]) {
                        /* test if template used a 'D' rather than an 'E' as the
                         * exponent character (added by WDP in 12/2010) */
                        savec = nc;
                        ...
                        if ((' ' == ngp_curline.value[nc]) ||
                            ...
                            ngp_curline.value[savec] =
                                    'D'; /* restore the original D character */
                        }
                    } else {
                        if ((' ' == ngp_curline.value[nc]) ||
                            ('\t' == ngp_curline.value[nc]) ||
            ...
            {
                if (1 ==
                    sscanf(ngp_curline.value, "%d%n", &(ngp_linkey.value.i), &nc)) {
                    if ((' ' == ngp_curline.value[nc]) ||
                        ('\t' == ngp_curline.value[nc]) ||
                        ('\n' == ngp_curline.value[nc]) ||
                        (0 == ngp_curline.value[nc])) {
        ...
        if (NULL != ngp_curline.comment) {
            strncpy(ngp_linkey.comment, ngp_curline.comment,
                    NGP_MAX_COMMENT); /* store comment */
            ngp_linkey.comment[NGP_MAX_COMMENT - 1] = 0;
        } else {
            ngp_linkey.comment[0] = 0;
        }

        strncpy(ngp_linkey.name, ngp_curline.name,
                NGP_MAX_NAME); /* and keyword's name */
        ngp_linkey.name[NGP_MAX_NAME - 1] = 0;

        if (strlen(ngp_linkey.name) >
            FLEN_KEYWORD) /* WDP: 20-Jun-2002:  mod to support HIERARCH */
        {
...
int ngp_keyword_is_write(NGP_TOKEN* ngp_tok) {
    ...
    static char* nmni[] = {"SIMPLE", "XTENSION", "BITPIX",  "NAXIS",
    ...
    for (i = l + 1; i < 8; i++) {
        if (spc) {
            if (' ' != ngp_tok->name[i]) return (NGP_OK);
        } else {
        ...
        if ((NGP_REALLY_ALL & mode) || (NGP_OK == r)) {
            switch (ngph->tok[i].type) {
                case NGP_TTYPE_BOOL:
                    ...
                    fits_write_key(ffp, TLOGICAL, ngph->tok[i].name, &ib,
                                   ngph->tok[i].comment, &r);
                    break;
                case NGP_TTYPE_STRING:
                    fits_write_key_longstr(ffp, ngph->tok[i].name, ngph->tok[i].value.s,
                                           ngph->tok[i].comment, &r);
                    break;
                case NGP_TTYPE_INT:
                    l = ngph->tok[i].value.i; /* bugfix - 22-Jan-99, BO - nonalignment
                                                 of OSF/Alpha */
                    fits_write_key(ffp, TLONG, ngph->tok[i].name, &l,
                                   ngph->tok[i].comment, &r);
                    break;
                case NGP_TTYPE_REAL:
                    fits_write_key(ffp, TDOUBLE, ngph->tok[i].name,
                                   &(ngph->tok[i].value.d), ngph->tok[i].comment, &r);
                    break;
                case NGP_TTYPE_COMPLEX:
                    fits_write_key(ffp, TDBLCOMPLEX, ngph->tok[i].name,
                                   &(ngph->tok[i].value.c), ngph->tok[i].comment, &r);
                    break;
                case NGP_TTYPE_NULL:
                    fits_write_key_null(ffp, ngph->tok[i].name, ngph->tok[i].comment,
                                        &r);
                    break;
                case NGP_TTYPE_RAW:
                    ...
                    snprintf(buf, 200, "%-8.8s%s", ngph->tok[i].name,
                             ngph->tok[i].comment);
                    fits_write_record(ffp, buf, &r);
                    break;
            }
        } else if (NGP_BAD_ARG == r) /* enhancement 10 dec 2003, James Peachey: template
                                        comments replace defaults */
        {
            r = NGP_OK; /* update comments of special keywords like TFORM */
            if (ngph->tok[i].comment &&
                *ngph->tok[i].comment) /* do not update with a blank comment */
            {
...
int ngp_hdu_insert_token(NGP_HDU* ngph, NGP_TOKEN* newtok) {
    ...
    if (0 == ngph->tokcnt)
        ...
        tkp = (NGP_TOKEN*)ngp_realloc(ngph->tok,
                                      (ngph->tokcnt + 1) * sizeof(NGP_TOKEN));
    ...
    if (NGP_TTYPE_STRING == newtok->type) {
        if (NULL != newtok->value.s) {
            ngph->tok[ngph->tokcnt].value.s =
                    (char*)ngp_alloc(1 + strlen(newtok->value.s));
            ...
            if (1 == sscanf(ngph->tok[i].name, "TFORM%d%c", &ngph_i, &ngph_ctmp)) {
                if ((NGP_TTYPE_STRING == ngph->tok[i].type) && (ngph_i == (j + 1))) {
                    my_tform = ngph->tok[i].value.s;
                       ...
                       sscanf(ngph->tok[i].name, "TTYPE%d%c", &ngph_i, &ngph_ctmp)) {
                if ((NGP_TTYPE_STRING == ngph->tok[i].type) && (ngph_i == (j + 1))) {
...
int ngp_read_xtension(fitsfile* ff, int parent_hn, int simple_mode) {
    ...
    switch (NGP_XTENSION_SIMPLE & simple_mode) {
        case 0:
            if (NGP_TOKEN_XTENSION != ngp_keyidx) return (NGP_TOKEN_NOT_EXPECT);
            break;
        default:
            if (NGP_TOKEN_SIMPLE != ngp_keyidx) return (NGP_TOKEN_NOT_EXPECT);
            break;
    }
            ...
            default:
                ...
                if ((l >= 2) && (l <= 6)) {
                    if ('#' == ngp_linkey.name[l - 1]) {
                        ...
                        if (((l - 1) == (int)strlen(incrementor_name)) &&
                            (0 == memcmp(incrementor_name, ngp_linkey.name, l - 1))) {
                            incrementor_index++;
                        }
                        snprintf(ngp_linkey.name + l - 1, NGP_MAX_NAME - l + 1, "%d",
                                 incrementor_index);
                    }
                }
    ...
    if (NGP_OK == r) { /* we should scan keywords, and calculate HDU's */
        ...
        for (i = 0; i < ngph.tokcnt; i++) {
            ...
            } else if (!strcmp("EXTNAME", ngph.tok[i].name)) /* assign EXTNAME, I hope
        ...
        switch (ngph_node_type) {
            case NGP_NODE_IMAGE:
                if (NGP_XTENSION_FIRST == ((NGP_XTENSION_FIRST | NGP_XTENSION_SIMPLE) &
                                           simple_mode)) { /* if caller signals that
                                                              this is 1st HDU in file */
            ...
            case NGP_NODE_BTABLE:
                ...
                fits_create_tbl(
                        ff,
                        ((NGP_NODE_ATABLE == ngph_node_type) ? ASCII_TBL : BINARY_TBL),
    ...
    if ((NGP_OK == r) && (NULL != ngph_extname)) {
        r = ngp_get_extver(ngph_extname,
                           &my_version); /* write correct ext version number */
        lv = my_version; /* bugfix - 22-Jan-99, BO - nonalignment of OSF/Alpha */
        fits_write_key(ff, TLONG, "EXTVER", &lv, "auto assigned by template parser",
                       &r);
    }
        ...
        switch (ngp_keyidx) {
            ...
            case NGP_TOKEN_GROUP:
                if (NGP_TTYPE_STRING == ngp_linkey.type) {
                    ...
                    snprintf(grnm, NGP_MAX_STRING, "DEFAULT_GROUP_%d",
                             master_grp_idx++);
                }
...
int fits_execute_template(fitsfile* ff, char* ngp_template, int* status) {
    ...
    for (i = strlen(ngp_template) - 1; i >= 0;
         i--) /* strlen is > 0, otherwise fopen failed */
    {
#ifdef MSDOS
    ...
    for (;;) {
        ...
        switch (ngp_keyidx) {
            ...
            case NGP_TOKEN_XTENSION:
                if (NGP_OK != (r = ngp_unread_line())) break;
                r = ngp_read_xtension(ff, 0,
                                      (first_extension ? NGP_XTENSION_FIRST : 0));
                first_extension = 0;
                break;
    ...
    /* all top level HDUs up to faulty one are left intact in case of i/o error. It is
       up to the caller to call fits_close_file or fits_delete_file when this function
       returns error. */
