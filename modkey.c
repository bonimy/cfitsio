...
*/
{
    ...
    if (datatype == TSTRING) {
        ...
        ffukyg(fptr, keyname, (double)*(unsigned int*)value, 0, comm, status);
    } else if (datatype == TLOGICAL) {
        ffukyl(fptr, keyname, *(int*)value, comm, status);
    } else if (datatype == TULONG) {
        ffukyg(fptr, keyname, (double)*(unsigned long*)value, 0, comm, status);
           ...
           int* status)         /* IO - error status      */
{
    ...
    if (ffgcrd(fptr, keyname, tcard, status) > 0) return (*status);

    ffmkey(fptr, card, status);

    /* calc position of keyword in header */
    keypos = (int)((((fptr->Fptr)->nextkey) -
           ...
           int* status)         /* IO - error status         */
{
    ...
    if (ffgkey(fptr, oldname, value, comm, status) > 0) return (*status);
           ...
           int* status)         /* IO - error status      */
{
    ...
    if (ffgkey(fptr, keyname, value, oldcomm, status) > 0) return (*status);
...
*/
{
    ...
    if (ffdkey(fptr, keyname, status) > 0) return (*status); /* keyword doesn't exist */
...
*/
{
    ...
    if (((fptr->Fptr)->datastart - (fptr->Fptr)->headend) ==
        80) /* only room for END card */
    {
        nblocks = 1;
    ...
    if (!fits_strncasecmp("COMMENT ", buff2, 8) ||
        !fits_strncasecmp("HISTORY ", buff2, 8) ||
        !fits_strncasecmp("        ", buff2, 8) ||
        !fits_strncasecmp("CONTINUE", buff2, 8))
        keylength = 8;
...
*/
{
    ...
    if (ffgkey(fptr, keyname, valstring, comm, status) > 0) /* read keyword */
    {
        snprintf(message, FLEN_ERRMSG,
                 "Could not find the %s keyword to delete (ffdkey)", keyname);
        ffpmsg(message);
        return (*status);
    }
...
*/
{
    ...
    {
        snprintf(message, FLEN_ERRMSG,
                 "Could not find the %s keyword to delete (ffdkey)", string);
        ffpmsg(message);
        return (*status);
    }
...
*/
{
    ...
    if (keypos < 1 ||
        keypos > (fptr->Fptr)->headend -
                         (fptr->Fptr)->headstart[(fptr->Fptr)->curhdu] / 80)
        return (*status = KEY_OUT_BOUNDS);

    (fptr->Fptr)->nextkey =
            (fptr->Fptr)->headstart[(fptr->Fptr)->curhdu] + (keypos - 1) * 80;

    nshift = (int)(((fptr->Fptr)->headend - (fptr->Fptr)->nextkey) /
                   80); /* no. keywords to shift */

    if (nshift <= 0) {
        snprintf(message, FLEN_ERRMSG,
                 "Cannot delete keyword number %d.  It does not exist.", keypos);
        ffpmsg(message);
        return (*status = KEY_OUT_BOUNDS);
    }
