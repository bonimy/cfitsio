...
*/
{
    ...
    */
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_write_compressed_pixels(fptr, TLONG, firstelem, nelem, 0, array,
                                     &nullvalue, status);
        return (*status);
    }
...
*/
{
    ...
    */
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_write_compressed_pixels(fptr, TLONG, firstelem, nelem, 1, array,
                                     &nullvalue, status);
        return (*status);
    }
...
*/
{
    ...
    if (naxis < 1 || naxis > 7) return (*status = BAD_DIMEN);
...
*/
{
    ...
    /*---------------------------------------------------*/
    if (ffgcprll(fptr, colnum, firstrow, firstelem, nelem, 1, &scale, &zero, tform,
    ...
    */
    if (scale == 1. && zero == 0. && MACHINE == NATIVE && tcode == TLONG &&
        LONGSIZE == 32) {
        writeraw = 1;
    ...
    while (remain) {
        ...
        */
        ...
        switch (tcode) {
            case (TLONG):
                if (writeraw) {
                    ...
                    ffi4fi4(&array[next], ntodo, scale, zero, (INT32BIT*)buffer,
                            status);
            ...
            case (TLONGLONG):
                ...
                ffi4fi8(&array[next], ntodo, scale, zero, (LONGLONG*)buffer, status);
                ffpi8b(fptr, ntodo, incre, (long*)buffer, status);
                break;

            case (TBYTE):

                ffi4fi1(&array[next], ntodo, scale, zero, (unsigned char*)buffer,
                        status);
            ...
            case (TSHORT):
                ...
                ffi4fi2(&array[next], ntodo, scale, zero, (short*)buffer, status);
                ffpi2b(fptr, ntodo, incre, (short*)buffer, status);
                break;

            case (TFLOAT):

                ffi4fr4(&array[next], ntodo, scale, zero, (float*)buffer, status);
                ffpr4b(fptr, ntodo, incre, (float*)buffer, status);
                break;

            case (TDOUBLE):
                ffi4fr8(&array[next], ntodo, scale, zero, (double*)buffer, status);
            ...
            case (TSTRING): /* numerical column in an ASCII table */
                ...
                {
                    ffi4fstr(&array[next], ntodo, scale, zero, cform, twidth,
                             (char*)buffer, status);

                    if (incre == twidth) /* contiguous bytes */
                        ffpbyt(fptr, ntodo * twidth, buffer, status);
                    else
                        ffpbytoff(fptr, twidth, ntodo, incre - twidth, buffer, status);
        ...
        {
            snprintf(message, FLEN_ERRMSG,
                     "Error writing elements %.0f thru %.0f of input data array "
                     "(ffpclj).",
...
*/
{
    ...
    if (scale == 1. && zero == 0.) {
        for (ii = 0; ii < ntodo; ii++) output[ii] = (INT32BIT)input[ii];
    } else {
        for (ii = 0; ii < ntodo; ii++) {
            dvalue = (input[ii] - zero) / scale;
...
*/
{
    ...
    if (scale == 1. && zero == 9223372036854775808.) {
        ...
        for (ii = 0; ii < ntodo; ii++) output[ii] = input[ii];
    } else {
        for (ii = 0; ii < ntodo; ii++) {
            dvalue = (input[ii] - zero) / scale;
...
*/
{
    ...
    if (scale == 1. && zero == 0.) {
        for (ii = 0; ii < ntodo; ii++) output[ii] = (float)input[ii];
    } else {
        for (ii = 0; ii < ntodo; ii++) output[ii] = (float)((input[ii] - zero) / scale);
    }
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (scale == 1. && zero == 0.) {
        for (ii = 0; ii < ntodo; ii++) output[ii] = (double)input[ii];
    } else {
        for (ii = 0; ii < ntodo; ii++) output[ii] = (input[ii] - zero) / scale;
    }
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    /*
       ...
       format in a temporary buffer that has been allocated for this purpose.
    */
    if (scale == 1. && zero == 0. && MACHINE == NATIVE && tcode == TLONGLONG) {
        writeraw = 1;
        ...
        switch (tcode) {
            case (TLONGLONG):
                if (writeraw) {
                    ...
                    /* convert the raw data before writing to FITS file */
                    ffi8fi8(&array[next], ntodo, scale, zero, (LONGLONG*)buffer,
                            status);
                    ffpi8b(fptr, ntodo, incre, (long*)buffer, status);
                }
            ...
            case (TLONG):
                ...
                ffi8fi4(&array[next], ntodo, scale, zero, (INT32BIT*)buffer, status);
                ffpi4b(fptr, ntodo, incre, (INT32BIT*)buffer, status);
                break;

            case (TBYTE):

                ffi8fi1(&array[next], ntodo, scale, zero, (unsigned char*)buffer,
                        status);
                ffpi1b(fptr, ntodo, incre, (unsigned char*)buffer, status);
                break;
            ...
            case (TSHORT):
                ...
                ffi8fi2(&array[next], ntodo, scale, zero, (short*)buffer, status);
                ffpi2b(fptr, ntodo, incre, (short*)buffer, status);
                break;

            case (TFLOAT):

                ffi8fr4(&array[next], ntodo, scale, zero, (float*)buffer, status);
                ffpr4b(fptr, ntodo, incre, (float*)buffer, status);
                break;

            case (TDOUBLE):
                ffi8fr8(&array[next], ntodo, scale, zero, (double*)buffer, status);
                ffpr8b(fptr, ntodo, incre, (double*)buffer, status);
                break;
            ...
            case (TSTRING): /* numerical column in an ASCII table */
                ...
                if (cform[1] != 's') /*  "%s" format is a string */
                {
                    ffi8fstr(&array[next], ntodo, scale, zero, cform, twidth,
                             (char*)buffer, status);
