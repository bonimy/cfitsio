...
*/
{
    ...
    */
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_write_compressed_pixels(fptr, TULONG, firstelem, nelem, 0, array,
                                     &nullvalue, status);
        return (*status);
    }
...
*/
{
    ...
    */
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_write_compressed_pixels(fptr, TULONG, firstelem, nelem, 1, array,
                                     &nullvalue, status);
        return (*status);
    }
...
*/
{
    ...
    if (ffgcprll(fptr, colnum, firstrow, firstelem, nelem, 1, &scale, &zero, tform,
                 &twidth, &tcode, &maxelem, &startpos, &elemnum, &incre, &repeat,
        ...
        */
        ...
        switch (tcode) {
            case (TLONG):
                ...
                ffu4fi4(&array[next], ntodo, scale, zero, (INT32BIT*)buffer, status);
                ffpi4b(fptr, ntodo, incre, (INT32BIT*)buffer, status);
                break;

            case (TLONGLONG):

                ffu4fi8(&array[next], ntodo, scale, zero, (LONGLONG*)buffer, status);
                ffpi8b(fptr, ntodo, incre, (long*)buffer, status);
                break;

            case (TBYTE):

                ffu4fi1(&array[next], ntodo, scale, zero, (unsigned char*)buffer,
                        status);
            ...
            case (TSHORT):
                ...
                ffu4fi2(&array[next], ntodo, scale, zero, (short*)buffer, status);
                ffpi2b(fptr, ntodo, incre, (short*)buffer, status);
                break;

            case (TFLOAT):

                ffu4fr4(&array[next], ntodo, scale, zero, (float*)buffer, status);
                ffpr4b(fptr, ntodo, incre, (float*)buffer, status);
                break;

            case (TDOUBLE):
                ffu4fr8(&array[next], ntodo, scale, zero, (double*)buffer, status);
            ...
            case (TSTRING): /* numerical column in an ASCII table */
                ...
                {
                    ffu4fstr(&array[next], ntodo, scale, zero, cform, twidth,
                             (char*)buffer, status);
...
*/
{
    ...
    if (scale == 1. && zero == 2147483648. && sizeof(long) == 4) {
        ...
        for (ii = 0; ii < ntodo; ii++) output[ii] = (*(long*)&input[ii]) ^ 0x80000000;
        ...
        */
        ...
        switch (tcode) {
            case (TLONGLONG):
                ...
                ffu8fi8(&array[next], ntodo, scale, zero, (LONGLONG*)buffer, status);
                ffpi8b(fptr, ntodo, incre, (long*)buffer, status);
                break;

            case (TLONG):

                ffu8fi4(&array[next], ntodo, scale, zero, (INT32BIT*)buffer, status);
                ffpi4b(fptr, ntodo, incre, (INT32BIT*)buffer, status);
                break;

            case (TBYTE):

                ffu8fi1(&array[next], ntodo, scale, zero, (unsigned char*)buffer,
                        status);
                ffpi1b(fptr, ntodo, incre, (unsigned char*)buffer, status);
                break;
            ...
            case (TSHORT):
                ...
                ffu8fi2(&array[next], ntodo, scale, zero, (short*)buffer, status);
                ffpi2b(fptr, ntodo, incre, (short*)buffer, status);
                break;

            case (TFLOAT):

                ffu8fr4(&array[next], ntodo, scale, zero, (float*)buffer, status);
                ffpr4b(fptr, ntodo, incre, (float*)buffer, status);
                break;

            case (TDOUBLE):
                ffu8fr8(&array[next], ntodo, scale, zero, (double*)buffer, status);
                ffpr8b(fptr, ntodo, incre, (double*)buffer, status);
                break;
            ...
            case (TSTRING): /* numerical column in an ASCII table */
                ...
                if (cform[1] != 's') /*  "%s" format is a string */
                {
                    ffu8fstr(&array[next], ntodo, scale, zero, cform, twidth,
                             (char*)buffer, status);
    ...
    for (ii = 0; ii < nelem; ii++) {
        if (array[ii] != nulvalue) /* is this a good pixel? */
        {
            ...
            if (ngood) /* write previous string of good pixels */
            {
                ...
                if (tcode > 0) { /* variable length arrays have already been written */
                    if (ffpclujj(fptr, colnum, fstrow, fstelm, ngood,
                                 &array[ii - ngood], status) > 0) {
                        if (*status == NUM_OVERFLOW) {
                            overflow = 1;
                            *status = 0;
                        } else {
