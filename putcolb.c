...
*/
{
    ...
    */
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_write_compressed_pixels(fptr, TBYTE, firstelem, nelem, 0, array,
                                     &nullvalue, status);
        return (*status);
    }
...
*/
{
    ...
    */
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_write_compressed_pixels(fptr, TBYTE, firstelem, nelem, 1, array,
                                     &nullvalue, status);
        return (*status);
    }
        ...
        switch (tcode) {
            case (TBYTE):
                if (writeraw) {
                    ...
                    ffi1fi1(&array[next], ntodo, scale, zero, (unsigned char*)buffer,
                            status);
            ...
            case (TLONGLONG):
                ...
                ffi1fi8(&array[next], ntodo, scale, zero, (LONGLONG*)buffer, status);
                ffpi8b(fptr, ntodo, incre, (long*)buffer, status);
                break;

            case (TSHORT):

                ffi1fi2(&array[next], ntodo, scale, zero, (short*)buffer, status);
                ffpi2b(fptr, ntodo, incre, (short*)buffer, status);
                break;

            case (TLONG):

                ffi1fi4(&array[next], ntodo, scale, zero, (INT32BIT*)buffer, status);
                ffpi4b(fptr, ntodo, incre, (INT32BIT*)buffer, status);
                break;

            case (TFLOAT):

                ffi1fr4(&array[next], ntodo, scale, zero, (float*)buffer, status);
                ffpr4b(fptr, ntodo, incre, (float*)buffer, status);
                break;

            case (TDOUBLE):
                ffi1fr8(&array[next], ntodo, scale, zero, (double*)buffer, status);
            ...
            case (TSTRING): /* numerical column in an ASCII table */
                ...
                {
                    ffi1fstr(&array[next], ntodo, scale, zero, cform, twidth,
                             (char*)buffer, status);
