...
*/
{
    ...
    ffgcls(fptr, colnum, firstrow, firstelem, nelem, 1, nulval, array, cdummy, anynul,
           status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgcls(fptr, colnum, firstrow, firstelem, nelem, 2, dummy, array, nularray, anynul,
           status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (tcode == TSTRING) {
        ...
        ffgcls2(fptr, colnum, firstrow, firstelem, nelem, nultyp, nulval, array,
        ...
        ffgcll(fptr, colnum, firstrow, firstelem, nelem, nultyp, *nulval, carray,
               nularray, anynul, status);
        ...
        ffgcle(fptr, colnum, firstrow, (firstelem - 1) * 2 + 1, nelem * 2, 1, 1,
               FLOATNULLVALUE, earray, nularray, anynul, status);
        ...
        ffgcld(fptr, colnum, firstrow, (firstelem - 1) * 2 + 1, nelem * 2, 1, 1,
               DOUBLENULLVALUE, darray, nularray, anynul, status);
        ...
        if (*status <= 0) {
            ...
            if (!cform[0]) strcpy(cform, "%23.15E");
        ...
        if (ffgcfjj(fptr, colnum, firstrow, firstelem, nelem, llarray, flgarray, anynul,
                    status) > 0) {
            free(flgarray);
            free(llarray);
            return (*status);
        ...
        if (ffgcfujj(fptr, colnum, firstrow, firstelem, nelem, ullarray, flgarray,
                     anynul, status) > 0) {
            free(flgarray);
            free(ullarray);
            return (*status);
        ...
        if (ffgcld(fptr, colnum, firstrow, firstelem, nelem, 1, nultyp, DOUBLENULLVALUE,
                   darray, nularray, anynul, status) > 0) {
            free(darray);
            return (*status);
        }
        ...
        if (ffgkyd(fptr, keyname, &tscale, NULL, &tstatus) == 0) {
            if (tscale != 1.0) scaled = 1; /* yes, this is a scaled column */
        }
...
*/
{
    ...
    *width = 0;
    ...
    if (ffgkys(fptr, keyname, dispfmt, NULL, &tstatus) == 0) {
        ...
        if (*cptr == 'A' || *cptr == 'a' || *cptr == 'I' || *cptr == 'i' ||
            *cptr == 'O' || *cptr == 'o' || *cptr == 'Z' || *cptr == 'z' ||
            *cptr == 'F' || *cptr == 'f' || *cptr == 'E' || *cptr == 'e' ||
            *cptr == 'D' || *cptr == 'd' || *cptr == 'G' || *cptr == 'g') {
            while (!isdigit((int)*cptr) && *cptr != '\0') /* find 1st digit */
                cptr++;

            *width = atoi(cptr);
            if (tcode >= TCOMPLEX) *width = (2 * (*width)) + 3;
        }
    }
            ...
            if (hdutype == ASCII_TBL) {
                ...
                *width = atoi(cptr);
                ...
                else if (tcode == TSTRING) /* 'A' */
                {
                    ...
                    if (ffgtcl(fptr, colnum, &typecode, &repeat, &rwidth, &gstatus) ==
                                0 &&
                        rwidth >= 1 && rwidth < repeat) {
                        *width = rwidth;

                    } else {
                        /* Hmmm, we couldn't parse the TFORM keyword by standard, so
                           just do simple parsing */
                        cptr = dispfmt;
                        while (!isdigit((int)*cptr) && *cptr != '\0') cptr++;

                        *width = atoi(cptr);
                    }

                    if (*width < 1) *width = 1; /* default is at least 1 column */
                }
            }
        }
    }
...
*/
{
    ...
    if (tcode == -TSTRING) /* variable length column in a binary table? */
    {
        ...
        if (ffgcprll(fptr, colnum, firstrow, 1, 1, 0, &scale, &zero, tform, &twidth,
                     &tcode, &maxelem, &startpos, &elemnum, &incre, &repeat, &rowlen,
                     &hdutype, &tnull, snull, status) > 0)
            return (*status);
