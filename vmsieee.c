...
void ieevpr(unsigned int* native, unsigned int* ieee, int* nelem) {
    ...
    for (i = 0; i < n; i++) {
        status = CVT$CONVERT_FLOAT(&native[i], CVT$K_VAX_F, &ieee[i], CVT$K_IEEE_S,
                                   options);
...
void ieevpd(unsigned long* native, unsigned long* ieee, int* nelem) {
    ...
    for (i = 0; i < n; i = i + 2) {
        status = CVT$CONVERT_FLOAT(&native[i], CVT$K_VAX_D, &ieee[i], CVT$K_IEEE_T,
                                   options);
        if (status != CVT$_NORMAL) {
            ieee[i] = *unanval;
            ieee[i + 1] = *unanval;
...
void ieevur(unsigned int* ieee, unsigned int* native, int* nelem) {
    ...
    for (j = 0; j < n; j++) {
        status = CVT$CONVERT_FLOAT(&ieee[j], CVT$K_IEEE_S, &native[j], CVT$K_VAX_F,
                                   options);
        if (status != CVT$_NORMAL) switch (status) {
                case CVT$_INVVAL:
                case CVT$_NEGINF:
                case CVT$_OVERFLOW:
                case CVT$_POSINF:
...
void ieevud(unsigned long* ieee, unsigned long* native, int* nelem) {
    ...
    for (j = 0; j < n; j = j + 2) {
        status = CVT$CONVERT_FLOAT(&ieee[j], CVT$K_IEEE_T, &native[j], CVT$K_VAX_D,
                                   options);
        if (status != CVT$_NORMAL) switch (status) {
                case CVT$_INVVAL:
                case CVT$_NEGINF:
