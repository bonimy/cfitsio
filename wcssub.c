...

{
    ...
    for (iwtb = 0; iwtb < nwtb; iwtb++, wtbp++) {
        ...
        if (fits_movnam_hdu(fptr, BINARY_TBL, (char*)(wtbp->extnam), wtbp->extver,
                            status)) {
            goto cleanup;
        }

        /* Locate the table column. */
        if (fits_get_colnum(fptr, CASEINSEN, (char*)(wtbp->ttype), &colnum, status)) {
            goto cleanup;
        }
        ...
        if (fits_read_col_dbl(fptr, colnum, wtbp->row, 1L, nelem, 0.0, *wtbp->arrayp,
                              &anynul, status)) {
            goto cleanup;
        }
    }
...
*/
{
    ...
    tstat = 0;
    if (ffgkyd(fptr, "CRVAL1", xrval, NULL, &tstat)) *xrval = 0.;

    tstat = 0;
    if (ffgkyd(fptr, "CRVAL2", yrval, NULL, &tstat)) *yrval = 0.;

    tstat = 0;
    if (ffgkyd(fptr, "CRPIX1", xrpix, NULL, &tstat)) *xrpix = 0.;

    tstat = 0;
    if (ffgkyd(fptr, "CRPIX2", yrpix, NULL, &tstat)) *yrpix = 0.;
    ...
    if (ffgkyd(fptr, "CDELT1", xinc, NULL, &tstat)) {
        ...
        } else /* no CD matrix keywords either */
        {
            *xinc = 1.;
            ...
            tstat = 0;
            if (ffgkyd(fptr, "CDELT2", yinc, NULL, &tstat)) *yinc = 1.;

            tstat = 0;
            if (ffgkyd(fptr, "CROTA2", rot, NULL, &tstat)) *rot = 0.;
        }
...
*/
{
    ...
    strcat(keyname, alt);
    if (ffgkyd(fptr, keyname, xrval, NULL, &tstat)) *xrval = 0.;

    tstat = 0;
    strcpy(keyname, "CRVAL2");
    strcat(keyname, alt);
    if (ffgkyd(fptr, keyname, yrval, NULL, &tstat)) *yrval = 0.;

    tstat = 0;
    strcpy(keyname, "CRPIX1");
    strcat(keyname, alt);
    if (ffgkyd(fptr, keyname, xrpix, NULL, &tstat)) *xrpix = 0.;

    tstat = 0;
    strcpy(keyname, "CRPIX2");
    strcat(keyname, alt);
    if (ffgkyd(fptr, keyname, yrpix, NULL, &tstat)) *yrpix = 0.;
    ...
    if (ffgkyd(fptr, keyname, xinc, NULL, &tstat)) {
        ...
        } else /* no CD matrix keywords either */
        {
            *xinc = 1.;
            ...
            strcat(keyname, alt);
            if (ffgkyd(fptr, keyname, yinc, NULL, &tstat)) *yinc = 1.;

            tstat = 0;
            strcpy(keyname, "CROTA2");
            strcat(keyname, alt);
            if (ffgkyd(fptr, keyname, rot, NULL, &tstat)) *rot = 0.;
        }
    } else /* Case 2: CDELTn + optional PC matrix */
    {
        strcpy(keyname, "CDELT2");
...
*/
{
    ...
    *header = calloc(1, 2401); /* room for up to 30 keywords */
    ...
    if (ffgkey(fptr, keyname, valstring, NULL, &tstatus)) valstring[0] = '\0';
    ...
    if (ffgkey(fptr, keyname, valstring, NULL, &tstatus)) strcpy(valstring, "1");
