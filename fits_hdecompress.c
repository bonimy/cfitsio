...
The following modifications have been made to the original code:
  ...
  - changed the first parameter in decode (and in lower level routines from a file
...
static int dodecode(unsigned char* infile, int a[], int nx, int ny,
                    unsigned char nbitplanes[3]);
static int dodecode64(unsigned char* infile, LONGLONG a[], int nx, int ny,
                      unsigned char nbitplanes[3]);
static int qtree_decode(unsigned char* infile, int a[], int n, int nqx, int nqy,
                        int nbitplanes);
static int qtree_decode64(unsigned char* infile, LONGLONG a[], int n, int nqx, int nqy,
                          int nbitplanes);
...
static void qtree_expand(unsigned char* infile, unsigned char a[], int nx, int ny,
                         unsigned char b[]);
static void qtree_bitins(unsigned char a[], int nx, int ny, int b[], int n, int bit);
static void qtree_bitins64(unsigned char a[], int nx, int ny, LONGLONG b[], int n,
                           int bit);
static void qtree_copy(unsigned char a[], int nx, int ny, unsigned char b[], int n);
static void read_bdirect(unsigned char* infile, int a[], int n, int nqx, int nqy,
                         unsigned char scratch[], int bit);
static void read_bdirect64(unsigned char* infile, LONGLONG a[], int n, int nqx, int nqy,
                           unsigned char scratch[], int bit);
...
int fits_hdecompress64(unsigned char* input, int smooth, LONGLONG* a, int* ny, int* nx,
    ...
    *status = stat;
    ...
    for (ii = 0; ii < nval; ii++) iarray[ii] = (int)a[ii];
...
*/
{
    ...
    int oddx, oddy;
    int shift, bit0, bit1, bit2, mask0, mask1, mask2, prnd0, prnd1, prnd2, nrnd0, nrnd1,
            nrnd2, lowbit0, lowbit1;
    int h0, hx, hy, hc;
    int s10, s00;
    int* tmp;
...
*/
{
    int i, j;
    int ny2, s10, s00;
    LONGLONG hm, h0, hp, hmm, hpm, hmp, hpp, hx2, hy2, diff, dmax, dmin, s, smax, m1,
            m2;
...
*/
{
    ...
    stat = qtree_decode64(infile, &a[ny * nx2 + ny2], ny, nx / 2, ny / 2,
                          nbitplanes[2]);
    if (stat) return (stat);
 ...
 *					in quadrant of 2-D array using binary quadtree
 *coding
 *
