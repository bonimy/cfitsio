...
*/
{
    ...
    {
        for (ii = 0; ii < nummsg; ii++) *txtbuff[ii] = '\0';
    ...
    } else if (action == DelMark) /* clear up to and including first marker */
    {
        while (nummsg > 0) {
            ...
            *txtbuff[nummsg] = '\0';     /* clear the buffer for this msg */
            ...
            if (markflag == ESMARKER) break; /* found a marker, so quit */
        }
...
*/
{
    ...
    for (ii = 0; ii < maxchr; ii++) {
        ...
        if ((testchar >= 'A' && testchar <= 'Z') ||
            (testchar >= '0' && testchar <= '9') || testchar == '-' ||
            testchar == '_') {
            if (spaces) {
                if (*status == 0) {
                    /* don't print error message if status < 0  */
                    snprintf(msg, FLEN_ERRMSG,
                             "Keyword name contains embedded space(s): %.8s", keyword);
                    ffpmsg(msg);
                }
            ...
            if (*status == 0) {
                /* don't print error message if status < 0  */
                snprintf(msg, FLEN_ERRMSG,
                         "Character %d in this keyword is illegal: %.8s", (int)(ii + 1),
                         keyword);
                ffpmsg(msg);
...
*/
{
    ...
    len = strlen(string);
    for (ii = 0; ii < len; ii++) string[ii] = toupper(string[ii]);
    return;
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    *card = '\0';
    ...
    if (namelen <= 8 && fftkey(tmpname, &tstatus) <= 0) {
        ...
        /*      if (ntoken > 1) { */
        if (ntoken >
            0) { /*  temporarily change so that this case should always be true  */
            /* for now at least, treat all cases as an implicit ESO HIERARCH keyword. */
            /* This could  change if FITS is ever expanded to directly support longer
             * keywords. */
    ...
    if (ntoken == 1 || specialchar == 1) {
        ffpmsg("Warning: the following keyword does not conform to the HIERARCH "
               "convention");
        /*  ffpmsg(" (e.g., name is not hierarchical or contains non-standard
         * characters)."); */
        ffpmsg(card);
    }
...
*/
{
    ...
    if (rootlen == 0 || value < 0) return (*status = 206);
    ...
    if (strlen(suffix) + strlen(keyname) > 8) return (*status = 206);
...
*/
{
    ...
    if (rootlen == 0 || rootlen > 7 || value < 0) return (*status = 206);

    snprintf(keyname, FLEN_VALUE, "%d", value); /* construct keyword prefix */

    if (rootlen + strlen(keyname) > 8) return (*status = 206);
...
*/
{
    ...
    *hdtype = 0;
    ...
    if (strncmp(tok, "--------------------", 20) == 0) return (*status = BAD_KEYCHAR);
    ...
    if (tok[0] == '-') /* is there a leading minus sign? */
    {
        ...
        *hdtype = -1;
        ...
        if (len >= FLEN_KEYWORD) return (*status = BAD_KEYCHAR);
        ...
        */
        ...
        if (tok[0] == '\0' || tok[0] == '=') return (*status); /* no second token */
        ...
        */
        ...
        if (*tok == '\'') /* is value enclosed in quotes? */
        {
            ...
            while (more) {
                ...
                tok--;
                if (len + 2 > remainlen) return (*status = BAD_KEYCHAR);
        ...
        } else /* not a quoted string value */
        {
            ...
            if (len > 139) return (*status = BAD_KEYCHAR);
            ...
            if (!((tok[0] == 'T' || tok[0] == 'F') &&
                ...
                if (*suffix != '\0' && *suffix != ' ' && *suffix != '/') {
                    ...
                    if (len > 137) return (*status = BAD_KEYCHAR);
                    strcpy(value, "'");
                    strncat(value, tok, len);
                    strcat(value, "'");
...

{
    ...
    if (*status > 0) return (*status);
    if ((inrec == 0) || (outrec == 0)) return (*status = NULL_INPUT_PTR);
    ...
    */
    ...
    for (pat = 0; pat < npat; pat++) {
        ...
        */
        ...
        for (ip = 0, ic = 0, firstfail = 1; (spat[ip]) && (ic < 8);
             ip++, ic++, firstfail = 0) {
            c = inrec[ic];
            s = spat[ip];
                ...
                if (isdigit(c)) {
                    ...
                    if (s == 'n') {
                        ...
                        if (val >= 1 && val <= 999 && /* Row range check */
                            (((n_range == 0) &&
                              (val == n_value)) || /* Strict equality */
                             ((n_range == -1) && (val <= n_value)) || /* n <= n_value */
                             ((n_range == +1) &&
                              (val >= n_value)))) { /* n >= n_value */
                            n1 = val;
                        } else {
                            pass = 0;
                        }
...
*/
{
    ...
    for (nrec = firstkey; (*status == 0) && (nrec <= nkeys); nrec++) {
        ...
        maxchr = strlen(rec);
        for (ii = 8; ii < maxchr; ii++) {
            if (rec[ii] < 32 || rec[ii] > 126) rec[ii] = ' ';
        }

        fits_translate_keyword(rec, outrec, patterns, npat, n_value, n_offset, n_range,
...
*/
{
    ...
    for (nrec = firstkey; nrec <= nkeys; nrec++) {
        ...
        fits_translate_pixkeyword(rec, outrec, patterns, npat, naxis, colnum, &pat_num,
                                  &iret, &jret, &nret, &mret, &lret, status);
...
int fits_translate_pixkeyword(
        ...
        int* colnum,         /* I - numbers of the columns to be binned */
        int* pat_num,        /* O - matched pattern number (0 based) or -1 */
        int* i, int* j, int* n, int* m, int* l, int* status) /* IO - error status */
           ...
           int* status)   /* IO - error status      */
{
    ...
    */
    ...
    if (form[0] == 'A')
        ...
        snprintf(message, FLEN_ERRMSG, "Illegal ASCII table TFORMn datatype: \'%s\'",
                 tform);
        ffpmsg(message);
        return (*status = BAD_TFORM_DTYPE);
    }

    if (dtcode) *dtcode = datacode;
        ...
        if (ffc2ii(form, &width, status) <= 0) /* read the width field */
        {
            if (width <= 0) {
                ...
                *status = BAD_TFORM;
                ...
                if (width <= 4 && datacode == TLONG) datacode = TSHORT;
            }
        }
    } else {
        /*-----------------------------------------------*/
    ...
    if (*status > 0) {
        *status = BAD_TFORM;
        snprintf(message, FLEN_ERRMSG, "Illegal ASCII table TFORMn code: \'%s\'",
                 tform);
        ffpmsg(message);
    }
           ...
           int* status)   /* IO - error status      */
{
    ...
    */
    ...
    ii = 0;
    while (isdigit((int)form[ii])) ii++; /* look for leading digits in the field */
    ...
    } else if (form[0] == 'W') /* internal code to signify unsigned long long integer */
    {
        ...
        */
        ...
        if (iread != 1 || (!variable && (width > repeat))) width = repeat;
    ...
    } else if (form[0] == 'S') /* internal code to signify signed byte */
    {
        ...
        snprintf(message, FLEN_ERRMSG, "Illegal binary table TFORMn datatype: \'%s\' ",
                 tform);
        ffpmsg(message);
        return (*status = BAD_TFORM_DTYPE);
    }

    if (variable) datacode = datacode * (-1); /* flag variable cols w/ neg type code */

    if (dtcode) *dtcode = datacode;

    if (trepeat) *trepeat = repeat;
    ...
    */
    ...
    } else if (form[0] == 'W') /* internal code to signify unsigned long long integer */
    {
        ...
        */
        ...
        if (iread != 1 || (!variable && (width > repeat))) width = (long)repeat;
...
*/
{
    ...
    if (tform[ii] == 0) return; /* input format string was blank */
    ...
    if (tform[ii] == 'F') strcat(cform, "f");
    if (tform[ii] == 'E') strcat(cform, "E");
    if (tform[ii] == 'D') strcat(cform, "E");
...
*/
{
    ...
    *colnum = 0;
    ...
    if (founde) {
        ...
        if (!unique || tstatus == COL_NOT_UNIQUE) *status = COL_NOT_UNIQUE;
    } else {
        ...
        if (tstatus == 0 && ivalue <= (fptr->Fptr)->tfield && ivalue > 0) {
            ...
            *status = COL_NOT_FOUND;
            if (tstatus != COL_NOT_UNIQUE) {
                snprintf(errmsg, FLEN_ERRMSG, "ffgcnn could not find column: %.45s",
                         templt);
                ffpmsg(errmsg);
            }
        }
...
*/
{
    ...
    *exact = TRUE;
    ...
    for (ii = strlen(temp) - 1; ii >= 0 && temp[ii] == ' '; ii--) temp[ii] = '\0';

    for (ii = strlen(col) - 1; ii >= 0 && col[ii] == ' '; ii--) col[ii] = '\0';
...
*/
{
    ...
    if (repeat) *repeat = (long)trepeat;

    if (width) *width = (long)twidth;
...
*/
{
    ...
    if (colnum < 1 || colnum > (fptr->Fptr)->tfield) return (*status = BAD_COL_NUM);

    colptr = (fptr->Fptr)->tableptr; /* pointer to first column */
    colptr += (colnum - 1);          /* offset to correct column */

    if (ffghdt(fptr, &hdutype, status) > 0) return (*status);
    ...
    if (hdutype == ASCII_TBL) {
        ...
        *width = tmpwidth;
        ...
        if (typecode) *typecode = colptr->tdatatype;

        if (width) *width = colptr->twidth;

        if (repeat) *repeat = colptr->trepeat;
    }
...
*/
{
    ...
    } else if ((min_val >= -9.2233720368547755808E18) &&
               (max_val <= 9.2233720368547755807E18)) {
        effcode = TLONGLONG;
...
*/
{
    ...
    if ((fptr->Fptr)->hdutype == IMAGE_HDU) return (*status = NOT_TABLE);
...
*/
{
    ...
    if (ttype) strcpy(ttype, colptr->ttype);

    if (tbcol) *tbcol = (long)((colptr->tbcol) + 1); /* first col is 1, not 0 */

    if (tform) strcpy(tform, colptr->tform);

    if (tscal) *tscal = colptr->tscale;

    if (tzero) *tzero = colptr->tzero;

    if (tnull) strcpy(tnull, colptr->strnull);
...
*/
{
    ...
    ffgbclll(fptr, colnum, ttype, tunit, dtype, &trepeat, tscal, tzero, &ttnull, tdisp,
             status);

    if (repeat) *repeat = (long)trepeat;

    if (tnull) *tnull = (long)ttnull;
...
*/
{
    ...
    /* reset position to the correct HDU if necessary */
    if (fptr->HDUposition != (fptr->Fptr)->curhdu) {
        if (ffmahd(fptr, (fptr->HDUposition) + 1, NULL, status) > 0) return (*status);
    } else if ((fptr->Fptr)->datastart == DATA_UNDEFINED) {
        if (ffrdef(fptr, status) > 0) /* rescan header */
            return (*status);
    }

    if (headstart) *headstart = (fptr->Fptr)->headstart[(fptr->Fptr)->curhdu];

    if (datastart) *datastart = (fptr->Fptr)->datastart;

    if (dataend) *dataend = (fptr->Fptr)->headstart[((fptr->Fptr)->curhdu) + 1];
...
*/
{
    ...
    if (headstart) *headstart = (OFF_T)(fptr->Fptr)->headstart[(fptr->Fptr)->curhdu];

    if (datastart) *datastart = (OFF_T)(fptr->Fptr)->datastart;

    if (dataend) *dataend = (OFF_T)(fptr->Fptr)->headstart[((fptr->Fptr)->curhdu) + 1];
...
*/
{
    ...
    else if (!strcmp(name, "XTENSION")) /* this is an XTENSION keyword */
    {
        ...
        if (!strcmp(xtension, "TABLE")) {
            ...
            if (hdutype != NULL) *hdutype = 1;
        }

        else if (!strcmp(xtension, "BINTABLE") || !strcmp(xtension, "A3DTABLE") ||
                 !strcmp(xtension, "3DTABLE")) {
            ffbinit(fptr, status); /* initialize the binary table */
            if (hdutype != NULL) *hdutype = 2;
        }
    ...
    if ((fptr->Fptr)->headstart[(fptr->Fptr)->curhdu + 1] < (fptr->Fptr)->logfilesize) {
        ...
        if (!strcmp(urltype, "mem://") || !strcmp(urltype, "memkeep://")) {
            fftrun(fptr, (fptr->Fptr)->headstart[(fptr->Fptr)->curhdu + 1], status);
        }
    }
    return (*status);
}
...
*/
{
    ...
    ffgphd(fptr, 999, &simple, &bitpix, &naxis, naxes, &pcount, &gcount, &extend,
           &bscale, &bzero, &blank, &nspace, status);
            ...
            int* status)    /* IO - error status     */
{
    ...
    */
    ...
    if (ffgttb(fptr, &rowlen, &nrows, &pcount, &tfield, status) > 0) return (*status);
    ...
    /* free the tile-compressed image cache, if it exists */
    if ((fptr->Fptr)->tilerow) {
        ntilebins = (((fptr->Fptr)->znaxis[0] - 1) / ((fptr->Fptr)->tilesize[0])) + 1;
    ...
    */
    ...
    for (ii = 0; ii < tfield; ii++, colptr++) {
        ...
        if (colptr->tdatatype == -9999) {
            ...
            snprintf(message, FLEN_ERRMSG, "Required %s keyword not found (ffainit).",
                     name);
            ffpmsg(message);
            ...
            snprintf(message, FLEN_ERRMSG,
                     "Value of %s keyword out of range: %ld (ffainit).", name,
                     (long)tbcoln);
            ffpmsg(message);
            ...
            snprintf(message, FLEN_ERRMSG,
                     "Column %d is too wide to fit in table (ffainit)", ii + 1);
            ffpmsg(message);
    ...
    */
    ...
    /* the next HDU begins in the next logical block after the data  */
    (fptr->Fptr)->headstart[(fptr->Fptr)->curhdu + 1] =
            (fptr->Fptr)->datastart + (((LONGLONG)rowlen * nrows + 2879) / 2880 * 2880);
            ...
            int* status)    /* IO - error status     */
{
    ...
    */
    ...
    for (ii = 0; ii < tfield; ii++, colptr++) {
        if (colptr->tdatatype == -9999) {
            ffkeyn("TFORM", ii + 1, name, status); /* construct keyword name */
            snprintf(message, FLEN_ERRMSG, "Required %s keyword not found (ffbinit).",
                     name);
            ffpmsg(message);
            return (*status = NO_TFORM);
        }
    ...
    for (ii = 0; ii < tfields; ii++, colptr++) {
        ...
        if (colptr->tdatatype == TSTRING) {
            ...
            if (*cptr == 'P')
                ...
                snprintf(message, FLEN_ERRMSG, "unknown binary table column type: %s",
                         colptr->tform);
                ffpmsg(message);
                *status = BAD_TFORM;
                return (*status);
    ...
    */
    ...
    if (!FSTRNCMP(name + 1, "TYPE", 4)) {
        ...
        if (ffc2dd(value, &dvalue, &tstatus) > 0) {
            snprintf(message, FLEN_ERRMSG, "Error reading value of %s as a double: %s",
                     name, value);
            ffpmsg(message);
        ...
        if (!loc) return (*status); /* not a proper TDIM keyword */
...
*/
{
    ...
    if (firstrow < 1) {
        ...
        snprintf(message, FLEN_ERRMSG,
                 "Tried to read or write less than 0 elements: %.0f", (double)nelem);
        ffpmsg(message);
    ...
    *startpos = datastart + ((LONGLONG)(firstrow - 1) * *rowlen) + tbcol;
    ...
    {                                       /*
                                            ...
                                            */
        if (*repeat < *elemnum + nelem) *repeat = *elemnum + nelem;
    } else if (*tcode > 0) /*  Fixed length table column  */
    {
        if (*elemnum >= *repeat) {
            snprintf(message, FLEN_ERRMSG,
                     "First element to write is too large: %ld; max allowed value is "
                     "%ld",
            ...
            if (endrow > (fptr->Fptr)->numrows && rangecheck) {
                if (*hdutype == IMAGE_HDU) /*  Primary Array or IMAGE */
                {
                    if (firstrow > (fptr->Fptr)->numrows) {
                        snprintf(message, FLEN_ERRMSG,
                                 "Attempted to read from group %ld of the HDU,",
                                 (long)firstrow);
                        ffpmsg(message);
                        ...
                        snprintf(message, FLEN_ERRMSG, "  Image has  %ld elements;",
                                 (long)*repeat);
                        ffpmsg(message);
                    ...
                    snprintf(message, FLEN_ERRMSG,
                             "  Tried to read %.0f elements starting at row %.0f, "
                             "element %.0f.",
        ...
        if (*repeat == 1 && nelem > 1 &&
            writemode !=
                    2) { /*
                           ...
                           This allows multiple values (up to the maxelem number of
                           elements that will fit in the buffer) to be read or written
                           with a single routine call, which increases the efficiency.
        ...
        *tcode *= (-1);
        ...
        if (writemode) /* return next empty heap address for writing */
        {
                    ...
                    if (lrepeat >= *repeat) /* enough existing space? */
                    {
                        *startpos = datastart + heapoffset + (fptr->Fptr)->heapstart;
                        ...
                        if (colptr->tdatatype <= -TCOMPLEX) {
                            ...
                            ffpdes(fptr, colnum, firstrow, *repeat / 2, heapoffset,
                                   status);
                        } else {
                            ffpdes(fptr, colnum, firstrow, *repeat, heapoffset, status);
                        }
                        return (*status);
                    }
                }
            ...
            *startpos = datastart + (fptr->Fptr)->heapstart + (fptr->Fptr)->heapsize;
            ...
            if (colptr->tdatatype <= -TCOMPLEX) {
                ...
                ffpdes(fptr, colnum, firstrow, *repeat, (fptr->Fptr)->heapsize, status);
            }
            ...
            if (!((fptr->Fptr)->lasthdu)) {
                endpos = datastart + (fptr->Fptr)->heapstart + (fptr->Fptr)->heapsize +
                         (*repeat * (*incre));
                ...
                if (endpos > (fptr->Fptr)->headstart[(fptr->Fptr)->curhdu + 1]) {
                    ...
                    {
                        snprintf(message, FLEN_ERRMSG,
                                 "Failed to extend the size of the variable length "
                                 "heap by %ld blocks.",
                                 nblock);
                        ffpmsg(message);
                        return (*status);
        ...
        } else /*  get the read start position in the heap */
        {
            ...
            *repeat = lrepeat;
            ...
            if (*elemnum >= *repeat) {
                snprintf(message, FLEN_ERRMSG,
                         "Starting element to read in variable length column is too "
                         "large: %ld",
                         (long)firstelem);
                ffpmsg(message);
                snprintf(message, FLEN_ERRMSG, "  This row only contains %ld elements",
                         (long)*repeat);
                ffpmsg(message);
                return (*status = BAD_ELEM_NUM);
            }
...
*/
{
    ...
    for (jj = 1; jj <= (fptr->Fptr)->tfield && *status <= 0; jj++) {
        ...
        if (typecode > 0) continue; /* ignore fixed length columns */
...
*/
{
    ...
    if (!valid) return (*status = BAD_HEAP_PTR); /* bad heap pointers */
    ...
    for (jj = 1; jj <= (fptr->Fptr)->tfield && *status <= 0; jj++) {
        ...
        for (ii = 1; ii <= (fptr->Fptr)->numrows; ii++) {
            ...
            ffmbyt(fptr, writeheapstart + (fptr->Fptr)->heapsize, IGNORE_EOF, status);
            ffpbyt(fptr, nbytes, buffer, status);

            /* write descriptor */
            ffpdes(fptr, jj, ii, repeat, (fptr->Fptr)->heapsize, status);
...
*/
{
    ...
    bytepos = (fptr->Fptr)->datastart + ((fptr->Fptr)->rowlength * (rownum - 1)) +
              colptr->tbcol;

    if (colptr->tform[0] == 'P' || colptr->tform[1] == 'P') {
        /* read 4-byte descriptor */
        if (ffgi4b(fptr, bytepos, 2, 4, (INT32BIT*)descript4, status) <= 0) {
            if (length) *length = (LONGLONG)descript4[0]; /* 1st word is the length  */
            if (heapaddr)
    ...
    } else /* this is for 'Q' columns */
    {
        ...
        if (ffgi8b(fptr, bytepos, 2, 8, (long*)descript8, status) <= 0) {
            if (length) *length = descript8[0];     /* 1st word is the length  */
            if (heapaddr) *heapaddr = descript8[1]; /* 2nd word is the address */
        }
    }
...
*/
{
    ...
    bytepos = (fptr->Fptr)->datastart + (rowsize * (firstrow - 1)) + colptr->tbcol;
...
*/
{
    ...
    if (colptr->tdatatype >= 0) *status = NOT_VARI_LEN;
    ...
    if (colptr->tform[0] == 'P' || colptr->tform[1] == 'P') {
        if (length > UINT_MAX || length < 0 || heapaddr > UINT_MAX || heapaddr < 0) {
           ...
           int* status)    /* IO - error status     */
{
    ...
    */
    ...
    if (fptr->HDUposition != (fptr->Fptr)->curhdu) {
        ...
        if (((fptr->Fptr)->driver != stdriver)) ffrdef(fptr, status);
    ...
    if ((fptr->Fptr)->open_count == 1) {
        ...
        if ((fptr->Fptr)->tableptr) {
            ...
            if ((fptr->Fptr)->tilerow) {
                ntilebins =
                        (((fptr->Fptr)->znaxis[0] - 1) / ((fptr->Fptr)->tilesize[0])) +
                        1;
    ...
    if (*status > 0 && *status != NO_CLOSE_ERROR) {
        snprintf(message, FLEN_ERRMSG, "Error while closing HDU number %d (ffchdu).",
                 (fptr->Fptr)->curhdu);
        ffpmsg(message);
    }
    return (*status);
...
*/
{
    ...
    for (ii = 1; ii <= tflds; ii++) /* loop over all the columns */
    {
        ...
        if (ffgkys(fptr, keyname, tform, comment, status) > 0) {
            snprintf(message, FLEN_ERRMSG,
                     "Error while updating variable length vector TFORMn values "
                     "(ffuptf).");
            ffpmsg(message);
            return (*status);
        }
        ...
        if ((fptr->Fptr)->datastart != DATA_UNDEFINED) {
            ...
            if ((fptr->Fptr)->hdutype != IMAGE_HDU) {
                ...
                /* we will assume that the current value is correct. */
                {
                    ...
                    snprintf(valstring, FLEN_VALUE, "%.0f",
                             (double)((fptr->Fptr)->numrows));
...
*/
{
    ...
    if (*status > 0 || morekeys < 1) return (*status);
    ...
    if (fptr->HDUposition != (fptr->Fptr)->curhdu) {
        ...
        delta = (((fptr->Fptr)->headend + (morekeys * 80)) / 2880 + 1) * 2880 -
                (fptr->Fptr)->datastart;
...
*/
{
    ...
    for (ii = 0; ii < nspace; ii++) {
        ...
        if (tstatus) break;
        if (strncmp(keyrec, blankkey, 80) && strncmp(keyrec, endkey, 80)) break;
    }
    ...
    */
    ...
    if (*status > 0) ffpmsg("Error while writing END card (ffwend).");
...
*/
{
    ...
    fillstart =
            (fptr->Fptr)->datastart + (fptr->Fptr)->heapstart + (fptr->Fptr)->heapsize;
    ...
    if (!nfill) /* no fill bytes; just check that entire table exists */
    {
        ...
        if (tstatus == 0) return (*status); /* no EOF error, so everything is OK */
    } else {
        ...
        if (tstatus == 0) {
            for (ii = 0; ii < nfill; ii++) {
                if (fill[ii] != chfill) break;
            }

            if (ii == nfill) return (*status); /* all the fill values were correct */
        }
    }
    ...
    if (*status > 0) ffpmsg("Error writing Data Unit fill bytes (ffpdfl).");
    ...
    for (i = 0; i < nblank; i++) {
        ...
        if (!strncmp(rec, "END     ", 8)) {
            ...
            if (strncmp(rec + 8, blanks + 8, 72)) {
                ...
                *status = END_JUNK;
                ffpmsg("Warning: END keyword contains extraneous non-blank "
                       "characters:");
            }
        } else if (gotend) {
            if (strncmp(rec, blanks, 80)) {
                ...
                *status = BAD_HEADER_FILL;
                ffpmsg("Warning: Header fill area contains extraneous non-blank "
                       "characters:");
            }
        }
    ...
    for (i = 0; i < nfill; i++) {
        if (chbuff[i] != chfill) {
            *status = BAD_DATA_FILL;
            if ((fptr->Fptr)->hdutype == ASCII_TBL)
                ffpmsg("Warning: remaining bytes following ASCII table data are not "
                       "filled with blanks.");
            else
                ffpmsg("Warning: remaining bytes following data are not filled with "
                       "zeros.");
            return (*status);
        }
    }
...
*/
{
    ...
    if (*status > 0 || nblocks <= 0) return (*status);
    ...
    */
    ...
    for (ii = 0; ii < nblocks; ii++) ffpbyt(fptr, 2880L, buffer, status);
...
*/
{
    ...
    if (fptr->HDUposition == 0 && (fptr->Fptr)->headend == 0) {
        ...
        *exttype = (fptr->Fptr)->hdutype; /* return the type of HDU */
        ...
        if ((fptr->Fptr)->compressimg) *exttype = IMAGE_HDU;
    }
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    /*  check if this is a compressed image */
    if ((fptr->Fptr)->compressimg) return (1);
...
*/
{
    ...
    if (bitpix) fits_get_img_type(infptr, bitpix, status); /* get BITPIX value */

    if (naxis) fits_get_img_dim(infptr, naxis, status); /* get NAXIS value */
...
*/
{
    ...
    while (((fptr->Fptr)->curhdu) + 1 != hdunum) /* at the correct HDU? */
    {
        ...
        if ((fptr->Fptr)->headstart[moveto] < (fptr->Fptr)->logfilesize) {
            ...
            {
                if (ffgext(fptr, moveto, exttype, status) >
                    0) { /* failed to get the requested extension */
    ...
    if (exttype != NULL) ffghdt(fptr, exttype, status);
...
*/
{
    ...
    */
    ...
    for (ii = 1; 1; ii++) /* loop over all HDUs until EOF */
    {
        ...
        alttype = -1;
        if (fits_is_compressed_image(fptr, status)) alttype = BINARY_TBL;
...
*/
{
    ...
    *nhdu = extnum - 1;
    ...
    if ((fptr->Fptr)->datastart == DATA_UNDEFINED) return (*status);
...
*/
{
    ...
    if (ffmbyt(fptr, (fptr->Fptr)->headstart[hdunum], REPORT_EOF, status) <= 0) {
        ...
        if (ffrhdu(fptr, exttype, status) >
            0) { /* failed to get the new HDU, so restore previous values */
...
*/
{
    ...
    if (*status > 0 || nblock <= 0) return (*status);
    ...
    {
        insertpt = (fptr->Fptr)->datastart + (fptr->Fptr)->heapstart +
                   (fptr->Fptr)->heapsize;
...

    {
        ...
        for (ii = 0; ii < nshift; ii++) {
            ...
            if (ffmbyt(fptr, jpoint, REPORT_EOF, status) > 0) return (*status);
...
*/
{
    ...
    if (*card == 'Z') {
        if (FSTRNCMP(card1, "IMAGE  ", 7) == 0)
            ...
            if (*card5 >= '0' && *card5 <= '9') return (TYP_CMPRS_KEY);
        } else if (FSTRNCMP(card1, "VAL", 3) == 0) {
            if (*(card + 4) >= '0' && *(card + 4) <= '9') return (TYP_CMPRS_KEY);
        ...
        if (FSTRNCMP(card1, "ITPIX  ", 7) == 0) return (TYP_STRUC_KEY);
        if (FSTRNCMP(card1, "LOCKED ", 7) == 0) return (TYP_STRUC_KEY);

        if (FSTRNCMP(card1, "LANK   ", 7) == 0) return (TYP_NULL_KEY);

        if (FSTRNCMP(card1, "SCALE  ", 7) == 0) return (TYP_SCAL_KEY);
        if (FSTRNCMP(card1, "ZERO   ", 7) == 0) return (TYP_SCAL_KEY);

        if (FSTRNCMP(card1, "UNIT   ", 7) == 0) return (TYP_UNIT_KEY);
    } else if (*card == 'C') {
        ...
        if (FSTRNCMP(card1, "HECKSUM", 7) == 0) return (TYP_CKSUM_KEY);

        if (FSTRNCMP(card1, "ONTINUE", 7) == 0) return (TYP_CONT_KEY);

        if (FSTRNCMP(card1, "TYPE", 4) == 0) {
            if (*card5 >= '0' && *card5 <= '9') return (TYP_WCS_KEY);
            ...
            if (*(card + 2) >= '0' && *(card + 2) <= '9') return (TYP_WCS_KEY);
        }
    } else if (*card == 'D') {
        if (FSTRNCMP(card1, "ATASUM ", 7) == 0) return (TYP_CKSUM_KEY);
        if (FSTRNCMP(card1, "ATAMIN ", 7) == 0) return (TYP_RANG_KEY);
        if (FSTRNCMP(card1, "ATAMAX ", 7) == 0) return (TYP_RANG_KEY);
        if (FSTRNCMP(card1, "ATE-OBS", 7) == 0) return (TYP_REFSYS_KEY);
    } else if (*card == 'E') {
        if (FSTRNCMP(card1, "XTEND  ", 7) == 0) return (TYP_STRUC_KEY);
        if (FSTRNCMP(card1, "ND     ", 7) == 0) return (TYP_STRUC_KEY);
        ...
        if (FSTRNCMP(card1, "XTVER  ", 7) == 0) return (TYP_HDUID_KEY);
        if (FSTRNCMP(card1, "XTLEVEL", 7) == 0) return (TYP_HDUID_KEY);

        if (FSTRNCMP(card1, "QUINOX", 6) == 0) return (TYP_REFSYS_KEY);
        if (FSTRNCMP(card1, "QUI", 3) == 0) {
            if (*(card + 4) >= '0' && *(card + 4) <= '9') return (TYP_REFSYS_KEY);
        }
        if (FSTRNCMP(card1, "POCH   ", 7) == 0) return (TYP_REFSYS_KEY);
    } else if (*card == 'G') {
        if (FSTRNCMP(card1, "COUNT  ", 7) == 0) return (TYP_STRUC_KEY);
        if (FSTRNCMP(card1, "ROUPS  ", 7) == 0) return (TYP_STRUC_KEY);
    } else if (*card == 'H') {
        if (FSTRNCMP(card1, "DUNAME ", 7) == 0) return (TYP_HDUID_KEY);
        if (FSTRNCMP(card1, "DUVER  ", 7) == 0) return (TYP_HDUID_KEY);
        if (FSTRNCMP(card1, "DULEVEL", 7) == 0) return (TYP_HDUID_KEY);
        ...
        if (FSTRNCMP(card1, "ONPOLE", 6) == 0) return (TYP_WCS_KEY);
        if (FSTRNCMP(card1, "ATPOLE", 6) == 0) return (TYP_WCS_KEY);
        if (FSTRNCMP(card1, "ONP", 3) == 0) {
            if (*(card + 4) >= '0' && *(card + 4) <= '9') return (TYP_WCS_KEY);
        } else if (FSTRNCMP(card1, "ATP", 3) == 0) {
            if (*(card + 4) >= '0' && *(card + 4) <= '9') return (TYP_WCS_KEY);
        }
    } else if (*card == 'M') {
        if (FSTRNCMP(card1, "JD-OBS ", 7) == 0) return (TYP_REFSYS_KEY);
        if (FSTRNCMP(card1, "JDOB", 4) == 0) {
            if (*(card + 5) >= '0' && *(card + 5) <= '9') return (TYP_REFSYS_KEY);
        }
    } else if (*card == 'N') {
        ...
        if (FSTRNCMP(card1, "ADECSYS", 7) == 0) return (TYP_REFSYS_KEY);
        if (FSTRNCMP(card1, "ADESYS", 6) == 0) return (TYP_REFSYS_KEY);
        if (FSTRNCMP(card1, "ADE", 3) == 0) {
            if (*(card + 4) >= '0' && *(card + 4) <= '9') return (TYP_REFSYS_KEY);
        }
    } else if (*card == 'S') {
        if (FSTRNCMP(card1, "IMPLE  ", 7) == 0) return (TYP_STRUC_KEY);
    } else if (*card == 'T') {
        if (FSTRNCMP(card1, "TYPE", 4) == 0) {
            if (*card5 >= '0' && *card5 <= '9') return (TYP_STRUC_KEY);
            ...
            if (*card5 >= '0' && *card5 <= '9') return (TYP_NULL_KEY);
        }

        else if (FSTRNCMP(card1, "DIM", 3) == 0) {
            if (*(card + 4) >= '0' && *(card + 4) <= '9') return (TYP_DIM_KEY);
        }

        else if (FSTRNCMP(card1, "UNIT", 4) == 0) {
            if (*card5 >= '0' && *card5 <= '9') return (TYP_UNIT_KEY);
        }

        else if (FSTRNCMP(card1, "DISP", 4) == 0) {
            if (*card5 >= '0' && *card5 <= '9') return (TYP_DISP_KEY);
        }

        else if (FSTRNCMP(card1, "SCAL", 4) == 0) {
            if (*card5 >= '0' && *card5 <= '9') return (TYP_SCAL_KEY);
        } else if (FSTRNCMP(card1, "ZERO", 4) == 0) {
            if (*card5 >= '0' && *card5 <= '9') return (TYP_SCAL_KEY);
        }

        else if (FSTRNCMP(card1, "LMIN", 4) == 0) {
            if (*card5 >= '0' && *card5 <= '9') return (TYP_RANG_KEY);
        ...
        if (FSTRNCMP(card1, "TENSION", 7) == 0) return (TYP_STRUC_KEY);
    } else if (*card == 'W') {
        if (FSTRNCMP(card1, "CSAXES", 6) == 0) return (TYP_WCS_KEY);
        if (FSTRNCMP(card1, "CSNAME", 6) == 0) return (TYP_WCS_KEY);
        ...
        if (*card1 == 'C') {
            ...
            if (*(card + 2) >= '0' && *(card + 2) <= '9') return (TYP_WCS_KEY);
                   ...
                   *card1 <= '9') { /* 2 digits at beginning of keyword */
...
*/
{
    ...
    if (cval[0] == '\0') return (*status = VALUE_UNDEFINED); /* null value string */
...
*/
{
    ...
    if (dtype == 'C' || dtype == 'X') *status = BAD_LOGICALKEY;
...
*/
{
    ...
    *ival = strtoul(cval, &loc, 10); /* read the string as an integer */
    ...
    if (errno == ERANGE) {
        strcpy(msg,
               "Range Error in ffc2ujj converting string to unsigned longlong int: ");
        strncat(msg, cval, 25);
        ffpmsg(msg);
