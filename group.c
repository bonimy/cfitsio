...

{
    ...
    do {
        ...
        *status = fits_insert_btbl(fptr, 0, tfields, ttype, tform, NULL, NULL, pcount,
                                   status);
        ...
        */
        ...
        fits_insert_key_str(fptr, "EXTNAME", extname, "HDU contains a Grouping Table",
                            status);
        ...
        */
        ...
        for (i = 0; i < tfields && *status == 0; ++i) {
            if (fits_strcasecmp(ttype[i], "MEMBER_POSITION") == 0 ||
                ...
                *status = fits_read_key_str(fptr, keyword, keyvalue, comment, status);
    ...
    do {
        ...
        */
        ...
        for (i = 0; i < ncols && *status == 0; ++i)
            *status =
                    fits_insert_col(gfptr, tfields + i + 1, ttype[i], tform[i], status);
        ...
        */
        ...
        for (i = 0; i < ncols && *status == 0; ++i) {
            if (fits_strcasecmp(ttype[i], "MEMBER_POSITION") == 0 ||
                ...
                *status = fits_get_colnum(gfptr, CASESEN, ttype[i], &colnum, status);

                snprintf(keyword, FLEN_KEYWORD, "TFORM%d", colnum);

                *status = fits_read_key_str(gfptr, keyword, keyvalue, comment, status);

                snprintf(keyword, FLEN_KEYWORD, "TNULL%d", colnum);

                *status = fits_insert_key_lng(gfptr, keyword, 0, "Column Null Value",
                                              status);
        ...
        if (*status != 0) {
            *firstfailed = i;
            snprintf(errstr, FLEN_VALUE,
                     "Group table verify failed for member %ld (ffgtvf)", i);
            ffpmsg(errstr);
            continue;
        }
...
*/
{
    ...
    do {
        ...
        if (grpid > ngroups) {
            *status = BAD_GROUP_ID;
            snprintf(comment, FLEN_COMMENT,
                     "GRPID index %d larger total GRPID keywords %ld (ffgtop)", grpid,
                     ngroups);
            ffpmsg(comment);
            continue;
        }
        ...
        */
        ...
        do {
            if (grpExtver > 0) {
                ...
                *status = BAD_GROUP_ID;
                snprintf(comment, FLEN_COMMENT,
                         "Invalid value of %ld for GRPID%d (ffgtop)", grpExtver, grpid);
                ffpmsg(comment);
                continue;
            }
            ...
            */
            ...
            /* SPR 1738 */
            *status =
                    fits_read_key_longstr(mfptr, keyword, &tkeyvalue, comment, status);
            if (0 == *status) {
        ...
        *status = fits_movnam_hdu(*gfptr, ANY_HDU, "GROUPING", (int)grpExtver, status);
...

{
    ...
    do {
        ...
        *status = fits_read_key_str(tmpfptr, "XTENSION", memberHDUtype, card, status);
        ...
        *status = fits_read_key_str(tmpfptr, "EXTNAME", memberExtname, card, status);
        ...
        *status = fits_get_url(tmpfptr, memberFileName, memberLocation, memberAccess1,
                               memberAccess2, &memberIOstate, status);
        ...
        *status = fits_get_url(gfptr, groupFileName, groupLocation, groupAccess1,
                               groupAccess2, &groupIOstate, status);
        ...
        if (fits_strcasecmp(groupAccess1, "file://") &&
            ...
            */
            ...
            if (fits_strcasecmp(memberAccess1, "file://") == 0) {
                ...
                *status = fits_clean_url(memberLocation, memberFileName, status);
            }
            ...
            */
            ...
            if (fits_strcasecmp(groupAccess1, "file://") == 0 &&
                ...
                fits_url2relurl(memberFileName, groupFileName, groupLocation, status);
                fits_url2relurl(groupFileName, memberFileName, memberLocation, status);
            ...
            *status = ffgmf(gfptr, memberHDUtype, memberExtname, memberExtver,
            ...
            if (*status == MEMBER_NOT_FOUND)
                *status = 0;
                ...
                ffpmsg("Specified HDU is already a member of the Grouping table "
                       "(ffgtam)");
                continue;
            } else
                continue;
            ...
            *tmpPtr = memberExtname;
            ...
            if (extnameCol != 0) {
                if (strlen(memberExtname) != 0)
                    fits_write_col_str(gfptr, extnameCol, nmembers, 1, 1, tmpPtr,
                                       status);
                else
                    /* WILL THIS WORK FOR VAR LENTH CHAR COLS??????*/
                    fits_write_col_byt(gfptr, extnameCol, nmembers, 1, 1, charNull,
                                       status);
            }
            ...
            if (positionCol != 0)
                fits_write_col_int(gfptr, positionCol, nmembers, 1, 1, &memberPosition,
                                   status);
            ...
            *tmpPtr = memberFileName;
            ...
            if (locationCol != 0) {
                ...
                */
                ...
                if ((tmpfptr->Fptr != gfptr->Fptr) &&
                    strncmp(tmprootname, grootname, FLEN_FILENAME))
                    fits_write_col_str(gfptr, locationCol, nmembers, 1, 1, tmpPtr,
                                       status);
                else
                    /* WILL THIS WORK FOR VAR LENTH CHAR COLS??????*/
                    fits_write_col_byt(gfptr, locationCol, nmembers, 1, 1, charNull,
                                       status);
            }
                    ...
                    */
                    ...
                    if (0 < strlen(cwd)) {
                        ...
                        if (!fits_is_url_absolute(grplc)) {
                            ...
                            if (groupLocation[0] != '/') {
                                ...
                                if (strlen(tmp) + strlen(groupLocation) + 1 >
                                    FLEN_FILENAME - 1) {
                                    ffpmsg("path and group location is too long "
                                           "(ffgtam)");
                                    *status = URL_PARSE_ERROR;
                                    continue;
                                }
                        ...
                        if (!fits_is_url_absolute(groupFileName)) {
                            fits_path2url(groupFileName, FLEN_FILENAME, groupLocation,
                                          status);
        ...
        */
        ...
        if (ngroups == 0) {
            ...
            */
            ...
            for (i = 0, *status = KEY_NO_EXIST; i < nkeys && *status == KEY_NO_EXIST;
                 ++i) {
                *status = 0;
            ...
            if (offset > 0) {
                ...
                *status = fits_read_key_longstr(mfptr, keyword, &tkeyvalue, comment,
                ...
                if (0 == *status) {
                    ...
                    fits_insert_key_longstr(mfptr, newKeyword, tkeyvalue, comment,
                                            status);
...

{
    ...
    do {
        ...
        */
        ...
        if (xtensionCol != 0) {
            *status = fits_read_col_str(gfptr, xtensionCol, member, 1, 1, nstr, tmpPtr,
                                        &dummy, status);
        ...
        if (extnameCol != 0)
            *status = fits_read_col_str(gfptr, extnameCol, member, 1, 1, nstr, tmpPtr,
                                        &dummy, status);
        ...
        if (locationCol != 0)
            *status = fits_read_col_str(gfptr, locationCol, member, 1, 1, nstr, tmpPtr,
                                        &dummy, status);
        tmpPtr[0] = uri;

        if (uriCol != 0)
            *status = fits_read_col_str(gfptr, uriCol, member, 1, 1, nstr, tmpPtr,
                                        &dummy, status);
                    ...
                    */
                    ...
                    do {
                        ...
                        if (*mbrLocation1 == '/') {
                            ...
                            *status = fits_url2path(mbrLocation1, mbrLocation2, status);
                            ...
                            if (!fits_is_url_absolute(grpLocation1) &&
                                *grpLocation1 != '/') {
                                ...
                                if (strlen(cwd) + strlen(grpLocation1) + 1 >
                                    FLEN_FILENAME - 1) {
                                    ffpmsg("cwd and group location1 is too long "
                                           "(ffgmop)");
                                    *status = URL_PARSE_ERROR;
                                    continue;
                                }
                            ...
                            if (!fits_is_url_absolute(grpLocation2) &&
                                *grpLocation2 != '/') {
                                ...
                                if (strlen(cwd) + strlen(grpLocation2) + 1 >
                                    FLEN_FILENAME - 1) {
                                    ffpmsg("cwd and group location2 is too long "
                                           "(ffgmop)");
                                    *status = URL_PARSE_ERROR;
                                    continue;
                                }
...

{
    ...
    do {
            ...
            *status = fits_copy_hdu(tmpfptr, mfptr, 0, status);
            ...
            while (*status == 0) {
                ...
                *status = fits_get_hdrpos(mfptr, &numkeys, &keypos, status);
                /* SPR 1738 */
                *status = fits_read_keyn(mfptr, keypos - 1, keyname, value, comment,
                                         status);
        ...
        */
        ...
        if (strlen(extname) == 0) {
            if (fits_get_hdu_num(tmpfptr, &hdunum) == 1) {
                ...
                *status =
                        fits_write_key_str(mfptr, "EXTNAME", extname,
                                           "HDU was Formerly a Primary Array", status);
            } else {
                strcpy(extname, "DEFAULT");
                *status = fits_write_key_str(mfptr, "EXTNAME", extname,
                                             "default EXTNAME set by CFITSIO", status);
            }
        }
        ...
        *status = fits_modify_key_lng(mfptr, "EXTVER", 0, NULL, status);
        ...
        if (*status == KEY_NO_EXIST) {
            *status = 0;
            *status = fits_read_key_str(mfptr, "EXTNAME", extname, comment, status);
            *status = fits_insert_key_lng(mfptr, "EXTVER", 0, "Extension version ID",
                                          status);
        }
        ...
        */
        ...
        if (rmopt == OPT_RM_MBR) {
            ...
            *status = ffgmul(mfptr, 0, status);
            ...
            if (iomode != READONLY) *status = fits_delete_hdu(mfptr, &hdutype, status);
            ...
            */
            ...
            if (iomode == READWRITE) {
                ...
                *status =
                        fits_read_key_lng(gfptr, "EXTVER", &groupExtver, card, status);
                /* Now, if either the Fptr values are the same, or the root filenames
                   are the same, then assume these refer to the same file.
                */
                ...
                */
                ...
                for (index = 1, found = 0; index <= ngroups && *status == 0 && !found;
                     ++index) {
                    /* read the next GRPIDn keyword in the series */
                    ...
                    if (grpid == groupExtver && grpid > 0) {
                        ...
                        */
                        ...
                        if (*grplc != 0 && !fits_is_url_absolute(grplc) &&
                            ...
                            *status = fits_clean_url(grpLocation3, grplc, status);
                        }
...

{
    ...
    do {
        ...
        *status =
                fits_get_colnum(gfptr, CASESEN, "MEMBER_XTENSION", xtensionCol, status);
        ...
        *status = fits_get_colnum(gfptr, CASESEN, "MEMBER_VERSION", extverCol, status);
        ...
        *status =
                fits_get_colnum(gfptr, CASESEN, "MEMBER_POSITION", positionCol, status);
        ...
        *status =
                fits_get_colnum(gfptr, CASESEN, "MEMBER_LOCATION", locationCol, status);
        ...
        *status = fits_get_colnum(gfptr, CASESEN, "MEMBER_URI_TYPE", uriCol, status);
        ...
        */
        ...
        if (*xtensionCol && *extnameCol && *extverCol && *positionCol && *locationCol &&
            *uriCol)
            *grptype = GT_ID_ALL_URI;

        else if (*xtensionCol && *extnameCol && *extverCol && *locationCol && *uriCol)
...

{
    ...
    do {
        ...
        *status = fits_get_url(mfptr, mbrLocation1, mbrLocation2, NULL, NULL, NULL,
                               status);
        ...
        *status = fits_get_num_groups(mfptr, &ngroups, status);
        ...
        for (index = 1; index <= ngroups && *status == 0; ++index) {
            ...
            */
            ...
            if (*status == 0) *status = fits_delete_rows(gfptr, memberID, 1, status);
...

{
    ...
    */
    ...
    if (location == NULL) {
        ...
        *tmpLocation = 0;
        ...
        if (*tmpLocation != '/') {
            fits_get_cwd(cwd, status);
            if (strlen(cwd) + strlen(tmpLocation) + 1 > FLEN_FILENAME - 1) {
    ...
    */
    ...
    for (i = 1; i <= nmembers && *member == 0 && *status == 0; ++i) {
        if (xtensionCol != 0) {
            fits_read_col_str(gfptr, xtensionCol, i, 1, 1, nstr, tmpPtr, &dummy,
                              status);
        ...
        if (extverCol != 0) {
            fits_read_col_lng(gfptr, extverCol, i, 1, 1, 0, (long*)&mextver, &dummy,
                              status);
        ...
        if (positionCol != 0 && (grptype == GT_ID_POS || grptype == GT_ID_POS_URI)) {
            fits_read_col_int(gfptr, positionCol, i, 1, 1, 0, &mposition, &dummy,
                              status);
        ...
        */
        ...
        if (locationCol != 0) {
            fits_read_col_str(gfptr, locationCol, i, 1, 1, nstr, tmpPtr, &dummy,
                              status);
        ...
        if (*mbrLocation1 == 0) {
            ...
            *status = fits_get_url(gfptr, mbrLocation1, mbrLocation2, NULL, NULL, NULL,
                                   status);
            ...
            if (*mbrLocation1 != 0 && !fits_is_url_absolute(mbrLocation1) &&
                *mbrLocation1 != '/') {
                fits_get_cwd(cwd, status);
                if (strlen(cwd) + strlen(mbrLocation1) + 1 > FLEN_FILENAME - 1) {
            ...
            if (*mbrLocation2 != 0 && !fits_is_url_absolute(mbrLocation2) &&
                *mbrLocation2 != '/') {
                fits_get_cwd(cwd, status);
                if (strlen(cwd) + strlen(mbrLocation2) + 1 > FLEN_FILENAME - 1) {
                    ffpmsg("cwd and member locations are too long (ffgmf)");
                    *status = URL_PARSE_ERROR;
                    continue;
                }
        ...
        */
            ...
            *status = fits_get_url(gfptr, grpLocation1, grpLocation2, NULL, NULL, NULL,
                                   status);
            ...
            if (*grpLocation1 != 0) {
                ...
                if (!fits_is_url_absolute(grpLocation1) && *grpLocation1 != '/') {
                    fits_get_cwd(cwd, status);
                    if (strlen(cwd) + strlen(grpLocation1) + 1 > FLEN_FILENAME - 1) {
            ...
            if (*grpLocation2 != 0) {
                ...
                if (!fits_is_url_absolute(grpLocation2) && *grpLocation2 != '/') {
                    fits_get_cwd(cwd, status);
                    if (strlen(cwd) + strlen(grpLocation2) + 1 > FLEN_FILENAME - 1) {
                        ffpmsg("cwd and group locations are too long (ffgmf)");
                        *status = URL_PARSE_ERROR;
                        continue;
                    }
...

{
    ...
    char* includeList[] = {"*"};
    char* excludeList[] = {"EXTNAME", "EXTVER", "GRPNAME", "GRPID#",
                           "GRPLC#",  "THEAP",  "TDIM#",   "T????#"};
    ...
    do {
            ...
            case OPT_GCP_ALL:
                ...
                */
                ...
                for (i = 1; i <= nmembers && *status == 0; ++i) {
                    ...
                    *status =
                            fits_read_key_str(mfptr, "EXTNAME", keyvalue, card, status);
        ...
        *status = fits_get_hdrpos(outfptr, &numkeys, &keypos, status);
        ...
        while (*status == 0) {
            ...
            *status = fits_find_nextkey(infptr, includeList, 1, excludeList, nexclude,
                                        card, status);
...

{
    ...
    do {
        ...
        */
        ...
        for (i = 0; i < HDU->nHDU && !(HDU->position[i] == hdunum &&
 ...
 */
{
    ...
    */
    ...
    for (i = 0, j = 0, size = strlen(inpath), buff[0] = 0; i < size; j = strlen(buff)) {
        switch (inpath[i]) {
            case ':':
    ...
    for (i = 0, j = 0, size = strlen(inpath), buff[0] = 0;
        ...
        switch (inpath[i]) {
            ...
            case '-':
                ...
                if (!done) /* must be part of the directory path specification */
                    strcat(buff, "..");
                else {
    ...
    */
    ...
    for (i = 0, j = 0, firstColon = 1, size = strlen(inpath), buff[0] = 0; i < size;
         j = strlen(buff)) {
        switch (inpath[i]) {
            case ':':
 ...
 */
{
    ...
    */
    ...
    for (tmpStr = ffstrtok(buff, "/", &saveptr), outpath[0] = 0; tmpStr != NULL;
        ...
        */
        ...
        if (startURL != NULL) {
            if (strlen(tmpStr2) == 0)
                ...
                *status =
                        fits_path2url(tmpPtr, FLEN_FILENAME - i, startURL + i, status);
            }
        }
...
static grp_stack_item* grp_stack_append(grp_stack_item* last, grp_stack_data data);
