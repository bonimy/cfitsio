...
int main() {
    ...
    */
    ...
    double ongkey[3] = {14.1414141414141414, 15.1515151515151515, 16.1616161616161616};
    double ondkey[3] = {15.1515151515151515, 16.1616161616161616, 17.1717171717171717},
           indkey[3];
    ...
    */
    ...
    if (ffpkls(fptr, "key_pkls",
               "This is a very long string value that is continued over more than one "
               "keyword.",
               "fxpkls comment", &status) > 0)
        printf("ffpkls status = %d\n", status);

    if (ffplsw(fptr, &status) > 0) printf("ffplsw status = %d\n", status);
    ...
    */
    ...
    olkey = TLOGICAL;
    if (ffpky(fptr, TLOGICAL, "tlogical", &olkey, "tlogical comment", &status) > 0)
        printf("ffpky status = %d\n", status);
    ...
    odkey = TDOUBLE;
    if (ffpky(fptr, TDOUBLE, "tdouble", &odkey, "tdouble comment", &status) > 0)
        printf("ffpky status = %d\n", status);
    ...
    */
    ...
    if (ffpkyj(fptr, "BLANK", -99, "value to use for undefined pixels", &status) > 0)
    ...
    */
    ...
    if (anynull == 0) printf("ERROR: ffgpv_ did not detect null values\n");
    ...
    if (anynull == 0) printf("ERROR: ffgpf_ did not detect null values\n");
    ...
    */
    ...
    extend = -99;
    printf("\nRead back keywords:\n");
    ffghpr(fptr, 99, &simple, &bitpix, &naxis, naxes, &pcount, &gcount, &extend,
           &status);
    ...
    printf("%s\n", card);
    if (strncmp(card, "KEY_PREC= 'This", 15)) printf("ERROR in ffgrec\n");

    ffgkyn(fptr, 9, keyword, value, comment, &status);
    printf("%s : %s : %s :\n", keyword, value, comment);
    if (strncmp(keyword, "KEY_PREC", 8)) printf("ERROR in ffgkyn: %s\n", keyword);

    ffgcrd(fptr, keyword, card, &status);
    printf("%s\n", card);

    if (strncmp(keyword, card, 8)) printf("ERROR in ffgcrd: %s\n", keyword);

    ffgkey(fptr, "KY_PKNS1", value, comment, &status);
    printf("KY_PKNS1 : %s : %s :\n", value, comment);

    if (strncmp(value, "'first string'", 14)) printf("ERROR in ffgkey: %s\n", value);
    ...
    */
    ...
    /*
        ffcrtb(fptr, BINARY_TBL, nrows, tfields, ttype, tform, tunit, binname,
                &status);
    */
    ffibin(fptr, nrows, tfields, ttype, tform, tunit, binname, 0L, &status);
    ...
    */
    ...
    for (ii = 0; ii < tfields; ii++) {
        ...
        ffgbcl(fptr, ii + 1, ttype[0], tunit[0], cvalstr, &repeat, &scale, &zero,
               &jnulval, tdisp, &status);
        printf(" %s, %s, %c, %ld, %f, %f, %ld, %s.\n", ttype[0], tunit[0], cvalstr[0],
               repeat, scale, zero, jnulval, tdisp);
    }
    ...
    */
    ...
    if (ffmrhd(fptr, -1, &hdutype, &status) > 0) goto errstatus;
    ...
    */
    ...
    /*
      ...
      ################################
    */
    ffghtb(fptr, 99, &rowlen, &nrows, &tfields, ttype, tbcol, tform, tunit, tblname,
           &status);

    printf("\nASCII table: rowlen, nrows, tfields, extname: %ld %ld %d %s\n", rowlen,
           nrows, tfields, tblname);
    ...
    */
    ...
    for (ii = 0; ii < tfields; ii++) {
        ffgtcl(fptr, ii + 1, &typecode, &repeat, &width, &status);
        printf("%4s %3d %2ld %2ld", tform[ii], typecode, repeat, width);
        ffgacl(fptr, ii + 1, ttype[0], tbcol, tunit[0], tform[0], &scale, &zero, nulstr,
               tdisp, &status);
        printf(" %s, %ld, %s, %s, %f, %f, %s, %s.\n", ttype[0], tbcol[0], tunit[0],
               tform[0], scale, zero, nulstr, tdisp);
    }
    ...
    */
    ...
    if (ffirow(fptr, 2, 3, &status) > 0) goto errstatus;
    ...
    if (ffdrow(fptr, 10, 2, &status) > 0) goto errstatus;
    ...
    if (ffdcol(fptr, 3, &status) > 0) goto errstatus;
    ...
    if (fficol(fptr, 5, "INSERT_COL", "F14.6", &status) > 0) goto errstatus;
    ...
    */
    ...
    tfields = 0;
    rowlen = 0;
    ffitab(tmpfptr, rowlen, nrows, tfields, ttype, tbcol, tform, tunit, tblname,
           &status);
    ...
    ffibin(tmpfptr, nrows, tfields, ttype, tform, tunit, tblname, 0L, &status);
    ...
    */
    ...
    ffghbn(fptr, 99, &nrows, &tfields, ttype, tform, tunit, binname, &pcount, &status);

    printf("\nBinary table: nrows, tfields, extname, pcount: %ld %d %s %ld\n", nrows,
           tfields, binname, pcount);

    for (ii = 0; ii < tfields; ii++)
        printf("%8s %8s %8s \n", ttype[ii], tform[ii], tunit[ii]);

    for (ii = 0; ii < 40; ii++) larray[ii] = 0;
    ...
    for (jj = 0; jj < 5; jj++) {
        for (ii = 0; ii < 8; ii++) printf("%1d", larray[jj * 8 + ii]);
        printf(" ");
    }
    ...
    for (ii = 10; ii < nrows; ii++) {
        ...
        printf("%15s %d %3d %2d %3d \n", inskey[ii], larray[ii], xinarray[ii],
               binarray[ii], iinarray[ii]);
    }
    ffprec(fptr, "key_prec= 'This keyword was written by f_prec' / comment here",
           &status);
    ...
    */
    ...
    if (ffdcol(fptr, 6, &status) > 0) goto errstatus;
    ...
    if (fficol(fptr, 8, "INSERT_COL", "1E", &status) > 0) goto errstatus;
    ...
    */
    ...
    nrows = 22;
    tfields = 0;
    ffibin(tmpfptr, nrows, tfields, ttype, tform, tunit, binname, 0L, &status);
    printf("\nCreate binary table with 0 columns: ffibin status = %d\n", status);
    ...
    */
    ...
    ffg2di(fptr, 1, 0, 19, naxes[0], naxes[1], imgarray[0], &anynull, &status);
    ...
    lpixels[0] = 14;
    lpixels[1] = 14;
    ffpssi(fptr, 1, naxis, naxes, fpixels, lpixels, imgarray2[0], &status);
    ...
    ffgsvi(fptr, 1, naxis, naxes, fpixels, lpixels, inc, 0, imgarray[0], &anynull,
           &status);
    ...
    */
    ...
    for (ii = 1; ii <= 20; ii++) /* loop over rows 1 - 20 */
    {
        ...
        for (jj = 0; jj < ii; jj++) printf(" %2d", larray[jj]);
        printf(" %d\nX", status);
        ...
        for (jj = 0; jj < ii; jj++) printf(" %2d", boutarray[jj]);
        printf(" %d\nI", status);

        ffgcvi(fptr, 5, ii, 1, ii, 99, ioutarray, &anynull, &status);
        for (jj = 0; jj < ii; jj++) printf(" %2d", ioutarray[jj]);
        printf(" %d\nJ", status);

        ffgcvj(fptr, 6, ii, 1, ii, 99, joutarray, &anynull, &status);
        for (jj = 0; jj < ii; jj++) printf(" %2ld", joutarray[jj]);
        printf(" %d\nE", status);

        ffgcve(fptr, 7, ii, 1, ii, 99., eoutarray, &anynull, &status);
        for (jj = 0; jj < ii; jj++) printf(" %2.0f", eoutarray[jj]);
        printf(" %d\nD", status);

        ffgcvd(fptr, 8, ii, 1, ii, 99., doutarray, &anynull, &status);
        for (jj = 0; jj < ii; jj++) printf(" %2.0f", doutarray[jj]);
        printf(" %d\n", status);
    ...
    */
    ...
    for (ii = 0; ii < npixels; ii++) printf(" %2d", iinarray[ii]);
    printf("  %d (short)\n", anynull);
    for (ii = 0; ii < npixels; ii++) printf(" %2d", kinarray[ii]);
    printf("  %d (int)\n", anynull);
    for (ii = 0; ii < npixels; ii++) printf(" %2ld", jinarray[ii]);
    printf("  %d (long)\n", anynull);
    for (ii = 0; ii < npixels; ii++) printf(" %2.0f", einarray[ii]);
    ...
    */
    ...
    ffwldp(xpix, ypix, xrval, yrval, xrpix, yrpix, xinc, yinc, rot, ctype, &xpos, &ypos,
           &status);
    ...
    ffxypx(xpos, ypos, xrval, yrval, xrpix, yrpix, xinc, yinc, rot, ctype, &xpix, &ypix,
           &status);
...
errstatus: /* jump here on error */
    ...
    for (ii = 0; ii < 21; ii++) free(inskey[ii]);
    for (ii = 0; ii < 10; ii++) {
        free(ttype[ii]);
        free(tform[ii]);
        free(tunit[ii]);
