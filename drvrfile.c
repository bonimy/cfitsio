    ...
    if (*filename == '~') {
        if (filename[1] == '/') {
            ...
            if (cptr) {
                ...
                if (strlen(filename) > 1023) return (FILE_NOT_OPENED);
            ...
            if (strlen(pwd->pw_dir) + strlen(cptr) > 1023) return (FILE_NOT_OPENED);
        ...
        *diskfile = fopen(filename, mode);
        ...
        if (!(*diskfile) && (rwmode == READWRITE)) {
            ...
            if ((stbuf.st_mode & S_IFMT) == S_IFLNK) /* is this a soft link? */
            {
                ...
                {
                    if (strlen(filename) + 7 > 1023) return (FILE_NOT_OPENED);
...
*/
{
    if (fclose(handleTable[handle].fileptr)) return (FILE_NOT_CLOSED);
...
*/
{
    if (fflush(handleTable[handle].fileptr)) return (WRITE_ERROR);
    ...
    if (file_seek(handle, handleTable[handle].currentpos)) return (SEEK_ERROR);
...
*/
{
    ...
    if (fseeko(handleTable[handle].fileptr, (OFF_T)offset, 0) != 0) return (SEEK_ERROR);

#else

    if (fseek(handleTable[handle].fileptr, (OFF_T)offset, 0) != 0) return (SEEK_ERROR);
...
*/
{
    ...
    if (handleTable[hdl].last_io_op == IO_WRITE) {
        if (file_seek(hdl, handleTable[hdl].currentpos)) return (SEEK_ERROR);
    }
...
*/
{
    ...
    if (file_openfile(filename, 0, &diskfile)) {
        if (strlen(filename) > FLEN_FILENAME - 5) return (0);
                    ...
                    if (file_openfile(filename, 0, &diskfile)) {
                        ...
                        if (file_openfile(filename, 0, &diskfile)) {
                            ...
                            if (file_openfile(filename, 0, &diskfile)) {
                                ...
                                if (file_openfile(filename, 0, &diskfile)) {
                                    strcpy(filename,
                                           tmpfilename); /* restore original name */
...

{
    ...
    if (hdl != 1) return (1); /* can only read from stdin */
...
*/
{
    if (hdl != 2) return (1); /* can only write to stdout */

    if ((long)fwrite(buffer, 1, nbytes, stdout) != nbytes) return (WRITE_ERROR);
