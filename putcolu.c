...
*/
{
    ...
    if (tcode < 0) writemode = 0; /* this is a variable length column */
    ...
    if (ffgcprll(fptr, colnum, firstrow, largeelem, nelem, writemode, &scale, &zero,
                 tform, &twidth, &tcode, &maxelem, &startpos, &elemnum, &incre, &repeat,
    ...
    if (tcode == TSTRING) {
        ...
        if (!cstring) return (*status = MEMORY_ALLOCATION);
    ...
    } else if (tcode == TBYTE || tcode == TSHORT || tcode == TLONG ||
               tcode == TLONGLONG) {
        ...
        */
        ...
        switch (tcode) {
            case (TBYTE):
                ...
                for (ii = 0; ii < ntodo; ii++) ffpbyt(fptr, 1, &i1null, status);
                break;

            case (TSHORT):

                for (ii = 0; ii < ntodo; ii++) ffpbyt(fptr, 2, &i2null, status);
                break;

            case (TLONG):

                for (ii = 0; ii < ntodo; ii++) ffpbyt(fptr, 4, &i4null, status);
                break;

            case (TLONGLONG):

                for (ii = 0; ii < ntodo; ii++) ffpbyt(fptr, 8, &i8null, status);
                break;

            case (TFLOAT):

                for (ii = 0; ii < ntodo; ii++) ffpbyt(fptr, 4, jbuff, status);
                break;

            case (TDOUBLE):

                for (ii = 0; ii < ntodo; ii++) ffpbyt(fptr, 8, jbuff, status);
                break;

            case (TLOGICAL):

                for (ii = 0; ii < ntodo; ii++) ffpbyt(fptr, 1, &lognul, status);
                break;
