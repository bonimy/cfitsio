...
*/
{
    ...
    if (tbltype == BINARY_TBL) {
        ...
        ffphtb(fptr, naxis1, naxis2, tfields, ttype, tbcol, tform, tunit, extnm,
               status);
    } else
        *status = NOT_TABLE;
...
*/
{
    ...
    if (datatype == TSTRING) {
        ...
        ffpkyg(fptr, keyname, (double)*(unsigned int*)value, 0, comm, status);
    } else if (datatype == TINT) {
        ffpkyj(fptr, keyname, (LONGLONG) * (int*)value, comm, status);
    } else if (datatype == TLOGICAL) {
        ffpkyl(fptr, keyname, *(int*)value, comm, status);
    } else if (datatype == TULONG) {
        ffpkyuj(fptr, keyname, (ULONGLONG) * (unsigned long*)value, comm, status);
    } else if (datatype == TULONGLONG) {
        ffpkyuj(fptr, keyname, (ULONGLONG) * (ULONGLONG*)value, comm, status);
        ...
        snprintf(errmsg, FLEN_ERRMSG, "Bad keyword datatype code: %d (ffpky)",
                 datatype);
        ffpmsg(errmsg);
        *status = BAD_DATATYPE;
    }
...
*/
{
    ...
    if (!fits_strncasecmp("COMMENT ", tcard, 8) ||
        !fits_strncasecmp("HISTORY ", tcard, 8) ||
        !fits_strncasecmp("        ", tcard, 8) ||
        !fits_strncasecmp("CONTINUE", tcard, 8))
        keylength = 8;
    ...
    if (*status <= 0) (fptr->Fptr)->headend += 80; /* update end-of-header position */
...
*/
{
    ...
    while (remain > 0) {
        ...
        ffs2c(tstring, valstring,
              status); /* expand quotes, and put quotes around the string */
        ...
        {
            if (nocomment) {
                ffmkky("CONTINUE", valstring, NULL, card,
                       status); /* make keyword w/o comment */
            } else {
        ...
        /* make adjustment if necessary to allow reasonable room for a comment on last
           CONTINUE card only need to do this if a) there is a comment string, and b)
           the remaining value string characters could all fit on the next CONTINUE
           card, and c) there is not enough room on the next CONTINUE card for both the
           remaining value characters, and at least 47 characters of the comment string.
        */

        if (commlen > 0 && remain + nquote < 69 && remain + nquote + commlen > 65) {
            if (nchar > 18) { /* only false if there are a rediculous number of quotes
                                 in the string */
...
*/
{
    ...
    ffpcom(fptr, "  This FITS file may contain long string keyword values that are",
           status);
    ...
    ffpcom(fptr, "  character at the end of each substring which is then continued",
           status);
...
*/
{
    ...
    if (year < 0 || year > 9999) {
        snprintf(errmsg, FLEN_ERRMSG, "input year value = %d is out of range 0 - 9999",
                 year);
        ffpmsg(errmsg);
        return (*status = BAD_DATE);
    } else if (month < 1 || month > 12) {
        snprintf(errmsg, FLEN_ERRMSG, "input month value = %d is out of range 1 - 12",
                 month);
        ffpmsg(errmsg);
        return (*status = BAD_DATE);
    }
    ...
    if (ndays[month] == 31) {
        if (day < 1 || day > 31) {
            snprintf(errmsg, FLEN_ERRMSG,
                     "input day value = %d is out of range 1 - 31 for month %d", day,
                     month);
            ffpmsg(errmsg);
            return (*status = BAD_DATE);
        }
        ...
        if (day < 1 || day > 30) {
            snprintf(errmsg, FLEN_ERRMSG,
                     "input day value = %d is out of range 1 - 30 for month %d", day,
                     month);
            ffpmsg(errmsg);
            return (*status = BAD_DATE);
        }
        ...
        if (day < 1 || day > 28) {
            if (day == 29) {
                ...
                */
                ...
                snprintf(errmsg, FLEN_ERRMSG,
                         "input day value = %d is out of range 1 - 28 for February %d "
                         "(not leap year)",
                         day, year);
                ffpmsg(errmsg);
            } else {
                snprintf(errmsg, FLEN_ERRMSG,
                         "input day value = %d is out of range 1 - 28 (or 29) for "
                         "February",
                         day);
                ffpmsg(errmsg);
            }
...
*/
{
    ...
    *datestr = '\0';
    ...
    if (decimals < 0) {
        ...
        sprintf(datestr, "%.2d:%.2d:%0*.*f", hour, minute, width, decimals, second);
    } else {
        /* return both the time and date */
        sprintf(datestr, "%.4d-%.2d-%.2dT%.2d:%.2d:%0*.*f", year, month, day, hour,
                minute, width, decimals, second);
    }
    return (*status);
}
/*-----------------------------------------------------------------*/
...
*/
{
    ...
    if (datestr[2] == '/' || datestr[4] == '-') {
        ...
        if (ffs2dt(datestr, year, month, day, status) > 0) return (*status);
        ...
        if (slen == 8 || slen == 10)
            ...
            if (isdigit((int)datestr[11]) && isdigit((int)datestr[12]) &&
                ...
                if (hour) *hour = atoi(&datestr[11]);

                if (minute) *minute = atoi(&datestr[14]);

                if (second) *second = atof(&datestr[17]);
            } else {
                ffpmsg("input date string has illegal format:");
                ffpmsg(datestr);
                return (*status = BAD_DATE);
    ...
    } else /* no date fields */
    {
        if (datestr[2] == ':' && datestr[5] == ':') /* time string */
        {
            if (isdigit((int)datestr[0]) && isdigit((int)datestr[1]) &&
                ...
                if (hour) *hour = atoi(&datestr[0]);

                if (minute) *minute = atoi(&datestr[3]);

                if (second) *second = atof(&datestr[6]);
            } else {
                ffpmsg("input date string has illegal format:");
                ffpmsg(datestr);
                return (*status = BAD_DATE);
...
*/
{
    ...
    if (comm) {
        ...
        while (len > 0 && comm[0][len - 1] == ' ') len--; /* ignore trailing blanks */
...
*/
{
    ...
    if ((fptr->Fptr)->hdutype != BINARY_TBL) {
        ffpmsg("Error: The TDIMn keyword is only allowed in BINTABLE extensions "
               "(ffptdm)");
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (ii > 0) strcat(tdimstr, ","); /* append the comma separator */
...
*/
{
    ...
    for (ii = 0; (ii < naxis) && (ii < 20); ii++) naxesll[ii] = naxes[ii];
...
*/
{
    ...
    if (naxis != 0) /* never try to compress a null image */
    {
        if ((fptr->Fptr)->request_compress_type) {
            for (ii = 0; ii < naxis; ii++) tnaxes[ii] = (long)naxes[ii];
    ...
    if (ffpkyj(fptr, "BITPIX", longbitpix, comm, status) > 0) return (*status);
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (naxes[ii] < 0) {
            snprintf(message, FLEN_ERRMSG,
                     "Illegal negative value for NAXIS%d keyword: %.0f", ii + 1,
                     (double)(naxes[ii]));
            ffpmsg(message);
    ...
    if (bitpix == USHORT_IMG) {
        ...
        strcpy(card,
               "BZERO   =  9223372036854775808 / offset data range to that of unsigned "
               "long long");
        ffprec(fptr, card, status);
        strcpy(comm, "default scaling factor");
        ffpkyg(fptr, "BSCALE", 1.0, 0, comm, status);
...
*/
{
    ...
    extnm[0] = '\0';
    if (extnmx) strncat(extnm, extnmx, FLEN_VALUE - 1);
    ...
    for (ii = 0; ii < tfields; ii++) /* loop over every column */
    {
        ...
        if (tbcol[ii] < 1 || tbcol[ii] > rowlen) *status = BAD_TBCOL;
        ...
        if (*status > 0) break; /* abort loop on error */
    }

    if (extnm[0]) /* optional EXTNAME keyword */
        ffpkys(fptr, "EXTNAME", extnm, "name of this ASCII table extension", status);

    if (*status > 0) ffpmsg("Failed to write ASCII table header keywords (ffphtb)");

    if (gotmem) free(tbcol);
...
*/
{
    ...
    for (ii = 0; ii < tfields; ii++) /* sum the width of each field */
    {
        ...
        else if (tform[ii][0] == 'P' || tform[ii][1] == 'P' || tform[ii][0] == 'p' ||
                 tform[ii][1] == 'p')
    ...
    */
    ...
    for (ii = 0; ii < tfields; ii++) /* loop over every column */
    {
        ...
        if (datatype == TSTRING) {
            ...
            if (cptr) iread = sscanf(cptr, "%ld", &width);
    ...
    if (*status > 0) ffpmsg("Failed to write binary table header keywords (ffphbn)");
...
*/
{
    ...
    if (*status > 0) ffpmsg("Failed to write extension header keywords (ffphext)");
...
*/
{
    ...
    len = strlen(instr);
    if (len > 68) len = 68; /* limit input string to 68 chars */
