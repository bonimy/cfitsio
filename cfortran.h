...
#if defined(CLIPPERFortran) || defined(pgiFortran) || defined(__INTEL_COMPILER) || \
        defined(INTEL_COMPILER) || defined(gFortran)
#define f2cFortran
#endif
...
/* Split #if into 2 because some HP-UX can't handle long #if */
      ...
      defined(apolloFortran) || defined(sunFortran) || defined(IBMR2Fortran) || \
      defined(CRAYFortran))
...
#else
STIN int num_elem(strv, elem_len, term_char, num_term) char* strv;
unsigned elem_len;
int term_char;
int num_term;
#endif
...
#define PINT_cfVP(A, B) \
    int B __CFORTRAN__PCTYPE__UNUSED__ = (int)A; /* For ZSTRINGV_ARGS */
#else
#define PINT_cfVP(A, B)
#endif
#define PLOGICAL_cfVP(A, B) \
    int* B __CFORTRAN__PCTYPE__UNUSED__; /* Returning LOGICAL in FUNn and SUBn */
...
#define PSTRINGV_cfV(T, A, B, F)                                                      \
    static fstringvector B = {                                                        \
            0, DSC$K_DTYPE_T, DSC$K_CLASS_A, NULL, 0, 0, {0, 0, 1, 1, 1}, 1, 0, NULL, \
            0, {1, 0}};
#else
#define STRING_cfV(T, A, B, F)   \
    struct {                     \
...
#define ZTRINGV_cfA(M, I, A, B)                                                  \
    AATRINGV_cfA((char*)A, B, (_3(M, _ELEMS_, I)) * ((_3(M, _ELEMLEN_, I)) + 1), \
                 (_3(M, _ELEMS_, I)), (_3(M, _ELEMLEN_, I)) + 1)
#define PZTRINGV_cfA(M, I, A, B)                                                  \
    APATRINGV_cfA((char*)A, B, (_3(M, _ELEMS_, I)) * ((_3(M, _ELEMLEN_, I)) + 1), \
                  (_3(M, _ELEMS_, I)), (_3(M, _ELEMLEN_, I)) + 1)
...
#define PSTRINGV_cfW(A, B)                                                         \
    vkill_trailing(f2cstrv((char*)A, (char*)A, B.flen + 1, B.sizeofA), B.flen + 1, \
                   B.sizeofA, ' ');
#endif
 ...
 */
        ...
        (defined(VAXUltrix) || defined(__CF__APOLLO67) ||                              \
         (defined(sun) && !defined(__sun)) || defined(_CRAY) || defined(__ultrix__) || \
         (defined(__hpux) && defined(__CF__KnR)))
...
#define CFARGT14S(S, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE)          \
    S(T1, 1)                                                                          \
    S(T2, 2) S(T3, 3) S(T4, 4) S(T5, 5) S(T6, 6) S(T7, 7) S(T8, 8) S(T9, 9) S(TA, 10) \
            S(TB, 11) S(TC, 12) S(TD, 13) S(TE, 14)
#define CFARGT27S(S, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF, TG,  \
                  TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR)                         \
    S(T1, 1)                                                                          \
    S(T2, 2) S(T3, 3) S(T4, 4) S(T5, 5) S(T6, 6) S(T7, 7) S(T8, 8) S(T9, 9) S(TA, 10) \
            S(TB, 11) S(TC, 12) S(TD, 13) S(TE, 14) S(TF, 15) S(TG, 16) S(TH, 17)     \
...
#define CFARGT14FS(F, S, M, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE) \
    ...
    F(T2, 2, 1) F(T3, 3, 1) F(T4, 4, 1) F(T5, 5, 1) F(T6, 6, 1) F(T7, 7, 1)         \
            F(T8, 8, 1) F(T9, 9, 1) F(TA, 10, 1) F(TB, 11, 1) F(TC, 12, 1)          \
                    F(TD, 13, 1) F(TE, 14, 1) M CFARGT14S(                          \
                            S, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE)
#define CFARGT27FS(F, S, M, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE,    \
                   TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR)                 \
    F(T1, 1, 0)                                                                        \
    F(T2, 2, 1) F(T3, 3, 1) F(T4, 4, 1) F(T5, 5, 1) F(T6, 6, 1) F(T7, 7, 1) F(         \
            T8, 8, 1) F(T9, 9, 1) F(TA, 10, 1) F(TB, 11, 1) F(TC, 12, 1) F(TD, 13, 1)  \
            F(TE, 14, 1) F(TF, 15, 1) F(TG, 16, 1) F(TH, 17, 1) F(TI, 18, 1)           \
                    F(TJ, 19, 1) F(TK, 20, 1) F(TL, 21, 1) F(TM, 22, 1) F(TN, 23, 1)   \
                            F(TO, 24, 1) F(TP, 25, 1) F(TQ, 26, 1) F(TR, 27, 1)        \
                                    M CFARGT27S(S, T1, T2, T3, T4, T5, T6, T7, T8, T9, \
                                                TA, TB, TC, TD, TE, TF, TG, TH, TI,    \
                                                TJ, TK, TL, TM, TN, TO, TP, TQ, TR)
...
#define CFARGT14(F, S, M, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE) \
    ...
    F(T2, 2, 1) F(T3, 3, 1) F(T4, 4, 1) F(T5, 5, 1) F(T6, 6, 1) F(T7, 7, 1)       \
            F(T8, 8, 1) F(T9, 9, 1) F(TA, 10, 1) F(TB, 11, 1) F(TC, 12, 1)        \
                    F(TD, 13, 1) F(TE, 14, 1) M CFARGT14S(                        \
                            S, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE)
#define CFARGT27(F, S, M, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,  \
...
#define CFARGT20(F, S, M, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF, \
                 TG, TH, TI, TJ, TK)                                                  \
    F(T1, 1, 0)                                                                       \
    F(T2, 2, 1) F(T3, 3, 1) F(T4, 4, 1) F(T5, 5, 1) F(T6, 6, 1) F(T7, 7, 1) F(        \
            T8, 8, 1) F(T9, 9, 1) F(TA, 10, 1) F(TB, 11, 1) F(TC, 12, 1) F(TD, 13, 1) \
            F(TE, 14, 1) F(TF, 15, 1) F(TG, 16, 1) F(TH, 17, 1) F(TI, 18, 1)          \
                    F(TJ, 19, 1) F(TK, 20, 1) S(T1, 1) S(T2, 2) S(T3, 3) S(T4, 4)     \
                            S(T5, 5) S(T6, 6) S(T7, 7) S(T8, 8) S(T9, 9) S(TA, 10)    \
                                    S(TB, 11) S(TC, 12) S(TD, 13) S(TE, 14) S(TF, 15) \
                                            S(TG, 16) S(TH, 17) S(TI, 18) S(TJ, 19)   \
                                                    S(TK, 20)
#define CFARGTA14(F, S, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, A1, \
                  A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, AE)               \
    F(T1, A1, 1, 0)                                                                 \
    F(T2, A2, 2, 1) F(T3, A3, 3, 1) F(T4, A4, 4, 1) F(T5, A5, 5, 1) F(T6, A6, 6, 1) \
            F(T7, A7, 7, 1) F(T8, A8, 8, 1) F(T9, A9, 9, 1) F(TA, AA, 10, 1)        \
                    F(TB, AB, 11, 1) F(TC, AC, 12, 1) F(TD, AD, 13, 1)              \
                            F(TE, AE, 14, 1) S(T1, 1) S(T2, 2) S(T3, 3) S(T4, 4)    \
                                    S(T5, 5) S(T6, 6) S(T7, 7) S(T8, 8) S(T9, 9)    \
                                            S(TA, 10) S(TB, 11) S(TC, 12) S(TD, 13) \
                                                    S(TE, 14)
#if MAX_PREPRO_ARGS > 31
#define CFARGTA20(F, S, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,    \
                  TG, TH, TI, TJ, TK, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC,  \
                  AD, AE, AF, AG, AH, AI, AJ, AK)                                      \
    F(T1, A1, 1, 0)                                                                    \
    F(T2, A2, 2, 1) F(T3, A3, 3, 1) F(T4, A4, 4, 1) F(T5, A5, 5, 1) F(T6, A6, 6, 1) F( \
            T7, A7, 7, 1) F(T8, A8, 8, 1) F(T9, A9, 9, 1) F(TA, AA, 10, 1)             \
            F(TB, AB, 11, 1) F(TC, AC, 12, 1) F(TD, AD, 13, 1) F(TE, AE, 14, 1) F(     \
                    TF, AF, 15, 1) F(TG, AG, 16, 1) F(TH, AH, 17, 1) F(TI, AI, 18, 1)  \
                    F(TJ, AJ, 19, 1) F(TK, AK, 20, 1) S(T1, 1) S(T2, 2) S(T3, 3)       \
                            S(T4, 4) S(T5, 5) S(T6, 6) S(T7, 7) S(T8, 8) S(T9, 9)      \
                                    S(TA, 10) S(TB, 11) S(TC, 12) S(TD, 13) S(TE, 14)  \
                                            S(TF, 15) S(TG, 16) S(TH, 17) S(TI, 18)    \
                                                    S(TJ, 19) S(TK, 20)
#define CFARGTA27(F, S, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,    \
                  TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR, A1, A2, A3, A4, A5,  \
                  A6, A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK, AL, AM,  \
                  AN, AO, AP, AQ, AR)                                                  \
    F(T1, A1, 1, 0)                                                                    \
    F(T2, A2, 2, 1) F(T3, A3, 3, 1) F(T4, A4, 4, 1) F(T5, A5, 5, 1) F(T6, A6, 6, 1) F( \
            T7, A7, 7, 1) F(T8, A8, 8, 1) F(T9, A9, 9, 1) F(TA, AA, 10, 1)             \
            F(TB, AB, 11, 1) F(TC, AC, 12, 1) F(TD, AD, 13, 1) F(TE, AE, 14, 1) F(     \
                    TF, AF, 15, 1) F(TG, AG, 16, 1) F(TH, AH, 17, 1) F(TI, AI, 18, 1)  \
                    F(TJ, AJ, 19, 1) F(TK, AK, 20, 1) F(TL, AL, 21, 1) F(              \
                            TM, AM, 22, 1) F(TN, AN, 23, 1) F(TO, AO, 24, 1)           \
                            F(TP, AP, 25, 1) F(TQ, AQ, 26, 1) F(TR, AR, 27, 1) S(      \
                                    T1, 1) S(T2, 2) S(T3, 3) S(T4, 4) S(T5, 5)         \
                                    S(T6, 6) S(T7, 7) S(T8, 8) S(T9, 9) S(TA, 10) S(   \
                                            TB, 11) S(TC, 12) S(TD, 13) S(TE, 14)      \
                                            S(TF, 15) S(TG, 16) S(TH, 17) S(TI, 18) S( \
                                                    TJ, 19) S(TK, 20) S(TL, 21)        \
                                                    S(TM, 22) S(TN, 23) S(TO, 24)      \
                                                            S(TP, 25) S(TQ, 26)        \
                                                                    S(TR, 27)
#endif
#else
    ...
    S(T1, 1) F(T2, 2, 1) S(T2, 2) F(T3, 3, 1) S(T3, 3) F(T4, 4, 1) S(T4, 4)            \
            F(T5, 5, 1) S(T5, 5) F(T6, 6, 1) S(T6, 6) F(T7, 7, 1) S(T7, 7) F(T8, 8, 1) \
                    S(T8, 8) F(T9, 9, 1) S(T9, 9) F(TA, 10, 1) S(TA, 10) F(TB, 11, 1)  \
                            S(TB, 11) F(TC, 12, 1) S(TC, 12) F(TD, 13, 1) S(TD, 13)    \
                                    F(TE, 14, 1) S(TE, 14)
#define CFARGT27(F, S, M, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,  \
                 TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR)                       \
    F(T1, 1, 0)                                                                        \
    S(T1, 1) F(T2, 2, 1) S(T2, 2) F(T3, 3, 1) S(T3, 3) F(T4, 4, 1) S(T4, 4) F(         \
            T5, 5, 1) S(T5, 5) F(T6, 6, 1) S(T6, 6) F(T7, 7, 1) S(T7, 7) F(T8, 8, 1)   \
            S(T8, 8) F(T9, 9, 1) S(T9, 9) F(TA, 10, 1) S(TA, 10) F(TB, 11, 1) S(       \
                    TB, 11) F(TC, 12, 1) S(TC, 12) F(TD, 13, 1) S(TD, 13) F(TE, 14, 1) \
                    S(TE, 14) F(TF, 15, 1) S(TF, 15) F(TG, 16, 1) S(TG, 16) F(         \
                            TH, 17, 1) S(TH, 17) F(TI, 18, 1) S(TI, 18) F(TJ, 19, 1)   \
                            S(TJ, 19) F(TK, 20, 1) S(TK, 20) F(TL, 21, 1) S(TL, 21)    \
                                    F(TM, 22, 1) S(TM, 22) F(TN, 23, 1) S(TN, 23)      \
                                            F(TO, 24, 1) S(TO, 24) F(TP, 25, 1)        \
                                                    S(TP, 25) F(TQ, 26, 1) S(TQ, 26)   \
                                                            F(TR, 27, 1) S(TR, 27)
...
#define CFARGT20(F, S, M, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,  \
    ...
    S(T1, 1) F(T2, 2, 1) S(T2, 2) F(T3, 3, 1) S(T3, 3) F(T4, 4, 1) S(T4, 4) F(         \
            ...
            S(T8, 8) F(T9, 9, 1) S(T9, 9) F(TA, 10, 1) S(TA, 10) F(TB, 11, 1)          \
                    S(TB, 11) F(TC, 12, 1) S(TC, 12) F(TD, 13, 1) S(TD, 13)            \
                            F(TE, 14, 1) S(TE, 14) F(TF, 15, 1) S(TF, 15) F(TG, 16, 1) \
                                    S(TG, 16) F(TH, 17, 1) S(TH, 17) F(TI, 18, 1)      \
                                            S(TI, 18) F(TJ, 19, 1) S(TJ, 19)           \
                                                    F(TK, 20, 1) S(TK, 20)
#define CFARGTA14(F, S, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, A1,    \
                  A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, AE)                  \
    F(T1, A1, 1, 0)                                                                    \
    S(T1, 1) F(T2, A2, 2, 1) S(T2, 2) F(T3, A3, 3, 1) S(T3, 3) F(T4, A4, 4, 1)         \
            S(T4, 4) F(T5, A5, 5, 1) S(T5, 5) F(T6, A6, 6, 1) S(T6, 6) F(T7, A7, 7, 1) \
                    S(T7, 7) F(T8, A8, 8, 1) S(T8, 8) F(T9, A9, 9, 1) S(T9, 9)         \
                            F(TA, AA, 10, 1) S(TA, 10) F(TB, AB, 11, 1) S(TB, 11)      \
                                    F(TC, AC, 12, 1) S(TC, 12) F(TD, AD, 13, 1)        \
...
#define CFARGTA20(F, S, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,   \
                  ...
                  AD, AE, AF, AG, AH, AI, AJ, AK)                                     \
    F(T1, A1, 1, 0)                                                                   \
    S(T1, 1) F(T2, A2, 2, 1) S(T2, 2) F(T3, A3, 3, 1) S(T3, 3) F(T4, A4, 4, 1) S(     \
            T4, 4) F(T5, A5, 5, 1) S(T5, 5) F(T6, A6, 6, 1) S(T6, 6) F(T7, A7, 7, 1)  \
            S(T7, 7) F(T8, A8, 8, 1) S(T8, 8) F(T9, A9, 9, 1) S(T9, 9)                \
                    F(TA, AA, 10, 1) S(TA, 10) F(TB, AB, 11, 1) S(TB, 11) F(          \
                            TC, AC, 12, 1) S(TC, 12) F(TD, AD, 13, 1) S(TD, 13)       \
                            F(TE, AE, 14, 1) S(TE, 14) F(TF, AF, 15, 1) S(TF, 15)     \
                                    F(TG, AG, 16, 1) S(TG, 16) F(TH, AH, 17, 1)       \
                                            S(TH, 17) F(TI, AI, 18, 1) S(TI, 18)      \
                                                    F(TJ, AJ, 19, 1) S(TJ, 19)        \
...
#define CFARGTA27(F, S, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,    \
    ...
    S(T1, 1) F(T2, A2, 2, 1) S(T2, 2) F(T3, A3, 3, 1) S(T3, 3) F(T4, A4, 4, 1) S(      \
            T4, 4) F(T5, A5, 5, 1) S(T5, 5) F(T6, A6, 6, 1) S(T6, 6) F(T7, A7, 7, 1)   \
            S(T7, 7) F(T8, A8, 8, 1) S(T8, 8) F(T9, A9, 9, 1) S(T9, 9) F(              \
                    TA, AA, 10, 1) S(TA, 10) F(TB, AB, 11, 1) S(TB, 11)                \
                    F(TC, AC, 12, 1) S(TC, 12) F(TD, AD, 13, 1) S(TD, 13) F(           \
                            TE, AE, 14, 1) S(TE, 14) F(TF, AF, 15, 1) S(TF, 15)        \
                            F(TG, AG, 16, 1) S(TG, 16) F(TH, AH, 17, 1) S(TH, 17) F(   \
                                    TI, AI, 18, 1) S(TI, 18) F(TJ, AJ, 19, 1)          \
                                    S(TJ, 19) F(TK, AK, 20, 1) S(TK, 20) F(            \
                                            TL, AL, 21, 1) S(TL, 21) F(TM, AM, 22, 1)  \
                                            S(TM, 22) F(TN, AN, 23, 1) S(TN, 23)       \
                                                    F(TO, AO, 24, 1) S(TO, 24) F(      \
                                                            TP, AP, 25, 1) S(TP, 25)   \
...
#define PROTOCCALLSFSUB1(UN, LN, T1)                                              \
    PROTOCCALLSFSUB14(UN, LN, T1, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, \
                      CF_0, CF_0, CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB2(UN, LN, T1, T2)                                              \
    PROTOCCALLSFSUB14(UN, LN, T1, T2, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, \
                      CF_0, CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB3(UN, LN, T1, T2, T3)                                        \
    PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, \
                      CF_0, CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB4(UN, LN, T1, T2, T3, T4)                                  \
    PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, T4, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, \
                      CF_0, CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB5(UN, LN, T1, T2, T3, T4, T5)                                  \
    PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, \
                      CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB6(UN, LN, T1, T2, T3, T4, T5, T6)                            \
    PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, CF_0, CF_0, CF_0, CF_0, CF_0, \
                      CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB7(UN, LN, T1, T2, T3, T4, T5, T6, T7)                      \
    PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, CF_0, CF_0, CF_0, CF_0, \
                      CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB8(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8)                      \
    PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, CF_0, CF_0, CF_0, CF_0, \
                      CF_0, CF_0)
#define PROTOCCALLSFSUB9(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9)                \
    PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, CF_0, CF_0, CF_0, \
                      CF_0, CF_0)
#define PROTOCCALLSFSUB10(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA)         \
    PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, CF_0, CF_0, \
                      CF_0, CF_0)
#define PROTOCCALLSFSUB11(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB)         \
    PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, CF_0, CF_0, \
                      CF_0)
#define PROTOCCALLSFSUB12(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC)   \
    PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, CF_0, \
                      CF_0)
...
#define PROTOCCALLSFSUB15(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF)                                                     \
    PROTOCCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                      TF, CF_0, CF_0, CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB16(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG)                                                 \
    PROTOCCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                      TF, TG, CF_0, CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB17(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH)                                             \
    PROTOCCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                      TF, TG, TH, CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB18(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI)                                         \
    PROTOCCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                      TF, TG, TH, TI, CF_0, CF_0)
#define PROTOCCALLSFSUB19(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ)                                     \
    PROTOCCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                      TF, TG, TH, TI, TJ, CF_0)

#define PROTOCCALLSFSUB21(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK, TL)                             \
    PROTOCCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                      TF, TG, TH, TI, TJ, TK, TL, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB22(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK, TL, TM)                         \
    PROTOCCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                      TF, TG, TH, TI, TJ, TK, TL, TM, CF_0, CF_0, CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB23(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN)                     \
    PROTOCCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                      TF, TG, TH, TI, TJ, TK, TL, TM, TN, CF_0, CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB24(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO)                 \
    PROTOCCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                      TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, CF_0, CF_0, CF_0)
#define PROTOCCALLSFSUB25(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP)             \
    PROTOCCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                      TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, CF_0, CF_0)
#define PROTOCCALLSFSUB26(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ)         \
    PROTOCCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                      TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, CF_0)
...
#define PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE)                                                         \
    PROTOCCALLSFFUN14(VOID, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC,   \
                      TD, TE)
#define PROTOCCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK)                                 \
    PROTOCCALLSFFUN20(VOID, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC,   \
                      TD, TE, TF, TG, TH, TI, TJ, TK)
#define PROTOCCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR)     \
    PROTOCCALLSFFUN27(VOID, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC,   \
...
#define PROTOCCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE)                                                         \
    _(VOID, _cfPU)                                                                    \
    (CFC_(UN, LN))(CFARGT14(NCF, KCF, _Z, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, \
                            TC, TD, TE));
#define PROTOCCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK)                                 \
    _(VOID, _cfPU)                                                                    \
    (CFC_(UN, LN))(CFARGT20(NCF, KCF, _Z, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, \
                            TC, TD, TE, TF, TG, TH, TI, TJ, TK));
#define PROTOCCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR)     \
    _(VOID, _cfPU)                                                                    \
    (CFC_(UN, LN))(CFARGT27(NCF, KCF, _Z, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, \
...
#define CCALLSFSUB5(UN, LN, T1, T2, T3, T4, T5, A1, A2, A3, A4, A5)                    \
    CCALLSFSUB10(UN, LN, T1, T2, T3, T4, T5, CF_0, CF_0, CF_0, CF_0, CF_0, A1, A2, A3, \
                 A4, A5, 0, 0, 0, 0, 0)
#define CCALLSFSUB6(UN, LN, T1, T2, T3, T4, T5, T6, A1, A2, A3, A4, A5, A6)          \
    CCALLSFSUB10(UN, LN, T1, T2, T3, T4, T5, T6, CF_0, CF_0, CF_0, CF_0, A1, A2, A3, \
                 A4, A5, A6, 0, 0, 0, 0)
#define CCALLSFSUB7(UN, LN, T1, T2, T3, T4, T5, T6, T7, A1, A2, A3, A4, A5, A6, A7)    \
    CCALLSFSUB10(UN, LN, T1, T2, T3, T4, T5, T6, T7, CF_0, CF_0, CF_0, A1, A2, A3, A4, \
                 A5, A6, A7, 0, 0, 0)
#define CCALLSFSUB8(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, A1, A2, A3, A4, A5, A6,  \
                    A7, A8)                                                          \
    CCALLSFSUB10(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, CF_0, CF_0, A1, A2, A3, A4, \
                 A5, A6, A7, A8, 0, 0)
#define CCALLSFSUB9(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, A1, A2, A3, A4, A5,    \
                    A6, A7, A8, A9)                                                    \
    CCALLSFSUB10(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, CF_0, A1, A2, A3, A4, A5, \
                 A6, A7, A8, A9, 0)
#define CCALLSFSUB10(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, A1, A2, A3, A4, \
                     A5, A6, A7, A8, A9, AA)                                         \
    CCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, CF_0, CF_0, CF_0,   \
                 CF_0, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, 0, 0, 0, 0)
#define CCALLSFSUB11(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, A1, A2, A3,   \
                     A4, A5, A6, A7, A8, A9, AA, AB)                                   \
    CCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, CF_0, CF_0, CF_0, \
                 A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, 0, 0, 0)
#define CCALLSFSUB12(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, A1, A2, \
                     A3, A4, A5, A6, A7, A8, A9, AA, AB, AC)                         \
    CCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, CF_0, CF_0, \
                 A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, 0, 0)
#define CCALLSFSUB13(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, A1,   \
                     A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD)                   \
    CCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, CF_0, A1, \
                 A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, 0)

#ifdef __cplusplus
#define CPPPROTOCLSFSUB0(UN, LN)
#define CPPPROTOCLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE)
#define CPPPROTOCLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK)
#define CPPPROTOCLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                          TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR)
#else
...
#define CCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                     A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, AE)         \
    CCALLSFFUN14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, A1, \
                 A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, AE)
#else
...
#define CCALLSFSUB14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE,   \
    ...
    do {                                                                               \
        VVCF(T1, A1, B1)                                                               \
        VVCF(T2, A2, B2) VVCF(T3, A3, B3) VVCF(T4, A4, B4) VVCF(T5, A5, B5) VVCF(      \
                T6, A6, B6) VVCF(T7, A7, B7) VVCF(T8, A8, B8) VVCF(T9, A9, B9)         \
                VVCF(TA, AA, B10) VVCF(TB, AB, B11) VVCF(TC, AC, B12)                  \
                        VVCF(TD, AD, B13) VVCF(TE, AE, B14) CPPPROTOCLSFSUB14(         \
                                ...
                                ACF(LN, T3, A3, 3) ACF(LN, T4, A4, 4) ACF(             \
                                        LN, T5, A5, 5) ACF(LN, T6, A6, 6)              \
                                        ACF(LN, T7, A7, 7) ACF(LN, T8, A8, 8) ACF(     \
                                                LN, T9, A9, 9) ACF(LN, TA, AA, 10)     \
                                                ACF(LN, TB, AB, 11) ACF(               \
                                                        LN, TC, AC,                    \
                                                        12) ACF(LN, TD, AD, 13)        \
        ...
        WCF(T2, A2, 2) WCF(T3, A3, 3) WCF(T4, A4, 4) WCF(T5, A5, 5) WCF(T6, A6, 6)     \
                WCF(T7, A7, 7) WCF(T8, A8, 8) WCF(T9, A9, 9) WCF(TA, AA, 10)           \
                        WCF(TB, AB, 11) WCF(TC, AC, 12) WCF(TD, AD, 13)                \
                                WCF(TE, AE, 14)                                        \
    } while (0)
#endif
...
#if MAX_PREPRO_ARGS > 31
#define CCALLSFSUB15(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE,   \
                     TF, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, AE, AF)   \
    CCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,   \
                 CF_0, CF_0, CF_0, CF_0, CF_0, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, \
                 AB, AC, AD, AE, AF, 0, 0, 0, 0, 0)
#define CCALLSFSUB16(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                     TF, TG, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, AE, \
                     AF, AG)                                                         \
    CCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF, \
                 TG, CF_0, CF_0, CF_0, CF_0, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, \
                 AB, AC, AD, AE, AF, AG, 0, 0, 0, 0)
#define CCALLSFSUB17(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE,   \
                     TF, TG, TH, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD,   \
                     AE, AF, AG, AH)                                                   \
    CCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,   \
                 TG, TH, CF_0, CF_0, CF_0, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, \
                 AC, AD, AE, AF, AG, AH, 0, 0, 0)
#define CCALLSFSUB18(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                     TF, TG, TH, TI, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, \
                     AD, AE, AF, AG, AH, AI)                                         \
    CCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF, \
                 TG, TH, TI, CF_0, CF_0, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, \
                 AC, AD, AE, AF, AG, AH, AI, 0, 0)
#define CCALLSFSUB19(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE,   \
                     TF, TG, TH, TI, TJ, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB,   \
                     AC, AD, AE, AF, AG, AH, AI, AJ)                                   \
    CCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,   \
                 TG, TH, TI, TJ, CF_0, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, \
                 AD, AE, AF, AG, AH, AI, AJ, 0)

#ifdef CFSUBASFUN
#define CCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                     TF, TG, TH, TI, TJ, TK, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, \
                     AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK)                         \
    CCALLSFFUN20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF, \
                 TG, TH, TI, TJ, TK, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, \
                 AD, AE, AF, AG, AH, AI, AJ, AK)
#else
#define CCALLSFSUB20(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE,   \
    ...
    do {                                                                               \
        ...
        VVCF(T2, A2, B2) VVCF(T3, A3, B3) VVCF(T4, A4, B4) VVCF(T5, A5, B5) VVCF(      \
                T6, A6, B6) VVCF(T7, A7, B7) VVCF(T8, A8, B8) VVCF(T9, A9, B9)         \
                VVCF(TA, AA, B10) VVCF(TB, AB, B11) VVCF(TC, AC, B12) VVCF(            \
                        TD, AD, B13) VVCF(TE, AE, B14) VVCF(TF, AF, B15) VVCF(TG, AG,  \
                                                                              B16)     \
                        VVCF(TH, AH, B17) VVCF(TI, AI, B18) VVCF(TJ, AJ, B19) VVCF(    \
                                TK, AK, B20) CPPPROTOCLSFSUB20(UN, LN, T1, T2, T3, T4, \
                                                               T5, T6, T7, T8, T9, TA, \
                                                               TB, TC, TD, TE, TF, TG, \
                                                               TH, TI, TJ,             \
                                                               TK) ACF(LN, T1, A1, 1)  \
...
#if MAX_PREPRO_ARGS > 31
#define CCALLSFSUB21(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                     TF, TG, TH, TI, TJ, TK, TL, A1, A2, A3, A4, A5, A6, A7, A8, A9, \
                     AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK, AL)                 \
    CCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF, \
                 TG, TH, TI, TJ, TK, TL, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, A1, A2, \
                 A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, \
                 AK, AL, 0, 0, 0, 0, 0, 0)
#define CCALLSFSUB22(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE,   \
                     TF, TG, TH, TI, TJ, TK, TL, TM, A1, A2, A3, A4, A5, A6, A7, A8,   \
                     A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK, AL, AM)           \
    CCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,   \
                 TG, TH, TI, TJ, TK, TL, TM, CF_0, CF_0, CF_0, CF_0, CF_0, A1, A2, A3, \
                 A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK,   \
                 AL, AM, 0, 0, 0, 0, 0)
#define CCALLSFSUB23(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                     TF, TG, TH, TI, TJ, TK, TL, TM, TN, A1, A2, A3, A4, A5, A6, A7, \
                     A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK, AL, AM, AN) \
    CCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF, \
                 TG, TH, TI, TJ, TK, TL, TM, TN, CF_0, CF_0, CF_0, CF_0, A1, A2, A3, \
                 A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK, \
                 AL, AM, AN, 0, 0, 0, 0)
#define CCALLSFSUB24(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE,   \
                     TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, A1, A2, A3, A4, A5, A6,   \
                     A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK, AL, AM,   \
                     AN, AO)                                                           \
    CCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,   \
                 TG, TH, TI, TJ, TK, TL, TM, TN, TO, CF_0, CF_0, CF_0, A1, A2, A3, A4, \
                 A5, A6, A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK, AL,   \
                 AM, AN, AO, 0, 0, 0)
#define CCALLSFSUB25(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                     TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, A1, A2, A3, A4, A5, \
                     A6, A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK, AL, \
                     AM, AN, AO, AP)                                                 \
    CCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF, \
                 TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, CF_0, CF_0, A1, A2, A3, A4, \
                 A5, A6, A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK, AL, \
                 AM, AN, AO, AP, 0, 0)
#define CCALLSFSUB26(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE,   \
                     TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, A1, A2, A3, A4,   \
                     A5, A6, A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK,   \
                     AL, AM, AN, AO, AP, AQ)                                           \
    CCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF,   \
                 TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, CF_0, A1, A2, A3, A4, A5, \
                 A6, A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK, AL, AM,   \
                 AN, AO, AP, AQ, 0)

#ifdef CFSUBASFUN
#define CCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                     TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR, A1, A2, A3, \
                     A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, \
                     AK, AL, AM, AN, AO, AP, AQ, AR)                                 \
    CCALLSFFUN27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF, \
                 TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR, A1, A2, A3, A4, A5, \
                 A6, A7, A8, A9, AA, AB, AC, AD, AE, AF, AG, AH, AI, AJ, AK, AL, AM, \
                 AN, AO, AP, AQ, AR)
#else
#define CCALLSFSUB27(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE,                                 \
    ...
    do {                                                                                                             \
        VVCF(T1, A1, B1)                                                                                             \
        VVCF(T2, A2, B2) VVCF(T3, A3, B3) VVCF(T4, A4, B4) VVCF(T5, A5, B5) VVCF(                                    \
                T6, A6, B6) VVCF(T7, A7, B7) VVCF(T8, A8,                                                            \
                                                  B8) VVCF(T9, A9,                                                   \
                                                           B9) VVCF(TA, AA,                                          \
                                                                    B10) VVCF(TB, AB,                                \
                                                                              B11)                                   \
                VVCF(TC, AC, B12) VVCF(TD, AD, B13) VVCF(TE, AE, B14) VVCF(                                          \
                        TF, AF, B15) VVCF(TG, AG, B16) VVCF(TH, AH, B17) VVCF(TI, AI,                                \
                                                                              B18)                                   \
                        VVCF(TJ, AJ, B19) VVCF(TK, AK, B20) VVCF(TL, AL, B21) VVCF(                                  \
                                TM, AM,                                                                              \
                                B22) VVCF(TN, AN,                                                                    \
                                          B23) VVCF(TO, AO, B24)                                                     \
                                VVCF(TP, AP, B25) VVCF(TQ, AQ, B26) VVCF(                                            \
                                        TR, AR,                                                                      \
                                        B27) CPPPROTOCLSFSUB27(UN, LN, T1, T2, T3, T4,                               \
                                                               T5, T6,                                               \
                                                               T7, T8,                                               \
                                                               T9, TA,                                               \
                                                               TB, TC,                                               \
                                                               ...
                                                               TR) ACF(LN, T1, A1,                                   \
                                                                       1) ACF(LN, T2,                                \
                                                                              ...
                                                                              2) ACF(LN,                             \
                                                                                     ...
                                                                                     A3,                             \
                                                                                     3) ACF(LN, T4, A4, 4)           \
                                        ACF(LN, T5, A5, 5) ACF(LN, T6, A6, 6) ACF(LN, T7, A7, 7) ACF(                \
                                                LN,                                                                  \
                                                T8,                                                                  \
                                                A8,                                                                  \
                                                8) ACF(LN, T9, A9,                                                   \
                                                ...
                                                ACF(LN, TG, AG, 16) ACF(                                             \
                                                        ...
                                                        ACF(LN, TJ, AJ, 19) ACF(                                     \
                                                                ...
                                                                ACF(LN, TM, AM, 22) ACF(                             \
                                                                        ...
                                                                        AN,                                          \
                                                                        23) ACF(LN, TO, AO, 24)                      \
                                                                        ACF(LN, TP, AP, 25) ACF(LN, TQ, AQ, 26) ACF( \
                                                                                LN,                                  \
                                                                                TR,                                  \
                                                                                AR,                                  \
                                                                                27) CFC_(UN,                         \
        ...
        WCF(T1, A1, 1)                                                                                               \
        WCF(T2, A2, 2) WCF(T3, A3, 3) WCF(T4, A4, 4) WCF(T5, A5, 5) WCF(T6, A6, 6)                                   \
                WCF(T7, A7, 7) WCF(T8, A8, 8) WCF(T9, A9, 9) WCF(TA, AA, 10) WCF(                                    \
                        TB, AB, 11) WCF(TC, AC, 12) WCF(TD, AD, 13) WCF(TE, AE, 14)                                  \
                        WCF(TF, AF, 15) WCF(TG, AG, 16) WCF(TH, AH, 17)                                              \
                                WCF(TI, AI, 18) WCF(TJ, AJ, 19) WCF(TK, AK, 20) WCF(                                 \
                                        TL, AL, 21) WCF(TM, AM, 22) WCF(TN, AN, 23)                                  \
                                        WCF(TO, AO, 24) WCF(TP, AP, 25)                                              \
...
#define CCALLSFFUN5(UN, LN, T1, T2, T3, T4, T5, A1, A2, A3, A4, A5)                    \
    CCALLSFFUN10(UN, LN, T1, T2, T3, T4, T5, CF_0, CF_0, CF_0, CF_0, CF_0, A1, A2, A3, \
                 A4, A5, 0, 0, 0, 0, 0)
#define CCALLSFFUN6(UN, LN, T1, T2, T3, T4, T5, T6, A1, A2, A3, A4, A5, A6)          \
    CCALLSFFUN10(UN, LN, T1, T2, T3, T4, T5, T6, CF_0, CF_0, CF_0, CF_0, A1, A2, A3, \
                 A4, A5, A6, 0, 0, 0, 0)
#define CCALLSFFUN7(UN, LN, T1, T2, T3, T4, T5, T6, T7, A1, A2, A3, A4, A5, A6, A7)    \
    CCALLSFFUN10(UN, LN, T1, T2, T3, T4, T5, T6, T7, CF_0, CF_0, CF_0, A1, A2, A3, A4, \
                 A5, A6, A7, 0, 0, 0)
#define CCALLSFFUN8(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, A1, A2, A3, A4, A5, A6,  \
                    A7, A8)                                                          \
    CCALLSFFUN10(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, CF_0, CF_0, A1, A2, A3, A4, \
                 A5, A6, A7, A8, 0, 0)
#define CCALLSFFUN9(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, A1, A2, A3, A4, A5,    \
                    A6, A7, A8, A9)                                                    \
    CCALLSFFUN10(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, CF_0, A1, A2, A3, A4, A5, \
                 A6, A7, A8, A9, 0)
#define CCALLSFFUN10(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, A1, A2, A3, A4, \
                     A5, A6, A7, A8, A9, AA)                                         \
    CCALLSFFUN14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, CF_0, CF_0, CF_0,   \
                 CF_0, A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, 0, 0, 0, 0)
#define CCALLSFFUN11(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, A1, A2, A3,   \
                     A4, A5, A6, A7, A8, A9, AA, AB)                                   \
    CCALLSFFUN14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, CF_0, CF_0, CF_0, \
                 A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, 0, 0, 0)
#define CCALLSFFUN12(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, A1, A2, \
                     A3, A4, A5, A6, A7, A8, A9, AA, AB, AC)                         \
    CCALLSFFUN14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, CF_0, CF_0, \
                 A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, 0, 0)
#define CCALLSFFUN13(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, A1,   \
                     A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD)                   \
    CCALLSFFUN14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, CF_0, A1, \
                 A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, 0)

#define CCALLSFFUN14(UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, \
                     A1, A2, A3, A4, A5, A6, A7, A8, A9, AA, AB, AC, AD, AE)         \
    ((CFFUN(UN)(BCF(T1, A1, 0) BCF(T2, A2, 1) BCF(T3, A3, 1) BCF(T4, A4, 1) BCF(     \
            T5, A5, 1) BCF(T6, A6, 1) BCF(T7, A7, 1) BCF(T8, A8, 1) BCF(T9, A9, 1)   \
                        BCF(TA, AA, 1) BCF(TB, AB, 1) BCF(TC, AC, 1) BCF(            \
                                TD, AD, 1) BCF(TE, AE, 1) SCF(T1, LN, 1, A1)         \
                                SCF(T2, LN, 2, A2) SCF(T3, LN, 3, A3) SCF(           \
                                        T4, LN, 4, A4) SCF(T5, LN, 5, A5)            \
                                        SCF(T6, LN, 6, A6) SCF(T7, LN, 7, A7) SCF(   \
                                                T8, LN, 8, A8) SCF(T9, LN, 9, A9)    \
                                                SCF(TA, LN, 10, AA) SCF(TB, LN, 11,  \
                                                                        AB)          \
                                                        SCF(TC, LN, 12, AC) SCF(     \
                                                                TD, LN, 13, AD)      \
                                                                SCF(TE, LN, 14, AE))))
...
#define PROTOCCALLSFFUN10(T0, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA)         \
    PROTOCCALLSFFUN14(T0, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, CF_0, CF_0, \
                      CF_0, CF_0)
#define PROTOCCALLSFFUN11(T0, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB)   \
    PROTOCCALLSFFUN14(T0, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, CF_0, \
                      CF_0, CF_0)
#define PROTOCCALLSFFUN12(T0, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC) \
    PROTOCCALLSFFUN14(T0, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC,     \
                      CF_0, CF_0)
#define PROTOCCALLSFFUN13(T0, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                          TD)                                                         \
    PROTOCCALLSFFUN14(T0, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                      CF_0)

/* HP/UX 9.01 cc requires the blank between '_Icf(3,G,T0,UN,LN) CCCF(T1,1,0)' */

#ifndef __CF__KnR
#define PROTOCCALLSFFUN14(T0, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                          TD, TE)                                                     \
...
#define FCALLSCSUB14(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE)                                                               \
    FCALLSCFUN14(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE)
#define FCALLSCSUB15(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF)                                                           \
    FCALLSCFUN15(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF)
#define FCALLSCSUB16(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG)                                                       \
    FCALLSCFUN16(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG)
#define FCALLSCSUB17(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG, TH)                                                   \
    FCALLSCFUN17(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH)
#define FCALLSCSUB18(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG, TH, TI)                                               \
    FCALLSCFUN18(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI)
#define FCALLSCSUB19(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG, TH, TI, TJ)                                           \
    FCALLSCFUN19(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ)
#define FCALLSCSUB20(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG, TH, TI, TJ, TK)                                       \
    FCALLSCFUN20(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK)
#define FCALLSCSUB21(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG, TH, TI, TJ, TK, TL)                                   \
    FCALLSCFUN21(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL)
#define FCALLSCSUB22(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG, TH, TI, TJ, TK, TL, TM)                               \
    FCALLSCFUN22(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, TM)
#define FCALLSCSUB23(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN)                           \
    FCALLSCFUN23(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN)
#define FCALLSCSUB24(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO)                       \
    FCALLSCFUN24(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO)
#define FCALLSCSUB25(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP)                   \
    FCALLSCFUN25(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP)
#define FCALLSCSUB26(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ)               \
    FCALLSCFUN26(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ)
#define FCALLSCSUB27(CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD,   \
                     TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR)           \
    FCALLSCFUN27(VOID, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR)
...
#define FCALLSCFUN10(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA)         \
    FCALLSCFUN14(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, CF_0, CF_0, \
                 CF_0, CF_0)
#define FCALLSCFUN11(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB)   \
    FCALLSCFUN14(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, CF_0, \
                 CF_0, CF_0)
#define FCALLSCFUN12(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC)   \
    FCALLSCFUN14(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, CF_0, \
                 CF_0)
#define FCALLSCFUN13(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD)                                                             \
    FCALLSCFUN14(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 CF_0)


#define FCALLSCFUN15(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF)                                                     \
    FCALLSCFUN20(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, CF_0, CF_0, CF_0, CF_0, CF_0)
#define FCALLSCFUN16(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF, TG)                                                 \
    FCALLSCFUN20(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, CF_0, CF_0, CF_0, CF_0)
#define FCALLSCFUN17(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF, TG, TH)                                             \
    FCALLSCFUN20(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, CF_0, CF_0, CF_0)
#define FCALLSCFUN18(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF, TG, TH, TI)                                         \
    FCALLSCFUN20(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, CF_0, CF_0)
#define FCALLSCFUN19(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF, TG, TH, TI, TJ)                                     \
    FCALLSCFUN20(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, CF_0)
#define FCALLSCFUN20(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF, TG, TH, TI, TJ, TK)                                 \
    FCALLSCFUN27(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0)
#define FCALLSCFUN21(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF, TG, TH, TI, TJ, TK, TL)                             \
    FCALLSCFUN27(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, CF_0, CF_0, CF_0, CF_0, CF_0, CF_0)
#define FCALLSCFUN22(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF, TG, TH, TI, TJ, TK, TL, TM)                         \
    FCALLSCFUN27(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, TM, CF_0, CF_0, CF_0, CF_0, CF_0)
#define FCALLSCFUN23(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN)                     \
    FCALLSCFUN27(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, CF_0, CF_0, CF_0, CF_0)
#define FCALLSCFUN24(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO)                 \
    FCALLSCFUN27(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, CF_0, CF_0, CF_0)
#define FCALLSCFUN25(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP)             \
    FCALLSCFUN27(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, CF_0, CF_0)
#define FCALLSCFUN26(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, \
                     TD, TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ)         \
    FCALLSCFUN27(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, \
                 TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, CF_0)
...
#define FCALLSCFUN14(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC,  \
                     TD, TE)                                                          \
                ...
                TCF(LN, T1, 1, 0) TCF(LN, T2, 2, 1) TCF(LN, T3, 3, 1)                 \
                        TCF(LN, T4, 4, 1) TCF(LN, T5, 5, 1) TCF(LN, T6, 6, 1)         \
                                TCF(LN, T7, 7, 1) TCF(LN, T8, 8, 1) TCF(LN, T9, 9, 1) \
                                        TCF(LN, TA, 10, 1) TCF(LN, TB, 11, 1)         \
                                                TCF(LN, TC, 12, 1) TCF(LN, TD, 13, 1) \
                                                        TCF(LN, TE, 14, 1));          \
        _Icf(0, K, T0, 0, 0) CFARGT14S(RCF, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA,   \
                                       TB, TC, TD, TE) _(T0, _cfI)                    \
    }

#define FCALLSCFUN27(T0, CN, UN, LN, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC,                                 \
                     TD, TE, TF, TG, TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR)                                     \
                                 CFextern _(T0,_cfF)(UN,LN)                    \
 CFARGT27(NCF,DCF,ABSOFT_cf2(T0),T1,T2,T3,T4,T5,T6,T7,T8,T9,TA,TB,TC,TD,TE,TF,TG,TH,TI,TJ,TK,TL,TM,TN,TO,TP,TQ,TR) ) \
 {                                                                                                                   \
        CFARGT27S(QCF, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA, TB, TC, TD, TE, TF, TG,                               \
                  TH, TI, TJ, TK, TL, TM, TN, TO, TP, TQ, TR)                                                        \
        _Icf(2, UU, T0, A0, 0);                                                                                      \
        _Icf(0, L, T0, 0, 0)                                                                                         \
                CN(TCF(LN, T1, 1, 0) TCF(LN, T2, 2, 1) TCF(LN, T3, 3, 1) TCF(                                        \
                        LN, T4, 4, 1) TCF(LN, T5, 5, 1) TCF(LN, T6, 6, 1)                                            \
                           TCF(LN, T7, 7, 1) TCF(LN, T8, 8, 1) TCF(LN, T9, 9, 1) TCF(                                \
                                   LN, TA, 10, 1) TCF(LN, TB, 11, 1)                                                 \
                                   TCF(LN, TC, 12, 1) TCF(LN, TD, 13, 1) TCF(                                        \
                                           LN, TE, 14, 1) TCF(LN, TF, 15,                                            \
                                                              1) TCF(LN, TG, 16, 1)                                  \
                                           TCF(LN, TH, 17, 1) TCF(LN, TI, 18, 1) TCF(                                \
                                                   LN, TJ, 19, 1) TCF(LN, TK, 20, 1)                                 \
                                                   TCF(LN, TL, 21, 1) TCF(                                           \
                                                           LN, TM, 22, 1) TCF(LN, TN,                                \
                                                                              23, 1)                                 \
                                                           TCF(LN, TO, 24, 1) TCF(                                   \
                                                                   LN, TP, 25, 1)                                    \
                                                                   TCF(LN, TQ, 26,                                   \
                                                                       1) TCF(LN, TR,                                \
                                                                              27, 1));                               \
        _Icf(0, K, T0, 0, 0) CFARGT27S(RCF, T1, T2, T3, T4, T5, T6, T7, T8, T9, TA,                                  \
