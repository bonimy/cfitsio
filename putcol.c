...
*/
{
    ...
    if (datatype == TBYTE) {
        ...
        ffpprui(fptr, group, firstelem, nelem, (unsigned short*)array, status);
...
*/
{
    ...
    if (datatype == TBYTE) {
        ...
        ffppni(fptr, group, firstelem, nelem, (short*)array, *(short*)nulval, status);
    } else if (datatype == TUINT) {
        ffppnuk(fptr, group, firstelem, nelem, (unsigned int*)array,
                *(unsigned int*)nulval, status);
    } else if (datatype == TINT) {
        ffppnk(fptr, group, firstelem, nelem, (int*)array, *(int*)nulval, status);
    } else if (datatype == TULONG) {
        ffppnuj(fptr, group, firstelem, nelem, (unsigned long*)array,
                *(unsigned long*)nulval, status);
    } else if (datatype == TLONG) {
        ffppnj(fptr, group, firstelem, nelem, (long*)array, *(long*)nulval, status);
    } else if (datatype == TULONGLONG) {
        ffppnujj(fptr, group, firstelem, nelem, (ULONGLONG*)array, *(ULONGLONG*)nulval,
                 status);
    } else if (datatype == TLONGLONG) {
        ffppnjj(fptr, group, firstelem, nelem, (LONGLONG*)array, *(LONGLONG*)nulval,
                status);
    } else if (datatype == TFLOAT) {
        ffppne(fptr, group, firstelem, nelem, (float*)array, *(float*)nulval, status);
    } else if (datatype == TDOUBLE) {
        ffppnd(fptr, group, firstelem, nelem, (double*)array, *(double*)nulval, status);
    } else
        *status = BAD_DATATYPE;
...
*/
{
    ...
    if (datatype == TBYTE) {
        ffpssb(fptr, 1, naxis, naxes, blc, trc, (unsigned char*)array, status);
    } else if (datatype == TSBYTE) {
        ffpsssb(fptr, 1, naxis, naxes, blc, trc, (signed char*)array, status);
    } else if (datatype == TUSHORT) {
        ffpssui(fptr, 1, naxis, naxes, blc, trc, (unsigned short*)array, status);
    } else if (datatype == TSHORT) {
        ffpssi(fptr, 1, naxis, naxes, blc, trc, (short*)array, status);
    } else if (datatype == TUINT) {
        ffpssuk(fptr, 1, naxis, naxes, blc, trc, (unsigned int*)array, status);
    } else if (datatype == TINT) {
        ffpssk(fptr, 1, naxis, naxes, blc, trc, (int*)array, status);
    } else if (datatype == TULONG) {
        ffpssuj(fptr, 1, naxis, naxes, blc, trc, (unsigned long*)array, status);
    } else if (datatype == TLONG) {
        ffpssj(fptr, 1, naxis, naxes, blc, trc, (long*)array, status);
    } else if (datatype == TULONGLONG) {
        ffpssujj(fptr, 1, naxis, naxes, blc, trc, (ULONGLONG*)array, status);
    } else if (datatype == TLONGLONG) {
        ffpssjj(fptr, 1, naxis, naxes, blc, trc, (LONGLONG*)array, status);
    } else if (datatype == TFLOAT) {
        ffpsse(fptr, 1, naxis, naxes, blc, trc, (float*)array, status);
    } else if (datatype == TDOUBLE) {
        ffpssd(fptr, 1, naxis, naxes, blc, trc, (double*)array, status);
    } else
        *status = BAD_DATATYPE;
...
*/
{
    ...
    if (datatype == TBIT) {
        ...
        ffpclb(fptr, colnum, firstrow, firstelem, nelem, (unsigned char*)array, status);
    } else if (datatype == TSBYTE) {
        ffpclsb(fptr, colnum, firstrow, firstelem, nelem, (signed char*)array, status);
    } else if (datatype == TUSHORT) {
        ffpclui(fptr, colnum, firstrow, firstelem, nelem, (unsigned short*)array,
                status);
    } else if (datatype == TSHORT) {
        ffpcli(fptr, colnum, firstrow, firstelem, nelem, (short*)array, status);
    } else if (datatype == TUINT) {
        ffpcluk(fptr, colnum, firstrow, firstelem, nelem, (unsigned int*)array, status);
    } else if (datatype == TINT) {
...
*/
{
    ...
    if (datatype == TBYTE) {
        ...
        ffpcnk(fptr, colnum, firstrow, firstelem, nelem, (int*)array, *(int*)nulval,
               status);
    } else if (datatype == TULONG) {
        ffpcnuj(fptr, colnum, firstrow, firstelem, nelem, (unsigned long*)array,
                *(unsigned long*)nulval, status);
    } else if (datatype == TLONG) {
        ffpcnj(fptr, colnum, firstrow, firstelem, nelem, (long*)array, *(long*)nulval,
               status);
    } else if (datatype == TULONGLONG) {
        ...
        ffpcne(fptr, colnum, firstrow, firstelem, nelem, (float*)array, *(float*)nulval,
               status);
    } else if (datatype == TDOUBLE) {
        ...
        ffpcnl(fptr, colnum, firstrow, firstelem, nelem, (char*)array, *(char*)nulval,
               status);
    } else if (datatype == TSTRING) {
        ffpcns(fptr, colnum, firstrow, firstelem, nelem, (char**)array, (char*)nulval,
               status);
    } else
        *status = BAD_DATATYPE;
...
int ffiter(int n_cols, iteratorCol* cols, long offset, long n_per_loop,
           int (*work_fn)(long total_n, long offset, long first_n, long n_values,
                          int n_cols, iteratorCol* cols, void* userPointer),
...
*/
{
    ...
    for (jj = 0; jj < n_cols; jj++) {
        ...
        if (type != 0 && type != TBYTE && type != TSBYTE && type != TLOGICAL &&
            type != TSTRING && type != TSHORT && type != TINT && type != TLONG &&
            type != TFLOAT && type != TDOUBLE && type != TCOMPLEX && type != TULONG &&
            type != TUSHORT && type != TDBLCOMPLEX && type != TLONGLONG) {
            if (type < 0) {
                snprintf(message, FLEN_ERRMSG,
                         "Variable length array not allowed for output column number "
                         "%d (ffiter)",
                         jj + 1);
            } else {
                snprintf(message, FLEN_ERRMSG,
                         "Illegal datatype for column number %d: %d  (ffiter)", jj + 1,
                         cols[jj].datatype);
            }
        ...
        } else /* operating on FITS tables */
        {
            ...
            if (cols[jj].colnum < 1) {
                ...
                if (ffgcno(cols[jj].fptr, CASEINSEN, cols[jj].colname, &cols[jj].colnum,
                           status)) {
                    snprintf(message, FLEN_ERRMSG,
            ...
            if (tstatus) cols[jj].colname[0] = '\0';
    ...
    if (hdutype == IMAGE_HDU) /* get total number of pixels in the image */
    {
        ...
        for (ii = 1; ii < naxis; ii++) totaln *= naxes[ii];
    ...
    if (n_per_loop == 0) {
        ...
        for (jj = 1; jj < n_cols; jj++) {
            for (ii = 0; ii < jj; ii++) {
                if (cols[ii].fptr == cols[jj].fptr) break;
            }
    ...
    for (jj = 0; jj < n_cols; jj++) {
        ...
        if (hdutype == IMAGE_HDU) /* get total number of pixels in the image */
        {
            ...
            if (ffgtcl(cols[jj].fptr, cols[jj].colnum, &typecode, &rept, &width,
                       status) > 0)
                goto cleanup;
            ...
            if (typecode < 0) { /* if any variable length arrays, then the */
                ...
                if (cols[jj].iotype == OutputCol) {
                    snprintf(message, FLEN_ERRMSG,
                             "Variable length array not allowed for output column "
                             "number %d (ffiter)",
                             jj + 1);
                    ffpmsg(message);
                    return (*status = BAD_DATATYPE);
        ...
        if (hdutype == IMAGE_HDU || cols[jj].datatype == TSTRING) {
            ...
            if (abs(typecode) == TBYTE || abs(typecode) == TSHORT ||
                abs(typecode) == TLONG || abs(typecode) == TINT ||
            ...
            if (typecode < 0) {
                ...
                for (ii = 0; ii < totaln; ii++) {
                    ffgdes(cols[jj].fptr, cols[jj].colnum, frow + ii, &rowrept, NULL,
                           status);
                ...
                if (hdutype == ASCII_TBL) /* TNULLn value is a string */
                {
                    ...
                    if (tstatus) {
                        ...
                        if (*cptr == '\0') /* TNULLn is all blanks? */
                            ...
                            if (tstatus) tnull = LONG_MIN; /* choose smallest value */
                        }                                  /* to represent nulls */
                    }
        ...
        switch (cols[jj].datatype) {
            ...
            case TSTRING:
                ...
                if (stringptr) {
                    ...
                    if (stringptr[0]) {
                        ...
                        ffkeyn("TNULL", cols[jj].colnum, keyname, &tstatus);
                        ffgkys(cols[jj].fptr, keyname, nullstr, 0, &tstatus);
                        if (!tstatus) strncat(col[jj].null.stringnull, nullstr, rept);
                    } else {
            ...
            case TLONGLONG:
                ...
                if (abs(typecode) == TBYTE || abs(typecode) == TSHORT ||
                    abs(typecode) == TLONG || abs(typecode) == TLONGLONG ||
                    abs(typecode) == TINT) {
    ...
    while (nleft) {
        ...
        for (jj = 0; jj < n_cols; jj++) {
            if (cols[jj].iotype != OutputCol) {
                ...
                if (hdutype == IMAGE_HDU) {
                    if (ffgpv(cols[jj].fptr, cols[jj].datatype, felement,
                              cols[jj].repeat * ntodo, defaultnull, dataptr, &anynul,
                              status) > 0) {
                        break;
                    }
                } else {
                    ...
                    if (typecode < 0) {
                        ...
                        ffgdes(cols[jj].fptr, cols[jj].colnum, frow, &cols[jj].repeat,
                               NULL, status);
                    }

                    if (ffgcv(cols[jj].fptr, cols[jj].datatype, cols[jj].colnum, frow,
                              felement, cols[jj].repeat * ntodo, defaultnull, dataptr,
                              &anynul, status) > 0) {
                        break;
                    }
                }
        ...
        if (*status > 0) break; /* looks like an error occurred; quit immediately */

        /* call work function */

        if (hdutype == IMAGE_HDU)
            *status =
                    work_fn(totaln, offset, felement, ntodo, n_cols, cols, userPointer);
        else
        ...
        for (jj = 0; jj < n_cols; jj++) {
            if (cols[jj].iotype != InputCol) {
                ...
                if (memcmp(nullptr, &zeros, nbytes)) {
                    ...
                    if (hdutype == IMAGE_HDU) {
                        if (ffppn(cols[jj].fptr, cols[jj].datatype, felement,
                                  cols[jj].repeat * ntodo, dataptr, nullptr,
                                  &tstatus) > 0)
                            break;
                    } else {
                        ...
                        {
                            ffgdes(cols[jj].fptr, cols[jj].colnum, frow,
                                   &cols[jj].repeat, NULL, status);
                        }

                        if (ffpcn(cols[jj].fptr, cols[jj].datatype, cols[jj].colnum,
                                  frow, felement, cols[jj].repeat * ntodo, dataptr,
                                  nullptr, &tstatus) > 0)
                            break;
                    }
                } else {
                    ...
                    if (hdutype == IMAGE_HDU) {
                        if (ffppr(cols[jj].fptr, cols[jj].datatype, felement,
                                  cols[jj].repeat * ntodo, dataptr, &tstatus) > 0)
                            break;
                    } else {
                        if (ffgtcl(cols[jj].fptr, cols[jj].colnum, &typecode, &rept,
                                   &width, status) > 0)
                        ...
                        if (ffpcl(cols[jj].fptr, cols[jj].datatype, cols[jj].colnum,
                                  frow, felement, cols[jj].repeat * ntodo, dataptr,
                                  &tstatus) > 0)
                            break;
                    }
        ...
        if (*status) break; /* exit on any error */
