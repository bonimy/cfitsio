...
int zuncompress2mem(char* filename, FILE* diskfile, char** buffptr, size_t* buffsize,
                    void* (*mem_realloc)(void* p, size_t newsize), size_t* filesize,
                    int* status);

#if HAVE_BZIP2
/* prototype for .bz2 uncompression function (in this file) */
void bzip2uncompress2mem(char* filename, FILE* diskfile, int hdl, size_t* filesize,
                         int* status);
#endif
...
*/
{
    ...
    if (memTable[handle].mem_realloc) { /* explicit LONGLONG->size_t cast */
        ptr = (memTable[handle].mem_realloc)(*(memTable[handle].memaddrptr),
                                             (size_t)filesize);
        if (!ptr) {
        ...
        if (filesize > *(memTable[handle].memsizeptr)) {
            memset(ptr + *(memTable[handle].memsizeptr), 0,
...
*/
{
    if (strlen(outfile)) {
        stdin_outfile[0] = '\0';
        strncat(stdin_outfile, outfile,
                FLEN_FILENAME - 1); /* an output file is specified */
...
*/
{
    ...
    if (fwrite(memTable[handle].memaddr, 1, ((size_t)memTable[handle].fitsfilesize),
...
*/
{
    ...
    if (*(memTable[*hdl].memsizeptr) > (((size_t)memTable[*hdl].fitsfilesize) + 256L)) {
...
*/
{
    ...
    if (rwmode != READONLY) {
        ffpmsg("cannot open compressed input stream with WRITE access "
               "(mem_compress_stdin_open)");
        return (READONLY_FILE);
    }
...
*/
{
    ...
    status = iraf2mem(filename, memTable[*hdl].memaddrptr, memTable[*hdl].memsizeptr,
                      &filesize, &status);
...
*/
{
    ...
    *rootfile = '\0';
    ...
    while (*cptr == ' ') cptr++; /* skip leading blanks */
    ...
    if (offset > 0) fseek(diskfile, offset, 0); /* offset to start of the data */

    /* read the raw data into memory */
    ptr = *memTable[*hdl].memaddrptr + 2880;

    if (fread((char*)ptr, 1, datasize, diskfile) != datasize) status = READ_ERROR;
...
*/
{
    ...
    if (memTable[handle].fileptr != stdout) fclose(memTable[handle].fileptr);
...
*/
{
    if (offset > memTable[handle].fitsfilesize) return (END_OF_FILE);
...
*/
{
    ...
    if ((size_t)(memTable[hdl].currentpos + nbytes) > *(memTable[hdl].memsizeptr)) {
        ...
        newsize = maxvalue(
                (size_t)(((memTable[hdl].currentpos + nbytes - 1) / 2880) + 1) * 2880,
                *(memTable[hdl].memsizeptr) + memTable[hdl].deltasize);

        /* call the realloc function */
        ptr = (memTable[hdl].mem_realloc)(*(memTable[hdl].memaddrptr), newsize);
        if (!ptr) {
    ...
    memTable[hdl].fitsfilesize =
            maxvalue(memTable[hdl].fitsfilesize, memTable[hdl].currentpos);
    return (0);
}
            ...
            *status = mem_write(hdl, buf, nread);
            if (*status) {
                ...
                if (*status == MEMORY_ALLOCATION)
                    ffpmsg("Failed to reallocate memory while uncompressing bzip2 "
                           "file");
                return;
            }
            total_read += nread;
