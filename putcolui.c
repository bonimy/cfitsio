...
*/
{
    ...
    */
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_write_compressed_pixels(fptr, TUSHORT, firstelem, nelem, 0, array,
                                     &nullvalue, status);
        return (*status);
    }
...
*/
{
    ...
    */
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_write_compressed_pixels(fptr, TUSHORT, firstelem, nelem, 1, array,
                                     &nullvalue, status);
        return (*status);
    }
...
*/
{
    ...
    */
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_write_compressed_img(fptr, TUSHORT, fpixel, lpixel, 0, array, NULL,
                                  status);
        ...
        */
        ...
        switch (tcode) {
            case (TSHORT):
                ...
                ffu2fi2(&array[next], ntodo, scale, zero, (short*)buffer, status);
                ffpi2b(fptr, ntodo, incre, (short*)buffer, status);
                break;

            case (TLONGLONG):

                ffu2fi8(&array[next], ntodo, scale, zero, (LONGLONG*)buffer, status);
                ffpi8b(fptr, ntodo, incre, (long*)buffer, status);
                break;

            case (TBYTE):

                ffu2fi1(&array[next], ntodo, scale, zero, (unsigned char*)buffer,
                        status);
            ...
            case (TLONG):
                ...
                ffu2fi4(&array[next], ntodo, scale, zero, (INT32BIT*)buffer, status);
                ffpi4b(fptr, ntodo, incre, (INT32BIT*)buffer, status);
                break;

            case (TFLOAT):

                ffu2fr4(&array[next], ntodo, scale, zero, (float*)buffer, status);
                ffpr4b(fptr, ntodo, incre, (float*)buffer, status);
                break;

            case (TDOUBLE):
                ffu2fr8(&array[next], ntodo, scale, zero, (double*)buffer, status);
            ...
            case (TSTRING): /* numerical column in an ASCII table */
                ...
                {
                    ffu2fstr(&array[next], ntodo, scale, zero, cform, twidth,
                             (char*)buffer, status);
...
*/
{
    ...
    if (scale == 1. && zero == 32768.) {
        ...
        for (ii = 0; ii < ntodo; ii++) output[ii] = (*(short*)&input[ii]) ^ 0x8000;
...
*/
{
    ...
    if (scale == 1. && zero == 0.) {
        ...
        for (ii = 0; ii < ntodo; ii++) output[ii] = ((double)input[ii] - zero) / scale;
    }
    return (*status);
}
/*--------------------------------------------------------------------------*/
