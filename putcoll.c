        ...
        if (array[ii] != nulvalue) /* is this a good pixel? */
        {
            ...
            if (ngood) /* write previous string of good pixels */
            {
                ...
                /*  good values have already been written
                            if (ffpcll(fptr, colnum, fstrow, fstelm, ngood,
                   &array[ii-ngood], status) > 0) return(*status);
                */
                ngood = 0;
            }
...
*/
{
    ...
    if (ffgcprll(fptr, colnum, frow, fbyte, nbyte, 1, &dummyd, &dummyd, tform, &twidth,
                 &tcode, &maxelem, &bstart, &elemnum, &incre, &repeat, &rowlen,
