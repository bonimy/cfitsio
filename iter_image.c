...
main() {
    ...
    fits_iterate_data(n_cols, cols, offset, rows_per_loop, zero_image, 0L, &status);
...
/*--------------------------------------------------------------------------*/
int zero_image(long totalrows, long offset, long firstrow, long nrows, int ncols,
...
*/
{
    ...
    /*--------------------------------------------------------*/
    if (firstrow == 1) {
        if (ncols != 1) return (-1); /* number of columns incorrect */
