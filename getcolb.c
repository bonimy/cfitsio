...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TBYTE, firstelem, nelem, nullcheck,
    ...
    */
    ...
    ffgclb(fptr, 2, row, firstelem, nelem, 1, 1, nulval, array, &cdummy, anynul,
           status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TBYTE, firstelem, nelem, nullcheck, NULL,
    ...
    */
    ...
    ffgclb(fptr, 2, row, firstelem, nelem, 1, 2, 0, array, nularray, anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffg3db(fptr, group, nulval, ncols, naxis2, naxis1, naxis2, 1, array, anynul,
           status);
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TBYTE, fpixel, lpixel, inc, nullcheck,
                                 &nullvalue, array, NULL, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    if (ncols == naxis1 && nrows == naxis2) /* arrays have same size? */
    {
        ...
        ffgclb(fptr, 2, tablerow, 1, naxis1 * naxis2 * naxis3, 1, 1, nulval, array,
    ...
    for (jj = 0; jj < naxis3; jj++) {
        ...
        for (ii = 0; ii < naxis2; ii++) {
            if (ffgclb(fptr, 2, tablerow, nfits, naxis1, 1, 1, nulval, &array[narray],
...
*/
{
    ...
    if (naxis < 1 || naxis > 9) {
        snprintf(msg, FLEN_ERRMSG, "NAXIS = %d in call to ffgsvb is out of range",
                 naxis);
        ffpmsg(msg);
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TBYTE, blcll, trcll, inc, nullcheck, &nullvalue,
        ...
        if (trc[ii] < blc[ii]) {
            if (hdutype == IMAGE_HDU) {
                ...
                snprintf(msg, FLEN_ERRMSG,
                         "ffgsvb: illegal range specified for axis %ld", ii + 1);
                ffpmsg(msg);
                            ...
                            for (i3 = str[3] * dir[3]; i3 <= stp[3] * dir[3];
                                ...
                                for (i2 = str[2] * dir[2]; i2 <= stp[2] * dir[2];
                                    ...
                                    for (i1 = str[1] * dir[1]; i1 <= stp[1] * dir[1];
                                        ...
                                        if (ffgclb(fptr, numcol, row, felem, nelem,
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TBYTE, blcll, trcll, inc, nullcheck, NULL, array,
    ...
    */
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (trc[ii] < blc[ii]) {
            snprintf(msg, FLEN_ERRMSG, "ffgsvb: illegal range specified for axis %ld",
                     ii + 1);
            ffpmsg(msg);
            return (*status = BAD_PIX_NUM);
        }
...
*/
{
    ...
    */
    ...
    ffgclb(fptr, 1, row, firstelem, nelem, 1, 1, 0, array, &cdummy, &idummy, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgclb(fptr, colnum, firstrow, firstelem, nelem, 1, 1, nulval, array, &cdummy,
           anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgclb(fptr, colnum, firstrow, firstelem, nelem, 1, 2, dummy, array, nularray,
           anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgcprll(fptr, colnum, firstrow, firstelem, nelem, readcheck, &scale, &zero, tform,
             &twidth, &tcode, &maxelem2, &startpos, &elemnum, &incre, &repeat, &rowlen,
        ...
        */
        ...
        switch (tcode) {
            ...
            case (TSHORT):
                ...
                fffi2i1((short*)buffer, ntodo, scale, zero, nulcheck, (short)tnull,
            ...
            case (TLONGLONG):
                ...
                fffi8i1((LONGLONG*)buffer, ntodo, scale, zero, nulcheck, tnull, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TFLOAT):
                ffgr4b(fptr, readptr, ntodo, incre, (float*)buffer, status);
                fffr4i1((float*)buffer, ntodo, scale, zero, nulcheck, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TDOUBLE):
                ffgr8b(fptr, readptr, ntodo, incre, (double*)buffer, status);
                fffr8i1((double*)buffer, ntodo, scale, zero, nulcheck, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TSTRING):
                ...
                fffstri1((char*)buffer, ntodo, scale, zero, twidth, power, nulcheck,
