...
int main(int argc, char** argv) {
    ...
    if (0 == cmdok) {
        ...
        printf("smem -r         - unconditionally reset all shared memory "
               "segments\n\t\t(does not block, recovers zombie handles left by kill "
               "-9)\n");
        printf("smem -r id      - unconditionally reset specific shared memory "
               "segment\n");
    }
