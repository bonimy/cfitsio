...
main() {
    ...
    fits_iterate_data(n_cols, cols, offset, rows_per_loop, str_iter, 0L, &status);
...
/*--------------------------------------------------------------------------*/
int str_iter(long totalrows, long offset, long firstrow, long nrows, int ncols,
...
*/
{
    ...
    /*--------------------------------------------------------*/
    if (firstrow == 1) {
        if (ncols != 2) return (-1); /* number of columns incorrect */
