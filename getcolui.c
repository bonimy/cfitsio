...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TUSHORT, firstelem, nelem, nullcheck,
    ...
    */
    ...
    ffgclui(fptr, 2, row, firstelem, nelem, 1, 1, nulval, array, &cdummy, anynul,
            status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TUSHORT, firstelem, nelem, nullcheck, NULL,
    ...
    */
    ...
    ffgclui(fptr, 2, row, firstelem, nelem, 1, 2, 0, array, nularray, anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffg3dui(fptr, group, nulval, ncols, naxis2, naxis1, naxis2, 1, array, anynul,
            status);
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TUSHORT, fpixel, lpixel, inc, nullcheck,
                                 &nullvalue, array, NULL, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    if (ncols == naxis1 && nrows == naxis2) /* arrays have same size? */
    {
        ...
        ffgclui(fptr, 2, tablerow, 1, naxis1 * naxis2 * naxis3, 1, 1, nulval, array,
    ...
    for (jj = 0; jj < naxis3; jj++) {
        ...
        for (ii = 0; ii < naxis2; ii++) {
            if (ffgclui(fptr, 2, tablerow, nfits, naxis1, 1, 1, nulval, &array[narray],
...
*/
{
    ...
    if (naxis < 1 || naxis > 9) {
        snprintf(msg, FLEN_ERRMSG, "NAXIS = %d in call to ffgsvui is out of range",
                 naxis);
        ffpmsg(msg);
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TUSHORT, blcll, trcll, inc, nullcheck,
                                 &nullvalue, array, NULL, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (trc[ii] < blc[ii]) {
            snprintf(msg, FLEN_ERRMSG, "ffgsvui: illegal range specified for axis %ld",
                     ii + 1);
            ffpmsg(msg);
                                ...
                                for (i2 = str[2]; i2 <= stp[2]; i2 += incr[2]) {
                                    for (i1 = str[1]; i1 <= stp[1]; i1 += incr[1]) {
                                        felem = str[0] + (i1 - 1) * dsize[1] +
                                                ...
                                                (i7 - 1) * dsize[7] +
                                                (i8 - 1) * dsize[8];
                                        if (ffgclui(fptr, numcol, row, felem, nelem,
...
*/
{
    ...
    if (naxis < 1 || naxis > 9) {
        snprintf(msg, FLEN_ERRMSG, "NAXIS = %d in call to ffgsvi is out of range",
                 naxis);
        ffpmsg(msg);
        return (*status = BAD_DIMEN);
    }
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TUSHORT, blcll, trcll, inc, nullcheck, NULL,
    ...
    */
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (trc[ii] < blc[ii]) {
            snprintf(msg, FLEN_ERRMSG, "ffgsvi: illegal range specified for axis %ld",
                     ii + 1);
            ffpmsg(msg);
            return (*status = BAD_PIX_NUM);
        }
...
*/
{
    ...
    */
    ...
    ffgclui(fptr, 1, row, firstelem, nelem, 1, 1, 0, array, &cdummy, &idummy, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgclui(fptr, colnum, firstrow, firstelem, nelem, 1, 1, nulval, array, &cdummy,
            anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgclui(fptr, colnum, firstrow, firstelem, nelem, 1, 2, dummy, array, nularray,
            anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    while (remain) {
        ...
        */
        ...
        switch (tcode) {
            case (TSHORT):
                ffgi2b(fptr, readptr, ntodo, incre, (short*)&array[next], status);
                fffi2u2((short*)&array[next], ntodo, scale, zero, nulcheck,
                        (short)tnull, nulval, &nularray[next], anynul, &array[next],
                        status);
                break;
            case (TLONGLONG):

                ffgi8b(fptr, readptr, ntodo, incre, (long*)buffer, status);
                fffi8u2((LONGLONG*)buffer, ntodo, scale, zero, nulcheck, tnull, nulval,
            ...
            case (TFLOAT):
                ...
                fffr4u2((float*)buffer, ntodo, scale, zero, nulcheck, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TDOUBLE):
                ffgr8b(fptr, readptr, ntodo, incre, (double*)buffer, status);
                fffr8u2((double*)buffer, ntodo, scale, zero, nulcheck, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TSTRING):
                ...
                fffstru2((char*)buffer, ntodo, scale, zero, twidth, power, nulcheck,
