...
static int FnMeanSigma_short(short* array, long npix, int nullcheck, short nullvalue,
...
static int FnMeanSigma_int(int* array, long npix, int nullcheck, int nullvalue,
                           long* ngoodpix, double* mean, double* sigma, int* status);
static int FnMeanSigma_float(float* array, long npix, int nullcheck, float nullvalue,
                             long* ngoodpix, double* mean, double* sigma, int* status);
static int FnMeanSigma_double(double* array, long npix, int nullcheck, double nullvalue,
                              long* ngoodpix, double* mean, double* sigma, int* status);
...
static int FnNoise5_int(int* array, long nx, long ny, int nullcheck, int nullvalue,
                        long* ngood, int* minval, int* maxval, double* n2, double* n3,
                        double* n5, int* status);
...
static int FnNoise5_double(double* array, long nx, long ny, int nullcheck,
                           double nullvalue, long* ngood, double* minval,
                           double* maxval, double* n2, double* n3, double* n5,
                           int* status);

static int FnNoise3_short(short* array, long nx, long ny, int nullcheck,
                          short nullvalue, long* ngood, short* minval, short* maxval,
                          double* noise, int* status);
static int FnNoise3_int(int* array, long nx, long ny, int nullcheck, int nullvalue,
                        long* ngood, int* minval, int* maxval, double* noise,
                        int* status);
...
static int FnNoise3_double(double* array, long nx, long ny, int nullcheck,
                           double nullvalue, long* ngood, double* minval,
                           double* maxval, double* noise, int* status);

static int FnNoise1_short(short* array, long nx, long ny, int nullcheck,
                          short nullvalue, double* noise, int* status);
static int FnNoise1_int(int* array, long nx, long ny, int nullcheck, int nullvalue,
                        double* noise, int* status);
static int FnNoise1_float(float* array, long nx, long ny, int nullcheck,
                          float nullvalue, double* noise, int* status);
static int FnNoise1_double(double* array, long nx, long ny, int nullcheck,
                           double nullvalue, double* noise, int* status);
...
int fits_quantize_float(long row, float fdata[], long nxpix, long nypix, int nullcheck,
                        float in_null_value, float qlevel, int dither_method,
    ...
    */
    ...
    if (qlevel >= 0.) {
        ...
        FnNoise5_float(fdata, nxpix, nypix, nullcheck, in_null_value, &ngood, &minval,
                       &maxval, &noise2, &noise3, &noise5, &status);
        ...
        if (delta == 0.) return (0); /* don't quantize */
        ...
        FnNoise3_float(fdata, nxpix, nypix, nullcheck, in_null_value, &ngood, &minval,
                       &maxval, 0, &status);
    }
        ...
        if (row > 0) { /* dither the values when quantizing */
            for (i = 0; i < nx; i++) {
                if (dither_method == SUBTRACTIVE_DITHER_2 && fdata[i] == 0.0) {
                    ...
                    idata[i] = NINT((((double)fdata[i] - zeropt) / delta) +
                                    fits_rand_value[nextrand] - 0.5);
                }
...
/*---------------------------------------------------------------------------*/
int fits_quantize_double(long row, double fdata[], long nxpix, long nypix,
                         int nullcheck, double in_null_value, float qlevel,
    ...
    */
    ...
    int status, iseed = 0;
    long i, nx, ngood = 0;
    double stdev, noise2 = 0., noise3 = 0.,
                  noise5 = 0.;       /* MAD 2nd, 3rd, and 5th order noise values */
    ...
    if (qlevel >= 0.) {
        /* estimate background noise using MAD pixel differences */
        FnNoise5_double(fdata, nxpix, nypix, nullcheck, in_null_value, &ngood, &minval,
                        &maxval, &noise2, &noise3, &noise5, &status);
        ...
        /* only nned to calculate the min and max values */
        FnNoise3_double(fdata, nxpix, nypix, nullcheck, in_null_value, &ngood, &minval,
                        &maxval, 0, &status);
    }
...
*/
{
    ...
    if (mean || sigma) {
        FnMeanSigma_short(array, nx * ny, nullcheck, nullvalue, &ngood, &xmean, &xsigma,
                          status);
    ...
    if (noise1) {
        FnNoise1_short(array, nx, ny, nullcheck, nullvalue, &xnoise, status);

        *noise1 = xnoise;
    }

    if (minvalue || maxvalue || noise3) {
        FnNoise5_short(array, nx, ny, nullcheck, nullvalue, &ngood, &minval, &maxval,
                       &xnoise2, &xnoise3, &xnoise5, status);
...
*/
{
    ...
    /* need to calculate mean and/or sigma and/or limits? */
    if (mean || sigma) {
        FnMeanSigma_int(array, nx * ny, nullcheck, nullvalue, &ngood, &xmean, &xsigma,
                        status);
    ...
    if (noise1) {
        FnNoise1_int(array, nx, ny, nullcheck, nullvalue, &xnoise, status);

        *noise1 = xnoise;
    }

    if (minvalue || maxvalue || noise3) {
        FnNoise5_int(array, nx, ny, nullcheck, nullvalue, &ngood, &minval, &maxval,
                     &xnoise2, &xnoise3, &xnoise5, status);
...
*/
{
    ...
    /* need to calculate mean and/or sigma and/or limits? */
    if (mean || sigma) {
        FnMeanSigma_float(array, nx * ny, nullcheck, nullvalue, &ngood, &xmean, &xsigma,
                          status);
    ...
    if (noise1) {
        FnNoise1_float(array, nx, ny, nullcheck, nullvalue, &xnoise, status);

        *noise1 = xnoise;
    }

    if (minvalue || maxvalue || noise3) {
        FnNoise5_float(array, nx, ny, nullcheck, nullvalue, &ngood, &minval, &maxval,
                       &xnoise2, &xnoise3, &xnoise5, status);
    ...
    for (jj = 0; jj < ny; jj++) {
        ...
        for (ii++; ii < nx; ii++) {
            ...
            if (!(v3 == v4 && v4 == v5 && v5 == v6 && v6 == v7)) {
                ...
                differences5[nvals] = abs((6 * (int)v5) - (4 * (int)v3) -
                                          (4 * (int)v7) + (int)v1 + (int)v9);
                nvals++;
            } else {
    ...
    for (jj = 0; jj < ny; jj++) {
        ...
        for (ii++; ii < nx; ii++) {
            ...
            if (!(v3 == v4 && v4 == v5 && v5 == v6 && v6 == v7)) {
                ...
                tdiff = (6 * (LONGLONG)v5) - (4 * (LONGLONG)v3) - (4 * (LONGLONG)v7) +
                        (LONGLONG)v1 + (LONGLONG)v9;
                if (tdiff < 0)
    ...
    for (jj = 0; jj < ny; jj++) {
        ...
        for (ii++; ii < nx; ii++) {
            ...
            if (!(v3 == v4 && v4 == v5 && v5 == v6 && v6 == v7)) {
                ...
                differences5[nvals] =
                        (float)fabs((6 * v5) - (4 * v3) - (4 * v7) + v1 + v9);
                nvals++;
            } else {
                /* ignore constant background regions */
                ngoodpix++;
    ...
    if (nrows == 0) {
        ...
        for (ii = 0; ii < nrows; ii++) {
            if (fabs(diffs[ii] - xnoise) <= sigma) {
                if (jj != ii) diffs[jj] = diffs[ii];
                jj++;
            }
        }
        if (ii != jj) FnMeanSigma_double(diffs, jj, 0, 0.0, 0, &xnoise, &sigma, status);
    }
                ...
                for (iter = 0; iter < NITER; iter++) {
                    ...
                    for (ii = 0; ii < nvals; ii++) {
                        if (fabs(differences[ii] - mean) < SIGMA_CLIP * stdev) {
                            if (kk < ii) differences[kk] = differences[ii];
                            kk++;
                        }
                    }
                    if (kk == nvals) break;
                    ...
                    nvals = kk;
                    FnMeanSigma_short(differences, nvals, 0, 0, 0, &mean, &stdev,
                                      status);
                }
            }
        ...
        if (nvals < 2)
            ...
            if (stdev > 0.) {
                for (iter = 0; iter < NITER; iter++) {
                    ...
                    nvals = kk;
                    FnMeanSigma_double(differences, nvals, 0, 0, 0, &mean, &stdev,
                                       status);
                }
            }
