           ...
           int* status)      /* IO - error status                    */
{
    ...
    */
    ...
    if (bytepos < 0) return (*status = NEG_FILE_POS);
    ...
    if (*status <= 0) (fptr->Fptr)->bytepos = bytepos; /* save new file position */
    ...
    if (nbytes >= MINDIRECT) {
        ...
        for (ii = 0; ii < NIOBUF; ii++) /* flush any affected buffers to disk */
        {
            if ((fptr->Fptr)->bufrecnum[ii] >= recstart &&
                ...
                (fptr->Fptr)->bufrecnum[ii] =
                        -1; /* disassociate buffer from the file */
            }
        }

        /* move to the correct write position */
        if ((fptr->Fptr)->io_pos != filepos) ffseek(fptr->Fptr, filepos);
        ...
        if ((fptr->Fptr)->io_pos >= (fptr->Fptr)->filesize) /* at the EOF? */
        {
            ...
            if ((fptr->Fptr)->hdutype == ASCII_TBL)
                memset((fptr->Fptr)->iobuffer + (nbuff * IOBUFLEN), 32,
                       IOBUFLEN); /* blank fill */
            else
                memset((fptr->Fptr)->iobuffer + (nbuff * IOBUFLEN), 0,
                       IOBUFLEN); /* zero fill */
        } else {
            /* read next record */
            ffread(fptr->Fptr, IOBUFLEN, (fptr->Fptr)->iobuffer + (nbuff * IOBUFLEN),
                   status);
        ...
        (fptr->Fptr)->logfilesize =
                maxvalue((fptr->Fptr)->logfilesize, (LONGLONG)(recend + 1) * IOBUFLEN);
        (fptr->Fptr)->bytepos = filepos + nwrite + ntodo;
    } else {
        /* bufpos is the starting position in IO buffer */
        bufpos = (long)((fptr->Fptr)->bytepos -
                        ((LONGLONG)(fptr->Fptr)->bufrecnum[(fptr->Fptr)->curbuf] *
                         IOBUFLEN));
        nspace = IOBUFLEN - bufpos; /* amount of space left in the buffer */
        ...
        while (ntodo) {
            ...
            memcpy((fptr->Fptr)->iobuffer + ((fptr->Fptr)->curbuf * IOBUFLEN) + bufpos,
                   cptr, nwrite);
            ntodo -= nwrite; /* decrement remaining number of bytes */
            cptr += nwrite;
            (fptr->Fptr)->bytepos += nwrite; /* increment file position pointer */
            (fptr->Fptr)->dirty[(fptr->Fptr)->curbuf] =
                    TRUE; /* mark record as modified */

            if (ntodo) /* load next record into a buffer */
            {
                ffldrc(fptr, (long)((fptr->Fptr)->bytepos / IOBUFLEN), IGNORE_EOF,
                       status);
                bufpos = 0;
                nspace = IOBUFLEN;
            }
...
*/
{
    ...
    bufpos = (long)((fptr->Fptr)->bytepos -
                    ((LONGLONG)record * IOBUFLEN)); /* start pos */
    ...
    for (ii = 1; ii < ngroups; ii++) /* write all but the last group */
    {
        ...
        {
            (fptr->Fptr)->dirty[bcurrent] =
                    TRUE; /* mark record as having been modified */
            record++;
    ...
    (fptr->Fptr)->dirty[bcurrent] = TRUE; /* mark record as having been modified */
    (fptr->Fptr)->bytepos =
            (fptr->Fptr)->bytepos + (ngroups * gsize) + (ngroups - 1) * offset;
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (nbytes >= MINDIRECT) {
        ...
        for (ii = 0; ii < NIOBUF; ii++) /* flush any affected buffers to disk */
        {
            if ((fptr->Fptr)->dirty[ii] && (fptr->Fptr)->bufrecnum[ii] >= recstart &&
                (fptr->Fptr)->bufrecnum[ii] <= recend) {
        ...
        if ((fptr->Fptr)->curbuf < 0) /* no current data buffer for this file */
        {                             /* so reload the last one that was used */
            ffldrc(fptr, (long)(((fptr->Fptr)->bytepos) / IOBUFLEN), REPORT_EOF,
                   status);
        }
        ...
        while (ntodo) {
            ...
            memcpy(cptr,
                   (fptr->Fptr)->iobuffer + ((fptr->Fptr)->curbuf * IOBUFLEN) + bufpos,
                   nread);
           ...
           int* status)    /* IO - error status                 */
{
    ...
    */
    ...
    if (rstart >= (fptr->Fptr)->filesize) /* EOF? */
    {
        ...
        (fptr->Fptr)->logfilesize =
                maxvalue((fptr->Fptr)->logfilesize, rstart + IOBUFLEN);

        (fptr->Fptr)->dirty[nbuff] = TRUE; /* mark record as having been modified */
    } else                                 /* not EOF, so read record from disk */
    {
        if ((fptr->Fptr)->io_pos != rstart) ffseek(fptr->Fptr, rstart);
...
updatebuf:
    ...
    if (ibuff < 0) {
        ...
        for (ibuff = 0; ibuff < NIOBUF; ibuff++)
            if ((fptr->Fptr)->ageindex[ibuff] == nbuff) break;
    }
           ...
           int* status)    /* IO - error status                  */
{
    ...
    */
    ...
    if (filepos <= Fptr->filesize) {
        ...
        /* move to the correct write position */
        if (Fptr->io_pos != filepos) ffseek(Fptr, filepos);
    ...
    {
        /* move to EOF */
        if (Fptr->io_pos != Fptr->filesize) ffseek(Fptr, Fptr->filesize);
...
*/
{
    ...
    {
        *ndata = (long)(((NIOBUF - 1) * IOBUFLEN) /
                        maxvalue(1, (fptr->Fptr)->rowlength));
...
*/
{
    ...
    bytepos = (fptr->Fptr)->datastart + ((fptr->Fptr)->rowlength * (firstrow - 1)) +
              firstchar - 1;
...
*/
{
    ...
    if (endrow > (fptr->Fptr)->numrows) {
        ...
        if (!((fptr->Fptr)->lasthdu) || (fptr->Fptr)->heapsize > 0) {
            ...
            (fptr->Fptr)->heapstart += ((LONGLONG)(endrow - (fptr->Fptr)->numrows) *
                                        (fptr->Fptr)->rowlength);
...
*/
{
    ...
    for (ii = 0; ii < nvals; ii++) values[ii] *= 0.25;
