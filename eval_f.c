...
/*---------------------------------------------------------------------------*/
{
    ...
    FFLOCK;
    if (ffiprs(fptr, 0, expr, MAXDIMS, &Info.datatype, &nelem, &naxis, naxes, status)) {
        ffcprs();
        FFUNLOCK;
        return (*status);
    }
    ...
    if (constant) { /* No need to call parser... have result from ffiprs */
        ...
        *n_good_rows = nrows;
        for (elem = 0; elem < nrows; elem++) row_status[elem] = result;
    } else {
        ...
        if (ffiter(gParse.nCols, gParse.colData, firstrow - 1, 0, parse_data,
                   (void*)&Info, status) == -1)
...
/*--------------------------------------------------------------------------*/
{
    ...
    FFLOCK;
    if (ffiprs(infptr, 0, expr, MAXDIMS, &Info.datatype, &nelem, &naxis, naxes,
               status)) {
        ffcprs();
        FFUNLOCK;
        return (*status);
    ...
    if ((outfptr->Fptr)->datastart < 0) ffrdef(outfptr, status);
    if (*status) {
        ffcprs();
        FFUNLOCK;
        return (*status);
    ...
    if (!outExt.numRows) (outfptr->Fptr)->heapsize = 0L;
    ...
    if (constant) { /*  Set all rows to the same value from constant result  */
        ...
        ffiter(gParse.nCols, gParse.colData, 0L, 0L, parse_data, (void*)&Info, status);
    ...
    if (*status) {
        ...
        if (infptr == outfptr) { /* Skip initial good rows if input==output file */
            ...
            if (outloc > 1) ffirow(outfptr, outExt.numRows, nGood, status);
        }
        ...
        if (infptr == outfptr) {
            ...
            ffukyj(outfptr, "PCOUNT", inExt.heapSize + outExt.heapSize, NULL, status);
...
/*---------------------------------------------------------------------------*/
{
    ...
    FFLOCK;
    if (ffiprs(fptr, 0, expr, MAXDIMS, &Info.datatype, &nelem1, &naxis, naxes,
               status)) {
        ffcprs();
        FFUNLOCK;
        return (*status);
    ...
    if (ffiter(gParse.nCols, gParse.colData, firstrow - 1, 0, parse_data, (void*)&Info,
               status) == -1)
        *status = 0; /* -1 indicates exitted without error before end... OK */
...
/* a row range of 1-MAX.                                                    */
{
    ...
    return ffcalc_rng(infptr, expr, outfptr, parName, parInfo, 1, &start, &end, status);
}
...
/*--------------------------------------------------------------------------*/
{
        ...
        *status = 0;
        if (parName[0] == '#') {
            ...
            if ((fits_strcasecmp(parName, "HISTORY") == 0 ||
                 fits_strcasecmp(parName, "COMMENT") == 0) &&
        ...
        if (colNo < 0) {
            ...
            *status = 0;
            ...
            if (parInfo == NULL || *parInfo == '\0') {
                ...
                if (Info.datatype == TBIT && *parInfo == 'B') nelem = (nelem + 7) / 8;
                snprintf(tform, 16, "%ld%s", nelem, parInfo);
                parInfo = tform;
            }
            fficol(outfptr, colNo, parName, parInfo, status);
            if (naxis > 1) ffptdm(outfptr, colNo, naxis, naxes, status);
    ...
    if (colNo > 0) {
        ...
        fits_iter_set_by_num(gParse.colData + col_cnt, outfptr, colNo, 0, OutputCol);
        gParse.nCols++;
        ...
        for (i = 0; i < nRngs; i++) {
           ...
           */
            ...
            if ((Info.maxRows >= 10) && (nRngs == 1) && (start[0] == 1) &&
                (end[0] == totaln))
                nPerLp = 0;
            else
                nPerLp = Info.maxRows;

            if (ffiter(gParse.nCols, gParse.colData, start[i] - 1, nPerLp, parse_data,
                       (void*)&Info, status) == -1)
                *status = 0;
            else if (*status) {
                ffcprs();
...
/*--------------------------------------------------------------------------*/
{
    ...
    if (gParse.hdutype == IMAGE_HDU) {
        ...
        for (i = 0; i < xaxis; ++i) gParse.totalRows *= xaxes[i];
        if (DEBUG_PIXFILTER)
    ...
    *nelem = result->value.nelem;
    for (i = 0; i < *naxis && i < maxdim; i++) naxes[i] = result->value.naxes[i];
...
/*---------------------------------------------------------------------------*/
{
    ...
    if (firstrow == offset + 1) {
        ...
        if (userInfo->dataPtr == NULL) {
            ...
            jnull = 0;
            if (gParse.hdutype == IMAGE_HDU) {
                if (gParse.pixFilter->blank) jnull = (LONGLONG)gParse.pixFilter->blank;
            } else {
                ffgknjj(outcol->fptr, "TNULL", outcol->colnum, 1, &jnull, (int*)&jj,
                        &status);
        ...
        switch (result->type) {
            ...
            case DOUBLE:
                if (constant) {
                    ...
                    for (kk = 0; kk < ntodo; kk++)
                        for (jj = 0; jj < repeat; jj++)
                            ffcvtn(gParse.datatype, &(result->value.data), &undef,
                                   result->value.nelem /* 1 */, userInfo->datatype,
                                   Null, (char*)Data + (kk * repeat + jj) * datasize,
                                   &anyNullThisTime, &gParse.status);
                } else {
                    if (repeat == result->value.nelem) {
                        ffcvtn(gParse.datatype, result->value.data.ptr,
                               result->value.undef, result->value.nelem * ntodo,
                               userInfo->datatype, Null, Data, &anyNullThisTime,
                               &gParse.status);
                ...
                if (gParse.status == OVERFLOW_ERR) {
                    ...
                    ffpmsg("Numerical overflow while converting expression to "
                           "necessary datatype");
                }
                break;
    ...
    for (i = 0; i < nCols; i++) {
        ...
        switch (varData->type) {
            ...
            case STRING:
                ...
                if (varData->undef) free(varData->undef);
        ...
        if (gParse.status) { /*  Deallocate NULL arrays of previous columns */
            while (i--) {
                ...
                if (varData->type == BITSTR) FREE(((char**)varData->data)[0]);
                FREE(varData->undef);
                varData->undef = NULL;
            }
            return;
        ...
        case TBYTE:
            switch (inputType) {
                ...
                case TBYTE:
                    ...
                    break;
                case TSHORT:
                    fffi2i1((short*)input, ntodo, 1., 0., 0, 0, 0, NULL, NULL,
        ...
        case TINT:
            switch (inputType) {
                ...
                case TBYTE:
                    ...
                    break;
                case TSHORT:
                    for (i = 0; i < ntodo; i++) ((int*)output)[i] = ((short*)input)[i];
                    break;
                case TLONG:
                    for (i = 0; i < ntodo; i++) ((int*)output)[i] = ((long*)input)[i];
                    break;
                case TFLOAT:
        ...
        case TLONG:
            switch (inputType) {
                ...
                case TBYTE:
                    ...
                    break;
                case TSHORT:
                    for (i = 0; i < ntodo; i++) ((long*)output)[i] = ((short*)input)[i];
                    break;
                case TLONG:
                    for (i = 0; i < ntodo; i++) ((long*)output)[i] = ((long*)input)[i];
                    break;
                case TFLOAT:
...
/*---------------------------------------------------------------------------*/
{
    ...
    if (ffiprs(fptr, 1, expr, MAXDIMS, &Info.datatype, &nelem, &naxis, naxes, status)) {
        ffcprs();
        return (*status);
    }
    if (nelem < 0) {
    ...
    while (parNo--) {
        switch (gParse.colData[parNo].datatype) {
            ...
            case TDOUBLE:
                ...
                break;
            case TSTRING:
                if (!fits_get_coltype(fptr, gParse.valCol, &typecode, &alen, &width,
                                      status)) {
                    alen++;
                    if ((gParse.colData[parNo].array =
                                 (char**)malloc((ntimes + 1) * sizeof(char*)))) {
                        if ((((char**)gParse.colData[parNo].array)[0] = (char*)malloc(
                                     (ntimes + 1) * sizeof(char) * alen))) {
                            for (elem = 1; elem <= ntimes; elem++)
                                ((char**)gParse.colData[parNo].array)[elem] =
                                        ((char**)gParse.colData[parNo]
                                                 .array)[elem - 1] +
                                        alen;
    ...
    if (!uncompress_hkdata(fptr, ntimes, times, status)) {
        if (constant) {
            ...
            Info.nullPtr = NULL;
            Info.maxRows = ntimes;
            *status = parse_data(ntimes, 0, 1, ntimes, gParse.nCols, gParse.colData,
                                 (void*)&Info);
        }
    }
...
/*---------------------------------------------------------------------------*/
{
    ...
    for (row = 1; row <= naxis2; row++) {
        if (ffgcvd(fptr, gParse.timeCol, row, 1L, 1L, 0.0, &newtime, &anynul, status))
            return (*status);
        if (newtime != currtime) {
        ...
        if (ffgcvs(fptr, gParse.parCol, row, 1L, 1L, "", sPtr, &anynul, status))
            return (*status);
        parNo = gParse.nCols;
        while (parNo--)
            ...
            switch (gParse.colData[parNo].datatype) {
                case TLONG:
                    ffgcvj(fptr, gParse.valCol, row, 1L, 1L,
                           ...
                           ((long*)gParse.colData[parNo].array) + currelem, &anynul,
                           status);
                    break;
                case TDOUBLE:
                    ffgcvd(fptr, gParse.valCol, row, 1L, 1L,
                           ((double*)gParse.colData[parNo].array)[0],
                           ((double*)gParse.colData[parNo].array) + currelem, &anynul,
                           status);
                    break;
                case TSTRING:
                    ffgcvs(fptr, gParse.valCol, row, 1L, 1L,
                           ((char**)gParse.colData[parNo].array)[0],
                           ((char**)gParse.colData[parNo].array) + currelem, &anynul,
                           status);
                    break;
            }
            if (*status) return (*status);
...
/*---------------------------------------------------------------------------*/
{
    ...
    FFLOCK;
    if (ffiprs(fptr, 0, expr, MAXDIMS, &dtype, &nelem, &naxis, naxes, status)) {
        ffcprs();
        FFUNLOCK;
        return (*status);
    }
    ...
    *rownum = 0;
    if (constant) { /* No need to call parser... have result from ffiprs */
        ...
        if (ffiter(gParse.nCols, gParse.colData, 0, 0, ffffrw_work, (void*)rownum,
                   status) == -1)
            *status = 0; /* -1 indicates exitted without error before end... OK */
    }
...
static int find_column(char* colName, void* itslval) {
    ...
    if (DEBUG_PIXFILTER) printf("find_column(%s)\n", colName);

    if (*colName == '#') return (find_keywd(colName + 1, itslval));
    ...
    if (gParse.hdutype == IMAGE_HDU) {
        ...
        for (i = 0; i < gParse.pixFilter->count; ++i) {
            if (!fits_strcasecmp(colName, gParse.pixFilter->tag[i])) colnum = i;
        }
        if (colnum < 0) {
        ...
        fits_get_img_param(fptr, MAXDIMS, &typecode, /* actually bitpix */
                           &varInfo->naxis, &varInfo->naxes[0], &status);
        varInfo->nelem = 1;
        type = COLUMN;
    ...
    if (gParse.hdutype != IMAGE_HDU) {
            ...
            case TLONG:
                ...
                */
                ...
                if (tscale == 1.0 && (tzero == 0.0 || tzero == 32768.0)) {
                    varInfo->type = LONG;
                    colIter->datatype = TLONG;
                    /*    Reading an unsigned long column as a long can cause overflow
                       errors. Treat the column as a double instead. } else if (tscale
                       == 1.0 &&  tzero == 2147483648.0 ) { varInfo->type     = LONG;
        ...
        if (repeat > 1 && typecode != TSTRING) {
            if (fits_read_tdim(fptr, colnum, MAXDIMS, &varInfo->naxis,
...
static int find_keywd(char* keyname, void* itslval) {
    ...
    if (fits_read_keyword(fptr, keyname, keyvalue, NULL, &status)) {
        if (status == KEY_NO_EXIST) {
            ...
            snprintf(keyvalue, FLEN_VALUE, "ffgkey could not find keyword: %s",
                     keyname);
            ffpmsg(keyvalue);
        }
        gParse.status = status;
...
static int allocateCol(int nCol, int* status) {
    if ((nCol % 25) == 0) {
        if (nCol) {
            ...
            gParse.varData =
                    (DataInfo*)realloc(gParse.varData, (nCol + 25) * sizeof(DataInfo));
        } else {
...
static int load_column(int varNum, long fRow, long nRows, void* data, char* undef) {
    ...
    if (gParse.hdutype == IMAGE_HDU) {
        ...
        fits_read_imgnull(var->fptr, var->datatype, fRow, nRows, data, undef, &anynul,
                          &status);
        if (DEBUG_PIXFILTER)
        ...
        switch (var->datatype) {
            case TBYTE:
                ...
                ffgcvb(var->fptr, var->colnum, fRow, 1L, nbytes, 0, bytes, &anynul,
                       &status);
                ...
                break;
            case TSTRING:
                ffgcfs(var->fptr, var->colnum, fRow, 1L, nRows, (char**)data, undef,
                       &anynul, &status);
                break;
            case TLOGICAL:
                ffgcfl(var->fptr, var->colnum, fRow, 1L, nelem, (char*)data, undef,
                       &anynul, &status);
                break;
            case TLONG:
                ffgcfj(var->fptr, var->colnum, fRow, 1L, nelem, (long*)data, undef,
                       &anynul, &status);
                break;
            case TDOUBLE:
                ffgcfd(var->fptr, var->colnum, fRow, 1L, nelem, (double*)data, undef,
                       &anynul, &status);
                break;
            default:
...
/*--------------------------------------------------------------------------*/
{
    ...
    if (!filter->tag || !filter->tag[0] || !filter->tag[0][0]) {
        ...
        if (DEBUG_PIXFILTER) printf("using default tag '%s'\n", filter->tag[0]);
    }
    ...
    if (ffiprs(infptr, 0, filter->expression, MAXDIMS, &Info.datatype, &nelem, &naxis,
               naxes, status)) {
        goto CLEANUP;
    }
...

    {
        ...
        switch (Info.datatype) {
            ...
            case TBIT:
                type = "BIT";
                if (DEBUG_PIXFILTER) printf("hmm, image from bits?\n");
                break;
            default:
                type = "UNKNOWN?!";
                *status = pERROR;
        ...
        if (DEBUG_PIXFILTER) printf("result type is %s [%d]\n", type, Info.datatype);
        if (*status) goto CLEANUP;
    }

    if (fits_get_img_param(infptr, MAXDIMS, &bitpix, &naxis, &naxes[0], status)) {
        ffpmsg("pixel_filter: unable to read input image parameters");
        goto CLEANUP;
    }

    if (DEBUG_PIXFILTER) printf("input bitpix %d\n", bitpix);
    ...
    if (filter->bitpix) bitpix = filter->bitpix;
    if (DEBUG_PIXFILTER) printf("output bitpix %d\n", bitpix);
    ...
    if (bitpix > 0) { /* arrange for NULLs in output */
        ...
        if (DEBUG_PIXFILTER) printf("using blank %ld\n", nullVal);
    }
    ...
    if (!filter->keyword[0]) {
        ...
        /*************************************/
        col_cnt = gParse.nCols;
        if (allocateCol(col_cnt, status)) goto CLEANUP;
        gParse.nCols++;
        ...
        if (ffiter(gParse.nCols, gParse.colData, 0, 0, parse_data, &Info, status) == -1)
            *status = 0;
        else if (*status)
            goto CLEANUP;
        ...
        if (Info.anyNull) {
            if (writeBlankKwd) {
                fits_update_key_lng(outfptr, "BLANK", filter->blank, "NULL pixel value",
                                    status);
                if (*status) ffpmsg("pixel_filter: unable to write BLANK keyword");
                if (DEBUG_PIXFILTER) {
        ...
        switch (Info.datatype) {
            ...
            default:
                snprintf(msg, 256,
                         "pixel_filter: unexpected constant result type [%d]\n",
                         Info.datatype);
                ffpmsg(msg);
        }
    }
