...
*/
{
    ...
    */
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_write_compressed_pixels(fptr, TFLOAT, firstelem, nelem, 0, array,
                                     &nullvalue, status);
        return (*status);
    }
...
*/
{
    ...
    */
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_write_compressed_pixels(fptr, TFLOAT, firstelem, nelem, 1, array,
                                     &nullvalue, status);
        return (*status);
    }
...
*/
{
    ...
    */
    if (scale == 1. && zero == 0. && MACHINE == NATIVE && tcode == TFLOAT) {
        writeraw = 1;
        ...
        switch (tcode) {
            case (TFLOAT):
                if (writeraw) {
                    ...
                    ffr4fr4(&array[next], ntodo, scale, zero, (float*)buffer, status);
            ...
            case (TLONGLONG):
                ...
                ffr4fi8(&array[next], ntodo, scale, zero, (LONGLONG*)buffer, status);
                ffpi8b(fptr, ntodo, incre, (long*)buffer, status);
                break;

            case (TBYTE):

                ffr4fi1(&array[next], ntodo, scale, zero, (unsigned char*)buffer,
                        status);
            ...
            case (TSHORT):
                ...
                ffr4fi2(&array[next], ntodo, scale, zero, (short*)buffer, status);
                ffpi2b(fptr, ntodo, incre, (short*)buffer, status);
                break;

            case (TLONG):

                ffr4fi4(&array[next], ntodo, scale, zero, (INT32BIT*)buffer, status);
                ffpi4b(fptr, ntodo, incre, (INT32BIT*)buffer, status);
                break;

            case (TDOUBLE):
                ffr4fr8(&array[next], ntodo, scale, zero, (double*)buffer, status);
            ...
            case (TSTRING): /* numerical column in an ASCII table */
                ...
                {
                    ffr4fstr(&array[next], ntodo, scale, zero, cform, twidth,
                             (char*)buffer, status);
...
*/
{
    ...
    ffpcle(fptr, colnum, firstrow, (firstelem - 1) * 2 + 1, nelem * 2, array, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
