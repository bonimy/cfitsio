...
/*---------------------------------------------------------------------------*/
int fits_read_ascii_region(const char* filename, WCSdata* wcs, SAORegion** Rgn,
                           int* status)
        ...
        *status = 0;
        ...
        if (*currLoc == '#') {
            ...
            while (*currLoc != '\0') {
                ...
                if (!(aRgn->nShapes % 10)) {
                    if (aRgn->Shapes)
                        tmpShape = (RgnShape*)realloc(
                                aRgn->Shapes, (10 + aRgn->nShapes) * sizeof(RgnShape));
                    else
                ...
                if (!fits_strcasecmp(namePtr, "circle")) {
                    ...
                    if (nParams != 3) *status = PARSE_SYNTAX_ERR;
                    nCoords = 2;
                } else if (!fits_strcasecmp(namePtr, "annulus")) {
                    newShape->shape = annulus_rgn;
                    if (nParams != 4) *status = PARSE_SYNTAX_ERR;
                    nCoords = 2;
                    ...
                    newShape->shape = elliptannulus_rgn;
                    if (!(nParams == 8 || nParams == 6)) *status = PARSE_SYNTAX_ERR;
                    newShape->param.gen.p[6] = 0.0;
                    newShape->param.gen.p[7] = 0.0;
                    nCoords = 2;
                    ...
                    if (nParams < 4 || nParams > 5) *status = PARSE_SYNTAX_ERR;
                    newShape->param.gen.p[4] = 0.0;
                    nCoords = 4;
                    ...
                    newShape->shape = point_rgn;
                    if (nParams != 2) *status = PARSE_SYNTAX_ERR;
                    nCoords = 2;
                    ...
                    if (nParams < 6 || (nParams & 1)) *status = PARSE_SYNTAX_ERR;
                    nCoords = nParams;
                } else if (!fits_strcasecmp(namePtr, "panda")) {
                    newShape->shape = panda_rgn;
                    if (nParams != 8) *status = PARSE_SYNTAX_ERR;
                    nCoords = 2;
                } else if (!fits_strcasecmp(namePtr, "epanda")) {
                    newShape->shape = epanda_rgn;
                    if (nParams < 10 || nParams > 11) *status = PARSE_SYNTAX_ERR;
                ...
                if (newShape->shape == poly_rgn) {
                    newShape->param.poly.Pts =
                            (double*)malloc(nParams * sizeof(double));
                    ...
                    *(paramPtr++) = '\0';
                    ...
                    if (cFmt != pixel_fmt) {
                        ...
                        if (wcs == NULL || !wcs->exists) {
                            ffpmsg("WCS information needed to convert region "
                                   "coordinates.");
                            *status = NO_WCS_KEY;
                            goto error;
                        }

                        if (ffxypx(X, Y, wcs->xrefval, wcs->yrefval, wcs->xrefpix,
        ...
        if ((!comp_result && Shapes->sign) || (comp_result && !Shapes->sign)) {
            ...
            switch (Shapes->shape) {
                case box_rgn:
                    ...
                    x = xprime * Shapes->param.gen.cosT +
                        yprime * Shapes->param.gen.sinT;
                    y = -xprime * Shapes->param.gen.sinT +
                        yprime * Shapes->param.gen.cosT;

                    dx = 0.5 * Shapes->param.gen.p[2];
                    dy = 0.5 * Shapes->param.gen.p[3];
                    if ((x < -dx) || (x > dx) || (y < -dy) || (y > dy)) comp_result = 0;
                    break;
                ...
                case boxannulus_rgn:
                    ...
                    if ((x < -dx) || (x > dx) || (y < -dy) || (y > dy)) {
                        ...
                        y = -xprime * Shapes->param.gen.a +
                            yprime * Shapes->param.gen.b;
                ...
                case circle_rgn:
                    ...
                    r = x * x + y * y;
                    if (r > Shapes->param.gen.a) comp_result = 0;
                    break;
                ...
                case sector_rgn:
                    ...
                    if (x || y) {
                        ...
                        if (Shapes->param.gen.p[2] <= Shapes->param.gen.p[3]) {
                            if (r < Shapes->param.gen.p[2] ||
                                r > Shapes->param.gen.p[3])
                                comp_result = 0;
                        } else {
                            if (r < Shapes->param.gen.p[2] &&
                                r > Shapes->param.gen.p[3])
                ...
                case poly_rgn:
                    if (X < Shapes->xmin || X > Shapes->xmax || Y < Shapes->ymin ||
                        Y > Shapes->ymax)
                        comp_result = 0;
                    else
                ...
                case panda_rgn:
                    ...
                    if (r < Shapes->param.gen.a || r > Shapes->param.gen.b) {
                        ...
                        if (x || y) {
                            ...
                            if (Shapes->param.gen.p[2] <= Shapes->param.gen.p[3]) {
                                if (th < Shapes->param.gen.p[2] ||
                                    th > Shapes->param.gen.p[3])
                                    comp_result = 0;
                            } else {
                                if (th < Shapes->param.gen.p[2] &&
                                    th > Shapes->param.gen.p[3])
...
/*---------------------------------------------------------------------------*/
{
    ...
    for (i = 0; i < Rgn->nShapes; i++)
        if (Rgn->Shapes[i].shape == poly_rgn) free(Rgn->Shapes[i].param.poly.Pts);
    if (Rgn->Shapes) free(Rgn->Shapes);
    free(Rgn);
}
...
/*---------------------------------------------------------------------------*/
{
    ...
    for (i = 0; i < nPts; i += 2) {
        ...
        if (y != prevY) {
            ...
            if ((nextY - y) * (y - Pts[j]) > 0) flag = 1 - flag;
        }
    }
    return (flag);
}
...
void fits_set_region_components(SAORegion* aRgn) {
    ...
    */
        ...
        if (!aRgn->Shapes[i].sign) {
            ...
            while (j >= 0) {
                ...
                if (aRgn->Shapes[j].sign) {
                    aRgn->Shapes = (RgnShape*)realloc(
                            aRgn->Shapes, (1 + aRgn->nShapes) * sizeof(RgnShape));
                    aRgn->nShapes++;
    ...
    for (i = 0; i < aRgn->nShapes; i++) {
        ...
        /*
        printf("i = %d, shape = %d, sign = %d, comp = %d\n", i, aRgn->Shapes[i].shape,
        aRgn->Shapes[i].sign, aRgn->Shapes[i].comp);
        */
    }
...
void fits_setup_shape(RgnShape* newShape) {
    ...
    switch (newShape->shape) {
        ...
        case rectangle_rgn:
            ...
            newShape->param.gen.a =
                    fabs(X * newShape->param.gen.cosT + Y * newShape->param.gen.sinT);
            newShape->param.gen.b =
                    fabs(Y * newShape->param.gen.cosT - X * newShape->param.gen.sinT);
    ...
    switch (newShape->shape) {
        ...
        case box_rgn:
            R = sqrt(coords[2] * coords[2] + coords[3] * coords[3]) / 2.0;
            break;

        case boxannulus_rgn:
            R = sqrt(coords[4] * coords[5] + coords[4] * coords[5]) / 2.0;
            break;
        ...
        case bpanda_rgn:
            R = sqrt(coords[7] * coords[8] + coords[7] * coords[8]) / 2.0;
            break;
...
/*---------------------------------------------------------------------------*/
{
    ...
    char shapename[17][FLEN_VALUE] = {
            "POINT",  "CIRCLE",     "ELLIPSE",   "ANNULUS",      "ELLIPTANNULUS", "BOX",
            "ROTBOX", "BOXANNULUS", "RECTANGLE", "ROTRECTANGLE", "POLYGON",       "PIE",
            "SECTOR", "DIAMOND",    "RHOMBUS",   "ROTDIAMOND",   "ROTRHOMBUS"};
    ...
    for (i = 1; i <= aRgn->nShapes; i++) {
        ...
        for (j = 0; j < npos; j++) {
            if (ffgcvd(fptr, icol[0], i, j + 1, 1, DOUBLENULLVALUE, coords, &anynul,
                       status)) {
                ffpmsg("Failed to read X column for polygon region");
                goto error;
            }
            if (*coords ==
                DOUBLENULLVALUE) { /* check for null value end of array marker */
                npos = j;
            ...
            if (ffgcvd(fptr, icol[1], i, j + 1, 1, DOUBLENULLVALUE, coords, &anynul,
                       status)) {
            ...
            if (j == 0) { /* save the first X and Y coordinate */
                ...
                /* if point has same coordinate as first point, this marks the end of
                 * the array */
                npos = j + 1;
                newShape->param.poly.nPts = npos * 2;
                break;
        ...
        if (dotransform) {
            ...
            Ysave = coords[1];
            for (j = 0; j < npos; j++) {
                ffwldp(coords[2 * j], coords[2 * j + 1], regwcs->xrefval,
                       regwcs->yrefval, regwcs->xrefpix, regwcs->yrefpix, regwcs->xinc,
                       regwcs->yinc, regwcs->rot, regwcs->type, &Xpos, &Ypos, status);
                ffxypx(Xpos, Ypos, wcs->xrefval, wcs->yrefval, wcs->xrefpix,
                       wcs->yrefpix, wcs->xinc, wcs->yinc, wcs->rot, wcs->type,
                       &coords[2 * j], &coords[2 * j + 1], status);
                if (*status) {
        ...
        if (npos > 0) {
            ...
            if (dotransform) {
                for (j = 0; j < npos; j++) {
                    ...
                    ffxypx(Xpos, Ypos, wcs->xrefval, wcs->yrefval, wcs->xrefpix,
                           wcs->yrefpix, wcs->xinc, wcs->yinc, wcs->rot, wcs->type, &X,
                           &Y, status);
                    if (*status) {
                        ffpmsg("Failed to transform coordinates");
                        goto error;
                    }
                    *(coords++) = sqrt(pow(X - newShape->param.gen.p[0], 2) +
                                       pow(Y - newShape->param.gen.p[1], 2));
                }
            } else {
                coords += npos;
            }
        ...
        if (got_component) {
            if (ffgcv(fptr, TINT, icol[5], i, 1, 1, 0, &newShape->comp, &anynul,
                      status)) {
