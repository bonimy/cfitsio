...
*/
{
    ...
    *sum = (hi << 16) + lo;
    if (complm) *sum = 0xFFFFFFFF - *sum; /* complement each bit of the value */
...
*/
{
    ...
    if (ffpdfl(fptr, status) > 0) return (*status);

    /* calc size of data unit, in FITS 2880-byte blocks */
    if (ffghadll(fptr, &headstart, &datastart, &dataend, status) > 0) return (*status);
    ...
    if (nrec > 0) {
        ...
        if (ffcsum(fptr, nrec, &dsum, status) > 0) return (*status);
    }
    ...
    if (strcmp(checksum, "0000000000000000")) {
        ...
        sum = dsum;
        if (ffcsum(fptr, nrec, &sum, status) > 0) return (*status);

        if (sum == 0 || sum == 0xFFFFFFFF) return (*status); /* CHECKSUM is correct */
...
*/
{
    ...
    if (ffgkys(fptr, "CHECKSUM", checksum, comm, status) == KEY_NO_EXIST) {
        *status = tstatus;
        ...
        if (ffwend(fptr, status) > 0) return (*status);
...
*/
{
    ...
    *hdustatus = -1;
    ...
    if (ffgcks(fptr, &datasum, &hdusum, status) > 0) return (*status);

    if (*datastatus)
        if (datasum == olddatasum) *datastatus = 1;

    if (*hdustatus)
        if (hdusum == 0 || hdusum == 0xFFFFFFFF) *hdustatus = 1;
...
/* calculate the checksums of the data unit and the total HDU */
{
    ...
    *datasum = 0;
    ...
    if (nrec > 0) {
        /* accumulate the 32-bit 1's complement checksum */
        ffmbyt(fptr, datastart, REPORT_EOF, status);
        if (ffcsum(fptr, nrec, datasum, status) > 0) return (*status);
    }
