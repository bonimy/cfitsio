...
int gsiftp_init(void) {
    if (getenv("GSIFTP_TMPFILE")) {
        ...
        gsiftp_tmpfile =
                malloc(strlen(gsiftp_tmpdir) + strlen("/gsiftp_buffer.tmp") + 1);
        gsiftp_tmpfile[0] = 0;
        free_gsiftp_tmp = 1;
...
int gsiftp_shutdown(void) {
    free(gsiftpurl);
    if (free_gsiftp_tmp) free(gsiftp_tmpfile);

    return file_shutdown();
}

int gsiftp_setoptions(int options) { return file_setoptions(options); }

int gsiftp_getoptions(int* options) { return file_getoptions(options); }

int gsiftp_getversion(int* version) { return file_getversion(version); }
...
int gsiftp_size(int handle, LONGLONG* filesize) { return file_size(handle, filesize); }
...
int gsiftp_seek(int handle, LONGLONG offset) { return file_seek(handle, offset); }
...
static void data_cb_read(void* user_arg, globus_ftp_client_handle_t* handle,
                         globus_object_t* err, globus_byte_t* buffer,
    ...
    if (err) {
        ...
        if (!eof) {
            globus_ftp_client_register_read(handle, buffer, MAX_BUFFER_SIZE_R,
                                            data_cb_read, (void*)fd);
        }
    }
    return;
}

static void data_cb_write(void* user_arg, globus_ftp_client_handle_t* handle,
                          globus_object_t* err, globus_byte_t* buffer,
    ...
    if (err) {
        ...
        if (!eof) {
            ...
            globus_ftp_client_register_write(handle, buffer, rc, curr_offset,
                                             feof(fd) != 0, data_cb_write, (void*)fd);
        } else {
...
int gsiftp_get(char* filename, FILE** gsiftpfile, int num_streams) {
    ...
    *gsiftpfile = fopen(gsiftp_tmpfile, "w+");
    ...
    result = globus_ftp_client_get(&handle, gsiurl, &attr, &restart, done_cb, 0);
    if (result != GLOBUS_SUCCESS) {
        ...
        globus_ftp_client_register_read(&handle, buffer, buffer_length, data_cb_read,
                                        (void*)*gsiftpfile);
    }
...
int gsiftp_put(char* filename, FILE** gsiftpfile, int num_streams) {
    ...
    *gsiftpfile = fopen(gsiftp_tmpfile, "r");
    ...
    result = globus_ftp_client_put(&handle, gsiurl, &attr, &restart, done_cb, 0);
    if (result != GLOBUS_SUCCESS) {
        globus_object_t* err;
        err = globus_error_get(result);
        fprintf(stderr, "%s", globus_object_printable_to_string(err));
        ...
        for (i = 0; i < 2 * num_streams && feof(*gsiftpfile) == 0; i++) {
            ...
            globus_mutex_unlock(&lock);
            globus_ftp_client_register_write(&handle, buffer, rc, curr_offset,
                                             feof(*gsiftpfile) != 0, data_cb_write,
                                             (void*)*gsiftpfile);
        }
    }
