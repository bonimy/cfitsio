...
static int irafrdimage(char** buffptr, size_t* buffsize, size_t* filesize, int* status);
static int iraftofits(char* hdrname, char* irafheader, int nbiraf, char** buffptr,
                      size_t* nbfits, size_t* fitssize, int* status);
...
static int getirafpixname(const char* hdrname, char* irafheader, char* pixfilename,
                          int* status);
...
*/
{
    ...
    *filesize = 0;
    ...
    iraftofits(filename, irafheader, lenirafhead, buffptr, buffsize, filesize, status);
        ...
        int* lihead)          /* Length of IRAF image header in bytes (returned) */
{
    ...
    *lihead = nihead;
    ...
    if (nbr < LEN_PIXHDR) {
        snprintf(errmsg, FLEN_ERRMSG, "IRAFRHEAD header file: %d / %d bytes read.", nbr,
                 LEN_PIXHDR);
        ffpmsg(errmsg);
        ffpmsg(filename);
        free(irafheader);
...
/*--------------------------------------------------------------------------*/
static int irafrdimage(char** buffptr,   /* FITS image header (filled) */
    ...
    if (nbr < lpixhead) {
        snprintf(errmsg, FLEN_ERRMSG, "IRAF pixel file: %d / %d bytes read.", nbr,
                 LEN_PIXHDR);
        ffpmsg(errmsg);
        free(pixheader);
        fclose(fd);
    ...
    if (nax > 2) hgeti4(fitsheader, "NAXIS3", &naxis3);
    if (nax > 3) hgeti4(fitsheader, "NAXIS4", &naxis4);
    ...
    *filesize = newfilesize;
    ...
    if (npaxis1 == naxis1) nbr = fread(image, 1, nbimage, fd);
    ...
    /* Read IRAF image one line at a time if physical and image dimensions differ */
        ...
        linebuff = image;
        nbr = 0;
        if (naxis2 == 1 && naxis3 > 1) naxis2 = naxis3;
        for (i = 0; i < naxis2; i++) {
    ...
    if (swapdata) irafswap(bitpix, image, nbimage);
...
static int head_version(char* irafheader) /* IRAF image header from file */
...
static int pix_version(char* irafheader) /* IRAF image header from file */
{
...

{
    ...
    if ((line = iraf2str(irafheader, nc)) == NULL) return (1);
...
static int iraftofits(char* hdrname,    /* IRAF header file name (may be path) */
    ...
    if (imhver == 2) {
        ...
        for (i = imu; i < nbiraf; i++) {
            ...
            if (irafchar == 0)
                ...
                if (irafchar > 32 && irafchar < 127) fitsline[j] = irafchar;
                j++;
            }
        }
    } else {
...
static int getirafpixname(const char* hdrname, /* IRAF header file name (may be path) */
                          char* irafheader,    /* IRAF image header */
...

{
    ...
    for (i = 0; i < nc; i++) {
        ...
        if (ctemp[i] > 0 && ctemp[i] < 32) ctemp[i] = ' ';
    }
...
static int hgeti4(hstring, keyword, ival)
...
static int hgets(hstring, keyword, lstr, str)
...
static char* hgetc(hstring, keyword0)
                          ...
                          (the first 8 characters must be unique) */
{
    ...
    if (brack1 == NULL) brack1 = strsrch(keyword, comma);
    if (brack1 != NULL) {
        *brack1 = '\0';
        brack1++;
    }
...
static char* blsearch(hstring, keyword)
                       ...
                       or '$'.  it is truncated to 8 characters. */
{
    ...
    if (lhead0)
        ...
        lhstr = 0;
        while (lhstr < 57600 && hstring[lhstr] != 0) lhstr++;
    }
    ...
    if (pval == hstring) return (NULL);

    /* Find last nonblank line before requested keyword */
    bval = pval - 80;
    while (!strncmp(bval, "        ", 8)) bval = bval - 80;
    bval = bval + 80;
...
static char* strnsrch(s1, s2, ls1)
...
static void hputi4(hstring, keyword, ival)
...
static void hputl(hstring, keyword, lval)
...
static void hputs(hstring, keyword, cval)
...
static void hputc(hstring, keyword, value)
                ...
                keyword.  trailing and leading blanks are removed.  */
{
    ...
    if (lkeyword == 7 &&
        (strncmp(keyword, "COMMENT", 7) == 0 || strncmp(keyword, "HISTORY", 7) == 0)) {
        /* Find end of header */
        ...
        /* Pad with spaces */
        for (vp = v1 + lkeyword; vp < v2; vp++) *vp = ' ';
...
static void hputcom(hstring, keyword, comment)
