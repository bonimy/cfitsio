...
*************************************************************************/
        ...
        (defined(IBMR2Fortran) && defined(__64BIT__)) || defined(__ia64__) || \
        defined(__sparcv9) || (defined(__sparc__) && defined(__arch64__)) ||  \
        defined(__x86_64__) || defined(_SX) || defined(__powerpc64__) ||      \
        defined(__s390x__)
...
#define LONGV_cfT(M, I, A, B, D) \
    ((_(B, N) = *_3(M, _LONGV_A, I)), B = F2Clongv(_(B, N), A))
...
#define STRINGV_cfT(M, I, A, B, D)                 \
    TTTTSTRV(A->dsc$a_pointer, B, A->dsc$w_length, \
             num_elem(A->dsc$a_pointer, A->dsc$w_length, _3(M, _STRV_A, I)))
#else
#ifdef CRAYFortran
...
#define STRINGV_cfQ(B) \
    char** B;          \
    unsigned int _(B, N), _(B, M);
#define STRINGV_cfR(A, B, D) \
    free(B[0]);              \
    free(B);
#define TTSTR(A, B, D)                                                          \
...
#define TTTTSTRV(A, B, D, E)                                      \
    (_(B, N) = _cfMAX(E, 1), _(B, M) = _cfMAX(D, gMinStrLen) + 1, \
     B = (char**)malloc(_(B, N) * sizeof(char*)),                 \
     B[0] = (char*)malloc(_(B, N) * _(B, M)),                     \
     vindex(B, _(B, M), _(B, N), f2cstrv2(A, B[0], D, _(B, M), _(B, N))))
#define RRRRPSTRV(A, B, D) c2fstrv2(B[0], A, _(B, M), D, _(B, N)), free(B[0]), free(B);
...
static char* c2fstrv2(char* cstr, char* fstr, int celem_len, int felem_len, int nelem) {
    int i, j;
...
static char* f2cstrv2(char* fstr, char* cstr, int felem_len, int celem_len, int nelem) {
    int i, j;
...
#define LOGICALV_cfQ(B) \
    char* B;            \
    unsigned int _(B, N);
#define LOGICALV_cfT(M, I, A, B, D) \
    (_(B, N) = *_3(M, _LOGV_A, I), B = F2CcopyLogVect(_(B, N), A))
