...
void Cfffiou(int unit, int* status) {
    ...
    if (unit == -1) {
        int i;
        for (i = 50; i < NMAXFILES;) gFitsFiles[i++] = NULL;
...
/* in turn call CFITSIO... Modelled after code by Ning Gan.          */
{
    ...
    if (last_unit && fptr == gFitsFiles[last_unit]) return (last_unit);
    ...
    for (last_unit = 1; last_unit < NMAXFILES; last_unit++) {
        if (fptr == gFitsFiles[last_unit]) return (last_unit);
    }
...
fitsfile* CUnit2FITS(int unit) {
    if (unit < 1 || unit >= NMAXFILES) return (0);
...
FCALLSCSUB9(ffiurl, FTIURL, ftiurl, STRING, PSTRING, PSTRING, PSTRING, PSTRING, PSTRING,
            PSTRING, PSTRING, PINT)
...
void Cffopen(fitsfile** fptr, const char* filename, int iomode, int* blocksize,
             int* status);
void Cffopen(fitsfile** fptr, const char* filename, int iomode, int* blocksize,
             int* status) {
    int hdutype;
...
void Cffdkopn(fitsfile** fptr, const char* filename, int iomode, int* blocksize,
              int* status);
void Cffdkopn(fitsfile** fptr, const char* filename, int iomode, int* blocksize,
              int* status) {
    int hdutype;
...
void Cffgtmo(int* secs) { *secs = ffgtmo(); }
