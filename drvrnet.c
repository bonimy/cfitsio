...
HREF="http://heasarc.gsfc.nasa.gov/FTP/software/ftools/release/other/image.fits.gz">here</A>.<P>
   </BODY></HTML>
  ...
  ****************************************************************
   ...
   HTTP/1.1 302 Found
   LOCATION:
http://heasarc.gsfc.nasa.gov/FTP/software/ftools/release/other/image.fits.gz
...
static int NET_ParseUrl(const char* url, char* proto, char* host, int* port, char* fn);
static int CreateSocketAddress(struct sockaddr_in* sockaddrPtr, char* host, int port);
...
static int ftp_open_network(char* url, FILE** ftpfile, FILE** command, int* sock);
...
static int ssl_get_with_curl(char* url, curlmembuf* buffer, char* username,
                             char* password);
...
int http_open(char* filename, int rwmode, int* handle) {
    ...
    if (setjmp(env) != 0) {
        ...
        snprintf(errorstr, MAXLEN, "Download timeout exceeded: %d seconds",
                 net_timeout);
        ffpmsg(errorstr);
    ...
    if (http_open_network(filename, &httpfile, contentencoding, &contentlength)) {
        alarm(0);
    ...
    if (!strcmp(contentencoding, "x-gzip") || !strcmp(contentencoding, "x-compress") ||
        strstr(filename, ".gz") || strstr(filename, ".Z") || ('\037' == firstchar)) {
        ...
        if (contentlength % 2880) {
            snprintf(errorstr, MAXLEN,
                     "Content-Length not a multiple of 2880 (http_open) %d",
                     contentlength);
            ffpmsg(errorstr);
        }
...
int http_compress_open(char* url, int rwmode, int* handle) {
    ...
    alarm(net_timeout);
    if ((status = http_open_network(url, &httpfile, contentencoding, &contentlength))) {
        alarm(0);
...
static int http_open_network(char* url, FILE** httpfile, char* contentencoding,
    ...
    */
    ...
    /*  snprintf(tmpstr1,SHORTLEN,"User-Agent:
     * HEASARC/CFITSIO/%-8.3f\r\n",ffvers(&version)); */

    /*  snprintf(tmpstr1,SHORTLEN,"User-Agent:
     * CFITSIO/HEASARC/%-8.3f\r\n",ffvers(&version)); */
    ...
    if (status != 200) {
        if (status == 301 || status == 302) {
            ...
            */
                    ...
                    */
                    ...
                    if (scratchstr2 != NULL) {
                        ...
                        *httpfile = 0;
                        ...
                        return http_open_network(turl, httpfile, contentencoding,
                                                 contentlength);
                    }
                    ...
                    if (scratchstr2 != NULL) {
                        ...
                        /* return the new URL string, and set contentencoding to "ftp"
                           as a flag to the http_checkfile routine
                        */
                        if (strlen(scratchstr2) > FLEN_FILENAME - 1) {
                            ffpmsg("Error: redirected url string too long "
                                   "(http_open_network)");
                            fclose(*httpfile);
                            *httpfile = 0;
                    ...
                    if (scratchstr2 != NULL) {
                        ...
                        /* return the new URL string, and set contentencoding to "https"
                           as a flag to the http_checkfile routine
                        */
                        if (strlen(scratchstr2) > FLEN_FILENAME - 1) {
                            ffpmsg("Error: redirected url string too long "
            ...
            ffpmsg("but we were unable to find the redirected url in the servers "
                   "response");
        }
        ...
        if (!strcmp(tmpstr, "Content-Encoding:")) {
            if (NULL != (scratchstr = strstr(recbuf, ":"))) {
                ...
                if (strlen(scratchstr) > SHORTLEN - 1) {
                    ffpmsg("Error: content-encoding string too long "
                           "(http_open_network)");
                    fclose(*httpfile);
                    *httpfile = 0;
                    return URL_PARSE_ERROR;
...
int https_open(char* filename, int rwmode, int* handle) {
    ...
    if (inmem.size % 2880) {
        snprintf(errStr, MAXLEN,
                 "Content-Length not a multiple of 2880 (https_open) %u", inmem.size);
        ffpmsg(errStr);
    }
...
int curlProgressCallback(void* clientp, double dltotal, double dlnow, double ultotal,
                         double ulnow) {
    ...
    if (isComplete && percent < 100) isFirst = 1;
    if (!isComplete || percent < 100) {
        ...
        if (nToDisplay > fullBar) nToDisplay = fullBar;
        fprintf(stderr, "%3d%% [", percent);
        for (i = 0; i < nToDisplay; ++i) fprintf(stderr, "=");
        /* print remaining spaces */
        for (i = nToDisplay; i < fullBar; ++i) fprintf(stderr, " ");
        fprintf(stderr, "]\r");
        if (isComplete) fprintf(stderr, "\n");
        fflush(stderr);
    }
    return 0;
}
...
int fits_net_timeout(int sec) {
    ...
    if (sec > 0) net_timeout = (unsigned int)sec;
...
int ftps_open(char* filename, int rwmode, int* handle) {
    ...
    if (firstByte == 0x1f && secondByte == 0x8b || strstr(localFilename, ".Z")) {
#ifdef HAVE_FMEMOPEN
        ...
        if (inmem.size % 2880) {
            snprintf(errStr, MAXLEN,
                     "Content-Length not a multiple of 2880 (ftps_open) %u",
                     inmem.size);
            ffpmsg(errStr);
        }
        status = mem_write(*handle, inmem.memory, inmem.size);
...
int ftps_compress_open(char* filename, int rwmode, int* handle) {
    ...
    if ((firstByte == 0x1f && secondByte == 0x8b) || strstr(localFilename, ".gz") ||
        ...
        ffpmsg("Cannot write uncompressed infile to compressed outfile "
               "(ftps_compress_open)");
        free(inmem.memory);
        return (FILE_NOT_OPENED);
    }
...
int ftps_open_network(char* filename, curlmembuf* buffer) {
    ...
    if (!username || strlen(username) == 0) username = "anonymous";
    if (!password || strlen(password) == 0) {
        snprintf(agentStr, SHORTLEN, "User-Agent: FITSIO/HEASARC/%-8.3f",
                 ffvers(&version));
        password = agentStr;
    }
    ...
    */
    ...
    if ((len - origLen) == 2 || (len - origLen) == 3) {
        if (strlen(filename) > FLEN_FILENAME - 4) {
            ffpmsg("Filename is too long to append compression ext "
                   "(ftps_open_network)");
...
int ssl_get_with_curl(char* url, curlmembuf* buffer, char* username, char* password) {
    ...
    */
    ...
    int experimentWithCompression =
            (!strstr(url, ".gz") && !strstr(url, ".Z") && !strstr(url, "?"));
    int notFound = 1;
#ifdef CFITSIO_HAVE_CURL
    CURL* curl = 0;
    CURLcode res;
    ...
    if (strstr(url, ".Z") && !isFtp) {
        ffpmsg("x-compress .Z format not currently supported with curl https "
               "transfers");
        return (FILE_NOT_OPENED);
    }
    ...
    if (experimentWithCompression) strcat(tmpUrl, ".gz");
    ...
    if (res != CURLE_OK && res != CURLE_HTTP_RETURNED_ERROR &&
        ...
        if (verify) {
            if (verify[0] == 'T' || verify[0] == 't') {
                ...
                ffpmsg(errStr);
                if (strlen(curlErrBuf)) ffpmsg(curlErrBuf);
        ...
        if (res != CURLE_OK) {
            if (isFtp && experimentWithCompression) {
                ...
                if (res == CURLE_OK) {
                    ...
                    fprintf(stderr,
                            "Warning: Unable to perform SSL verification on https "
                            "transfer from: %s\n",
                            tmpUrl);
                    notFound = 0;
                }
            ...
            fprintf(stderr,
                    "Warning: Unable to perform SSL verification on https transfer "
                    "from: %s\n",
                    tmpUrl);
...
#else
    ffpmsg("ERROR: This CFITSIO build was not compiled with the libcurl library "
           "package ");
...
int ftp_open(char* filename, int rwmode, int* handle) {
    ...
    if (strstr(filename, ".gz") || strstr(filename, ".Z") || ('\037' == firstchar)) {
        status = 0;
...
int ftp_file_open(char* url, int rwmode, int* handle) {
    ...
    if (strstr(url, ".gz") || strstr(url, ".Z") || ('\037' == firstchar)) {
        /* to make this more cfitsioish we use the file driver calls to create
           the file */
        /* Create the output file */
        if ((status = file_create(netoutfile, handle))) {
...
int NET_TcpConnect(char* hostname, int port) {
    ...
    if ((stat = connect(sock, (struct sockaddr*)&sockaddr, sizeof(sockaddr))) < 0) {
        close(sock);
        /*
...
static int NET_ParseUrl(const char* url, char* proto, char* host, int* port, char* fn) {
    ...
    *port = 80;
    ...
    if (!strcmp(proto, "http:")) {
        ...
        if ((thost = strchr(urlcopy, '@')) != NULL) urlcopy = thost + 1;
...
int http_checkfile(char* urltype, char* infile, char* outfile1) {
    ...
    */
            ...
            if (!http_open_network(newinfile, &httpfile, contentencoding,
                ...
                if (!strcmp(contentencoding, "ftp://")) {
                    ...
                    if (ftp_file_exist(newinfile) > 0) {
                        ...
                        if (strlen(outfile1)) {
                            /* there is an output file;  might need to modify the
                             * urltype */

                            if (!strncmp(outfile1, "mem:", 4)) {
                                /* copy the file to memory, with READ and WRITE access
                                In this case, it makes no difference whether the ftp
                                file and or the output file are compressed or not.   */

                                strcpy(urltype, "ftpmem://"); /* use special driver */
                            } else {
                                /* input file is compressed */
                                if (strstr(outfile1, ".gz") ||
                                    (strstr(outfile1, ".Z"))) {
                                    strcpy(urltype, "ftpcompress://");
                                } else {
                                    strcpy(urltype, "ftpfile://");
    ...
    if (!foundfile) {
        ...
        strcpy(newinfile, infile);
        if (!http_open_network(newinfile, &httpfile, contentencoding, &contentlength)) {
            if (!strcmp(contentencoding, "ftp://")) {
                /* this is a signal from http_open_network that indicates that */
                /* the http server returned a 301 or 302 redirect to a FTP URL. */
                /* Check that the file exists, because redirect many not be reliable */
...
int ftp_checkfile(char* urltype, char* infile, char* outfile1) {
    ...
    if (!strstr(infile, ".gz") && (!strstr(infile, ".Z"))) {
        ...
        if (status > 0) {
            foundfile = 1;
            if (strlen(newinfile) > FLEN_FILENAME - 1) return URL_PARSE_ERROR;
            strcpy(infile, newinfile);
        } else if (status < 0) {
...
static int ftp_status(FILE* ftp, char* statusstr) {
    ...
    while (1) {
        if (!(fgets(recbuf, MAXLEN, ftp))) {
            snprintf(errorstr, SHORTLEN,
                     "ERROR: ftp_status wants %s but fgets returned 0", statusstr);
            ffpmsg(errorstr);
        ...
        if (recbuf[0] > '3') {
            ...
            snprintf(errorstr, SHORTLEN, "ERROR ftp_status wants %s but got %s",
                     statusstr, recbuf);
            ffpmsg(errorstr);
        ...
        snprintf(errorstr, SHORTLEN, "ERROR ftp_status wants %s but got unexpected %s",
                 statusstr, recbuf);
        ffpmsg(errorstr);
    }
}
...
static int CreateSocketAddress(struct sockaddr_in* sockaddrPtr, /* Socket address */
