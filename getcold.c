...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TDOUBLE, firstelem, nelem, nullcheck,
    ...
    */
    ...
    ffgcld(fptr, 2, row, firstelem, nelem, 1, 1, nulval, array, &cdummy, anynul,
           status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TDOUBLE, firstelem, nelem, nullcheck, NULL,
    ...
    */
    ...
    ffgcld(fptr, 2, row, firstelem, nelem, 1, 2, 0., array, nularray, anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffg3dd(fptr, group, nulval, ncols, naxis2, naxis1, naxis2, 1, array, anynul,
           status);
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TDOUBLE, fpixel, lpixel, inc, nullcheck,
                                 &nullvalue, array, NULL, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    if (ncols == naxis1 && nrows == naxis2) /* arrays have same size? */
    {
        ...
        ffgcld(fptr, 2, tablerow, 1, naxis1 * naxis2 * naxis3, 1, 1, nulval, array,
    ...
    for (jj = 0; jj < naxis3; jj++) {
        ...
        for (ii = 0; ii < naxis2; ii++) {
            if (ffgcld(fptr, 2, tablerow, nfits, naxis1, 1, 1, nulval, &array[narray],
...
*/
{
    ...
    if (naxis < 1 || naxis > 9) {
        snprintf(msg, FLEN_ERRMSG, "NAXIS = %d in call to ffgsvd is out of range",
                 naxis);
        ffpmsg(msg);
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TDOUBLE, blcll, trcll, inc, nullcheck,
                                 &nullvalue, array, NULL, anynul, status);
        return (*status);
    }
        ...
        if (trc[ii] < blc[ii]) {
            if (hdutype == IMAGE_HDU) {
                ...
                snprintf(msg, FLEN_ERRMSG,
                         "ffgsvd: illegal range specified for axis %ld", ii + 1);
                ffpmsg(msg);
                            ...
                            for (i3 = str[3] * dir[3]; i3 <= stp[3] * dir[3];
                                ...
                                for (i2 = str[2] * dir[2]; i2 <= stp[2] * dir[2];
                                    ...
                                    for (i1 = str[1] * dir[1]; i1 <= stp[1] * dir[1];
                                        ...
                                        if (ffgcld(fptr, numcol, row, felem, nelem,
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TDOUBLE, blcll, trcll, inc, nullcheck, NULL,
    ...
    */
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (trc[ii] < blc[ii]) {
            snprintf(msg, FLEN_ERRMSG, "ffgsvd: illegal range specified for axis %ld",
                     ii + 1);
            ffpmsg(msg);
            return (*status = BAD_PIX_NUM);
        }
...
*/
{
    ...
    */
    ...
    ffgcld(fptr, 1, row, firstelem, nelem, 1, 1, 0., array, &cdummy, &idummy, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgcld(fptr, colnum, firstrow, firstelem, nelem, 1, 1, nulval, array, &cdummy,
           anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgcld(fptr, colnum, firstrow, (firstelem - 1) * 2 + 1, nelem * 2, 1, 1, nulval,
...
*/
{
    ...
    ffgcld(fptr, colnum, firstrow, firstelem, nelem, 1, 2, dummy, array, nularray,
           anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgcld(fptr, colnum, firstrow, (firstelem - 1) * 2 + 1, nelem * 2, 1, 2, dummy,
        ...
        */
        ...
        switch (tcode) {
            case (TDOUBLE):
                ...
                if (convert)
                    fffr8r8(&array[next], ntodo, scale, zero, nulcheck, nulval,
            ...
            case (TSHORT):
                ...
                fffi2r8((short*)buffer, ntodo, scale, zero, nulcheck, (short)tnull,
            ...
            case (TLONGLONG):
                ...
                fffi8r8((LONGLONG*)buffer, ntodo, scale, zero, nulcheck, tnull, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TFLOAT):
                ffgr4b(fptr, readptr, ntodo, incre, (float*)buffer, status);
                fffr4r8((float*)buffer, ntodo, scale, zero, nulcheck, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TSTRING):
                ...
                fffstrr8((char*)buffer, ntodo, scale, zero, twidth, power, nulcheck,
