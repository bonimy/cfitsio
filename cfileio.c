...
static int pixel_filter_helper(fitsfile** fptr, char* outfile, char* expr, int* status);
...
#ifdef __GLIBC__
        status = pthread_mutexattr_settype(&mutex_init, PTHREAD_MUTEX_RECURSIVE_NP);
#else
        status = pthread_mutexattr_settype(&mutex_init, PTHREAD_MUTEX_RECURSIVE);
#endif
        if (status) {
...
*/
{
    ...
    *fptr = 0; /* initialize null file pointer */
    ...
    fits_parse_input_url(url, urltype, infile, outfile, extspec, rowfilter, binspec,
                         colspec, status);
    ...
    *status = mem_openmem(buffptr, buffsize, deltasize, mem_realloc, &handle);
    FFUNLOCK;
    ...
    *fptr = (fitsfile*)calloc(1, sizeof(fitsfile));
    ...
    if (ffrhdu(*fptr, &hdutyp, status) > 0) /* determine HDU structure */
    {
        ...
        if (*status == UNKNOWN_REC) ffpmsg("This does not look like a FITS file.");
    ...
    if (*extspec) {
        ...
        ffexts(extspec, &extnum, extname, &extvers, &movetotype, imagecolname,
               rowexpress, status);
        ...
        if (*status > 0) {
            ...
            if (extnum > 0) {
                ...
                snprintf(errmsg, FLEN_ERRMSG, " extension with EXTNAME = %s,", extname);
                ffpmsg(errmsg);

                if (extvers) {
                    snprintf(errmsg, FLEN_ERRMSG, "           and with EXTVERS = %d,",
                             extvers);
                    ffpmsg(errmsg);
                }

                if (movetotype != ANY_HDU) {
                    snprintf(errmsg, FLEN_ERRMSG, "           and with XTENSION = %s,",
                             hdtype[movetotype]);
                    ffpmsg(errmsg);
                }
...
*/
{
    ...
    if (ffopen(fptr, name, mode, status) > 0) return (*status);
...
*/
{
    ...
    *status = SKIP_IMAGE;
    ...
    if (ffghdt(*fptr, &hdutype, status) <= 0) {
        if (hdutype == IMAGE_HDU) *status = NOT_TABLE;
    }
...
*/
{
    ...
    *status = SKIP_TABLE;
    ...
    if (ffghdt(*fptr, &hdutype, status) <= 0) {
        if (hdutype != IMAGE_HDU) *status = NOT_IMAGE;
    }
...
*/
{
    if (soname != CFITSIO_SONAME) {
        printf("\nERROR: Mismatch in the CFITSIO_SONAME value in the fitsio.h include "
               "file\n");
        printf("that was used to build the CFITSIO library, and the value in the "
               "include file\n");
...
*/
{
    ...
    *fptr = 0;     /* initialize null file pointer */
    ...
    if (open_disk_file) {
        ...
        ffifile2(url, urltype, infile, outfile, extspec, rowfilter, binspec, colspec,
                 pixfilter, compspec, status);
    }
    ...
    FFLOCK;
    if (fits_already_open(fptr, url, urltype, infile, extspec, rowfilter, binspec,
                          colspec, mode, open_disk_file, &isopen, status) > 0) {
        FFUNLOCK;
        return (*status);
    }
    FFUNLOCK;
    ...
    *fptr = (fitsfile*)calloc(1, sizeof(fitsfile));
    ...
    ((*fptr)->Fptr)->noextsyntax =
            open_disk_file; /* true if extended syntax is disabled */
    ...
    if (ffrhdu(*fptr, &hdutyp, status) > 0) /* determine HDU structure */
    {
        ffpmsg("ffopen could not interpret primary array header of file: ");
        ffpmsg(url);
...
move2hdu:
            ...
            if (naxis == 0 || skip_image) /* skip primary array */
            {
                while (1) {
                    ...
                    if (hdutyp == IMAGE_HDU && skip_image) {
                        ...
                        fits_get_img_dim(*fptr, &naxis, status);
                        if (naxis > 0) break; /* found a non-null image */

                    } else {
                        tstatus = 0;
                        tblname[0] = '\0';
                        fits_read_key(*fptr, TSTRING, "EXTNAME", tblname, NULL,
                                      &tstatus);
    ...
    if (*imagecolname) {
        ...
        if (fits_copy_cell2image(*fptr, newptr, imagecolname, rownum, status) > 0) {
    ...
    if (*rowfilter) {
        ...
        if (hdutyp == IMAGE_HDU) {
            ...
            if (*binspec) {
                ...
                if (!rowselect) {
                    ffpmsg("failed to allocate memory for selected columns array "
                           "(ffopen)");
                ...
                if (fits_find_rows(*fptr, rowfilter, 1L, nrows, &goodrows, rowselect,
                                   status) > 0) {
                ...
                if (!writecopy) {
                    if (*filtfilename && *outfile == '\0')
                        strcpy(outfile, filtfilename); /* the original outfile name */
                             ...
                             '\0') /* output filename not already defined? */
                ...
                ffphis(*fptr,
                       "CFITSIO used the following filtering expression to create this "
                       "table:",
                       status);
    ...
    if (*binspec) {
        ...
        ffbins(binspec, &imagetype, &haxis, colname, minin, maxin, binsizein, minname,
               maxname, binname, &weight, wtcol, &recip, status);
        ...
        ffhist2(fptr, outfile, imagetype, haxis, colname, minin, maxin, binsizein,
                minname, maxname, binname, weight, wtcol, recip, rowselect, status);
        ...
        /* write history records */
        ffphis(*fptr, "CFITSIO used the following expression to create this histogram:",
               status);
        ffphis(*fptr, name, status);
    }
    ...
    if (*pixfilter) {
        ...
        if (hdutyp == IMAGE_HDU) {
            ...
            /* write history records */
            ffphis(*fptr, "CFITSIO used the following expression to create this image:",
                   status);
            ffphis(*fptr, name, status);
        } else {
...
/*--------------------------------------------------------------------------*/
int fits_already_open(
        fitsfile** fptr, /* I/O - FITS file pointer       */
        char* url, char* urltype, char* infile, char* extspec, char* rowfilter,
...
*/
{
    ...
    */
    if (mode == 0) return (*status);

    strcpy(tmpinfile, infile);
    if (fits_strcasecmp(urltype, "FILE://") == 0) {
        if (standardize_path(tmpinfile, status)) return (*status);
    }
        ...
        if (FptrTable[ii] != 0) {
            ...
            if (oldFptr->noextsyntax) {
                ...
                if (fits_strcasecmp(urltype, "FILE://") == 0) {
                    ...
                    if (standardize_path(oldinfile, status)) return (*status);
                    ...
                    if (!strcmp(tmpinfile, oldinfile)) {
                        ...
                        if (noextsyn || (!rowfilter[0] && !binspec[0] && !colspec[0])) {
                            if (mode == READWRITE && oldFptr->writemode == READONLY) {
                                /*
                                  cannot assume that a file previously opened with
                                  READONLY can now be written to (e.g., files on CDROM,
                                  or over the the network, or STDIN), so return with an
                                  error.
                                */

                                ffpmsg("cannot reopen file READWRITE when previously "
                                       "opened READONLY");
                                ffpmsg(url);
                                return (*status = FILE_NOT_OPENED);
                            }
                ...
                fits_parse_input_url(oldFptr->filename, oldurltype, oldinfile,
                                     oldoutfile, oldextspec, oldrowfilter, oldbinspec,
                                     oldcolspec, status);
                ...
                if (!strcmp(urltype, oldurltype) && !strcmp(tmpinfile, oldinfile)) {
                    ...
                    if ((!rowfilter[0] && !oldrowfilter[0] && !binspec[0] &&
                         !oldbinspec[0] && !colspec[0] && !oldcolspec[0])
                        ...
                        (!strcmp(rowfilter, oldrowfilter) &&
                         !strcmp(binspec, oldbinspec) && !strcmp(colspec, oldcolspec) &&
...
int standardize_path(char* fullpath, int* status) {
    ...
    if (fits_path2url(fullpath, FLEN_FILENAME, tmpPath, status)) return (*status);
...
/*
    replace commas with semicolons, unless the comma is within a quoted or bracketed
   expression
*/
{
    char* tstr;
...
/*--------------------------------------------------------------------------*/
int ffedit_columns(fitsfile** fptr, /* IO - pointer to input table; on output it  */
...
*/
{
    ...
    if (*outfile) {
        ...
        if (!((*fptr)->Fptr)->only_one) {
            for (ii = 1; 1; ii++) {
                if (fits_movabs_hdu(*fptr, ii, NULL, status) > 0) break;
    ...
    while (*cptr == ' ') cptr++; /* skip leading white space */
    ...
    if (*cptr == '@') {
        ...
        cptr = file_expr;
        while (*cptr == ' ') cptr++; /* skip leading white space... again */
    }
        ...
        if (clause[0] == '!' || clause[0] == '-') {
            ...
            */
            ...
            /* Case Ic. delete keyword(s)
                    ...
                    -KEYNAME+,-#KEYNAME+ - delete 0-or-more exact matches of exact
               keyword -KEYNAM*+,-#KEYNAM*+ - delete 0-or-more wildcard matches Note the
               preceding # is optional if no conflicting column name exists and that
               wildcard patterns are described in "colfilter" section of documentation.
            */
            else {
                int delall = 0;
                int haswild = 0;
                ...
                *status = 0;
                ...
                if (strchr(clause1, '?') || strchr(clause1, '*') ||
                    strchr(clause1, '#')) {
                    haswild = 1;
                }
                ...
                if (haswild) {
                    /* ffdkey() behaves differently if the pattern has a wildcard:
                       it only checks from the "current" header position to the end, and
                       doesn't check before the "current" header position.  Therefore,
                       for the
            ...
            if (slen == 0 || *status) {
                ...
                if (file_expr) free(file_expr);
                if (clause) free(clause);
                if (*status == 0) *status = URL_PARSE_ERROR;
                return (*status);
            }
            ...
            */
            if (*cptr2 == '(') {
                if (fits_get_token2(&cptr2, ")", &tstbuff, NULL, status) == 0) {
                    ...
                    if ((strlen(tstbuff) + strlen(colname) + 1) > FLEN_VALUE - 1) {
            ...
            while (*cptr2 == ' ') cptr2++; /* skip white space */
            ...
            if (*cptr2 != '=') {
                ...
                while (*status == COL_NOT_UNIQUE) {
                    ...
                    if (!colindex) colindex = (int*)calloc(999, sizeof(int));
                ...
                if (*cptr2 == '=') {
                    ...
                    if (fits_get_token2(&cptr2, " ", &tstbuff, NULL, status) == 0) {
                        ...
                        if (strlen(tstbuff) > FLEN_VALUE - 1) {
                            ffpmsg("error: column name syntax is too long "
                                   "(ffedit_columns):");
                            if (file_expr) free(file_expr);
                            if (clause) free(clause);
                            free(tstbuff);
                    ...
                    if (cptr3[0] == '(') {
                        ...
                        if (fits_get_token2(&cptr3, ")", &tstbuff, NULL, status) == 0) {
                            ...
                            if (strlen(tstbuff) > FLEN_VALUE - 1) {
                                ffpmsg("column expression is too long "
                                       "(ffedit_columns)");
                                if (colindex) free(colindex);
                                if (file_expr) free(file_expr);
                                if (clause) free(clause);
...

{
    ...
    /*---------------------------------------------------*/
    if (ffgcprll(fptr, colnum, rownum, 1L, 1L, 0, &scale, &zero, tform, &twidth,
                 &typecode, &maxelem, &startpos, &elemnum, &incre, &repeat, &rowlen,
                 &hdutype, &tnull, (char*)buffer, status) > 0)
        return (*status);
    ...
    fits_translate_keywords(fptr, newptr, 9, patterns, npat, colnum, 0, 0, status);

    /* add some HISTORY  */
    snprintf(card, FLEN_CARD,
             "HISTORY  This image was copied from row %ld of column '%s',", rownum,
             colname);
...
/*--------------------------------------------------------------------------*/
int fits_copy_image2cell(fitsfile* fptr,   /* I - pointer to input image extension */
...
*/
{
    ...
    if (*status) {
        *status = 0;
        ...
        if (*status > 0 || naxis != naxis1) {
            ffpmsg("Input image dimensions and output table cell dimensions do not "
                   "match.");
    ...
    if (copykeyflag) {
        ...
        fits_translate_keywords(fptr, newptr, 5, patterns, npat, colnum, 0, 0, status);
    }
    ...
    */
    ...
    ffghdn(fptr, &hdunum);
    snprintf(filename + strlen(filename), FLEN_FILENAME + 20 - strlen(filename), "[%d]",
             hdunum - 1);
    /*
...
/*--------------------------------------------------------------------------*/
int fits_copy_image_section(fitsfile* fptr,   /* I - pointer to input image */
    ...
    */
    ...
    fits_get_img_dim(fptr, &naxis, status);
    if (fits_get_img_size(fptr, naxis, naxes, status) > 0) return (*status);
    ...
    for (ii = 0; ii < naxis; ii++) {
        ...
        if (fpixels[ii] != 1 || incs[ii] != 1) {
            for (kk = -1; kk < 26; kk++) /* modify any alternate WCS keywords */
            {
                ...
                tstatus = 0;
                if (fits_read_key(fptr, TDOUBLE, keyname, &crpix, NULL, &tstatus) ==
                    0) {
                    ...
                    if (incs[ii] != 1 || fpixels[ii] > lpixels[ii]) {
                        ...
                        tstatus = 0;
                        if (fits_read_key(fptr, TDOUBLE, keyname, &cdelt, NULL,
                                          &tstatus) == 0) {
                            ...
                            /* modify the value in the output file */
                            fits_modify_key_dbl(newptr, keyname, cdelt, 15, NULL,
                                                status);
                        }
        ...
        for (jiter = 0; jiter < nsliceiter; jiter++) {
            ...
            for (iiter = 0; iiter < nrowiter; iiter++) {
                ...
                if (bitpix == 8) {
                    ...
                    ffpprb(newptr, 1, firstpix, outsize, (unsigned char*)buffer,
                           status);
                } else if (bitpix == 16) {
                    ...
                    ffgsve(fptr, 1, naxis, naxes, fpixels, lpixels, incs,
                           FLOATNULLVALUE, (float*)buffer, &anynull, status);

                    ffppne(newptr, 1, firstpix, outsize, (float*)buffer, FLOATNULLVALUE,
                           status);
                } else if (bitpix == -64) {
                    ffgsvd(fptr, 1, naxis, naxes, fpixels, lpixels, incs,
                           DOUBLENULLVALUE, buffer, &anynull, status);
...
*/
{
    ...
    slen = fits_get_token2(ptr, " ,:", &tstbuff, &isanumber,
                           status); /* get 1st token */
    if (slen == 0) {
    ...
    } else if (*token == '-' && *(token + 1) == '*') /* invert the whole range */
    {
        ...
        *secmax = 1;
        ...
        if (slen == 0 || !isanumber || **ptr != ':') return (*status = URL_PARSE_ERROR);
    ...
    if (*secmin < 0 || *secmax < 0 || *incre < 1) *status = URL_PARSE_ERROR;

    return (*status);
}
/*--------------------------------------------------------------------------*/
int ffselect_table(fitsfile** fptr, /* IO - pointer to input table; on output it  */
                                    /*      points to the new selected rows table */
                   char* outfile,   /* I - name for output file */
    ...
    if (*outfile) {
        ...
        /* copy all preceding extensions to the output file, if the 'only_one' flag is
         * not set */
        if (!((*fptr)->Fptr)->only_one) {
            for (ii = 1; ii < hdunum; ii++) {
                fits_movabs_hdu(*fptr, ii, NULL, status);
    ...
    if (fits_select_rows(*fptr, newptr, expr, status) > 0) {
        if (*outfile) ffclos(newptr, status);
...
*/
{
    ...
    */
    ...
    while (isdigit((int)*ptr1) && ii < 9) {
        ...
        if (*ptr1 == ',') ptr1++; /* skip over the comma */
...
*/
{
    ...
    *status = urltype2driver(urltype, &driver);
    ...
    if (clobber) {
        if (driverTable[driver].remove) (*driverTable[driver].remove)(outfile);
    }
    ...
    *fptr = (fitsfile*)calloc(1, sizeof(fitsfile));
    ...
    ((*fptr)->Fptr)->validcode = VALIDSTRUC;     /* flag denoting valid structure */
    ((*fptr)->Fptr)->noextsyntax =
            create_disk_file; /* true if extended syntax is disabled */
    ...
    if (tmplfile[0]) ffoptplt(*fptr, tmplfile, status);

    /* parse and save image compression specification, if given */
    if (compspec[0]) ffparsecompspec(*fptr, compspec, status);
...
*/
{
    ...
    u.ival = 1;
    if ((BYTESWAPPED && u.cval[0] != 1) || (BYTESWAPPED == FALSE && u.cval[1] != 1)) {
        printf("\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    ...
    /* 1--------------------disk file driver-----------------------*/
    status = fits_register_driver("file://", file_init, file_shutdown, file_setoptions,
                                  file_getoptions, file_getversion, file_checkfile,
...
#endif
                                  file_close, file_remove, file_size, file_flush,
                                  file_seek, file_read, file_write);
    ...
    status = fits_register_driver("mem://", mem_init, mem_shutdown, mem_setoptions,
                                  mem_getoptions, mem_getversion,
                                  NULL, /* checkfile not needed */
                                  NULL, /* open function not allowed */
                                  mem_create, mem_truncate, mem_close_free,
                                  NULL,           /* remove function not required */
                                  mem_size, NULL, /* flush function not required */
    ...
    status = fits_register_driver(
            "memkeep://", NULL, mem_shutdown, mem_setoptions, mem_getoptions,
            mem_getversion, NULL, /* checkfile not needed */
            NULL,                 /* file open driver function is not used */
            NULL,                 /* create function not allowed */
            mem_truncate, mem_close_keep, NULL, /* remove function not required */
            mem_size, NULL,                     /* flush function not required */
            mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver("stdin://", NULL, mem_shutdown, mem_setoptions,
                                  mem_getoptions, mem_getversion, stdin_checkfile,
                                  stdin_open, NULL, /* create function not allowed */
                                  mem_truncate, mem_close_free,
                                  NULL,           /* remove function not required */
                                  mem_size, NULL, /* flush function not required */
                                  mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver("stdinfile://", NULL, mem_shutdown, mem_setoptions,
                                  mem_getoptions, mem_getversion,
                                  NULL,             /* checkfile not needed */
                                  stdin_open, NULL, /* create function not allowed */
#ifdef HAVE_FTRUNCATE
    ...
    status = fits_register_driver("stdout://", NULL, mem_shutdown, mem_setoptions,
                                  mem_getoptions, mem_getversion,
                                  NULL, /* checkfile not needed */
                                  NULL, /* open function not required */
                                  mem_create, mem_truncate, stdout_close,
                                  NULL,           /* remove function not required */
                                  mem_size, NULL, /* flush function not required */
                                  mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver(
            "irafmem://", NULL, mem_shutdown, mem_setoptions, mem_getoptions,
            mem_getversion, NULL,               /* checkfile not needed */
            mem_iraf_open, NULL,                /* create function not required */
            mem_truncate, mem_close_free, NULL, /* remove function not required */
            mem_size, NULL,                     /* flush function not required */
            mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver(
            "rawfile://", NULL, mem_shutdown, mem_setoptions, mem_getoptions,
            mem_getversion, NULL,               /* checkfile not needed */
            mem_rawfile_open, NULL,             /* create function not required */
            mem_truncate, mem_close_free, NULL, /* remove function not required */
            mem_size, NULL,                     /* flush function not required */
            mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver(
            "compress://", NULL, mem_shutdown, mem_setoptions, mem_getoptions,
            mem_getversion, NULL,               /* checkfile not needed */
            mem_compress_open, NULL,            /* create function not required */
            mem_truncate, mem_close_free, NULL, /* remove function not required */
            mem_size, NULL,                     /* flush function not required */
            mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver(
            "compressmem://", NULL, mem_shutdown, mem_setoptions, mem_getoptions,
            mem_getversion, NULL,               /* checkfile not needed */
            mem_compress_openrw, NULL,          /* create function not required */
            mem_truncate, mem_close_free, NULL, /* remove function not required */
            mem_size, NULL,                     /* flush function not required */
            mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver("compressfile://", NULL, file_shutdown,
                                  file_setoptions, file_getoptions, file_getversion,
                                  NULL, /* checkfile not needed */
                                  file_compress_open, file_create,
#ifdef HAVE_FTRUNCATE
                                  file_truncate,
#else
                                  NULL, /* no file truncate function */
    ...
    status = fits_register_driver(
            "compressoutfile://", NULL, mem_shutdown, mem_setoptions, mem_getoptions,
            mem_getversion, NULL, /* checkfile not needed */
            NULL,                 /* open function not allowed */
            mem_create_comp, mem_truncate, mem_close_comp,
    ...
    if (status) {
        ffpmsg("failed to register the compressoutfile:// driver (init_cfitsio)");
        FFUNLOCK;
        return (status);
    }
    ...
    status = fits_register_driver(
            "root://", root_init, root_shutdown, root_setoptions, root_getoptions,
            root_getversion, NULL,        /* checkfile not needed */
            root_open, root_create, NULL, /* No truncate possible */
            root_close, NULL,             /* No remove possible */
            root_size,                    /* no size possible */
            root_flush, root_seek,        /* Though will always succeed */
            root_read, root_write);
    ...
    /* 14--------------------http  driver-----------------------*/
    status = fits_register_driver("http://", NULL, mem_shutdown, mem_setoptions,
                                  mem_getoptions, mem_getversion, http_checkfile,
                                  http_open, NULL, /* create function not required */
                                  mem_truncate, mem_close_free,
                                  NULL,           /* remove function not required */
                                  mem_size, NULL, /* flush function not required */
                                  mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver("httpfile://", NULL, file_shutdown, file_setoptions,
                                  file_getoptions, file_getversion,
                                  NULL, /* checkfile not needed */
                                  http_file_open, file_create,
#ifdef HAVE_FTRUNCATE
                                  file_truncate,
#else
                                  NULL, /* no file truncate function */
    ...
    status = fits_register_driver("httpmem://", NULL, mem_shutdown, mem_setoptions,
                                  mem_getoptions, mem_getversion, http_checkfile,
    ...
    status = fits_register_driver(
            "httpcompress://", NULL, mem_shutdown, mem_setoptions, mem_getoptions,
            mem_getversion, NULL,               /* checkfile not needed */
            http_compress_open, NULL,           /* create function not required */
            mem_truncate, mem_close_free, NULL, /* remove function not required */
            mem_size, NULL,                     /* flush function not required */
            mem_seek, mem_read, mem_write);
    ...
    /* 18--------------------ftp driver-----------------------*/
    status = fits_register_driver("ftp://", NULL, mem_shutdown, mem_setoptions,
                                  mem_getoptions, mem_getversion, ftp_checkfile,
                                  ftp_open, NULL, /* create function not required */
                                  mem_truncate, mem_close_free,
                                  NULL,           /* remove function not required */
                                  mem_size, NULL, /* flush function not required */
                                  mem_seek, mem_read, mem_write);
    ...
    /* 19--------------------ftp file driver-----------------------*/
    status = fits_register_driver("ftpfile://", NULL, file_shutdown, file_setoptions,
                                  file_getoptions, file_getversion,
                                  NULL, /* checkfile not needed */
                                  ftp_file_open, file_create,
#ifdef HAVE_FTRUNCATE
                                  file_truncate,
#else
                                  NULL, /* no file truncate function */
    ...
    status = fits_register_driver("ftpmem://", NULL, mem_shutdown, mem_setoptions,
                                  mem_getoptions, mem_getversion, ftp_checkfile,
    ...
    status = fits_register_driver(
            "ftpcompress://", NULL, mem_shutdown, mem_setoptions, mem_getoptions,
            mem_getversion, NULL,            /* checkfile not needed */
            ftp_compress_open, 0,            /* create function not required */
            mem_truncate, mem_close_free, 0, /* remove function not required */
            mem_size, 0,                     /* flush function not required */
            mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver(
            "shmem://", smem_init, smem_shutdown, smem_setoptions, smem_getoptions,
            smem_getversion, NULL,        /* checkfile not needed */
            smem_open, smem_create, NULL, /* truncate file not supported yet */
            smem_close, smem_remove, smem_size, smem_flush, smem_seek, smem_read,
            smem_write);
    ...
    /* 23--------------------gsiftp driver-----------------------*/
    status = fits_register_driver(
            "gsiftp://", gsiftp_init, gsiftp_shutdown, gsiftp_setoptions,
            gsiftp_getoptions, gsiftp_getversion, gsiftp_checkfile, gsiftp_open,
            gsiftp_create,
#ifdef HAVE_FTRUNCATE
            gsiftp_truncate,
#else
            NULL,
#endif
            gsiftp_close, NULL, /* remove function not yet implemented */
            gsiftp_size, gsiftp_flush, gsiftp_seek, gsiftp_read, gsiftp_write);
    ...
    status = fits_register_driver(
            "stream://", NULL, NULL, NULL, NULL, NULL, NULL, stream_open, stream_create,
            NULL,               /* no stream truncate function */
            stream_close, NULL, /* no stream remove */
            stream_size, stream_flush, stream_seek, stream_read, stream_write);
    ...
    /* 25--------------------https  driver-----------------------*/
    status = fits_register_driver("https://", NULL, mem_shutdown, mem_setoptions,
                                  mem_getoptions, mem_getversion, https_checkfile,
                                  https_open, NULL, /* create function not required */
                                  mem_truncate, mem_close_free,
                                  NULL,           /* remove function not required */
                                  mem_size, NULL, /* flush function not required */
                                  mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver("httpsfile://", NULL, file_shutdown, file_setoptions,
                                  file_getoptions, file_getversion,
                                  NULL, /* checkfile not needed */
                                  https_file_open, file_create,
#ifdef HAVE_FTRUNCATE
                                  file_truncate,
#else
                                  NULL, /* no file truncate function */
    ...
    status = fits_register_driver(
            "httpsmem://", NULL, mem_shutdown, mem_setoptions, mem_getoptions,
            mem_getversion, https_checkfile,
    ...
    /* 28--------------------ftps  driver-----------------------*/
    status = fits_register_driver("ftps://", NULL, mem_shutdown, mem_setoptions,
                                  mem_getoptions, mem_getversion, ftps_checkfile,
                                  ftps_open, NULL, mem_truncate, mem_close_free, NULL,
                                  mem_size, NULL, mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver("ftpsfile://", NULL, file_shutdown, file_setoptions,
                                  file_getoptions, file_getversion, NULL,
                                  ftps_file_open, file_create,
#ifdef HAVE_FTRUNCATE
                                  file_truncate,
#else
                                  NULL,
    ...
    status = fits_register_driver("ftpsmem://", NULL, mem_shutdown, mem_setoptions,
                                  mem_getoptions, mem_getversion, ftps_checkfile,
                                  ftps_file_open, NULL, mem_truncate, mem_close_free,
                                  NULL, mem_size, NULL, mem_seek, mem_read, mem_write);
    ...
    status = fits_register_driver(
            "ftpscompress://", NULL, mem_shutdown, mem_setoptions, mem_getoptions,
            mem_getversion, NULL,            /* checkfile not needed */
            ftps_compress_open, 0,           /* create function not required */
            mem_truncate, mem_close_free, 0, /* remove function not required */
            mem_size, 0,                     /* flush function not required */
            mem_seek, mem_read, mem_write);
...
/*--------------------------------------------------------------------------*/
int fits_register_driver(char* prefix, int (*init)(void), int (*shutdown)(void),
                         ...
                         int (*close)(int driverhandle), int (*fremove)(char* filename),
...
*/
{
    ...
    if (no_of_drivers + 1 > MAX_DRIVERS) return (TOO_MANY_DRIVERS);

    if (prefix == NULL) return (BAD_URL_PREFIX);
...
*/
{
    return ffifile2(url, urltype, infilex, outfile, extspec, rowfilterx, binspec,
                    colspec, 0, 0, status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (!infile) return (*status = MEMORY_ALLOCATION);
    ...
    if (*ptr1 == '-' && (*(ptr1 + 1) == 0 || *(ptr1 + 1) == ' ' || *(ptr1 + 1) == '[' ||
                         *(ptr1 + 1) == '(')) {
        ...
        if (urltype) strcat(urltype, "stdin://");
        ptr1++;
    ...
    slen = strlen(infile);
    while ((--slen) > 0 && infile[slen] == ' ') infile[slen] = '\0';

    if (outfile) {
        slen = strlen(outfile);
        while ((--slen) > 0 && outfile[slen] == ' ') outfile[slen] = '\0';
    }
    ...
    if (ptr4 && (*(ptr4 + 4) == '\0')) {
        if (urltype) strcpy(urltype, "irafmem://");
    }
    ...
    /* omitted and we have to guess what the user intended */
    {
        ...
        if (*ptr1 == 'b' || *ptr1 == 'B' || *ptr1 == 'i' || *ptr1 == 'I' ||
            ...
            *ptr1 == 'u' || *ptr1 == 'U') {
            ...
            ptr1++;
            if (*ptr1 == 'b' || *ptr1 == 'B' || *ptr1 == 'l' || *ptr1 == 'L') ptr1++;

            if (isdigit((int)*ptr1)) /* must have at least 1 digit */
            {
                while (isdigit((int)*ptr1)) ptr1++; /* skip over digits */
                ...
                if (*ptr1 == ',' || *ptr1 == ':' || *ptr1 == ']') {
                    ...
                    if (infilex) {
                        ...
                        if (ptr1) *(ptr1 + 1) = '\0'; /* terminate string after the ] */
                    }
                    ...
                    if (tmptr && rowfilterx) {
                        if (strlen(rowfilterx) + strlen(tmptr + 1) >
                            FLEN_FILENAME - 1) {
                            free(infile);
                            return (*status = URL_PARSE_ERROR);
                        }
        ...
        while (*tmptr == ' ') tmptr++; /* skip leading blanks */

        while (isdigit((int)*tmptr)) tmptr++; /* skip over leading digits */
        ...
        if (*tmptr == ':' || *tmptr == '*' || *tmptr == '-') {
            ...
            */
            ...
            if (!fits_strncasecmp(tmptr, "col ", 4)) colStart = 1;

            if (!fits_strncasecmp(tmptr, "bin", 3)) binStart = 1;

            if (!fits_strncasecmp(tmptr, "pix", 3)) pixStart = 1;
            ...
            if (rowFilter || (pixStart && spaceTerm) || (hasAt && hasDot) || hasOper ||
                compStart || (spaceTerm && followingOper)) {
    ...
    while ((--slen) >= 0 && rowfilter[slen] == ' ') rowfilter[slen] = '\0';
    ...
    if (!ptr1) ptr1 = strstr(rowfilter, "[BIN"); /* search for "[BIN" */
    if (!ptr1) ptr1 = strstr(rowfilter, "[Bin"); /* search for "[Bin" */

    if (ptr1) {
        ptr2 = ptr1 + 4; /* end of the '[bin' string */
        if (*ptr2 == 'b' || *ptr2 == 'i' || *ptr2 == 'j' || *ptr2 == 'r' ||
            *ptr2 == 'd')
    ...
    while (ptr1) {
        ...
        while (*ptr2 != ']') {
            ...
            if (*ptr2 == '\'') /* start of a literal string */
            {
                ...
                if (!ptr2) {
                    ffpmsg("literal string in input file URL is missing closing single "
                           "quote");
                    free(infile);
                    return (*status = URL_PARSE_ERROR); /* error, no closing ] */
                }
            ...
            if (*ptr2 == '[') /* set of nested square brackets */
            {
                ...
                if (!ptr2) {
                    ffpmsg("nested brackets in input file URL is missing closing "
                           "bracket");
                    free(infile);
                    return (*status = URL_PARSE_ERROR); /* error, no closing ] */
                }
    ...
    if (!ptr1) {
        ...
        if (!ptr1) ptr1 = strstr(rowfilter, "[Pix");
    }
    ...
    if (ptr1) {
        ...
        if (*ptr2 == '1') ptr2++; /* skip the single HDU indicator */

        if (*ptr2 != ' ') ptr1 = NULL; /* pix string must be followed by space */
    }
    ...
    if (rowfilterx && rowfilter[0]) {
        ...
        while ((*ptr1 == '[') && (ptr2 = strstr(rowfilter, "][")) - ptr1 > 2) {
            ...
            /* Add expression of the form "((expr))&&", note the addition of 6
             * characters */
        ...
        if (*ptr1 == '[' && *ptr2 == ']') {
            ...
            /* Check for overflow; add extra 4 characters if we have pre-existing
             * expression */
            if (strlen(rowfilterx) + (ptr2 - ptr1 + (*rowfilterx) ? 4 : 0) >
                FLEN_FILENAME - 1) {
                free(infile);
                return (*status = URL_PARSE_ERROR);
            }
...

{
    ...
    *infile = '\0';
    ...
    } else if (!ptr3) /* no bracket, so () enclose output file name */
    {
        ...
        ptr1 = strchr(ptr2, ')');                      /* search for closing ) */
        if (!ptr1) return (*status = URL_PARSE_ERROR); /* error, no closing ) */
...
/*--------------------------------------------------------------------------*/
int ffexts(char* extspec, int* extnum, char* extname, int* extvers, int* hdutype,
           char* imagecolname, char* rowexpress, int* status) {
    /*
    ...
    if (notint) {
        ...
        if (slen) {
            ...
            if (slen) {
                if (*ptr1 == 'b' || *ptr1 == 'B')
                    *hdutype = BINARY_TBL;
                else if (*ptr1 == 't' || *ptr1 == 'T' || *ptr1 == 'a' || *ptr1 == 'A')
                    *hdutype = ASCII_TBL;
...
int ffextn(char* url,          /* I - input filename/URL  */
    ...
    */
    ...
    {
        ffexts(extspec, &extnum, extname, &extvers, &hdutype, imagecolname, rowexpress,
               status);
        ...
        if (*extname) {
            ...
            if (!strcmp(urltype, "stdin://")) /* opening stdin would destroying it! */
                return (*status = URL_PARSE_ERROR);
...
/*--------------------------------------------------------------------------*/
int fits_get_token(char** ptr, char* delimiter, char* token,
...
/*--------------------------------------------------------------------------*/
int fits_get_token2(char** ptr, char* delimiter, char** token,
                    int* isanumber, /* O - is this token a number? */
                    int* status)
...
/*---------------------------------------------------------------------------*/
char* fits_split_names(char* list) /* I   - input list of names */
{
    /*
...
*/
{
    ...
    if ((driverTable[(fptr->Fptr)->driver].remove)) {
        ...
        if (!basename) return (*status = MEMORY_ALLOCATION);

        fits_parse_input_url((fptr->Fptr)->filename, NULL, basename, NULL, NULL, NULL,
                             NULL, NULL, &zerostatus);

        if ((*driverTable[(fptr->Fptr)->driver].remove)(basename)) {
            ffpmsg("failed to delete the following file: (ffdelt)");
            ffpmsg((fptr->Fptr)->filename);
            if (!(*status)) *status = FILE_NOT_CLOSED;
        }
        free(basename);
    }
...
*/
{
    if (driverTable[(fptr->Fptr)->driver].truncate) {
        ...
        return (*status = (*driverTable[(fptr->Fptr)->driver].truncate)(
                        (fptr->Fptr)->filehandle, filesize));
    } else
        return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    readstatus = (*driverTable[fptr->driver].read)(fptr->filehandle, buffer, nbytes);
...
int pixel_filter_helper(
    ...
    if (((*fptr)->Fptr)->only_one) singleHDU = 1;
    ...
    *fptr = filter.ofptr; /* reset the pointer to the new table */
    ...
    /* move back to the image subsection */
    if (ii - 1 != hdunum) fits_movabs_hdu(*fptr, hdunum, NULL, status);
