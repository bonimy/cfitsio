...
*/
{
    ...
    ffgcll(fptr, colnum, firstrow, firstelem, nelem, 1, nulval, array, &cdummy, anynul,
           status);
...
*/
{
    ...
    ffgcvl(fptr, colnum, firstrow, firstelem, nelem, nulval, array, &anynul, status);
...
*/
{
    ...
    ffgcll(fptr, colnum, firstrow, firstelem, nelem, 2, nulval, array, nularray, anynul,
           status);
...
*/
{
    ...
    if (tcode != TLOGICAL) return (*status = NOT_LOGICAL_COL);
        ...
        */
            ...
            } else /* some other illegal character; return the char value */
            {
                if (*buffptr == 1) {
                    /* this is an unfortunate case where the illegal value is the same
                       as what we set True values to, so set the value to the character
                       '1' instead, which has ASCII value 49.  */
                    array[next] = 49;
                } else {
        ...
        if (*status > 0) /* test for error during previous read operation */
        {
            ...
            snprintf(message, FLEN_ERRMSG,
                     "Error reading elements %.0f thruough %.0f of logical array "
                     "(ffgcl).",
                     dtemp + 1., dtemp + ntodo);
            ffpmsg(message);
            return (*status);
...
*/
{
    ...
    if (tcode > 0) {
        ...
        if (tcode == TBIT) repeat = (repeat + 7) / 8; /* convert from bits to bytes */

        if (fbyte > repeat) return (*status = BAD_ELEM_NUM);
        ...
        if (tcode == -TBIT) repeat = (repeat + 7) / 8;

        if ((fbit + nbit + 6) / 8 > repeat) return (*status = BAD_ELEM_NUM);
    ...
    if (ffmbyt(fptr, bstart, REPORT_EOF, status) > 0) return (*status);

    /* read the next byte */
    while (1) {
        if (ffgbyt(fptr, 1, &cbuff, status) > 0) return (*status);
...
*/
{
    ...
    if (*status > 0 || nrows == 0) return (*status);

    /*  check input parameters */
    if (firstrow < 1) {
        snprintf(message, FLEN_ERRMSG,
                 "Starting row number is less than 1: %ld (ffgcxui)", (long)firstrow);
        ffpmsg(message);
        return (*status = BAD_ROW_NUM);
    } else if (input_first_bit < 1) {
        snprintf(message, FLEN_ERRMSG,
                 "Starting bit number is less than 1: %ld (ffgcxui)", input_first_bit);
        ffpmsg(message);
        return (*status = BAD_ELEM_NUM);
    ...
    if (colnum > (fptr->Fptr)->tfield) {
        snprintf(message, FLEN_ERRMSG,
                 "Specified column number is out of range: %d (ffgcxui)", colnum);
        ffpmsg(message);
    ...
    for (ii = 0; ii < nrows; ii++) {
        ...
        if (ffgcvui(fptr, colnum, firstrow + ii, firstbyte, nbytes, 0, colbyte, NULL,
                    status) > 0) {
...
*/
{
    ...
    if (firstrow < 1) {
        snprintf(message, FLEN_ERRMSG,
                 "Starting row number is less than 1: %ld (ffgcxuk)", (long)firstrow);
        ffpmsg(message);
        return (*status = BAD_ROW_NUM);
    } else if (input_first_bit < 1) {
        snprintf(message, FLEN_ERRMSG,
                 "Starting bit number is less than 1: %ld (ffgcxuk)", input_first_bit);
        ffpmsg(message);
        return (*status = BAD_ELEM_NUM);
    ...
    if (colnum > (fptr->Fptr)->tfield) {
        snprintf(message, FLEN_ERRMSG,
                 "Specified column number is out of range: %d (ffgcxuk)", colnum);
        ffpmsg(message);
        snprintf(message, FLEN_ERRMSG, "  There are %d columns in this table.",
                 (fptr->Fptr)->tfield);
        ffpmsg(message);
    ...
    for (ii = 0; ii < nrows; ii++) {
        /* read the relevant bytes from the row */
        if (ffgcvuk(fptr, colnum, firstrow + ii, firstbyte, nbytes, 0, colbyte, NULL,
                    status) > 0) {
