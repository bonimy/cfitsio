...
int ffbins(char* binspec,               /* I - binning specification */
    ...
    *wtname = '\0';
    ...
    if (*ptr == '@') {
        ...
        ptr = file_expr;
        while (*ptr == ' ') ptr++; /* skip leading white space... again */
    }
    ...
    if (*ptr == '(') {
        ...
        {
            ffpmsg("binning specification has too many column names or is missing "
                   "closing ')':");
            ffpmsg(binspec);
        ...
        ffbinr(&ptr, tmpname, minin, maxin, binsizein, minname[0], maxname[0],
               binname[0], status);
        if (*status > 0) {
            ffpmsg("illegal binning specification in URL:");
            ffpmsg(binspec);
        ...
        if (*ptr == ';') goto getweight; /* a weighting factor is specified */
    ...
    {
        ffbinr(&ptr, colname[ii], &minin[ii], &maxin[ii], &binsizein[ii], minname[ii],
        ...
        if (*ptr == '\0' || *ptr == ';') break; /* reached the end of the string */
        ...
        if (*ptr == ' ') {
            ...
            if (*ptr == ',') ptr++; /* comma separates the next column specification */
        } else if (*ptr == ',') {
    ...
    if (*histaxis == 1 && *colname[0] == '\0' && minin[0] == DOUBLENULLVALUE &&
...
getweight:
    if (*ptr == ';') /* looks like a weighting factor is given */
    {
        ...
        ffbinr(&ptr, wtname, &dummy, &dummy, wt, tmpname, tmpname, tmpname, status);
...
/*--------------------------------------------------------------------------*/
int ffbinr(char** ptr, char* colname, double* minin, double* maxin, double* binsizein,
           char* minname, char* maxname, char* binname, int* status)
/*
...
*/
{
    ...
    slen = fits_get_token2(ptr, " ,=:;", &token, &isanumber,
    ...
    if (!isanumber && **ptr != ':') {
        ...
        if (**ptr != '=') return (*status); /* reached the end */
...
int ffhist2(fitsfile** fptr, /* IO - pointer to table with X and Y cols;    */
    ...
    if (fits_calc_binningd(*fptr, naxis, colname, minin, maxin, binsizein, minname,
                           maxname, binname, colnum, haxes, amin, amax, binsize,
                           status) > 0) {
    ...
    if (recip && weight != DOUBLENULLVALUE) /* take reciprocal of weight */
...
int ffhist(fitsfile** fptr, /* IO - pointer to table with X and Y cols;    */
    ...
    for (ii = 0; ii < naxis; ii++) {
        ...
        fits_get_eqcoltype(*fptr, histData.hcolnum[ii], &datatype, NULL, NULL, status);
        ...
        if (minin[ii] == DOUBLENULLVALUE) {
            ...
            if (ffgky(*fptr, TDOUBLE, keyname, amin + ii, NULL, status) > 0) {
                ...
                *status = 0;
                if (fits_get_col_minmax(*fptr, histData.hcolnum[ii], amin + ii,
                                        &datamax, status) > 0) {
                    strcpy(errmsg,
                           "Error calculating datamin and datamax for column: ");
                    strncat(errmsg, colname[ii], FLEN_ERRMSG - strlen(errmsg) - 1);
                    ffpmsg(errmsg);
                    return (*status);
                }
            ...
            if (ffgky(*fptr, TDOUBLE, keyname, &amax[ii], NULL, status) > 0) {
                *status = 0;
                if (datamax != DOUBLENULLVALUE) /* already computed max value */
                {
                    ...
                    if (fits_get_col_minmax(*fptr, histData.hcolnum[ii], &datamin,
                                            &amax[ii], status) > 0) {
                        strcpy(errmsg,
                               "Error calculating datamin and datamax for column: ");
                        strncat(errmsg, colname[ii], FLEN_ERRMSG - strlen(errmsg) - 1);
        ...
        if (binsizein[ii] == DOUBLENULLVALUE) {
            ...
            if (ffgky(*fptr, TDOUBLE, keyname, binsizein + ii, NULL, &tstatus) > 0) {
                ...
                if (binsizein[ii] > 1.) binsizein[ii] = 1.; /* use default bin size */
            }
        }
        ...
        if (datatype <= TLONG && (double)imin == amin[ii] && (double)imax == amax[ii] &&
            ...
            haxes[ii] = (imax - imin) / ibin + 1; /* last bin may only */
                                                  /* be partially full */
            maxbin[ii] = (double)(haxes[ii] +
                                  1.); /* add 1. instead of .5 to avoid roundoff */
    ...
    if (recip && histData.weight != DOUBLENULLVALUE) /* take reciprocal of weight */
    ...
    for (ii = 1; ii <= nkeys; ii++) {
        ...
        if (fits_get_keyclass(card) >= 120) fits_write_record(histptr, card, status);
    }
    ...
    if (fits_iterate_data(n_cols, imagepars, offset, n_per_loop, ffwritehisto,
                          (void*)&histData, status))
        return (*status);
        ...
        if (ii == 1) {
            ...
            if (!tstatus && dvalue != 0.) /* only write keyword if angle != 0 */
            {
                ...
                if (!tstatus && dvalue != 0.) /* only write keyword if angle != 0 */
                {
                    ...
                    ffkeyn("CROTA", ii + 1, keyname, &tstatus);
                    ffpky(histptr, TDOUBLE, keyname, &dvalue, "Rotation angle",
                          &tstatus);
                }
            }
        }
    ...
    if (histData.haxis == 2) {
        ...
        if (!tstatus) ffpky(histptr, TDOUBLE, "PC1_1", &dvalue, card, &tstatus);
        ...
        ffgky(*fptr, TDOUBLE, keyname, &dvalue, card, &tstatus);
        if (!tstatus) ffpky(histptr, TDOUBLE, "PC1_2", &dvalue, card, &tstatus);
        ...
        ffkeyn(card, histData.hcolnum[0], keyname, &tstatus);
        ffgky(*fptr, TDOUBLE, keyname, &dvalue, card, &tstatus);
        if (!tstatus) ffpky(histptr, TDOUBLE, "PC2_1", &dvalue, card, &tstatus);
        ...
        ffkeyn(card, histData.hcolnum[1], keyname, &tstatus);
        ffgky(*fptr, TDOUBLE, keyname, &dvalue, card, &tstatus);
        if (!tstatus) ffpky(histptr, TDOUBLE, "PC2_2", &dvalue, card, &tstatus);
...
int fits_calc_binning(
    ...
    fits_calc_binningd(fptr, naxis, colname, minin, maxin, binsizein, minname, maxname,
                       binname, colnum, haxes, amind, amaxd, binsized, status);
        ...
        */
        ...
        if (minin[ii] != DOUBLENULLVALUE) {
            ...
            if (ffgky(fptr, TDOUBLE, keyname, amin + ii, NULL, status) > 0) {
                /* use actual data minimum value for the histogram minimum */
                *status = 0;
                if (fits_get_col_minmax(fptr, colnum[ii], amin + ii, &datamax, status) >
                    0) {
                    strcpy(errmsg,
                           "Error calculating datamin and datamax for column: ");
                    strncat(errmsg, colname[ii], FLEN_ERRMSG - strlen(errmsg) - 1);
            ...
            if (ffgky(fptr, TDOUBLE, keyname, &amax[ii], NULL, status) > 0) {
                *status = 0;
                if (datamax != DOUBLENULLVALUE) /* already computed max value */
                {
                    ...
                    /* use actual data maximum value for the histogram maximum */
                    if (fits_get_col_minmax(fptr, colnum[ii], &datamin, &amax[ii],
                                            status) > 0) {
                        strcpy(errmsg,
                               "Error calculating datamin and datamax for column: ");
                        strncat(errmsg, colname[ii], FLEN_ERRMSG - strlen(errmsg) - 1);
        ...
        if (binsizein[ii] != DOUBLENULLVALUE) {
            ...
            if (ffgky(fptr, TDOUBLE, keyname, binsizein + ii, NULL, &tstatus) > 0) {
                ...
                if (binsize[ii] > 1.) binsize[ii] = 1.; /* use default bin size */
            }
        }
...
/*--------------------------------------------------------------------------*/
int fits_rebin_wcs(fitsfile* fptr, /* I - pointer to table to be binned           */
                   int naxis,      /* I - number of axes in the histogram image   */
...
/* Double precision version */
int fits_rebin_wcsd(fitsfile* fptr,  /* I - pointer to table to be binned           */
                    int naxis,       /* I - number of axes in the histogram image   */
...
int fits_make_hist(fitsfile* fptr,    /* IO - pointer to table with X and Y cols; */
    ...
    if (*status == 0) {
        ...
        fits_make_histd(fptr, histptr, bitpix, naxis, naxes, colnum, amind, amaxd,
                        binsized, weight, wtcolnum, recip, selectrow, status);
    }
...
int fits_make_histd(fitsfile* fptr,    /* IO - pointer to table with X and Y cols; */
    ...
    for (ii = 0; ii < naxis; ii++) {
        ...
        if (datatype <= TLONG && (double)imin == tmin[ii] && (double)imax == tmax[ii] &&
            (double)ibin == tbin[ii]) {
            /* This is an integer column and integer limits were entered. */
            /* Shift the lower and upper histogramming limits by 0.5, so that */
            /* the values fall in the center of the bin, not on the edge. */
    ...
    /* call the iterator function to write out the histogram image */
    fits_iterate_data(n_cols, imagepars, offset, n_per_loop, ffwritehisto,
                      (void*)&histData, status);
...
*/
{
    ...
    *datamax = -9.0E36;
    ...
    while (nrows) {
        ...
        ffgcv(fptr, TDOUBLE, colnum, firstrow, 1, ntodo, &nulval, array, &anynul,
              status);
...
*/
{
    ...
    for (ii = 0; ii < histData->haxis; ii++) {
        fits_iter_set_by_num(&colpars[ii], histData->tblptr, histData->hcolnum[ii],
                             TDOUBLE, InputCol);
    }
    ...
    FFLOCK;
    fits_iterate_data(ncols, colpars, offset, rows_per_loop, ffcalchist,
                      (void*)histData, &status);
    FFUNLOCK;
...
*/
{
    ...
    for (ii = 1; ii <= nrows; ii++) {
        ...
        if (ipix < 1 || ipix > histData.haxis1 || pix > histData.maxbin1) continue;

        if (histData.haxis > 1) {
            if (col2[ii] == DOUBLENULLVALUE) continue;

            axisbin = (col2[ii] - histData.amin2) / histData.binsize2;
            iaxisbin = (long)axisbin;

            if (axisbin < 0. || iaxisbin >= histData.haxis2 ||
                axisbin > histData.maxbin2)
                continue;

            ipix += (iaxisbin * incr2);

            if (histData.haxis > 2) {
                if (col3[ii] == DOUBLENULLVALUE) continue;

                axisbin = (col3[ii] - histData.amin3) / histData.binsize3;
                iaxisbin = (long)axisbin;
                if (axisbin < 0. || iaxisbin >= histData.haxis3 ||
                    axisbin > histData.maxbin3)
                    continue;

                ipix += (iaxisbin * incr3);

                if (histData.haxis > 3) {
                    if (col4[ii] == DOUBLENULLVALUE) continue;

                    axisbin = (col4[ii] - histData.amin4) / histData.binsize4;
                    iaxisbin = (long)axisbin;
                    if (axisbin < 0. || iaxisbin >= histData.haxis4 ||
                        axisbin > histData.maxbin4)
                        continue;
