...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TINT, firstelem, nelem, nullcheck, &nullvalue,
    ...
    */
    ...
    ffgclk(fptr, 2, row, firstelem, nelem, 1, 1, nulval, array, &cdummy, anynul,
           status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TINT, firstelem, nelem, nullcheck, NULL,
    ...
    */
    ...
    ffgclk(fptr, 2, row, firstelem, nelem, 1, 2, 0L, array, nularray, anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffg3dk(fptr, group, nulval, ncols, naxis2, naxis1, naxis2, 1, array, anynul,
           status);
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TINT, fpixel, lpixel, inc, nullcheck, &nullvalue,
                                 array, NULL, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    if (ncols == naxis1 && nrows == naxis2) /* arrays have same size? */
    {
        ...
        ffgclk(fptr, 2, tablerow, 1, naxis1 * naxis2 * naxis3, 1, 1, nulval, array,
    ...
    for (jj = 0; jj < naxis3; jj++) {
        ...
        for (ii = 0; ii < naxis2; ii++) {
            if (ffgclk(fptr, 2, tablerow, nfits, naxis1, 1, 1, nulval, &array[narray],
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TINT, blcll, trcll, inc, nullcheck, &nullvalue,
                                 array, NULL, anynul, status);
        return (*status);
    }
        ...
        if (trc[ii] < blc[ii]) {
            if (hdutype == IMAGE_HDU) {
                ...
                snprintf(msg, FLEN_ERRMSG,
                         "ffgsvk: illegal range specified for axis %ld", ii + 1);
                ffpmsg(msg);
                            ...
                            for (i3 = str[3] * dir[3]; i3 <= stp[3] * dir[3];
                                ...
                                for (i2 = str[2] * dir[2]; i2 <= stp[2] * dir[2];
                                    ...
                                    for (i1 = str[1] * dir[1]; i1 <= stp[1] * dir[1];
                                        ...
                                        if (ffgclk(fptr, numcol, row, felem, nelem,
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TINT, blcll, trcll, inc, nullcheck, NULL, array,
...
*/
{
    ...
    */
    ...
    ffgclk(fptr, 1, row, firstelem, nelem, 1, 1, 0L, array, &cdummy, &idummy, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgclk(fptr, colnum, firstrow, firstelem, nelem, 1, 1, nulval, array, &cdummy,
           anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgclk(fptr, colnum, firstrow, firstelem, nelem, 1, 2, dummy, array, nularray,
           anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
            ...
            switch (tcode) {
                case (TLONG):
                    ffgi4b(fptr, readptr, ntodo, incre, (INT32BIT*)&array[next],
                           status);
                    if (convert)
                        fffi4int((INT32BIT*)&array[next], ntodo, scale, zero, nulcheck,
                ...
                case (TLONGLONG):
                    ...
                    fffi8int((LONGLONG*)buffer, ntodo, scale, zero, nulcheck, tnull,
                ...
                case (TSHORT):
                    ...
                    fffi2int((short*)buffer, ntodo, scale, zero, nulcheck, (short)tnull,
                             nulval, &nularray[next], anynul, &array[next], status);
                    break;
                case (TFLOAT):
                    ffgr4b(fptr, readptr, ntodo, incre, (float*)buffer, status);
                    fffr4int((float*)buffer, ntodo, scale, zero, nulcheck, nulval,
                             &nularray[next], anynul, &array[next], status);
                    break;
                case (TDOUBLE):
                    ffgr8b(fptr, readptr, ntodo, incre, (double*)buffer, status);
                    fffr8int((double*)buffer, ntodo, scale, zero, nulcheck, nulval,
                             &nularray[next], anynul, &array[next], status);
                    break;
                case (TSTRING):
