...
int main(int argc, char* argv[]) {
    ...
    if (argc != 3) {
        ...
        printf("will be compressed using the tile-compressed format.  In this "
               "format,\n");
        printf("the image is divided into rectangular tiles and each tile of pixels\n");
        printf("is compressed and stored in a variable-length row of a binary "
               "table.\n");
        printf("If the [compress] qualifier is omitted, and the input image is\n");
        printf("in tile-compressed format, then the output image will be "
               "uncompressed.\n");
        printf("\n");
        printf("If an extension name or number is appended to the input file name, \n");
        printf("enclosed in square brackets, then only that single extension will "
               "be\n");
        printf("copied to the output file.  Otherwise, every extension in the input "
               "file\n");
        ...
        printf("\n");
        printf("    This compresses the input image using the default parameters, "
               "i.e.,\n");
        printf("    using the Rice compression algorithm and using row by row "
               "tiles.\n");
        printf("\n");
        ...
        printf("         quantization level = 16 (only used with floating point "
               "images)\n");
        printf("\n");
        ...
        printf("sigma is the estimated noise level in background areas of the "
               "image.\n");
        ...
        printf("\n");
        printf("SCALE is the integer scale factor that only applies to the "
               "HCOMPRESS\n");
        printf("algorithm.  The default value SCALE = 0 forces the image to be\n");
        printf("losslessly compressed; Greater amounts of lossy compression "
               "(resulting\n");
        printf("in smaller compressed files) can be specified with larger SCALE "
               "values.\n");
        printf("\n");
        printf("\n");
    ...
    for (; !status; hdupos++) /* Main loop through each extension */
    {
        ...
        if (hdutype == IMAGE_HDU) {
            ...
            for (ii = 0; ii < 9; ii++) naxes[ii] = 1;

            fits_get_img_param(infptr, 9, &bitpix, &naxis, naxes, &status);

            totpix = naxes[0] * naxes[1] * naxes[2] * naxes[3] * naxes[4] * naxes[5] *
                     naxes[6] * naxes[7] * naxes[8];
        }
        ...
        if (hdutype != IMAGE_HDU || naxis == 0 || totpix == 0) {
            ...
            /*
                      if (!fits_is_compressed_image(outfptr, &status)) {
                          tstatus = 0;
                          fits_read_key(outfptr, TSTRING, "EXTNAME", card, NULL,
               &tstatus); if (!tstatus) { if (strcmp(card, "COMPRESSED_IMAGE") == 0)
            ...
            */
            ...
            while (totpix > 0 && !status) {
                ...
                fits_read_img(infptr, datatype, first, npix, &nulval, array, &anynul,
                              &status);
