...
main() {
    ...
    fits_iterate_data(n_cols, cols, offset, n_per_loop, writehisto, 0L, &status);
...
/*--------------------------------------------------------------------------*/
int writehisto(long totaln, long offset, long firstn, long nvalues, int narrays,
               iteratorCol* histo, void* userPointer)
/*
...
*/
{
    ...
    if (totaln != nvalues) return (-1); /* whole image must be passed at one time */

    if (narrays != 1) return (-2); /* number of images is incorrect */
    ...
    fits_iterate_data(n_cols, cols, rowoffset, rows_per_loop, calchisto, histogram,
                      &status);
...
/*--------------------------------------------------------------------------*/
int calchisto(long totalrows, long offset, long firstrow, long nrows, int ncols,
              iteratorCol* cols, void* userPointer)
...
*/
{
    ...
    if (firstrow == 1) {
        /* do sanity checking of input values */
        if (ncols != 2) return (-3); /* number of arrays is incorrect */
        ...
        for (ii = 0; ii <= xsize * ysize; ii++) histogram[ii] = 0L;
    }
