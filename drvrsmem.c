...
static int shared_init_called =
        0; /* flag whether shared_init() has been called, used for delayed init */

/* static support routines prototypes */

static int shared_clear_entry(int idx); /* unconditionally clear entry */
static int shared_destroy_entry(
        int idx); /* unconditionally destroy sema & shseg and clear entry */
static int shared_mux(int idx,
                      int mode); /* obtain exclusive access to specified segment */
static int shared_demux(int idx,
                        int mode); /* free exclusive access to specified segment */

static int shared_process_count(int sem); /* valid only for time of invocation */
static int shared_delta_process(
        int sem, int delta); /* change number of processes hanging on segment */
static int shared_attach_process(int sem);
static int shared_detach_process(int sem);
static int shared_get_free_entry(int newhandle); /* get free entry in shared_key, or -1,
                                                    entry is set rw locked */
        ...
        int idx) /* unconditionally destroy sema & shseg and clear entry */
{
    ...
    if (SHARED_INVALID != shared_gt[idx].handle)
        r2 = shmctl(shared_gt[idx].handle, IPC_RMID,
                    0);         /* destroy shared memory segment */
...
void shared_cleanup(void) /* this must (should) be called during exit/abort */
{
    ...
    if (NULL != shared_gt) /* detach global index table */
    {
        ...
        if (-1 != fcntl(shared_fd, F_SETLK, &flk)) {
            filelocked =
                    1; /* success, scan global table, to see if there are any segs */
            ...
            if (0 == segmentspresent) /* if there are no segs ... */
                if (0 == shmctl(shared_gt_h, IPC_STAT,
                                &ds)) /* get number of processes attached to table */
                {
                    if (ds.shm_nattch <= 1)
                        oktodelete = 1; /* if only one (we), then it is safe (but see
                                           text 4 lines later) to unlink */
                }
        }
        shmdt((char*)shared_gt); /* detach global table */
        if (oktodelete) /* delete global table from system, if no shm seg present */
        {
            shmctl(shared_gt_h, IPC_RMID, 0); /* there is a race condition here - time
                                                 window between shmdt and shmctl */
...
int shared_init(int debug_msgs) /* initialize shared memory stuff, you have to call this
                                   routine once */
{
    int i;
    char buf[1000], *p;
    ...
    if (SHARED_INVALID ==
        shared_fd) /* create rw locking file (this file is never deleted) */
    {
        ...
        umask(oldumask);
        if (SHARED_INVALID ==
            shared_fd) /* or just open rw locking file, in case it already exists */
        {
    ...
    if (SHARED_INVALID ==
        shared_gt_h) /* global table not attached, try to create it in shared memory */
    {
        if (shared_debug) printf(" globalsharedtableinit=");
        shared_gt_h = shmget(
                shared_kbase, shared_maxseg * sizeof(SHARED_GTAB),
                IPC_CREAT | IPC_EXCL | shared_create_mode); /* try open as a master */
        if (SHARED_INVALID == shared_gt_h) /* if failed, try to open as a slave */
        {
            shared_gt_h = shmget(shared_kbase, shared_maxseg * sizeof(SHARED_GTAB),
                                 shared_create_mode);
            if (SHARED_INVALID == shared_gt_h)
                return (SHARED_IPCERR); /* means deleted ID residing in system, shared
                                           mem unusable ... */
    ...
    if (NULL == shared_lt) /* initialize local table */
    {
        ...
        if (NULL ==
            (shared_lt = (SHARED_LTAB*)malloc(shared_maxseg * sizeof(SHARED_LTAB))))
            return (SHARED_NOMEM);
        for (i = 0; i < shared_maxseg; i++) {
...
int shared_recover(int id) /* try to recover dormant segments after applic crash */
{
    ...
    for (i = 0; i < shared_maxseg; i++) {
        ...
        if ((shared_gt[i].nprocdebug > r2) || (0 == r2)) {
            if (shared_debug)
                printf("Bogus handle=%d nproc=%d sema=%d:", i, shared_gt[i].nprocdebug,
                       r2);
                      ...
                      int mode) /* obtain exclusive access to specified segment */
{
    ...
    if (-1 == fcntl(shared_fd, ((mode & SHARED_NOWAIT) ? F_SETLK : F_SETLKW), &flk)) {
        switch (errno) {
            ...
            case EACCES:
                if (shared_debug) printf("again]");
                return (SHARED_AGAIN);
            default:
                if (shared_debug) printf("err]");
...
static int shared_get_free_entry(
        int newhandle) /* get newhandle, or -1, entry is set rw locked */
{
...
static long shared_adjust_size(long size) /* size must be >= 0 !!! */
{
    return (((size + sizeof(BLKHEAD) + SHARED_GRANUL - 1) / SHARED_GRANUL) *
            SHARED_GRANUL);
}
...
int shared_malloc(long size, int mode, int newhandle) /* return idx or SHARED_INVALID */
{
    ...
    for (i = 0;; i++) {
        ...
        h = shmget(key, shared_adjust_size(size),
        ...
        if (((BLKHEAD*)SHARED_INVALID) ==
            bp) /* cannot attach, delete segment, try with another key */
        {
            shmctl(h, IPC_RMID, 0);
            continue;
        ...
        bp->s.tflag = BLOCK_SHARED; /* fill in data in segment's header (this is really
                                       not necessary) */
...
int shared_attach(int idx) {
    ...
    if (shared_gt[idx].attr &
        SHARED_RESIZE) /* if resizeable, detach and return special pointer */
    {
...
static int shared_map(int idx) /* map all tables for given idx, check for validity */
{
    ...
    if (((BLKHEAD*)SHARED_INVALID) == (bp = (BLKHEAD*)shmat(h, 0, 0)))
        return (SHARED_BADARG);
    if ((SHARED_ID_0 != bp->s.ID[0]) || (SHARED_ID_1 != bp->s.ID[1]) ||
        (BLOCK_SHARED != bp->s.tflag) || (h != shared_gt[idx].handle)) {
        shmdt((char*)bp); /* invalid segment, detach everything */
        return (SHARED_BADARG);
    }
    if (shared_gt[idx].sem !=
        semget(shared_gt[idx].semkey, 1,
               shared_create_mode)) /* check if sema is still there */
    {
...
SHARED_P shared_realloc(int idx, long newsize) /* realloc shared memory segment */
{
    ...
    for (i = 0;; i++) {
        ...
        h = shmget(key, shared_adjust_size(newsize),
                   IPC_CREAT | IPC_EXCL | shared_create_mode);
        if (SHARED_INVALID == h) continue; /* segment already accupied */
        bp = (BLKHEAD*)shmat(h, 0, 0);     /* try attach */
        if (((BLKHEAD*)SHARED_INVALID) ==
        ...
        *bp = *(shared_lt[idx].p); /* copy header, then data */
        transfersize =
                ((newsize < shared_gt[idx].size) ? newsize : shared_gt[idx].size);
        if (transfersize > 0)
...
int shared_free(
        int idx) /* detach segment, if last process & !PERSIST, destroy segment */
{
    int cnt, r, r2;
    ...
    if (SHARED_OK != (r = shared_validate(idx, SHARED_RDWRITE | SHARED_WAIT)))
        return (r);
    if (SHARED_OK !=
        (r = shared_detach_process(
                 shared_gt[idx].sem))) /* update number of processes using segment */
    {
        shared_demux(idx, SHARED_RDWRITE);
        return (r);
    }
    ...
    if (shared_lt[idx].tcnt > 0)
        return (shared_demux(
                idx,
                SHARED_RDWRITE)); /* if more threads are using segment we are done */
    if (shmdt((char*)(shared_lt[idx].p))) /* if, we are the last thread, try to detach
                                             segment */
    {
        shared_demux(idx, SHARED_RDWRITE);
        return (SHARED_IPCERR);
    ...
    shared_lt[idx].seekpos = 0L; /* r/w pointer positioned at beg of block */
    if (-1 ==
        (cnt = shared_process_count(
                 shared_gt[idx].sem))) /* get number of processes hanging on segment */
    {
        shared_demux(idx, SHARED_RDWRITE);
        return (SHARED_IPCERR);
    }
...
int shared_set_attr(int idx,
                    int newattr) /* get the attributes of the shared memory segment */
{
    int r;
...
int shared_list(int id) {
    ...
    for (i = 0; i < shared_maxseg; i++) {
        ...
        if (SHARED_INVALID == shared_gt[i].key) continue; /* unused slot */
        switch (shared_mux(
                i, SHARED_NOWAIT | SHARED_RDONLY)) /* acquire exclusive access to
                                                      segment, but do not wait */
...
int smem_open(char* filename, int rwmode, int* driverhandle) {
    ...
    if (NULL ==
        (sp = (DAL_SHM_SEGHEAD*)shared_lock(
                 h, ((READWRITE == rwmode) ? SHARED_RDWRITE : SHARED_RDONLY)))) {
        shared_free(h);
        return (SHARED_BADARG);
    }
...
int smem_write(int driverhandle, void* buffer, long nbytes) {
    ...
    if (nbytes < 0) return (SHARED_BADARG);
    if ((unsigned long)(shared_lt[driverhandle].seekpos + nbytes) >
        (unsigned long)(shared_gt[driverhandle].size -
                        sizeof(DAL_SHM_SEGHEAD))) { /* need to realloc shmem */
        if (NULL ==
            shared_realloc(driverhandle, shared_lt[driverhandle].seekpos + nbytes +
                                                 sizeof(DAL_SHM_SEGHEAD)))
            return (SHARED_NOMEM);
    }
