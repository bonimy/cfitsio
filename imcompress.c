...
int imcomp_write_nocompress_tile(fitsfile* outfptr, long row, int datatype,
                                 void* tiledata, long tilelen, int nullcheck,
                                 void* nullflagval, int* status);
int imcomp_convert_tile_tshort(fitsfile* outfptr, void* tiledata, long tilelen,
                               int nullcheck, void* nullflagval, int nullval,
                               int zbitpix, double scale, double zero,
                               double actual_bzero, int* intlength, int* status);
int imcomp_convert_tile_tushort(fitsfile* outfptr, void* tiledata, long tilelen,
                                int nullcheck, void* nullflagval, int nullval,
                                int zbitpix, double scale, double zero, int* intlength,
                                int* status);
int imcomp_convert_tile_tint(fitsfile* outfptr, void* tiledata, long tilelen,
                             int nullcheck, void* nullflagval, int nullval, int zbitpix,
                             double scale, double zero, int* intlength, int* status);
...
int imcomp_convert_tile_tsbyte(fitsfile* outfptr, void* tiledata, long tilelen,
                               ...
                               int zbitpix, double scale, double zero, int* intlength,
                               int* status);
int imcomp_convert_tile_tfloat(fitsfile* outfptr, long row, void* tiledata,
                               long tilelen, long tilenx, long tileny, int nullcheck,
                               void* nullflagval, int nullval, int zbitpix,
                               double scale, double zero, int* intlength, int* flag,
                               double* bscale, double* bzero, int* status);
int imcomp_convert_tile_tdouble(fitsfile* outfptr, long row, void* tiledata,
                                long tilelen, long tilenx, long tileny, int nullcheck,
                                void* nullflagval, int nullval, int zbitpix,
                                double scale, double zero, int* intlength, int* flag,
                                double* bscale, double* bzero, int* status);

static int unquantize_i1r4(
        long row, unsigned char* input, /* I - array of values to be converted     */
...
static int unquantize_i2r4(
        long row, short* input, /* I - array of values to be converted     */
        long ntodo,             /* I - number of elements in the array     */
        double scale,           /* I - FITS TSCALn or BSCALE value         */
        double zero,            /* I - FITS TZEROn or BZERO  value         */
        int dither_method,      /* I - which subtractive dither method to use */
        ...
        float* output,          /* O - array of converted pixels           */
        int* status);           /* IO - error status                       */
static int unquantize_i4r4(
        long row, INT32BIT* input, /* I - array of values to be converted     */
        long ntodo,                /* I - number of elements in the array     */
        double scale,              /* I - FITS TSCALn or BSCALE value         */
        double zero,               /* I - FITS TZEROn or BZERO  value         */
        int dither_method,         /* I - which subtractive dither method to use */
...
static int fits_read_write_compressed_img(
...
static int fits_short_to_int_inplace(short* intarray, long length, int shift,
                                     int* status);
static int fits_ushort_to_int_inplace(unsigned short* intarray, long length, int shift,
                                      int* status);
...
static int fits_calc_tile_rows(long* tlpixel, long* tfpixel, int ndim, long* trowsize,
                               long* ntrows, int* status);
                              ...
                              int* status) /* IO - error status */
{
    ...
    */
    ...
    if (ctype != RICE_1 && ctype != GZIP_1 && ctype != GZIP_2 && ctype != PLIO_1 &&
        ctype != HCOMPRESS_1 && ctype != BZIP2_1 && ctype != NOCOMPRESS && ctype != 0) {
                              ...
                              int* status) /* IO - error status */
{
    ...
    *ctype = (fptr->Fptr)->request_compress_type;
    ...
    if (*ctype != RICE_1 && *ctype != GZIP_1 && *ctype != GZIP_2 && *ctype != PLIO_1 &&
        *ctype != HCOMPRESS_1 && *ctype != BZIP2_1 && *ctype != NOCOMPRESS &&
        *ctype != 0)
        ...
        if (!strncmp(card, "FZ", 2)) {
            ...
            if (!strncmp(card + 2, "ALGOR", 5)) {
                ...
                if (!fits_strncasecmp(value, "'no_dither", 10)) {
                    ...
                    ivalue = SUBTRACTIVE_DITHER_2; /* dither, except preserve
                                                      zero-valued pixels */
                } else {
                ...
                if (!fits_strncasecmp(value, "'checksum", 9)) {
                    ...
                    if (*value == '\'')
                        ivalue = (int)atol(value +
                                           1); /* allow for leading quote character */
                    else
                        ivalue = (int)atol(value);

                    if (ivalue < 1 || ivalue > 10000) {
                        ffpmsg("Invalid value for FZDTHRSD keyword: "
                               "(set_compression_pref)");
                        ffpmsg(value);
                        return (*status = DATA_COMPRESSION_ERR);
                    }
                }
...
*/
{
    ...
    if (fits_get_img_param(infptr, MAX_COMPRESS_DIM, &bitpix, &naxis, naxes, status) >
        0)
        return (*status);
    ...
    */
    ...
    if (imcomp_copy_img2comp(infptr, outfptr, status) > 0) return (*status);
...
/*--------------------------------------------------------------------------*/
int imcomp_init_table(
        fitsfile* outfptr, int inbitpix, int naxis, long* naxes,
...
*/
{
    ...
    int nQualifyDims =
            0; /* For Hcompress, number of image dimensions with required pixels. */
    ...
    */
    ...
    memcpy(actual_tilesize, outfptr->Fptr->request_tilesize,
           MAX_COMPRESS_DIM * sizeof(long));
    ...
    if ((outfptr->Fptr)->request_compress_type == HCOMPRESS_1) {
        ...
        if (naxis < 2) {
            ffpmsg("Hcompress cannot be used with 1-dimensional images "
                   "(imcomp_init_table)");
            return (*status = DATA_COMPRESSION_ERR);
        }
        for (ii = 0; ii < naxis; ii++) {
            if (naxes[ii] >= 4) ++nQualifyDims;
        }
        if (nQualifyDims < 2) {
        ...
        if ((actual_tilesize[0] <= 0) && (actual_tilesize[1] == -1)) {
            ...
            /*
             ...
             If the image has less than 30 rows, then the entire image will be
             compressed as a single tile.  Otherwise the tiles will consist of 16 rows
             of the image. This keeps the tiles to a reasonable size, and it also
             includes enough rows to allow good compression efficiency.  If the last
             tile of the image happens to contain less than 4 rows, then find another
             tile size with between 14 and 30 rows (preferably even), so that the last
             tile has at least 4 rows
            */
            ...
            if (actual_tilesize[0] <= 0) actual_tilesize[0] = naxes[0];
            for (ii = 1; ii < naxis; ++ii) {
        ...
        for (ii = 0; ii < naxis; ++ii) {
            if (actual_tilesize[ii] > 1) {
                if (firstDim < 0)
                    ...
                    ffpmsg("Hcompress tiles can only have 2 dimensions "
                           "(imcomp_init_table)");
                    return (*status = DATA_COMPRESSION_ERR);
                }
            }
        ...
        /* check if requested tile size causes the last tile to to have less than 4
         * pixels */
        remain = naxes[firstDim] % (actual_tilesize[firstDim]); /* 1st dimension */
        if (remain > 0 && remain < 4) {
            ndiv = naxes[firstDim] /
                   actual_tilesize[firstDim]; /* integer truncation is intentional */
            ...
            if (remain > 0 && remain < 4) {
                ffpmsg("Last tile along 1st dimension has less than 4 pixels "
                       "(imcomp_init_table)");
                return (*status = DATA_COMPRESSION_ERR);
            }
        }
        ...
        if (remain > 0 && remain < 4) {
            ndiv = naxes[secondDim] /
                   actual_tilesize[secondDim]; /* integer truncation is intentional */
            addToDim = ceil((double)remain / ndiv);
            (actual_tilesize[secondDim]) += addToDim; /* increase tile size */

            remain = naxes[secondDim] % (actual_tilesize[secondDim]);
            if (remain > 0 && remain < 4) {
                ffpmsg("Last tile along 2nd dimension has less than 4 pixels "
                       "(imcomp_init_table)");
                return (*status = DATA_COMPRESSION_ERR);
            }
        }
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (ii == 0) { /* first axis is different */
            ...
            if (actual_tilesize[ii] < 0) {
                actual_tilesize[ii] =
                        naxes[ii]; /* negative value maean use whole length */
            } else if (actual_tilesize[ii] == 0) {
    ...
    ffcrtb(outfptr, BINARY_TBL, nrows, ncols, ttype, tform, tunit, 0, status);
    ...
    if (writebitpix) {
        ...
        ffpkyj(outfptr, "ZBITPIX", bitpix, "data type of original image", status);
        ffpkyj(outfptr, "ZNAXIS", naxis, "dimension of original image", status);

        for (ii = 0; ii < naxis; ii++) {
            snprintf(keyname, FLEN_KEYWORD, "ZNAXIS%d", ii + 1);
            ffpkyj(outfptr, keyname, naxes[ii], "length of original image axis",
                   status);
        }
    }
    ...
    for (ii = 0; ii < naxis; ii++) {
        ...
        ffpkyj(outfptr, keyname, actual_tilesize[ii], "size of tiles to be compressed",
               status);
    }
    ...
    if (bitpix < 0) {
        if ((outfptr->Fptr)->request_quantize_level == NO_QUANTIZE) {
            ...
            if ((outfptr->Fptr)->request_quantize_method == SUBTRACTIVE_DITHER_1) {
                ...
                ffpky(outfptr, TINT, "ZDITHER0",
                      &((outfptr->Fptr)->request_dither_seed),
                      "dithering offset when quantizing floats", status);

            } else if ((outfptr->Fptr)->request_quantize_method ==
                ...
                if (!strcmp(zcmptype, "RICE_1")) {
                    ...
                    /* ******* can remove this cludge after about June 2015, after most
                     * old versions of fpack are gone */
    ...
    if ((outfptr->Fptr)->request_compress_type == RICE_1) {
        ...
        ffpkyj(outfptr, "ZVAL1", 32, "pixels per block", status);
        ...
        ffpkye(outfptr, "ZVAL1", (outfptr->Fptr)->request_hcomp_scale, 7,
...
*/
{
    ...
    /*
     ...
     algorithm is used:
      ...
      PLIO requires 4-byte int values, so byte and short arrays must be converted to
     int. HCompress internally converts byte or short values to ints, and converts int
     values to 8-byte longlong integers.
    */
                        ...
                        for (i0 = 1; i0 <= naxes[0]; i0 += tilesize[0]) {
                            ...
                            if (datatype == TFLOAT) {
                                ffgsve(infptr, 1, naxis, naxes, fpixel, lpixel, incre,
                                       FLOATNULLVALUE, (float*)tiledata, &anynul,
                                       status);
                             ...
                             */
                            /*   NOTE: we don't have to worry about the presence of null
                               values in the array if it is an integer array:  the null
                               value is simply encoded in the compressed array just like
                               any other pixel value.

                                 If it is a floating point array, then we need to check
                               for null only if the anynul parameter returned a true
                               value when reading the tile
                            */
                            ...
                            if (anynul && datatype == TFLOAT) {
                                imcomp_compress_tile(outfptr, row, datatype, tiledata,
                                                     tilelen, trowsize, ntrows, 1,
                                                     &fltnull, status);
                                ...
                                imcomp_compress_tile(outfptr, row, datatype, tiledata,
                                                     tilelen, trowsize, ntrows, 0,
                                                     &dummy, status);
                            }

                            /* set flag if we found any null values */
                            if (anynul) gotnulls = 1;
...
int imcomp_compress_tile(fitsfile* outfptr,
                         long row, /* tile number = row in the binary table that holds
                                      the compressed data */
                         int datatype, void* tiledata, long tilelen, long tilenx,
...
*/
{
    ...
    if ((outfptr->Fptr)->quantize_level == NO_QUANTIZE) {
        if (((outfptr->Fptr)->compress_type != GZIP_1) &&
            ...
            switch (datatype) {
                ...
                case TCOMPLEX:
                case TDBLCOMPLEX:
                    ffpmsg("Lossless compression of floating point images must use "
                           "GZIP (imcomp_compress_tile)");
                    return (*status = DATA_COMPRESSION_ERR);
                default:
                    break;
            }
    ...
    if ((outfptr->Fptr)->tilerow) { /* has the tile cache been allocated? */
        ...
        tilecol = (row - 1) % ((long)(((outfptr->Fptr)->znaxis[0] - 1) /
                                      ((outfptr->Fptr)->tilesize[0])) +
                               1);
    ...
    if (datatype == TSHORT) {
        imcomp_convert_tile_tshort(outfptr, tiledata, tilelen, nullcheck, nullflagval,
                                   nullval, zbitpix, scale, zero, actual_bzero,
                                   &intlength, status);
        ...
        ffpmsg("Integer*8 Long datatype is not supported when writing to compressed "
               "images");
        return (*status = BAD_DATATYPE);
    } else if (datatype == TULONG && sizeof(long) == 8) {
        ffpmsg("Unsigned integer*8 datatype is not supported when writing to "
               "compressed images");
        return (*status = BAD_DATATYPE);
    } else if (datatype == TFLOAT) {
        imcomp_convert_tile_tfloat(outfptr, row, tiledata, tilelen, tilenx, tileny,
                                   nullcheck, nullflagval, nullval, zbitpix, scale,
                                   zero, &intlength, &flag, bscale, bzero, status);
    } else if (datatype == TDOUBLE) {
        imcomp_convert_tile_tdouble(outfptr, row, tiledata, tilelen, tilenx, tileny,
                                    nullcheck, nullflagval, nullval, zbitpix, scale,
                                    zero, &intlength, &flag, bscale, bzero, status);
    } else {
    ...
    if (*status > 0) return (*status); /* return if error occurs */
    ...
    if (flag) /* now compress the integer data array */
    {
        ...
        if ((outfptr->Fptr)->compress_type == RICE_1) {
            if (intlength == 2) {
                ...
                nelem = fits_rcomp_byte((signed char*)idata, tilelen,
                                        ...
                                        (outfptr->Fptr)->rice_blocksize);
            } else {
                nelem = fits_rcomp(idata, tilelen, (unsigned char*)cbuf, clen,
                                   (outfptr->Fptr)->rice_blocksize);
            }
            ...
            ffpclb(outfptr, (outfptr->Fptr)->cn_compressed, row, 1, nelem,
                   (unsigned char*)cbuf, status);
        }
         ...
         */
            ...
            /* Write the compressed byte stream. */
            ffpcli(outfptr, (outfptr->Fptr)->cn_compressed, row, 1, nelem, cbuf,
                   status);
        }
         ...
         */
            ...
            if ((outfptr->Fptr)->quantize_level == NO_QUANTIZE && datatype == TFLOAT) {
                /* Special case of losslessly compressing floating point pixels with
                 * GZIP */
                ...
                compress2mem_from_mem((char*)tiledata, tilelen * sizeof(float),
                                      (char**)&cbuf, &clen, realloc, &gzip_nelem,
                                      status);
            ...
            /* Write the compressed byte stream. */
            ffpclb(outfptr, (outfptr->Fptr)->cn_compressed, row, 1, gzip_nelem,
                   (unsigned char*)cbuf, status);
            ...
            /*  bzip2 is not supported in the public release.  This is only for test
               purposes. if (BZ2_bzBuffToBuffCompress( (char *) cbuf, &bzlen, (char *)
               idata, (unsigned int) (tilelen * intlength), 9, 0, 0) )
            */
            {
            ...
            /* Write the compressed byte stream. */
            ffpclb(outfptr, (outfptr->Fptr)->cn_compressed, row, 1, bzlen,
                   (unsigned char*)cbuf, status);
            ...
            */
            ...
            if (hcompscale > 0.) {
                fits_img_stats_int(idata, tilenx, tileny, nullcheck, nullval, 0, 0, 0,
                                   0, 0, 0, &noise2, &noise3, &noise5, status);
            ...
            if (zbitpix == BYTE_IMG || zbitpix == SHORT_IMG) {
                fits_hcompress(idata, tilenx, tileny, ihcompscale, (char*)cbuf,
                               &hcomp_len, status);
                ...
                fits_hcompress64(lldata, tilenx, tileny, ihcompscale, (char*)cbuf,
                                 &hcomp_len, status);
            }

            /* Write the compressed byte stream. */
            ffpclb(outfptr, (outfptr->Fptr)->cn_compressed, row, 1, hcomp_len,
                   (unsigned char*)cbuf, status);
        }
        ...
        if ((outfptr->Fptr)->cn_zscale > 0) {
            ...
            ffpcld(outfptr, (outfptr->Fptr)->cn_zscale, row, 1, 1, bscale, status);
        ...
        /* Write the compressed byte stream. */
        ffpclb(outfptr, (outfptr->Fptr)->cn_gzip_data, row, 1, gzip_nelem,
               (unsigned char*)cbuf, status);
...
int imcomp_write_nocompress_tile(fitsfile* outfptr, long row, int datatype,
    ...
    if ((outfptr->Fptr)->cn_uncompressed < 1) {
        ...
        if (datatype == TSHORT) {
            ...
            ffpmsg("NOCOMPRESSION option only supported for int*2, int*4, and float*4 "
                   "images");
            return (*status = DATA_COMPRESSION_ERR);
        }

        fits_insert_col(outfptr, 999, "UNCOMPRESSED_DATA", coltype,
                        status); /* create column */
    }

    fits_get_colnum(outfptr, CASEINSEN, "UNCOMPRESSED_DATA",
                    &(outfptr->Fptr)->cn_uncompressed, status); /* save col. num. */

    fits_write_col(outfptr, datatype, (outfptr->Fptr)->cn_uncompressed, row, 1, tilelen,
                   tiledata, status); /* write the tile data */
    return (*status);
}
/*--------------------------------------------------------------------------*/
int imcomp_convert_tile_tshort(fitsfile* outfptr, void* tiledata, long tilelen,
    ...
    if (zbitpix != SHORT_IMG || scale != 1.0 || zero != 0.0) {
        ffpmsg("Datatype conversion/scaling is not supported when writing to "
               "compressed images");
        return (*status = DATA_COMPRESSION_ERR);
    }
    ...
    if ((outfptr->Fptr)->compress_type == RICE_1 ||
        ...
        *intlength = 2;
        ...
        if (nullcheck == 1) {
            /* reset pixels equal to flagval to the FITS null value, prior to
             * compression */
            flagval = *(short*)(nullflagval);
            if (flagval != nullval) {
                for (ii = tilelen - 1; ii >= 0; ii--) {
                    if (sbuff[ii] == (short)flagval) sbuff[ii] = (short)nullval;
                }
            }
        }
    } else if ((outfptr->Fptr)->compress_type == HCOMPRESS_1) {
...
int imcomp_convert_tile_tushort(fitsfile* outfptr, void* tiledata, long tilelen,
                                ...
                                int* status) {
    /*  Prepare the input  tile array of pixels for compression. */
    /*  Convert input unsigned integer*2 tile array in place to 4 or 8-byte ints for
     * compression, */
    /*  If needed, convert 4 or 8-byte ints and do null value substitution. */
    /*  Note that the calling routine must have allocated the input array big enough */
    /* to be able to do this.  */
    ...
    if (zbitpix != SHORT_IMG || scale != 1.0 || zero != 32768.) {
        ffpmsg("Implicit datatype conversion is not supported when writing to "
               "compressed images");
        return (*status = DATA_COMPRESSION_ERR);
    }
    ...
    if ((outfptr->Fptr)->compress_type == RICE_1 ||
        ...
        *intlength = 2;
        ...
        /* It is more efficient to do this by just flipping the most significant of the
         * 16 bits */
        ...
        if (nullcheck == 1) {
            ...
            for (ii = tilelen - 1; ii >= 0; ii--) usbuff[ii] = (usbuff[ii]) ^ 0x8000;
        }
    } else {
...
int imcomp_convert_tile_tint(fitsfile* outfptr, void* tiledata, long tilelen,
    ...
    *intlength = 4;
    ...
    if (nullcheck == 1) {
        ...
        if (flagval != nullval) {
            for (ii = tilelen - 1; ii >= 0; ii--) {
                if (idata[ii] == flagval) idata[ii] = nullval;
            }
        }
    }
...
int imcomp_convert_tile_tuint(fitsfile* outfptr, void* tiledata, long tilelen,
                              ...
                              int* status) {
    /*  Prepare the input tile array of pixels for compression. */
    /*  Convert input unsigned integer tile array in place to 4 or 8-byte ints for
     * compression, */
    /*  If needed, do null value substitution. */
    ...
    *intlength = 4;
    ...
    /* It is more efficient to do this by just flipping the most significant of the 32
     * bits */
...
int imcomp_convert_tile_tbyte(fitsfile* outfptr, void* tiledata, long tilelen,
                              ...
                              int* status) {
    /*  Prepare the input tile array of pixels for compression. */
    /*  Convert input unsigned integer*1 tile array in place to 4 or 8-byte ints for
     * compression, */
    /*  If needed, convert 4 or 8-byte ints and do null value substitution. */
    /*  Note that the calling routine must have allocated the input array big enough */
    /* to be able to do this.  */
    ...
    if ((outfptr->Fptr)->compress_type == RICE_1 ||
        ...
        *intlength = 1;
        ...
        if (nullcheck == 1) {
            ...
            for (ii = tilelen - 1; ii >= 0; ii--) sbbuff[ii] = (sbbuff[ii]) ^ 0x80;
        }
...
int imcomp_convert_tile_tfloat(fitsfile* outfptr, long row, void* tiledata,
    ...
    /* datatype of input array is double.  We only support writing this datatype
       to a FITS image with BITPIX = -64 or -32, except we also support the special case
       where BITPIX = 32 and BZERO = 0 and BSCALE = 1.  */
    ...
    *intlength = 4;
    ...
    if ((outfptr->Fptr)->cn_zscale > 0) {
        ...
        if ((outfptr->Fptr)->quantize_method == SUBTRACTIVE_DITHER_1 ||
            ...
            if ((outfptr->Fptr)->request_dither_seed == 0 &&
                (outfptr->Fptr)->dither_seed == 0) {
                /* This means randomly choose the dithering offset based on the system
                 * time. */
                /* The offset will have a value between 1 and 10000, inclusive. */
                /* The time function returns an integer value that is incremented each
                 * second. */
                /* The clock function returns the elapsed CPU time, in integer
                 * CLOCKS_PER_SEC units. */
                /* The CPU time returned by clock is typically (on linux PC) only good
                 * to 0.01 sec */
                /* Summing the 2 quantities may help avoid cases where 2 executions of
                 * the program */
                /* (perhaps in a multithreaded environoment) end up with exactly the
                 * same dither seed */
                /* value.  The sum is incremented by the current HDU number in the file
                 * to provide */
                /* further randomization.  This randomization is desireable if multiple
                 * compressed */
                /* images will be summed (or differenced). In such cases, the benefits
                 * of dithering */
                /* may be lost if all the images use exactly the same sequence of random
                 * numbers when */
                /* calculating the dithering offsets. */

                (outfptr->Fptr)->dither_seed =
                        (((int)time(NULL) +
                          ((int)clock() / (int)(CLOCKS_PER_SEC / 100)) +
                          (outfptr->Fptr)->curhdu) %
                         10000) +
                        1;

                /* update the header keyword with this new value */
                fits_update_key(outfptr, TINT, "ZDITHER0",
                                &((outfptr->Fptr)->dither_seed), NULL, status);

            } else if ((outfptr->Fptr)->request_dither_seed < 0 &&
                       (outfptr->Fptr)->dither_seed < 0) {
                /* this means randomly choose the dithering offset based on some hash
                 * function */
                /* of the first input tile of data to be quantized and compressed.  This
                 * ensures that */
                /* the same offset value is used for a given image every time it is
                 * compressed. */

                usbbuff = (unsigned char*)tiledata;
                dithersum = 0;
                for (ii = 0; ii < 4 * tilelen; ii++) {
                    dithersum += usbbuff[ii]; /* doesn't matter if there is an integer
                                                 overflow */
                }
            ...
            irow = row + (outfptr->Fptr)->dither_seed -
                   1; /* dither the quantized values */
        ...
        *flag = fits_quantize_float(irow, (float*)tiledata, tilenx, tileny, nullcheck,
                                    floatnull, (outfptr->Fptr)->quantize_level,
                                    (outfptr->Fptr)->quantize_method, idata, bscale,
                                    bzero, &iminval, &imaxval);

        if (*flag > 1) return (*status = *flag);
    } else if ((outfptr->Fptr)->quantize_level != NO_QUANTIZE) {
        ...
        if ((scale != 1. || zero != 0.)) /* must scale the values */
            ...
            imcomp_nullfloats((float*)tiledata, tilelen, idata, nullcheck,
                              *(float*)(nullflagval), nullval, status);
    } else if ((outfptr->Fptr)->quantize_level == NO_QUANTIZE) {
        /* just convert null values to NaNs in place, if necessary, then do lossless
         * gzip compression */
        if (nullcheck == 1) {
            imcomp_float2nan((float*)tiledata, tilelen, (int*)tiledata,
                             *(float*)(nullflagval), status);
...
int imcomp_convert_tile_tdouble(fitsfile* outfptr, long row, void* tiledata,
    ...
    *intlength = 4;
    ...
    if ((outfptr->Fptr)->cn_zscale > 0) {
        ...
        *flag = fits_quantize_double(irow, (double*)tiledata, tilenx, tileny, nullcheck,
                                     doublenull, (outfptr->Fptr)->quantize_level,
                                     (outfptr->Fptr)->quantize_method, idata, bscale,
                                     bzero, &iminval, &imaxval);
        ...
        if ((scale != 1. || zero != 0.)) /* must scale the values */
            imcomp_nullscaledoubles((double*)tiledata, tilelen, idata, scale, zero,
                                    nullcheck, *(double*)(nullflagval), nullval,
                                    status);
        else
            imcomp_nulldoubles((double*)tiledata, tilelen, idata, nullcheck,
                               *(double*)(nullflagval), nullval, status);
    } else if ((outfptr->Fptr)->quantize_level == NO_QUANTIZE) {
        /* just convert null values to NaNs in place, if necessary, then do lossless
         * gzip compression */
        if (nullcheck == 1) {
...
*/
{
    ...
    for (ii = 0; ii < tilelen; ii++) {
        if (idata[ii] == nullflagval) idata[ii] = nullval;
    }
    return (*status);
}
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
int imcomp_nullscalei2(short* idata, long tilelen, short nullflagval, short nullval,
                       double scale, double zero, int* status)
/*
   do null value substitution AND scaling of the integer array.
   If array value = nullflagval, then set the value to nullval.
...
/*---------------------------------------------------------------------------*/
int imcomp_nullvaluesi2(short* idata, long tilelen, short nullflagval, short nullval,
                        int* status)
/*
   do null value substitution.
   If array value = nullflagval, then set the value to nullval.
...
/*---------------------------------------------------------------------------*/
int imcomp_scalevaluesi2(short* idata, long tilelen, double scale, double zero,
                         int* status)
/*
   do inverse scaling the integer values.
*/
...
/*---------------------------------------------------------------------------*/
int imcomp_nullscalefloats(float* fdata, long tilelen, int* idata, double scale,
                           double zero, int nullcheck, float nullflagval, int nullval,
                           int* status)
/*
   do null value substitution  of the float array.
   If array value = nullflagval, then set the output value to FLOATNULLVALUE.
...
/*---------------------------------------------------------------------------*/
int imcomp_nullscaledoubles(double* fdata, long tilelen, int* idata, double scale,
                            double zero, int nullcheck, double nullflagval, int nullval,
                            int* status)
/*
   do null value substitution  of the float array.
   If array value = nullflagval, then set the output value to FLOATNULLVALUE.
...
*/
{
    ...
    /* rescan header if data structure is undefined */
        ...
        if (ffrdef(fptr, status) > 0) return (*status);
    ...
    if ((fptr->Fptr)->compress_type == HCOMPRESS_1) { /* need 4 or 8 bytes per pixel */
        ...
        buffpixsiz = maxvalue(buffpixsiz, 4);
               ...
               (fptr->Fptr)->compress_type ==
                       BZIP2_1) { /* need 1, 2, or 4 bytes per pixel */
    ...
    for (ii = 0; ii < ndim; ii++) {
        ...
        ltile[ii] = minvalue((lpixel[ii] - 1) / tilesize[ii] + 1, tiledim[ii]);
            ...
            for (i3 = ftile[3]; i3 <= ltile[3]; i3++) {
                ...
                for (i2 = ftile[2]; i2 <= ltile[2]; i2++) {
                    ...
                    for (i1 = ftile[1]; i1 <= ltile[1]; i1++) {
                        ...
                        thistilesize[1] =
                                thistilesize[2] * (tlpixel[1] - tfpixel[1] + 1);
                        offset[1] = (i1 - 1) * rowdim[1] + offset[2];
                        for (i0 = ftile[0]; i0 <= ltile[0]; i0++) {
                            tfpixel[0] = (i0 - 1) * tilesize[0] + 1;
                            tlpixel[0] =
                                    minvalue(tfpixel[0] + tilesize[0] - 1, naxis[0]);
                            thistilesize[0] =
                                    thistilesize[1] * (tlpixel[0] - tfpixel[0] + 1);
                            ...
                            imcomp_decompress_tile(fptr, irow, thistilesize[0],
                                                   datatype, nullcheck, nullval, buffer,
                                                   bnullarray, &tilenul, status);
                            ...
                            imcomp_merge_overlap(buffer, pixlen, ndim, tfpixel, tlpixel,
                                                 bnullarray, array, fpixel, lpixel,
                                                 nullcheck, status);
                            ...
                            /* compress the tile again, and write it back to the FITS
                             * file */
                            imcomp_compress_tile(fptr, irow, datatype, buffer,
                                                 thistilesize[0], trowsize, ntrows,
...
*/
{
    ...
    if (naxis == 1) {
        ...
        fits_write_compressed_img(fptr, datatype, firstcoord, lastcoord, nullcheck,
                                  array, nullval, status);
        return (*status);
    } else if (naxis == 2) {
        nplane = 0; /* write 1st (and only) plane of the image */
        fits_write_compressed_img_plane(fptr, datatype, bytesperpixel, nplane,
                                        firstcoord, lastcoord, naxes, nullcheck, array,
                                        nullval, &nread, status);
    } else if (naxis == 3) {
        /* test for special case: writing an integral number of planes */
        if (firstcoord[0] == 0 && firstcoord[1] == 0 && lastcoord[0] == naxes[0] - 1 &&
        ...
        for (nplane = firstcoord[2]; nplane <= lastcoord[2]; nplane++) {
            ...
            fits_write_compressed_img_plane(fptr, datatype, bytesperpixel, nplane,
                                            firstcoord, lastcoord, naxes, nullcheck,
 ...
 */
{
    ...
    *nread = 0;
    ...
    if (firstcoord[0] != 0) {
        ...
        fits_write_compressed_img(fptr, datatype, blc, trc, nullcheck, arrayptr,
                                  nullval, status);
    ...
    fits_write_compressed_img(fptr, datatype, blc, trc, nullcheck, arrayptr, nullval,
                              status);
...

{
    ...
    fits_read_write_compressed_img(infptr, datatype, fpixel, lpixel, inc, nullcheck,
                                   nulladdr, &anynul, outfptr, status);
...

{
    ...
    fits_read_compressed_img(infptr, datatype, fpixel, lpixel, inc, nullcheck, nulladdr,
                             data, NULL, &anynul, status);

    /* write the image to the output file */
    if (anynul)
        fits_write_imgnull(outfptr, datatype, 1, imgsize, data, nulladdr, status);
    else
...
/*--------------------------------------------------------------------------*/
int fits_img_decompress_header(
        fitsfile* infptr,  /* image (bintable) to uncompress */
        fitsfile* outfptr, /* empty HDU for output uncompressed image */
        int* status)       /* IO - error status               */
...

{
    ...
    if (!fits_read_card(infptr, "ZSIMPLE", card, &tstatus)) {
        ...
        if (hdupos == 1) {      /* are we positioned at the primary array? */
            if (numkeys == 0) { /* primary HDU is completely empty */
                ...
                fits_get_img_param(outfptr, MAX_COMPRESS_DIM, &bitpix, &naxis, naxes,
                                   status);
    ...
    if (nullprime) {
        ...
        */
        ...
        for (ii = numkeys; ii > 0; ii--) fits_delete_record(outfptr, ii, status);
...
*/
{
    ...
    if (nullcheck == 1 && testnullval == 0.) nullcheck = 0;
                    ...
                    for (i1 = ftile[1]; i1 <= ltile[1]; i1++) {
                        ...
                        for (i0 = ftile[0]; i0 <= ltile[0]; i0++) {
                            ...
                            printf("row %d, %d %d, %d %d, %d %d; %d\n",
                                          irow,
                            tfpixel[0],tlpixel[0],tfpixel[1],tlpixel[1],tfpixel[2],tlpixel[2],
                                          thistilesize[0]);
                            */
                            /* test if there are any intersecting pixels in this tile
                             * and the output image */
                            if (imcomp_test_overlap(ndim, tfpixel, tlpixel, fpixel,
                                                    lpixel, inc, status)) {
                                /* read and uncompress this row (tile) of the table */
                                /* also do type conversion and undefined pixel
                                 * substitution */
                                /* at this point */

                                imcomp_decompress_tile(fptr, irow, thistilesize[0],
                                                       datatype, nullcheck, nullval,
                                                       buffer, bnullarray, &tilenul,
                                                       status);

                                if (tilenul && anynul)
                                    *anynul = 1; /* there are null pixels */
                                                 /*
                                                 printf(" pixlen=%d, ndim=%d, %d %d %d, %d %d %d, %d %d
                                                 %d\n",                  pixlen, ndim,
                                                 fpixel[0],lpixel[0],inc[0],fpixel[1],lpixel[1],inc[1],
                                                      fpixel[2],lpixel[2],inc[2]);
                                                 */
                                /* copy the intersecting pixels from this tile to the
                                 * output */
                                imcomp_copy_overlap(buffer, pixlen, ndim, tfpixel,
                                                    tlpixel, bnullarray, array, fpixel,
                                                    lpixel, inc, nullcheck, nullarray,
                                                    status);
                            }
                        }
                    }
                ...
                for (i2 = ftile[2]; i2 <= ltile[2]; i2++) {
                    ...
                    for (i1 = ftile[1]; i1 <= ltile[1]; i1++) {
                        ...
                        for (i0 = ftile[0]; i0 <= ltile[0]; i0++) {
                            ...
                            if (tilenul && anynul) {
                                /* this assumes that the tiled pixels are in the same
                                   order as in the uncompressed FITS image.  This is not
                                   necessarily the case, but it almost alway is in
                                   practice. Note that null checking is not performed
                                   for integer images, so this could only be a problem
                                   for tile compressed floating point images that use an
                                   unconventional tiling pattern.
                                */
                                fits_write_imgnull(outfptr, datatype, firstelem,
                                                   thistilesize[0], buffer, nullval,
                                                   status);
                            } else {
...
*/
{
    ...
    if (naxis == 1) {
        ...
        firstcoord[0] = firstcoord[0] + 1;
        lastcoord[0] = lastcoord[0] + 1;
        fits_read_compressed_img(fptr, datatype, firstcoord, lastcoord, inc, nullcheck,
                                 nullval, array, nullarray, anynul, status);
        return (*status);
    } else if (naxis == 2) {
        nplane = 0; /* read 1st (and only) plane of the image */

        fits_read_compressed_img_plane(
                fptr, datatype, bytesperpixel, nplane, firstcoord, lastcoord, inc,
                naxes, nullcheck, nullval, array, nullarray, anynul, &nread, status);
    } else if (naxis == 3) {
        ...
        if (firstcoord[0] == 0 && firstcoord[1] == 0 && lastcoord[0] == naxes[0] - 1 &&
            ...
            fits_read_compressed_img(fptr, datatype, firstcoord, lastcoord, inc,
                                     nullcheck, nullval, array, nullarray, anynul,
                                     status);
        ...
        for (nplane = (long)firstcoord[2]; nplane <= lastcoord[2]; nplane++) {
            ...
            fits_read_compressed_img_plane(fptr, datatype, bytesperpixel, nplane,
                                           firstcoord, lastcoord, inc, naxes, nullcheck,
                                           nullval, arrayptr, nullarrayptr, &planenul,
                                           &nread, status);

            if (planenul && anynul) *anynul = 1; /* there are null pixels */
            ...
            arrayptr = arrayptr + nread * bytesperpixel;
            if (nullarrayptr && (nullcheck == 2)) nullarrayptr = nullarrayptr + nread;
        }
    } else {
        ffpmsg("only 1D, 2D, or 3D images are currently supported");
        return (*status = DATA_DECOMPRESSION_ERR);
 ...
 */
{
    ...
    *nread = 0;
    ...
    if (firstcoord[0] != 0) {
        ...
        fits_read_compressed_img(fptr, datatype, blc, trc, inc, nullcheck, nullval,
                                 arrayptr, nullarrayptr, &tnull, status);

        *nread = *nread + (long)(trc[0] - blc[0] + 1);

        if (tnull && anynul) *anynul = 1; /* there are null pixels */
...
*/
{
    ...
    if (ffgky(infptr, TINT, "ZBITPIX", &(infptr->Fptr)->zbitpix, NULL, status) > 0) {
    ...
    for (ii = 0; ii < (infptr->Fptr)->zndim; ii++) {
        ...
        tstatus = 0;
        ffgky(infptr, TLONG, keyword, &(infptr->Fptr)->tilesize[ii], NULL, &tstatus);

        expect_nrows *=
                (((infptr->Fptr)->znaxis[ii] - 1) / (infptr->Fptr)->tilesize[ii] + 1);
    ...
    if ((infptr->Fptr)->compress_type == RICE_1) {
        if (ffgky(infptr, TINT, "ZVAL1", &(infptr->Fptr)->rice_blocksize, NULL,
                  status) > 0) {
        ...
        if (ffgky(infptr, TFLOAT, "ZVAL1", &(infptr->Fptr)->hcomp_scale, NULL, status) >
            0) {
            ffpmsg("required ZVAL1 compression keyword not found");
            return (*status);
        }
        ...
        tstatus = 0;
        ffgky(infptr, TINT, "ZVAL2", &(infptr->Fptr)->hcomp_smooth, NULL, &tstatus);
    }
    ...
    (infptr->Fptr)->maxelem = imcomp_calc_max_elem((infptr->Fptr)->compress_type,
                                                   maxtilelen, (infptr->Fptr)->zbitpix,
                                                   (infptr->Fptr)->rice_blocksize);

    /* Get Column numbers. */
    if (ffgcno(infptr, CASEINSEN, "COMPRESSED_DATA", &(infptr->Fptr)->cn_compressed,
               status) > 0) {
    ...
    tstatus = 0;
    ffgcno(infptr, CASEINSEN, "UNCOMPRESSED_DATA", &(infptr->Fptr)->cn_uncompressed,
           &tstatus);

    tstatus = 0;
    ffgcno(infptr, CASEINSEN, "GZIP_COMPRESSED_DATA", &(infptr->Fptr)->cn_gzip_data,
           &tstatus);

    tstatus = 0;
    if (ffgcno(infptr, CASEINSEN, "ZSCALE", &(infptr->Fptr)->cn_zscale, &tstatus) > 0) {
        /* CMPSCALE column doesn't exist; see if there is a keyword */
        tstatus = 0;
        if (ffgky(infptr, TDOUBLE, "ZSCALE", &(infptr->Fptr)->zscale, NULL, &tstatus) <=
            0)
            (infptr->Fptr)->cn_zscale = -1; /* flag for a constant ZSCALE */
    }

    tstatus = 0;
    if (ffgcno(infptr, CASEINSEN, "ZZERO", &(infptr->Fptr)->cn_zzero, &tstatus) > 0) {
        /* CMPZERO column doesn't exist; see if there is a keyword */
        tstatus = 0;
        if (ffgky(infptr, TDOUBLE, "ZZERO", &(infptr->Fptr)->zzero, NULL, &tstatus) <=
            0)
            (infptr->Fptr)->cn_zzero = -1; /* flag for a constant ZZERO */
    }

    tstatus = 0;
    if (ffgcno(infptr, CASEINSEN, "ZBLANK", &(infptr->Fptr)->cn_zblank, &tstatus) > 0) {
        /* ZBLANK column doesn't exist; see if there is a keyword */
        tstatus = 0;
        if (ffgky(infptr, TINT, "ZBLANK", &(infptr->Fptr)->zblank, NULL, &tstatus) <=
            0) {
            ...
            tstatus = 0;
            if (ffgky(infptr, TINT, "BLANK", &(infptr->Fptr)->zblank, NULL, &tstatus) <=
                0)
                (infptr->Fptr)->cn_zblank = -1; /* flag for a constant ZBLANK */
        }
    }
    ...
    tstatus = 0;
    if (ffgky(infptr, TDOUBLE, "BSCALE", &(infptr->Fptr)->cn_bscale, NULL, &tstatus) >
        0) {
        (infptr->Fptr)->cn_bscale = 1.0;
    }

    tstatus = 0;
    if (ffgky(infptr, TDOUBLE, "BZERO", &(infptr->Fptr)->cn_bzero, NULL, &tstatus) >
        0) {
...
*/
{
    ...
    for (ii = 5; ii <= nkeys; ii++) /* skip the first 4 keywords */
    {
        ...
        if ((keyclass <= TYP_CMPRS_KEY) || (keyclass == TYP_CKSUM_KEY)) continue;
...
*/
{
    ...
    fits_translate_keywords(infptr, outfptr, 1, patterns, npat, 0, 0, 0, status);
    ...
    */
    ...
    if (fits_read_card(outfptr, "ZQUANTIZ", card, &tstatus) == 0) {
        ...
        if (fits_strncasecmp(card2, "'NONE", 5)) {
            ...
            fits_write_history(outfptr,
                               "Image was compressed by CFITSIO using scaled integer "
                               "quantization:",
                               status);
    ...
    for (jj = 0; jj < nmore; jj++)
        for (ii = 0; ii < 36; ii++) fits_write_record(outfptr, "    ", status);
...
*/
{
    ...
    char* reqkeys[][2] = {
            {"ZSIMPLE", "SIMPLE"},    {"ZTENSION", "XTENSION"}, {"ZBITPIX", "BITPIX"},
            {"ZNAXIS", "NAXIS"},      {"ZNAXISm", "NAXISm"},    {"ZEXTEND", "EXTEND"},
            {"ZBLOCKED", "BLOCKED"},  {"ZPCOUNT", "PCOUNT"},    {"ZGCOUNT", "GCOUNT"},
            {"ZHECKSUM", "CHECKSUM"}, /* restore original checksums */
            {"ZDATASUM", "DATASUM"}};

    /* other special keywords */
    char* spkeys[][2] = {
            {"XTENSION", "-"}, {"BITPIX", "-"},  {"NAXIS", "-"},    {"NAXISm", "-"},
            {"PCOUNT", "-"},   {"GCOUNT", "-"},  {"TFIELDS", "-"},  {"TTYPEm", "-"},
            {"TFORMm", "-"},   {"THEAP", "-"},   {"ZIMAGE", "-"},   {"ZQUANTIZ", "-"},
            {"ZDITHER0", "-"}, {"ZTILEm", "-"},  {"ZCMPTYPE", "-"}, {"ZBLANK", "-"},
            {"ZNAMEm", "-"},   {"ZVALm", "-"},

            {"CHECKSUM", "-"},                   /* delete checksums */
            {"DATASUM", "-"},  {"EXTNAME", "+"}, /* we may change this, below */
            {"*", "+"}};
...
*/
{
    ...
    char* spkeys[][2] = {{"SIMPLE", "-"},  {"BITPIX", "-"},   {"NAXIS", "-"},
                         {"NAXISm", "-"},  {"PCOUNT", "-"},   {"EXTEND", "-"},
                         {"GCOUNT", "-"},  {"CHECKSUM", "-"}, {"DATASUM", "-"},
                         {"EXTNAME", "-"}, {"HISTORY", "-"},  {"COMMENT", "-"},
                         {"*", "+"}};
    ...
    /* translate and copy the keywords from the input file to the output */
    fits_translate_keywords(infptr, outfptr, 1, spkeys, nsp, 0, 0, 0, status);

    return (*status);
}
/*--------------------------------------------------------------------------*/
int imcomp_decompress_tile(
        fitsfile* infptr, int nrow, /* I - row of table to read and uncompress */
...
/* This routine decompresses one tile of the image */
{
    ...
    if ((infptr->Fptr)->tilerow == 0) {
        ...
        ntilebins =
                (((infptr->Fptr)->znaxis[0] - 1) / ((infptr->Fptr)->tilesize[0])) + 1;

        if ((infptr->Fptr)->znaxis[0] != (infptr->Fptr)->tilesize[0] ||
            (infptr->Fptr)->tilesize[1] !=
                    1) { /* don't cache the tile if only single row of the image */
    ...
    if ((infptr->Fptr)->tilerow) {
        /* calculate the column bin of the compressed tile */
        tilecol = (nrow - 1) % ((long)(((infptr->Fptr)->znaxis[0] - 1) /
                                       ((infptr->Fptr)->tilesize[0])) +
                                1);

        if (nrow == (infptr->Fptr)->tilerow[tilecol] &&
            datatype == (infptr->Fptr)->tiletype[tilecol]) {
            memcpy(buffer, ((infptr->Fptr)->tiledata)[tilecol],
                   (infptr->Fptr)->tiledatasize[tilecol]);
    ...
    ffgdesll(infptr, (infptr->Fptr)->cn_compressed, nrow, &nelemll, &offset, status);

    /* EOF error here indicates that this tile has not yet been written */
    if (*status == END_OF_FILE) return (*status = NO_COMPRESSED_TILE);
    ...
    if (nelemll == 0) /* special case: tile was not compressed normally */
    {
        if ((infptr->Fptr)->cn_uncompressed >= 1) {
            ...
            ffgdesll(infptr, (infptr->Fptr)->cn_uncompressed, nrow, &nelemll, &offset,
                     status);
            ...
            ffgdesll(infptr, (infptr->Fptr)->cn_gzip_data, nrow, &nelemll, &offset,
                     status);
            ...
            if (cbuf == NULL) {
                ffpmsg("error allocating memory for gzipped tile "
                       "(imcomp_decompress_tile)");
                return (*status = MEMORY_ALLOCATION);
            }

            /* read array of compressed bytes */
            if (fits_read_col(infptr, TBYTE, (infptr->Fptr)->cn_gzip_data, nrow, 1,
                              (long)nelemll, &charnull, cbuf, NULL, status) > 0) {
            ...
            if ((infptr->Fptr)->zbitpix == FLOAT_IMG) {
                ...
                ffpmsg("incompatible data type in gzipped floating-point "
            ...
            if (datatype != TFLOAT && datatype != TDOUBLE) {
                ffpmsg("attempting to read compressed float or double image into "
            ...
            if (datatype == TFLOAT && (infptr->Fptr)->zbitpix == DOUBLE_IMG) {
                ...
                if (tempdouble == NULL) {
                    ffpmsg("Memory allocation failure for tempdouble. "
                           "(imcomp_decompress_tile)");
                    free(cbuf);
                    return (*status = MEMORY_ALLOCATION);
                }
                ...
                if (tempfloat == NULL) {
                    ffpmsg("Memory allocation failure for tempfloat. "
                           "(imcomp_decompress_tile)");
                    free(cbuf);
                    return (*status = MEMORY_ALLOCATION);
                }
                ...
                /* uncompress the data directly into the output buffer in all other
                 * cases */
                if (uncompress2mem_from_mem((char*)cbuf, (long)nelemll, (char**)&buffer,
                                            &idatalen, NULL, &tilebytesize, status)) {
                    ffpmsg("failed to gunzip the image tile");
                    free(cbuf);
                    return (*status);
                }
            ...
            if (tilebytesize == 4 * tilelen) { /* float pixels */
                ...
                if (datatype == TFLOAT) {
                    ...
                    fffr4r4((float*)buffer, (long)tilelen, 1., 0., nullcheck, fnulval,
                            bnullarray, anynul, (float*)buffer, status);
                } else if (datatype == TDOUBLE) {
                    if (nulval) {
                    ...
                    fffr4r8((float*)tempfloat, (long)tilelen, 1., 0., nullcheck,
                            dnulval, bnullarray, anynul, (double*)buffer, status);
                    free(tempfloat);

                } else {
                    ffpmsg("implicit data type conversion is not supported for gzipped "
                           "image tiles");
                    return (*status = DATA_DECOMPRESSION_ERR);
                }
                ...
                if (datatype == TFLOAT) {
                    ...
                    fffr8r4((double*)tempdouble, (long)tilelen, 1., 0., nullcheck,
                            fnulval, bnullarray, anynul, (float*)buffer, status);
                    free(tempdouble);
                    tempdouble = 0;
                } else if (datatype == TDOUBLE) {
                    if (nulval) {
                    ...
                    fffr8r8((double*)buffer, (long)tilelen, 1., 0., nullcheck, dnulval,
                            bnullarray, anynul, (double*)buffer, status);
                } else {
                    ffpmsg("implicit data type conversion is not supported in "
    ...
    if ((infptr->Fptr)->cn_zscale == 0) {
        ...
        ffgcvd(infptr, (infptr->Fptr)->cn_zscale, nrow, 1, 1, 0., &bscale, NULL,
               status);
        ffgcvd(infptr, (infptr->Fptr)->cn_zzero, nrow, 1, 1, 0., &bzero, NULL, status);
        if (*status > 0) {
    ...
    if ((infptr->Fptr)->cn_zblank == 0) {
        ...
        ffgcvk(infptr, (infptr->Fptr)->cn_zblank, nrow, 1, 1, 0, &tnull, NULL, status);
        if (*status > 0) {
    ...
    if ((infptr->Fptr)->compress_type == PLIO_1) {
        fits_read_col(infptr, TSHORT, (infptr->Fptr)->cn_compressed, nrow, 1,
                      (long)nelemll, &snull, (short*)cbuf, NULL, status);
    } else {
        fits_read_col(infptr, TBYTE, (infptr->Fptr)->cn_compressed, nrow, 1,
                      (long)nelemll, &charnull, cbuf, NULL, status);
    }
    ...
    if ((infptr->Fptr)->compress_type == RICE_1) {
        ...
        if ((infptr->Fptr)->rice_bytepix == 1) {
            ...
            *status = fits_rdecomp(cbuf, (long)nelemll, (unsigned int*)idata, tilelen,
                                   blocksize);
            tiledatatype = TINT;
        }
        ...
        if (((infptr->Fptr)->zbitpix == BYTE_IMG ||
             (infptr->Fptr)->zbitpix == SHORT_IMG)) {
            *status = fits_hdecompress(cbuf, smooth, idata, &nx, &ny, &scale, status);
        ...
        /* ************************************************************* */
               ...
               ((infptr->Fptr)->compress_type == GZIP_2)) {
        uncompress2mem_from_mem((char*)cbuf, (long)nelemll, (char**)&idata, &idatalen,
                                realloc, &tilebytesize, status);
    ...
    if (nulval == 0) nulval = &dummy; /* set address to dummy value */
    ...
    if (datatype == TSHORT) {
        ...
        if ((infptr->Fptr)->quantize_level == NO_QUANTIZE) {
            ...
            if (tiledatatype == TINT) {
                fffr4i2((float*)idata, tilelen, bscale, bzero, nullcheck,
                        *(short*)nulval, bnullarray, anynul, (short*)buffer, status);
            } else {
            ...
            fffi1i2((unsigned char*)idata, tilelen, bscale, bzero, nullcheck,
                    (unsigned char)tnull, *(short*)nulval, bnullarray, anynul,
                    (short*)buffer, status);
        }
    } else if (datatype == TINT) {
        pixlen = sizeof(int);
        ...
        if ((infptr->Fptr)->quantize_level == NO_QUANTIZE) {
            ...
            if (tiledatatype == TINT) {
                fffr4int((float*)idata, tilelen, bscale, bzero, nullcheck,
                         *(int*)nulval, bnullarray, anynul, (int*)buffer, status);
            } else {
            ...
            fffi1int((unsigned char*)idata, tilelen, bscale, bzero, nullcheck,
                     (unsigned char)tnull, *(int*)nulval, bnullarray, anynul,
                     (int*)buffer, status);
    } else if (datatype == TLONG) {
        pixlen = sizeof(long);
        ...
        if ((infptr->Fptr)->quantize_level == NO_QUANTIZE) {
            ...
            if (tiledatatype == TINT) {
                fffr4i4((float*)idata, tilelen, bscale, bzero, nullcheck,
                        *(long*)nulval, bnullarray, anynul, (long*)buffer, status);
            } else {
            ...
            if ((infptr->Fptr)->compress_type == PLIO_1 && actual_bzero == 32768.) {
                ...
                fffi4i4(idata, tilelen, bscale, bzero, nullcheck, tnull, *(long*)nulval,
                        bnullarray, anynul, (long*)buffer, status);
            }
        else if (tiledatatype == TSHORT)
            ...
            fffi1i4((unsigned char*)idata, tilelen, bscale, bzero, nullcheck,
                    (unsigned char)tnull, *(long*)nulval, bnullarray, anynul,
                    (long*)buffer, status);
    } else if (datatype == TFLOAT) {
        pixlen = sizeof(float);
        if (nulval) {
        ...
        if ((infptr->Fptr)->quantize_level == NO_QUANTIZE) {
            ...
            if (tiledatatype == TINT) {
                fffr4r4((float*)idata, tilelen, bscale, bzero, nullcheck, fnulval,
                        bnullarray, anynul, (float*)buffer, status);
            } else {
                fffr8r4((double*)idata, tilelen, bscale, bzero, nullcheck, fnulval,
                        bnullarray, anynul, (float*)buffer, status);
            }
            ...
            if (tiledatatype == TINT)
                unquantize_i4r4(nrow + (infptr->Fptr)->dither_seed - 1, idata, tilelen,
                                bscale, bzero, (infptr->Fptr)->quantize_method,
                                nullcheck, tnull, fnulval, bnullarray, anynul,
                                (float*)buffer, status);
            else if (tiledatatype == TSHORT)
                unquantize_i2r4(nrow + (infptr->Fptr)->dither_seed - 1, (short*)idata,
                                tilelen, bscale, bzero, (infptr->Fptr)->quantize_method,
                                nullcheck, (short)tnull, fnulval, bnullarray, anynul,
                                (float*)buffer, status);
            else if (tiledatatype == TBYTE)
                unquantize_i1r4(nrow + (infptr->Fptr)->dither_seed - 1,
                                (unsigned char*)idata, tilelen, bscale, bzero,
                                (infptr->Fptr)->quantize_method, nullcheck,
                                (unsigned char)tnull, fnulval, bnullarray, anynul,
                                (float*)buffer, status);
            ...
            if (tiledatatype == TINT)
                if ((infptr->Fptr)->compress_type == PLIO_1 && actual_bzero == 32768.) {
                    ...
                    fffi4r4(idata, tilelen, bscale, bzero, nullcheck, tnull, fnulval,
                            bnullarray, anynul, (float*)buffer, status);
                }
            else if (tiledatatype == TSHORT)
        ...
        if ((infptr->Fptr)->quantize_level == NO_QUANTIZE) {
            ...
            if (tiledatatype == TINT) {
                fffr4r8((float*)idata, tilelen, bscale, bzero, nullcheck, dnulval,
                        bnullarray, anynul, (double*)buffer, status);
            } else {
                fffr8r8((double*)idata, tilelen, bscale, bzero, nullcheck, dnulval,
                        bnullarray, anynul, (double*)buffer, status);
            }
            ...
            /* use the new dithering algorithm (introduced in July 2009) */
            if (tiledatatype == TINT)
                unquantize_i4r8(nrow + (infptr->Fptr)->dither_seed - 1, idata, tilelen,
                                bscale, bzero, (infptr->Fptr)->quantize_method,
                                nullcheck, tnull, dnulval, bnullarray, anynul,
                                (double*)buffer, status);
            else if (tiledatatype == TSHORT)
                unquantize_i2r8(nrow + (infptr->Fptr)->dither_seed - 1, (short*)idata,
                                tilelen, bscale, bzero, (infptr->Fptr)->quantize_method,
                                nullcheck, (short)tnull, dnulval, bnullarray, anynul,
                                (double*)buffer, status);
            else if (tiledatatype == TBYTE)
                unquantize_i1r8(nrow + (infptr->Fptr)->dither_seed - 1,
                                (unsigned char*)idata, tilelen, bscale, bzero,
                                (infptr->Fptr)->quantize_method, nullcheck,
                                (unsigned char)tnull, dnulval, bnullarray, anynul,
                                (double*)buffer, status);
            ...
            if (tiledatatype == TINT) {
                if ((infptr->Fptr)->compress_type == PLIO_1 && actual_bzero == 32768.) {
                    ...
                    fffi4r8(idata, tilelen, bscale, bzero, nullcheck, tnull, dnulval,
                            bnullarray, anynul, (double*)buffer, status);
                }
            } else if (tiledatatype == TSHORT) {
        ...
        if (tiledatatype == TINT)
            fffi4i1(idata, tilelen, bscale, bzero, nullcheck, tnull,
                    *(unsigned char*)nulval, bnullarray, anynul, (unsigned char*)buffer,
                    status);
        else if (tiledatatype == TSHORT)
            ...
            fffi1i1((unsigned char*)idata, tilelen, bscale, bzero, nullcheck,
                    (unsigned char)tnull, *(unsigned char*)nulval, bnullarray, anynul,
                    (unsigned char*)buffer, status);
    } else if (datatype == TSBYTE) {
        pixlen = sizeof(char);
        if (tiledatatype == TINT)
            fffi4s1(idata, tilelen, bscale, bzero, nullcheck, tnull,
                    *(signed char*)nulval, bnullarray, anynul, (signed char*)buffer,
                    status);
        else if (tiledatatype == TSHORT)
            ...
            fffi1s1((unsigned char*)idata, tilelen, bscale, bzero, nullcheck,
                    (unsigned char)tnull, *(signed char*)nulval, bnullarray, anynul,
        ...
        if ((infptr->Fptr)->quantize_level == NO_QUANTIZE) {
            ...
            fffi1u2((unsigned char*)idata, tilelen, bscale, bzero, nullcheck,
                    (unsigned char)tnull, *(unsigned short*)nulval, bnullarray, anynul,
                    (unsigned short*)buffer, status);
    } else if (datatype == TUINT) {
        pixlen = sizeof(int);
        ...
        if ((infptr->Fptr)->quantize_level == NO_QUANTIZE) {
            ...
            fffi2uint((short*)idata, tilelen, bscale, bzero, nullcheck, (short)tnull,
                      *(unsigned int*)nulval, bnullarray, anynul, (unsigned int*)buffer,
                      status);
        else if (tiledatatype == TBYTE)
            fffi1uint((unsigned char*)idata, tilelen, bscale, bzero, nullcheck,
                      (unsigned char)tnull, *(unsigned int*)nulval, bnullarray, anynul,
                      (unsigned int*)buffer, status);
    } else if (datatype == TULONG) {
        pixlen = sizeof(long);
        ...
        if ((infptr->Fptr)->quantize_level == NO_QUANTIZE) {
            ...
            fffi2u4((short*)idata, tilelen, bscale, bzero, nullcheck, (short)tnull,
                    *(unsigned long*)nulval, bnullarray, anynul, (unsigned long*)buffer,
                    status);
        else if (tiledatatype == TBYTE)
            fffi1u4((unsigned char*)idata, tilelen, bscale, bzero, nullcheck,
                    (unsigned char)tnull, *(unsigned long*)nulval, bnullarray, anynul,
                    (unsigned long*)buffer, status);
    } else
        *status = BAD_DATATYPE;
    ...
    if ((infptr->Fptr)->tilerow) { /* make sure cache has been allocated */
        if ((infptr->Fptr)->znaxis[0] != (infptr->Fptr)->tilesize[0] ||
            ...
            if (tilesize != (infptr->Fptr)->tiledatasize[tilecol] ||
                ...
                if (((infptr->Fptr)->tiledata)[tilecol] == 0) return (*status);

                if (nullcheck == 2) { /* also need array of null pixel flags */
                    (infptr->Fptr)->tilenullarray[tilecol] = malloc(tilelen);
                    if ((infptr->Fptr)->tilenullarray[tilecol] == 0) return (*status);
                }
...
*/
{
    ...
    for (ii = 0; ii < ndim; ii++) {
        ...
        if (ii > 0) tiledim[ii] *= tiledim[ii - 1]; /* product of dimensions */
        ...
        while ((tf - (fpixel[ii] - 1)) % labs(inc[ii])) {
            tf++;
            if (tf > tl) return (0); /* no overlapping pixels */
        }
        ...
        imglpix[ii] = minvalue((tl - fpixel[ii] + 1) / labs(inc[ii]), imgdim[ii] - 1);
        ...
        while ((tfpixel[ii] + tilefpix[ii] - fpixel[ii]) % labs(inc[ii])) {
            (tilefpix[ii])++;
            if (tilefpix[ii] >= tiledim[ii]) return (0); /* no overlapping pixels */
        }

        if (ii > 0) imgdim[ii] *= imgdim[ii - 1]; /* product of dimensions */
    }
...
*/
{
    ...
    for (ii = 0; ii < ndim; ii++) {
        ...
        /* calc dimensions of the output image section */
        imgdim[ii] = (lpixel[ii] - fpixel[ii]) / labs(inc[ii]) + 1;
        if (imgdim[ii] < 1) return (*status = NEG_AXIS);

        /* calc dimensions of the tile */
        tiledim[ii] = tlpixel[ii] - tfpixel[ii] + 1;
        if (tiledim[ii] < 1) return (*status = NEG_AXIS);
        ...
        while ((tf - (fpixel[ii] - 1)) % labs(inc[ii])) {
            tf++;
            if (tf > tl) return (*status); /* no overlapping pixels */
        }
    ...
    for (i4 = 0, it4 = 0; i4 <= imglpix[4] - imgfpix[4]; i4++, it4++) {
        ...
        while (ndim > 4 &&
               (tfpixel[4] + tilefpix[4] - fpixel[4] + it4) % labs(inc[4]) != 0)
            it4++;
        ...
        for (i3 = 0, it3 = 0; i3 <= imglpix[3] - imgfpix[3]; i3++, it3++) {
            /* increment plane if it falls in the cracks of the subsampled image */
            while (ndim > 3 &&
                   (tfpixel[3] + tilefpix[3] - fpixel[3] + it3) % labs(inc[3]) != 0)
                it3++;
            ...
            for (i2 = 0, it2 = 0; i2 <= imglpix[2] - imgfpix[2]; i2++, it2++) {
                ...
                while (ndim > 2 &&
                       (tfpixel[2] + tilefpix[2] - fpixel[2] + it2) % labs(inc[2]) != 0)
                    it2++;
                ...
                for (i1 = 0, it1 = 0; i1 <= imglpix[1] - imgfpix[1]; i1++, it1++) {
                    ...
                    */
                    ...
                    for (ipos = imgfpix[0]; ipos <= imglpix[0]; ipos += overlap_flags) {
                        ...
                        /*
                        printf("  tilepix, tilepixbyte, imgpix, imgpixbyte= %d %d %d
...
/*--------------------------------------------------------------------------*/
static int unquantize_i1r4(
        long row,             /* tile number = row number in table  */
...
*/
{
    ...
    if (nullcheck == 0) /* no null checking required */
    {
        for (ii = 0; ii < ntodo; ii++) {
            /*
                            if (dither_method == SUBTRACTIVE_DITHER_2 && input[ii] ==
               ZERO_VALUE) output[ii] = 0.0; else
            */
            output[ii] = (float)(((double)input[ii] - fits_rand_value[nextrand] + 0.5) *
                                         scale +
                                 zero);
    ...
    } else /* must check for null values */
    {
        for (ii = 0; ii < ntodo; ii++) {
            if (input[ii] == tnull) {
                ...
                */
                output[ii] = (float)(((double)input[ii] - fits_rand_value[nextrand] +
                                      0.5) * scale +
                                     zero);
            }
...
/*--------------------------------------------------------------------------*/
static int unquantize_i2r4(
        long row,          /* seed for random values  */
            ...
            if (input[ii] == tnull) {
                *anynull = 1;
                ...
                if (dither_method == SUBTRACTIVE_DITHER_2 && input[ii] == ZERO_VALUE)
                    ...
                    output[ii] =
                            (float)(((double)input[ii] - fits_rand_value[nextrand] +
                                     0.5) * scale +
                                    zero);
            }
    ...
    if (nullcheck == 0) /* no null checking required */
    {
        for (ii = 0; ii < ntodo; ii++) {
            /*
               ...
               ZERO_VALUE) output[ii] = 0.0; else
            */
            output[ii] = (double)(((double)input[ii] - fits_rand_value[nextrand] +
                                   0.5) * scale +
                                  zero);
            ...
            if (input[ii] == tnull) {
                *anynull = 1;
                ...
                if (dither_method == SUBTRACTIVE_DITHER_2 && input[ii] == ZERO_VALUE)
                    ...
                    output[ii] =
                            (double)(((double)input[ii] - fits_rand_value[nextrand] +
                                      0.5) * scale +
                                     zero);
            }
...
/*
  ...
  which may contain fewer rows).   The chunks should not be too large to copy into
  memory (currently, about 100 MB max seems a reasonable size).
...
{
    long maxchunksize =
            10000000; /* default value for the size of each chunk of the table */

    char* cm_buffer; /* memory buffer for the transposed, Column-Major, chunk of the
                        table */
    LONGLONG cm_colstart[1000]; /* starting offset of each column in the cm_buffer */
    LONGLONG rm_repeat[1000];   /* repeat count of each column in the input row-major
                                   table */
    LONGLONG rm_colwidth[999];  /* width in bytes of each column in the input row-major
                                   table */
    LONGLONG cm_repeat[999];    /* total number of elements in each column of the
                                   transposed column-major table */

    int coltype[999]; /* data type code for each column */
    int compalgor[999],
            default_algor = 0; /* compression algorithm to be applied to each column */
    float cratio[999]; /* compression ratio for each column (for diagnostic purposes) */

    float compressed_size, uncompressed_size, tot_compressed_size,
            tot_uncompressed_size;
    ...
    char comm[FLEN_COMMENT], keyvalue[FLEN_VALUE], *cvlamem, tempstring[FLEN_VALUE],
            card[FLEN_CARD];
    ...
    /* get offset to the start of the data and total size of the table (including the
     * heap) */
    ...
    if (!fits_read_key(infptr, TSTRING, "FZALGOR", tempstring, NULL, &tstatus)) {
        ...
        } else if (!fits_strcasecmp(tempstring, "GZIP") ||
                   !fits_strcasecmp(tempstring, "GZIP_1")) {
    ...
    /* is less than 5760 bytes (2 blocks) in size, or compression directive keyword =
     * "NONE" */
    if (nrows < 1 || ncols < 1 || (dataend - datastart) < 5760 ||
        default_algor == NOCOMPRESS) {
    ...
    lastrows = (long)(nrows -
                      ((nchunks - 1) * rowspertile)); /* number of rows in last chunk */
    ...
    fits_copy_header(infptr, outfptr,
                     status); /* start with verbatim copy of the input header */

    fits_write_key(outfptr, TLOGICAL, "ZTABLE", &ltrue, "this is a compressed table",
                   status);
    fits_write_key(outfptr, TLONG, "ZTILELEN", &rowspertile,
    ...
    fits_modify_key_lng(outfptr, "NAXIS1", ncols * 16, "&",
                        status); /* 16 bytes for each 1QB column */
    ...
    for (ii = 0; ii < ncols; ii++) {
        ...
        fits_binary_tform(tform, coltype + ii, &repeat, &width,
                          status); /* get the repeat count and the width */
         ...
         */
        fits_modify_key_str(outfptr, keyname, "1QB", "&",
                            status); /* Use 'Q' pointers (64-bit) */

        /* deal with special cases: bit, string, and variable length array columns */
        if (coltype[ii] == TBIT) {
            repeat = (repeat + 7) /
                     8; /* convert from bits to equivalent number of bytes */
        ...
        rm_repeat[ii] = repeat;
        rm_colwidth[ii] =
                repeat * width; /* column width (in bytes)in the input table */
        ...
        compalgor[ii] = default_algor; /* initialize the column compression algorithm to
                                          the default */
        ...
        if (!fits_read_key(outfptr, TSTRING, keyname, tempstring, NULL, &tstatus)) {
            if (!fits_strcasecmp(tempstring, "GZIP") ||
                !fits_strcasecmp(tempstring, "GZIP_1")) {
                compalgor[ii] = GZIP_1;
            } else if (!fits_strcasecmp(tempstring, "GZIP_2")) {
                compalgor[ii] = GZIP_2;
        ...
        if (abs(coltype[ii]) == TLOGICAL || abs(coltype[ii]) == TBIT ||
            abs(coltype[ii]) == TSTRING) {
    ...
    for (ll = 0; ll < nchunks; ll++) {
        ...
        /*  At the same time, shuffle the bytes in each datum, if doing GZIP_2
         * compression */
        /* ================================================================================*/
        ...
        for (jj = 0; jj < rowspertile; jj++) { /* loop over rows */
            for (ii = 0; ii < ncols; ii++) {   /* loop over columns */
                ...
                if (rm_repeat[ii] >
                    0) { /*  skip virtual columns that have 0 elements */
                    ...
                    if (coltype[ii] == TSHORT && compalgor[ii] == GZIP_2) {
                        while (kk < rm_colwidth[ii]) {
                            cptr = cm_buffer +
                                   (cm_colstart[ii] + (jj * rm_repeat[ii]) + kk / 2);
                        ...
                        while (kk < rm_colwidth[ii]) {
                            cptr = cm_buffer +
                                   (cm_colstart[ii] + (jj * rm_repeat[ii]) + kk / 4);
                            ffgbyt(infptr, 1, cptr, status); /* get 1st byte */
                            cptr += cm_repeat[ii];
                            ffgbyt(infptr, 1, cptr, status); /* get 2nd byte */
                            cptr += cm_repeat[ii];
                        ...
                        while (kk < rm_colwidth[ii]) {
                            cptr = cm_buffer +
                                   (cm_colstart[ii] + (jj * rm_repeat[ii]) + kk / 8);
                            ffgbyt(infptr, 1, cptr, status); /* get 1st byte */
                            cptr += cm_repeat[ii];
                            ffgbyt(infptr, 1, cptr, status); /* get 2nd byte */
                            cptr += cm_repeat[ii];
                    ...
                    } else { /* all other cases: don't shuffle the bytes; simply
                                transpose the column */
                        cptr = cm_buffer +
                               (cm_colstart[ii] +
                                (jj * rm_colwidth[ii])); /* addr to copy to */
                        startbyte =
                                (infptr->Fptr)
                                        ->bytepos; /* save the starting byte location */
                        ffgbyt(infptr, rm_colwidth[ii], cptr,
                               status); /* copy all the bytes */

                        if (rm_colwidth[ii] >=
                            MINDIRECT) { /* have to explicitly move to next byte */
        ...
        fits_set_hdustruc(outfptr,
                          status); /* initialize structures in the output table */

        for (ii = 0; ii < ncols; ii++) { /* loop over columns */
            /* initialize the diagnostic compression results string */
            snprintf(results[ii], 30, "%3d %3d %3d ", ii + 1, coltype[ii],
                     compalgor[ii]);
            cratio[ii] = 0;
            ...
            if (rm_repeat[ii] > 0) { /* skip virtual columns with zero width */
                ...
                if (coltype[ii] <
                    0) { /* this is a variable length array (VLA) column */

                    /*=========================================================================*/
                    /* variable-length array columns are a complicated special case  */
                    /*=========================================================================*/

                    /* allocate memory to hold all the VLA descriptors from the input
                     * table, plus */
                    /* room to hold the descriptors to the compressed VLAs in the output
                     * table */
                    ...
                    datasize =
                            (size_t)(cm_colstart[ii + 1] -
                                     cm_colstart[ii]); /* size of input descriptors */

                    cdescript = calloc(datasize + (rowspertile * 16),
                                       1); /* room for both descriptors */
                    if (!cdescript) {
...
#if BYTESWAPPED
                    /* byte-swap the integer values into the native machine
                     * representation */
...
#endif
                    descriptors =
                            (LONGLONG*)cdescript; /* use this for Q type descriptors */
                    pdescriptors = (int*)
                            cdescript; /* use this instead for or P type descriptors */
                    /* pointer to the 2nd set of descriptors */
                    outdescript =
                            (LONGLONG*)(cdescript +
                                        datasize); /* this is a LONGLONG pointer */

                    for (jj = 0; jj < rowspertile;
                         jj++) { /* loop to compress each VLA in turn */
                        ...
                        if (vlalen > 0) { /* skip zero-length VLAs */
                            ...
                            vlamem = (LONGLONG*)malloc(
                                    (size_t)vlamemlen); /* memory for the input
                                                           uncompressed VLA */
                            if (!vlamem) {
                            ...
                            compmemlen = (size_t)(
                                    vlalen * ((LONGLONG)(-coltype[ii] / 10)) * 1.5);
                            if (compmemlen < 100) compmemlen = 100;
                            cvlamem = malloc(compmemlen); /* memory for the output
                                                             compressed VLA */
                            if (!cvlamem) {
                            ...
                            /* read the raw bytes directly from the heap, without any
                             * byte-swapping or null value detection */
                            bytepos = (infptr->Fptr)->datastart +
                                      (infptr->Fptr)->heapstart + vlastart;
                            ffmbyt(infptr, bytepos, REPORT_EOF, status);
                            ffgbyt(infptr, vlamemlen, vlamem,
                                   status); /* read the bytes */
                            uncompressed_size +=
                                    vlamemlen; /* total size of the uncompressed VLAs */
                            tot_uncompressed_size +=
                                    vlamemlen; /* total size of the uncompressed file */
...
#endif
                                    dlen = fits_rcomp_short((short*)(vlamem),
                                                            (int)vlalen,
                                                            (unsigned char*)cvlamem,
                                                            (int)compmemlen, 32);
                                } else if (-coltype[ii] == TLONG) {
#if BYTESWAPPED
                                    ffswap4((int*)(vlamem), (long)vlalen);
#endif
                                    dlen = fits_rcomp((int*)(vlamem), (int)vlalen,
                                                      (unsigned char*)cvlamem,
                                                      (int)compmemlen, 32);
                                } else if (-coltype[ii] == TBYTE) {
                                    dlen = fits_rcomp_byte((signed char*)(vlamem),
                                                           (int)vlalen,
                                                           (unsigned char*)cvlamem,
                                                           (int)compmemlen, 32);
                                } else {
                                    /* this should not happen */
                                    ffpmsg(" Error: cannot compress this column type "
                            ...
                            } else if (compalgor[ii] == GZIP_1 ||
                                       compalgor[ii] == GZIP_2) {
                                if (compalgor[ii] ==
                                    GZIP_2) { /* shuffle the bytes before gzipping them
                                               */
                                    if ((int)(-coltype[ii] / 10) == 2) {
                                        fits_shuffle_2bytes((char*)vlamem, vlalen,
                                                            status);
                                    } else if ((int)(-coltype[ii] / 10) == 4) {
                                        fits_shuffle_4bytes((char*)vlamem, vlalen,
                                                            status);
                                    } else if ((int)(-coltype[ii] / 10) == 8) {
                                        fits_shuffle_8bytes((char*)vlamem, vlalen,
                                                            status);
                                    }
                                }
                                /*: gzip compress the array of bytes */
                                compress2mem_from_mem((char*)vlamem, (size_t)vlamemlen,
                                                      &cvlamem, &compmemlen, realloc,
                                                      &dlen, status);
                            } else {
                                /* this should not happen */
                            ...
                            /* We use a trick of always writing the array to the same
                             * row of the output table */
                            /* and then copy the descriptor into the array of
                             * descriptors that we allocated. */

                            /* First, reset the descriptor */
                            fits_write_descript(outfptr, ii + 1, ll + 1, 0, 0, status);

                            /* write the compressed VLA if it is smaller than the
                             * original, else write */
                            /* the uncompressed array */
                            fits_set_tscale(
                                    outfptr, ii + 1, 1.0, 0.0,
                                    status); /* turn off any data scaling, first */
                            if (dlen < vlamemlen) {
                                fits_write_col(outfptr, TBYTE, ii + 1, ll + 1, 1, dlen,
                                               cvlamem, status);
                                compressed_size +=
                                        dlen; /* total size of the compressed VLAs */
                                tot_compressed_size +=
                                        dlen; /* total size of the compressed file */
                            } else {
                                if (-coltype[ii] != TBYTE && compalgor[ii] != GZIP_1) {
                                    /* it is probably faster to reread the raw bytes,
                                     * rather than unshuffle or unswap them */
                                    bytepos = (infptr->Fptr)->datastart +
                                              (infptr->Fptr)->heapstart + vlastart;
                                    ffmbyt(infptr, bytepos, REPORT_EOF, status);
                                    ffgbyt(infptr, vlamemlen, vlamem,
                                ...
                                fits_write_col(outfptr, TBYTE, ii + 1, ll + 1, 1,
                                               vlamemlen, vlamem, status);
                                compressed_size += vlamemlen;     /* total size of the
                                                                     compressed VLAs */
                                tot_compressed_size += vlamemlen; /* total size of the
                                                                     compressed file */
                            }

                            /* read back the descriptor and save it in the array of
                             * descriptors */
                            fits_read_descriptll(outfptr, ii + 1, ll + 1,
                                                 outdescript + (jj * 2),
                                                 outdescript + (jj * 2) + 1, status);
                            free(cvlamem);
                            free(vlamem);
                    ...
                    /* now we just have to compress the array of descriptors (both input
                     * and output) */
                    ...
                    compress2mem_from_mem((char*)cdescript,
                                          datasize + (rowspertile * 16), &cvlamem,
                                          &datasize, realloc, &dlen, status);

                    free(cdescript);

                    /* write the compressed descriptors to the output column */
                    fits_set_tscale(outfptr, ii + 1, 1.0, 0.0,
                                    status); /* turn off any data scaling, first */
                    fits_write_descript(outfptr, ii + 1, ll + 1, 0, 0,
                                        status); /* First, reset the descriptor */
                    fits_write_col(outfptr, TBYTE, ii + 1, ll + 1, 1, dlen, cvlamem,
                                   status);
                    free(cvlamem);

                    if (ll == 0) { /* only write the ZCTYPn keyword once, while
                ...
                if (compalgor[ii] == RICE_1) {
...
#endif
                        dlen = fits_rcomp_short((short*)(cm_buffer + cm_colstart[ii]),
                                                datasize / 2, (unsigned char*)cvlamem,
                                                datasize * 2, 32);
...
#endif
                        dlen = fits_rcomp((int*)(cm_buffer + cm_colstart[ii]),
                                          datasize / 4, (unsigned char*)cvlamem,
                                          datasize * 2, 32);

                    } else if (coltype[ii] == TBYTE) {
                        dlen = fits_rcomp_byte(
                                (signed char*)(cm_buffer + cm_colstart[ii]), datasize,
                                (unsigned char*)cvlamem, datasize * 2, 32);
                    } else { /* this should not happen */
                        ffpmsg(" Error: cannot compress this column type with the RICE "
                               "algorthm");
                        free(cvlamem);
                        free(cm_buffer);
                        *status = DATA_COMPRESSION_ERR;
                    ...
                    /* all other cases: gzip compress the column (bytes may have been
                     * shuffled previously) */
                    compress2mem_from_mem(cm_buffer + cm_colstart[ii], datasize,
                                          &cvlamem, &datasize, realloc, &dlen, status);
                }

                if (ll == 0) { /* only write the ZCTYPn keyword once, while processing
                                  the first column */
                    fits_make_keyn("ZCTYP", ii + 1, keyname, status);
                ...
                if (dlen != 0)
                    cratio[ii] = (float)datasize /
                                 (float)dlen; /* compression ratio of the column */
    ...
    if (print_report) {
        if (tot_compressed_size != 0)
            printf("\nTotal data size (MB) %.3f -> %.3f, ratio = %.3f\n",
                   tot_uncompressed_size / 1000000., tot_compressed_size / 1000000.,
...
*/
{
    ...
    int ncols, hdutype, inttype, anynull, tstatus, zctype[999], addspace = 0,
                                                                *pdescript = 0;
    char *cptr, keyname[9], tform[40];
    long pcount, zheapptr, naxis1, naxis2, ii, jj;
    char *ptr, comm[FLEN_COMMENT], zvalue[FLEN_VALUE], *uncompressed_vla = 0,
                                                       *compressed_vla;
    char card[FLEN_CARD];
    ...
    /* ==================================================================================
     */
    /* determine compression paramters for each column and write column-specific
     * keywords */
    /* ==================================================================================
     */
    for (ii = 0; ii < ncols; ii++) {
        ...
        if (abs(coltype[ii]) == TBIT) {
            ...
            addspace +=
                    16; /* need space for a second set of Q pointers for this column */
        }
    ...
    while (rowsremain) {
        ...
        for (ii = 0; ii < ncols; ii++) {
            ...
            if (rmajor_repeat[ii] > 0) { /* ignore columns with 0 elements */
                ...
                fits_read_descript(infptr, ii + 1, ntile, &vla_repeat, &vla_address,
                                   status);
                ...
                fits_set_tscale(infptr, ii + 1, 1.0, 0.0,
                                status); /* turn off any data scaling, first */
                fits_read_col_byt(infptr, ii + 1, ntile, 1, vla_repeat, 0,
                                  (unsigned char*)ptr, &anynull, status);
                ...
                switch (colcode[ii]) {
                    case 'I':
                        ...
                        if (zctype[ii] == RICE_1) {
                            dlen = fits_rdecomp_short((unsigned char*)ptr, vla_repeat,
                                                      (unsigned short*)cptr,
                                                      fullsize / 2, 32);
#if BYTESWAPPED
                            ...
                            uncompress2mem_from_mem(ptr, vla_repeat, &cptr, &fullsize,
                                                    realloc, &dlen, status);
                        }
                        break;
                    ...
                    case 'J':
                        ...
                        if (zctype[ii] == RICE_1) {
                            dlen = fits_rdecomp((unsigned char*)ptr, vla_repeat,
                                                (unsigned int*)cptr, fullsize / 4, 32);
#if BYTESWAPPED
                    ...
                    case 'B':
                        ...
                        if (zctype[ii] == RICE_1) {
                            dlen = fits_rdecomp_byte((unsigned char*)ptr, vla_repeat,
                                                     (unsigned char*)cptr, fullsize,
                                                     32);
                        } else { /* gunzip the data into the correct location */
                            uncompress2mem_from_mem(ptr, vla_repeat, &cptr, &fullsize,
                                                    realloc, &dlen, status);
                    ...
                    default:
                         ...
                         */
                        /* gunzip the data into the correct location in the full table
                         * buffer */
                        uncompress2mem_from_mem(ptr, vla_repeat, &cptr, &fullsize,
                                                realloc, &dlen, status);
        ...
        /* move each byte, in turn, from the cm_buffer to the appropriate place in the
         * rm_buffer */
        for (ii = 0; ii < ncols; ii++) { /* loop over columns */
            ptr = (char*)(cm_buffer +
                          cmajor_colstart[ii]); /* initialize ptr to start of the column
                                                   in the cm_buffer */
            if (rmajor_repeat[ii] > 0) {        /* skip columns with zero elements */
                if (coltype[ii] > 0) {          /* normal fixed length array columns */
                    if (zctype[ii] == GZIP_2) { /*  need to unshuffle the bytes */
                        ...
                        /* recombine the byte planes for the 2-byte, 4-byte, and 8-byte
                         * numeric columns */
                        switch (colcode[ii]) {
                            case 'I':
                                /* get the 1st byte of each I*2 value */
                                for (jj = 0; jj < rowspertile;
                                     jj++) { /* loop over number of rows in the output
                                                table */
                                    cptr = rm_buffer + (rmajor_colstart[ii] +
                                                        (jj * rmajor_colstart[ncols]));
                            ...
                            default: /*  should never get here */
                                ffpmsg("Error: unexpected attempt to use GZIP_2 to "
                                       "compress a column unsuitable data type");
                                *status = DATA_DECOMPRESSION_ERR;
                                free(rm_buffer);
                                free(cm_buffer);
                                return (*status);
                    ...
                    } else { /* not GZIP_2, don't have to shuffle bytes, so just
                                transpose the rows and columns */

                        for (jj = 0; jj < rowspertile;
                             jj++) { /* loop over number of rows in the output table */
                            cptr = rm_buffer +
                                   (rmajor_colstart[ii] +
                                    jj * rmajor_colstart[ncols]); /* addr to copy to */
                    ...
                    for (jj = 0; jj < rowspertile;
                         jj++) { /* loop over number of rows in the output uncompressed
                                    table */
                        cptr = rm_buffer +
                               (rmajor_colstart[ii] +
                                jj * rmajor_colstart[ncols]); /* addr to copy to */
...
#if BYTESWAPPED
                        ffswap4((int*)pdescript,
                                rowspertile * 2); /* byte-swap the descriptor */
#endif
                    } else if (rmajor_colwidth[ii] ==
                               16) { /* these are Q-type descriptors */
                        qdescript = (LONGLONG*)(cm_buffer + cmajor_colstart[ii]);
#if BYTESWAPPED
                        ffswap8((double*)qdescript,
                                rowspertile * 2); /* byte-swap the descriptor */
#endif
                    } else { /* this should never happen */
                        ffpmsg("Error: Descriptor column is neither 8 nor 16 bytes "
                               "wide");
                        free(rm_buffer);
                        free(cm_buffer);
                        *status = DATA_DECOMPRESSION_ERR;
                    ...
                    /* First, set pointer to the Q descriptors, and byte-swap them, if
                     * needed */
                    descript = (LONGLONG*)(cm_buffer + cmajor_colstart[ii] +
                    ...
                    for (jj = 0; jj < rowspertile; jj++) { /* loop over rows */
                        /* get the size and location of the compressed VLA in the
                         * compressed table */
                        cvlalen = descript[jj * 2];
                        cvlastart = descript[(jj * 2) + 1];
                        if (cvlalen > 0) {
                            /* get the size and location to write the uncompressed VLA
                             * in the uncompressed table */
                            ...
                            vlamemlen = (size_t)(
                                    vlalen *
                                    (-coltype[ii] /
                                     10)); /* size of the uncompressed VLA, in bytes */
                            ...
                            /* read the compressed VLA from the heap in the input
                             * compressed table */
                            bytepos = (size_t)((infptr->Fptr)->datastart +
                                               (infptr->Fptr)->heapstart + cvlastart);
                            ffmbyt(infptr, bytepos, REPORT_EOF, status);
                            ffgbyt(infptr, cvlalen, compressed_vla,
                                   status); /* read the bytes */
                            /* if the VLA couldn't be compressed, just copy it directly
                             * to the output uncompressed table */
                            if (cvlalen == vlamemlen) {
                                bytepos =
                                        (size_t)((outfptr->Fptr)->datastart +
                                                 (outfptr->Fptr)->heapstart + vlastart);
                                ffmbyt(outfptr, bytepos, IGNORE_EOF, status);
                                ffpbyt(outfptr, cvlalen, compressed_vla,
                                       status); /* write the bytes */
                            } else {            /* uncompress the VLA  */

                                /* allocate memory for the uncompressed VLA */
                                uncompressed_vla = malloc(vlamemlen);
                                if (!uncompressed_vla) {
                                    ffpmsg("Could not allocate buffer for uncompressed "
                                           "VLA");
                                    *status = MEMORY_ALLOCATION;
                                    free(compressed_vla);
                                    free(rm_buffer);
                                ...
                                if (zctype[ii] == RICE_1) {
                                    if (-coltype[ii] == TSHORT) {
                                        dlen = fits_rdecomp_short(
                                                (unsigned char*)compressed_vla,
                                                (int)cvlalen,
                                                (unsigned short*)uncompressed_vla,
                                                (int)vlalen, 32);
#if BYTESWAPPED
                                        ...
                                        dlen = fits_rdecomp(
                                                (unsigned char*)compressed_vla,
                                                (int)cvlalen,
                                                (unsigned int*)uncompressed_vla,
                                                (int)vlalen, 32);
#if BYTESWAPPED
                                        ffswap4((int*)uncompressed_vla, (long)vlalen);
#endif
                                    } else if (-coltype[ii] == TBYTE) {
                                        dlen = fits_rdecomp_byte(
                                                (unsigned char*)compressed_vla,
                                                (int)cvlalen,
                                                (unsigned char*)uncompressed_vla,
                                                (int)vlalen, 32);
                                    } else {
                                        /* this should not happen */
                                        ffpmsg(" Error: cannot uncompress this column "
                                               "type with the RICE algorithm");
                                ...
                                } else if (zctype[ii] == GZIP_1 ||
                                           zctype[ii] == GZIP_2) {
                                    /*: gzip uncompress the array of bytes */
                                    uncompress2mem_from_mem(
                                            compressed_vla, (size_t)cvlalen,
                                            &uncompressed_vla, &vlamemlen, realloc,
                                            &vlamemlen, status);
                                ...
                                bytepos =
                                        (size_t)((outfptr->Fptr)->datastart +
                                                 (outfptr->Fptr)->heapstart + vlastart);
                                ffmbyt(outfptr, bytepos, IGNORE_EOF, status);
                                ffpbyt(outfptr, vlamemlen, uncompressed_vla,
                                       status); /* write the bytes */
...
/*--------------------------------------------------------------------------*/
static int fits_short_to_int_inplace(short* shortarray, long length, int shift,
                                     int* status)
...
/*--------------------------------------------------------------------------*/
static int fits_sbyte_to_int_inplace(signed char* sbytearray, long length, int* status)
...
int fits_calc_tile_rows(long* tlpixel, long* tfpixel, int ndim, long* trowsize,
                        long* ntrows, int* status) {
