...
#define SHARED_PERSIST \
    (8) /* flag for shared_malloc, object is not deleted after last proc detaches */
...
typedef union {
    struct BLKHEADstruct {
        ...
        int handle; /* this is not necessary, used only for non-resizeable objects via
                       ptr */
    } s;
...
SHARED_P shared_realloc(int idx, long newsize); /* reallocate n-bytes of shared memory
                                                   (ON LOCKED SEGMENT ONLY) */
int shared_size(int idx); /* get size of attached shared memory segment (ON LOCKED
                             SEGMENT ONLY) */
int shared_attr(int idx); /* get attributes of attached shared memory segment (ON LOCKED
                             SEGMENT ONLY) */
int shared_set_attr(int idx, int newattr); /* set attributes of attached shared memory
