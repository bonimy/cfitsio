...
*/
{
    ...
    if ((fptr->Fptr)->datastart == DATA_UNDEFINED) {
        if (nmore) *nmore = -1; /* data not written yet, so room for any keywords */
    } else {
...
*/
{
    ...
    if (position)
        *position = (int)((((fptr->Fptr)->nextkey) -
                           ((fptr->Fptr)->headstart[(fptr->Fptr)->curhdu])) /
                                  80 +
                          1);
           ...
           int* status)    /* IO - error status                        */
{
    ...
    */
    ...
    (fptr->Fptr)->nextkey =
            (fptr->Fptr)->headstart[(fptr->Fptr)->curhdu] + ((nrec - 1) * 80);
...
*/
{
    ...
    */
    ...
    if (bytepos > endhead || bytepos < (fptr->Fptr)->headstart[(fptr->Fptr)->curhdu]) {
        nrec = (int)((bytepos - (fptr->Fptr)->headstart[(fptr->Fptr)->curhdu]) / 80 +
                     1);
        snprintf(message, FLEN_ERRMSG,
                 "Cannot get keyword number %d.  It does not exist.", nrec);
        ffpmsg(message);
...
*/
{
    ...
    if (nrec == 0) {
        ...
        if (card) card[0] = '\0'; /* and return null card */
    } else if (nrec > 0) {
...
*/
{
    ...
    *keyname = '\0';
    ...
    gotstar = 0;
    if (namelen < 9 && (strchr(keyname, '?') || (gotstar = strchr(keyname, '*')) ||
                        strchr(keyname, '#'))) {
        wild = 1;
                    ...
                    if (!(!wild && cardlen != namelen)) {
                        for (ii = 0; ii < cardlen; ii++) {
                            ...
                            if (cardname[ii] > 96) {
                                ...
                                cardname[ii] =
                                        toupper(cardname[ii]); /* make upper case */
                            }
                        }

                        if (wild) {
                            ffcmps(keyname, cardname, 1, &match, &exact);
                            if (match) return (*status); /* found a matching keyword */
...
*/
{
    ...
    if (comm[0] == '[') {
        ...
        if (loc) *loc = '\0';    /*  terminate the string */
...
*/
{
    ...
    *value = '\0';
    ...
    if (!valstring[0]) /* null value string? */
    {
        ...
        *tempstring = '\0';
        ...
        while (contin && *status <= 0) {
            if (len &&
                *(tempstring + len - 1) == '&') /*  is last char an anpersand?  */
            {
                ffgcnt(fptr, valstring, nextcomm, status);
                if (*valstring) /* a null valstring indicates no continuation */
                {
                    *(tempstring + len - 1) = '\0'; /* erase the trailing & char */
                    len += strlen(valstring) - 1;
                    tempstring =
                            (char*)realloc(tempstring, len + 1); /* increase size */
    ...
    if (tempstring) {
        len = strlen(tempstring);
        if (firstchar <= len) strncat(value, tempstring + (firstchar - 1), maxchar);
        free(tempstring);
...
*/
{
    ...
    {
        snprintf(message, FLEN_ERRMSG,
                 "keyword %s does not have a complex value (ffgkyc):", keyname);
        ffpmsg(message);
        ffpmsg(valstring);
...
*/
{
    ...
    {
        snprintf(message, FLEN_ERRMSG,
                 "keyword %s does not have a complex value (ffgkym):", keyname);
        ffpmsg(message);
        ffpmsg(valstring);
...
*/
{
    ...
    {
        snprintf(sbuff, FLEN_CARD,
                 "Name of keyword no. %d contains illegal character(s): %s", nkey,
                 keyname);
        ffpmsg(sbuff);
    ...
    *nfound = 0;
    ...
    for (ii = 3; ii <= nkeys; ii++) {
        ...
        if (strncmp(keyroot, card, lenroot) == 0) /* see if keyword matches */
        {
            ...
            strncat(keyindex, &card[lenroot],
                    equalssign - card - lenroot); /*  copy suffix  */
            tstatus = 0;
...
*/
{
    ...
    *naxis = 0;
    ...
    while (loc) {
        ...
        if (*naxis < maxdim) naxes[*naxis] = dimsize;
...
*/
{
    ...
    {
        *naxis = 1;                                 /* default = 1 dimensional */
        if (maxdim > 0) naxes[0] = colptr->trepeat; /* default length = repeat */
    } else {
        *naxis = 0;
...
*/
{
    ...
    ffgphd(fptr, maxdim, simple, bitpix, naxis, tnaxes, pcount, gcount, extend, &ddummy,
           &ddummy, &lldummy, &idummy, status);

    if (naxis && naxes) {
        for (ii = 0; (ii < *naxis) && (ii < maxdim); ii++) naxes[ii] = (long)tnaxes[ii];
    } else if (naxes) {
        for (ii = 0; ii < maxdim; ii++) naxes[ii] = (long)tnaxes[ii];
    }
...
*/
{
    ...
    ffgphd(fptr, maxdim, simple, bitpix, naxis, naxes, pcount, gcount, extend, &ddummy,
           &ddummy, &lldummy, &idummy, status);
...
*/
{
    ...
    if (!strcmp(name, "XTENSION")) {
        ...
        if ((value[0] != '\'') || /* first char must be a quote */
            ...
            snprintf(message, FLEN_ERRMSG, "This is not a TABLE extension: %s", value);
            ffpmsg(message);
    ...
    if (naxis1) *naxis1 = (long)llnaxis1;

    if (naxis2) *naxis2 = (long)llnaxis2;

    if (pcount != 0) {
        snprintf(message, FLEN_ERRMSG,
                 "PCOUNT = %.0f is illegal in ASCII table; must = 0", (double)pcount);
        ffpmsg(message);
        return (*status = BAD_PCOUNT);
    }

    if (tfields) *tfields = fields;
    ...
    if (maxf > 0) {
        ...
        if (ttype) ffgkns(fptr, "TTYPE", 1, maxf, ttype, &nfound, status);

        if (tunit) ffgkns(fptr, "TUNIT", 1, maxf, tunit, &nfound, status);
        ...
        if (tbcol) {
            ...
            if (*status > 0 || nfound != maxf) {
                ffpmsg("Required TBCOL keyword(s) not found in ASCII table header "
                       "(ffghtb).");
        ...
        if (tform) {
            ...
            if (*status > 0 || nfound != maxf) {
                ffpmsg("Required TFORM keyword(s) not found in ASCII table header "
                       "(ffghtb).");
...
*/
{
    ...
    if (naxis1) *naxis1 = llnaxis1;

    if (naxis2) *naxis2 = llnaxis2;
...
*/
{
    ...
    if (!strcmp(name, "XTENSION")) {
        ...
        /* allow any number of trailing blanks before the closing quote */
        if ((value[0] != '\'') || /* first char must be a quote */
            (strcmp(xtension, "BINTABLE") && strcmp(xtension, "A3DTABLE") &&
             strcmp(xtension, "3DTABLE"))) {
            snprintf(message, FLEN_ERRMSG, "This is not a BINTABLE extension: %s",
                     value);
            ffpmsg(message);
    ...
    if (naxis2) *naxis2 = (long)naxis2ll;

    if (pcount) *pcount = (long)pcountll;
    ...
    if (maxf > 0) {
        ...
        if (tform) {
            ...
            if (*status > 0 || nfound != maxf) {
                ffpmsg("Required TFORM keyword(s) not found in binary table header "
                       "(ffghbn).");
                return (*status = NO_TFORM);
            }
        }
...
*/
{
    ...
    if (naxis2) *naxis2 = naxis2ll;

    if (pcount) *pcount = pcountll;
    ...
    */
    ...
    if ((fptr->Fptr)->curhdu == 0) /* Is this the beginning of the FITS file? */
    {
        if (!strcmp(name, "SIMPLE")) {
            if (value[0] == 'F') {
                if (simple) *simple = 0; /* not a simple FITS file */
            } else if (value[0] != 'T')
    ...
    {    /* or it could be a compressed image in a binary table */
        ...
        if (!strcmp(name, "XTENSION")) {
            ...
            /* allow any number of trailing blanks before the closing quote */
            if ((value[0] != '\'') || /* first char must be a quote */
                (strcmp(xtension, "IMAGE") && strcmp(xtension, "IUEIMAGE"))) {
                unknown = 1; /* unknown type of extension; press on anyway */
                snprintf(message, FLEN_ERRMSG, "This is not an IMAGE extension: %s",
                         value);
                ffpmsg(message);
            }
        }
    ...
    if (unknown && (fptr->Fptr)->compressimg) {
        ...
        if (ffc2ii(value, &longbitpix, status) > 0) {
            ...
            snprintf(message, FLEN_ERRMSG, "Illegal value for BITPIX keyword: %s",
                     value);
            ffpmsg(message);
            return (*status = BAD_BITPIX);
        }
        if (bitpix) *bitpix = longbitpix; /* do explicit type conversion */
    ...
    if (bscale) *bscale = 1.0;
    if (bzero) *bzero = 0.0;
    if (pcount) *pcount = 0;
    if (gcount) *gcount = 1;
    if (extend) *extend = 0;
    if (blank) *blank = NULL_UNDEFINED; /* no default null value for BITPIX=8,16,32 */
        ...
        } else /* got the next keyword without error */
        {
            ...
            if (!strcmp(name, "BSCALE") && bscale) {
                *nspace = 0;                       /* reset count of blank keywords */
                ...
                if (ffc2dd(value, bscale, status) > 0) /* convert to double */
                {
                    ...
                    *bscale = 1.0;
                    ...
                    snprintf(message, FLEN_ERRMSG,
                             "Error reading BSCALE keyword value as a double: %s",
                             value);
                    ffpmsg(message);
                }
            }
                ...
                *nspace = 0;                       /* reset count of blank keywords */
                ...
                if (ffc2dd(value, bzero, status) > 0) /* convert to double */
                {
                    ...
                    *bzero = 0.0;
                    ...
                    snprintf(message, FLEN_ERRMSG,
                             "Error reading BZERO keyword value as a double: %s",
                             value);
                    ffpmsg(message);
                }
            }
                ...
                *nspace = 0;                       /* reset count of blank keywords */
                ...
                if (ffc2jj(value, blank, status) > 0) /* convert to LONGLONG */
                {
                    ...
                    *blank = NULL_UNDEFINED;
                    ...
                    snprintf(message, FLEN_ERRMSG,
                             "Error reading BLANK keyword value as an integer: %s",
                             value);
                    ffpmsg(message);
                }
            }
                ...
                *nspace = 0;                       /* reset count of blank keywords */
                ...
                {
                    snprintf(message, FLEN_ERRMSG,
                             "Error reading PCOUNT keyword value as an integer: %s",
                             value);
                    ffpmsg(message);
                }
            }
                ...
                *nspace = 0;                       /* reset count of blank keywords */
                ...
                {
                    snprintf(message, FLEN_ERRMSG,
                             "Error reading GCOUNT keyword value as an integer: %s",
                             value);
                    ffpmsg(message);
                }
            }
                ...
                *nspace = 0;                       /* reset count of blank keywords */
                ...
                if (ffc2ll(value, extend, status) > 0) /* convert to logical */
                {
                    ...
                    *extend = 0;
                    ...
                    snprintf(message, FLEN_ERRMSG,
                             "Error reading EXTEND keyword value as a logical: %s",
                             value);
                    ffpmsg(message);
                }
            }
    ...
    if (ffgkyn(fptr, numkey, keyname, valuestring, comm, status) <= 0) {
        if (strcmp(keyname, name))
            *status = BAD_ORDER; /* incorrect keyword name */
            ...
            if (*status > 0 || *value < 0) *status = NOT_POS_INT;
        }
        ...
        if (*status > 0) {
            ...
            snprintf(message, FLEN_ERRMSG, " found keyword %s with value %s", keyname,
                     valuestring);
            ffpmsg(message);
        }
    }
           ...
           int* status)    /* IO - error status                  */
{
    ...
    */
    ...
    if (ffgkyn(fptr, numkey, keyname, valuestring, comm, status) <= 0) {
        if (strcmp(keyname, name)) *status = BAD_ORDER; /* incorrect keyword name */
    ...
    if (*status > 0) {
        ...
        snprintf(message, FLEN_ERRMSG, " Expected keyword %s with value %s, but", name,
                 value);
        ffpmsg(message);
...
*/
{
    ...
    if (ffghsp(fptr, &nkeys, NULL, status) > 0) return (*status);
...
*/
{
    ...
    *nkeys = 0;
    ...
    /* get number of keywords in the header (doesn't include END) */
    if (ffghsp(fptr, &totkeys, NULL, status) > 0) return (*status);
    ...
    *header = (char*)calloc((totkeys + 1) * 80 + 1, 1);
    ...
    for (ii = 1; ii <= totkeys; ii++) {
        ...
        if (exclude_comm) {
            if (!FSTRCMP("COMMENT ", keyname) || !FSTRCMP("HISTORY ", keyname) ||
                !FSTRCMP("        ", keyname))
