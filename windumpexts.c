 ...
 */
char* SzStorageClass1[] = {"NULL",      "AUTOMATIC",       "EXTERNAL",
                           "STATIC",    "REGISTER",        "EXTERNAL_DEF",
                           "LABEL",     "UNDEFINED_LABEL", "MEMBER_OF_STRUCT",
                           "ARGUMENT",  "STRUCT_TAG",      "MEMBER_OF_UNION",
                           "UNION_TAG", "TYPE_DEFINITION", "UNDEFINED_STATIC",
                           "ENUM_TAG",  "MEMBER_OF_ENUM",  "REGISTER_PARAM",
                           "BIT_FIELD"};

/*
 * The names of the second group of possible symbol table storage classes
 */
char* SzStorageClass2[] = {"BLOCK", "FUNCTION", "END_OF_STRUCT",
                           "FILE",  "SECTION",  "WEAK_EXTERNAL"};
...
void DumpSymbolTable(PIMAGE_SYMBOL pSymbolTable, FILE* fout, unsigned cSymbols) {
    ...
    for (i = 0; i < cSymbols; i++) {
        ...
        fprintf(fout, " sect:%s aux:%X type:%02X st:%s\n", sectionName,
                pSymbolTable->NumberOfAuxSymbols, pSymbolTable->Type,
...
void DumpObjFile(PIMAGE_FILE_HEADER pImageFileHeader, FILE* fout, int full) {
    ...
    PCOFFSymbolTable = (PIMAGE_SYMBOL)((DWORD)pImageFileHeader +
