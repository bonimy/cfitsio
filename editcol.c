...
*/
{
    ...
    for (ii = 0; (ii < naxis) && (ii < 99); ii++) tnaxes[ii] = naxes[ii];
...
*/
{
    ...
    if (ffghprll(fptr, 99, &simple, &obitpix, &onaxis, onaxes, &pcount, &gcount,
                 &extend, status) > 0)
        return (*status);
    ...
    if (longbitpix != BYTE_IMG && longbitpix != SHORT_IMG && longbitpix != LONG_IMG &&
        longbitpix != LONGLONG_IMG && longbitpix != FLOAT_IMG &&
        longbitpix != DOUBLE_IMG) {
        snprintf(message, FLEN_ERRMSG, "Illegal value for BITPIX keyword: %d", bitpix);
        ffpmsg(message);
        return (*status = BAD_BITPIX);
    }

    if (naxis < 0 || naxis > 999) {
        snprintf(message, FLEN_ERRMSG, "Illegal value for NAXIS keyword: %d", naxis);
        ffpmsg(message);
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (naxes[ii] < 0) {
            snprintf(message, FLEN_ERRMSG, "Illegal value for NAXIS%d keyword: %.0f",
                     ii + 1, (double)(naxes[ii]));
            ffpmsg(message);
    ...
    if (onaxis == 0)
        ...
        oldsize = 1;
        for (ii = 0; ii < onaxis; ii++) oldsize *= onaxes[ii];
    ...
    if (newsize > oldsize) /* have to insert new blocks for image */
    {
        ...
        if (ffiblk(fptr, nblocks, 1, status) > 0) return (*status);
    } else if (oldsize > newsize) /* have to delete blocks from image */
    {
        nblocks = (long)(oldsize - newsize);
        if (ffdblk(fptr, nblocks, status) > 0) return (*status);
    }
...
*/
{
    ...
    if (firstrow > naxis2) {
        ffpmsg("Delete position greater than the number of rows in the table (ffdrow)");
        return (*status = BAD_ROW_NUM);
    } else if (firstrow < 1) {
    ...
    if (nblock > 0) ffdblk(fptr, nblock, status);
...
*/
{
    ...
    cptr = ranges;
    for (nranges = 1; (cptr = strchr(cptr, ',')); nranges++) cptr++;
...
/*--------------------------------------------------------------------------*/
int ffrwrg(char* rowlist,    /* I - list of rows and row ranges */
    ...
    *numranges = 0;
    ...
    while (*next != '\0') {
        ...
        if (minval <= maxrows) { /* ignore range if greater than maxrows */
            if (maxval > maxrows) maxval = (long)maxrows;
...
/*--------------------------------------------------------------------------*/
int ffrwrgll(char* rowlist,    /* I - list of rows and row ranges */
             ...
             int* status)      /* IO - status value */
{
    ...
    *numranges = 0;
    ...
    while (*next != '\0') {
        ...
        if (minval <= maxrows) { /* ignore range if greater than maxrows */
            if (maxval > maxrows) maxval = maxrows;
...
*/
{
    ...
    if (colnum <= tfields) ffkshf(fptr, colnum, tfields, ncols, status);
...
*/
{
    ...
    if ((fptr->Fptr)->hdutype != BINARY_TBL) {
        ffpmsg("Can only change vector length of a column in BINTABLE extension "
               "(ffmvec)");
        return (*status = NOT_TABLE);
    }
    ...
    /*  is the column number valid?  */
    tfields = (fptr->Fptr)->tfield;
    if (colnum < 1 || colnum > tfields) return (*status = BAD_COL_NUM);
    ...
    if (datacode == TSTRING) width = 1; /* width was equal to width of unit string */
    ...
    if (delbyte > 0) /* insert space for more elements */
    {
        ...
        if ((fptr->Fptr)->heapsize > 0) {
            ...
            if (ffshft(fptr, firstbyte, nbytes, ndelete, status) > 0) return (*status);
        }
...
*/
{
    ...
    if (create_col) /* insert new column in output table? */
    {
        ...
        if ((infptr->Fptr == outfptr->Fptr) &&
            (infptr->HDUposition == outfptr->HDUposition) && (colnum <= incol)) {
        ...
        if (ffrdef(outfptr, status)) return (*status);
    } else {
        colnum = outcol;
     ...
     */
        ...
        ujjvalues = (unsigned long long int*)calloc(maxloop,
                                                    sizeof(unsigned long long int));
        if (!ujjvalues) {
            ffpmsg("malloc failed to get memory for unsigned long long int (ffcpcl)");
            return (*status = ARRAY_TOO_BIG);
        }
    }
    /* These are long-long ints that are not rescaled to floating point numbers */
        ...
        if (!jjvalues) {
            ffpmsg("malloc failed to get memory for long long int (ffcpcl)");
            return (*status = ARRAY_TOO_BIG);
        }
    ...
    while (ntodo) /* iterate through the table */
    {
        ...
        if (typecode == TLOGICAL)
            ffgcl(infptr, incol, firstrow, firstelem, ntodo, lvalues, status);
        else if (typecode == TSTRING)
            ffgcvs(infptr, incol, firstrow, firstelem, ntodo, nulstr, strarray,
                   &anynull, status);

        else if (typecode == TCOMPLEX)
            ffgcvc(infptr, incol, firstrow, firstelem, ntodo, fnull, fvalues, &anynull,
                   status);

        else if (typecode == TDBLCOMPLEX)
            ffgcvm(infptr, incol, firstrow, firstelem, ntodo, dnull, dvalues, &anynull,
                   status);
        ...
        /* Neither TULONGLONG nor TLONGLONG does null checking.  Whatever
           ...
           should preserve null values */
            ...
            ffgcvujj(infptr, incol, firstrow, firstelem, ntodo, /*nulval*/ 0, ujjvalues,
                     &anynull, status);

        else if (typecode == TLONGLONG && etypecode != TDOUBLE)
            ffgcvjj(infptr, incol, firstrow, firstelem, ntodo, /*nulval*/ 0, jjvalues,
                    &anynull, status);

        else /* all numerical types */
            ffgcvd(infptr, incol, firstrow, firstelem, ntodo, dnull, dvalues, &anynull,
                   status);
        ...
        if (typecode == TLOGICAL) {
            ...
            ffpcnl(outfptr, colnum, firstrow, firstelem, ntodo, lvalues, nullflag,
                   status);
            ...
            if (anynull)
                ffpcns(outfptr, colnum, firstrow, firstelem, ntodo, strarray, nulstr,
                       status);
            else
                ffpcls(outfptr, colnum, firstrow, firstelem, ntodo, strarray, status);
        }

        else if (typecode == TCOMPLEX) { /* doesn't support writing nulls */
            ffpclc(outfptr, colnum, firstrow, firstelem, ntodo, fvalues, status);
        }

        else if (typecode == TDBLCOMPLEX) { /* doesn't support writing nulls */
            ffpclm(outfptr, colnum, firstrow, firstelem, ntodo, dvalues, status);
        }

        else if (typecode == TLONGLONG &&
                 etypecode == TULONGLONG) { /* No null checking because we did none to
                                               read */
            ffpclujj(outfptr, colnum, firstrow, firstelem, ntodo, ujjvalues, status);
        } else if (typecode == TLONGLONG &&
                   etypecode !=
                           TDOUBLE) { /* No null checking because we did none to read */
            ffpcljj(outfptr, colnum, firstrow, firstelem, ntodo, jjvalues, status);
        } else /* all other numerical types */
        {
            if (anynull)
                ffpcnd(outfptr, colnum, firstrow, firstelem, ntodo, dvalues, dnull,
                       status);
            else
                ffpcld(outfptr, colnum, firstrow, firstelem, ntodo, dvalues, status);
        }
    ...
    if (typecode == TLOGICAL) {
        ...
        for (ii = 0; ii < maxloop; ii++) free(strarray[ii]);
...
*/
{
    ...
    for (incol1 = incol, outcol1 = colnum, icol = 0; icol < ncols;
         icol++, incol1++, outcol1++) {
        ...
        if (create_col) {
            ...
            if (orepeat != repeat) {
                ffpmsg("Input and output vector columns must have same length "
                       "(ffccls)");
                return (*status = BAD_TFORM);
            }
        }
...
*/
{
    ...
    if (((infptr->Fptr)->hdutype == BINARY_TBL &&
         (outfptr->Fptr)->hdutype == ASCII_TBL) ||
        ((infptr->Fptr)->hdutype == ASCII_TBL &&
         (outfptr->Fptr)->hdutype == BINARY_TBL)) {
        ffpmsg("Copying rows between Binary and ASCII tables is not supported "
               "(ffcprw)");
        return (*status = NOT_BTABLE);
    }
        ...
        for (ii = firstrow; ii < firstrow + nrows; ii++) {
            ...
            for (icol = 0; icol < (infptr->Fptr)->tfield; ++icol) {
                if (iVarCol < nInVarCols && inVarCols[iVarCol] == icol + 1) {
                    ...
                    if (!((outfptr->Fptr)->lasthdu)) {
                        if (outPos + nVarBytes >
                            ...
                            nNewBlocks = (long)(((outPos + nVarBytes - 1 -
                                                  (outfptr->Fptr)
                                                          ->headstart[(outfptr->Fptr)
                                                                              ->curhdu +
                                                                      1]) /
                                                 2880) +
                                                1);
                            if (ffiblk(outfptr, nNewBlocks, 1, status) > 0) {
                                ffpmsg("Failed to extend the size of the variable "
                                       "length heap (ffcprw)");
                                goto CLEANUP_RETURN;
                            }
                        }
                    ...
                    if (nVarBytes) {
                        if (nVarBytes > nVarAllocBytes) {
                            /* Grow the copy buffer to accomodate the new maximum size.
                               Note it is safe to call realloc() with null input
                               pointer, which is equivalent to malloc(). */
                            unsigned char* varColBuff1 =
                                    (unsigned char*)realloc(varColBuff, nVarBytes);
                            if (!varColBuff1) {
                                *status = MEMORY_ALLOCATION;
                                ffpmsg("failed to allocate memory for variable column "
                                       "copy (ffcprw)");
                                goto CLEANUP_RETURN;
                            }
                    ...
                    ffpdes(outfptr, icol + 1, jj, hrepeat, (outfptr->Fptr)->heapsize,
                           status);
...
{
    int ii, tstatus;
    LONGLONG firstbyte, size, ndelete, nbytes, naxis1, naxis2, firstcol, delbyte,
            freespace;
    LONGLONG tbcol;
    long nblock, nspace;
...
*/
{
    ...
    if (newlen <= 10000) {
        ...
        */
        if ((fptr->Fptr)->logfilesize <
            (fptr->Fptr)->datastart + (fptr->Fptr)->heapstart) {
            (fptr->Fptr)->logfilesize =
                    (((fptr->Fptr)->datastart + (fptr->Fptr)->heapstart + 2879) /
                     2880) *
                    2880;
        }
...
{
    /*
       Internal function to identify which columns in a binary table are variable
       length. The colnums array will be filled with nvarcols elements - the 1-based
       numbers of all variable length columns in the table.  This ASSUMES calling
       function has passed in a colnums array large enough to hold these.
    */
    int tfields = 0, icol;
    tcolumn* colptr = 0;
    ...
    *nvarcols = 0;
    ...
    if ((fptr->Fptr)->hdutype != BINARY_TBL) {
        ffpmsg("Var-length column search can only be performed on Binary tables "
               "(fffvcl)");
        return (*status = NOT_BTABLE);
    }
...
#define shftbuffsize 100000
    ...
    if (nshift > 0) /* start at the end of the block and work backwards */
