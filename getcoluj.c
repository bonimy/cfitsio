...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TULONG, firstelem, nelem, nullcheck,
                                    &nullvalue, array, NULL, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    ffgcluj(fptr, 2, row, firstelem, nelem, 1, 1, nulval, array, &cdummy, anynul,
            status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TULONG, firstelem, nelem, nullcheck, NULL,
    ...
    */
    ...
    ffgcluj(fptr, 2, row, firstelem, nelem, 1, 2, 0L, array, nularray, anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffg3duj(fptr, group, nulval, ncols, naxis2, naxis1, naxis2, 1, array, anynul,
            status);
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TULONG, fpixel, lpixel, inc, nullcheck,
                                 &nullvalue, array, NULL, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    if (ncols == naxis1 && nrows == naxis2) /* arrays have same size? */
    {
        ...
        ffgcluj(fptr, 2, tablerow, 1, naxis1 * naxis2 * naxis3, 1, 1, nulval, array,
                &cdummy, anynul, status);
        return (*status);
    }

    if (ncols < naxis1 || nrows < naxis2) return (*status = BAD_DIMEN);
    ...
    for (jj = 0; jj < naxis3; jj++) {
        ...
        for (ii = 0; ii < naxis2; ii++) {
            if (ffgcluj(fptr, 2, tablerow, nfits, naxis1, 1, 1, nulval, &array[narray],
                        &cdummy, anynul, status) > 0)
                return (*status);
...
*/
{
    ...
    if (naxis < 1 || naxis > 9) {
        snprintf(msg, FLEN_ERRMSG, "NAXIS = %d in call to ffgsvuj is out of range",
                 naxis);
        ffpmsg(msg);
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TULONG, blcll, trcll, inc, nullcheck, &nullvalue,
    ...
    */
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (trc[ii] < blc[ii]) {
            snprintf(msg, FLEN_ERRMSG, "ffgsvuj: illegal range specified for axis %ld",
                     ii + 1);
            ffpmsg(msg);
                            ...
                            for (i3 = str[3]; i3 <= stp[3]; i3 += incr[3]) {
                                for (i2 = str[2]; i2 <= stp[2]; i2 += incr[2]) {
                                    for (i1 = str[1]; i1 <= stp[1]; i1 += incr[1]) {
                                        ...
                                        if (ffgcluj(fptr, numcol, row, felem, nelem,
                                                    ninc, nultyp, nulval, &array[i0],
                                                    &ldummy, &anyf, status) > 0)
                                            return (*status);

                                        if (anyf && anynul) *anynul = TRUE;
...
*/
{
    ...
    if (naxis < 1 || naxis > 9) {
        snprintf(msg, FLEN_ERRMSG, "NAXIS = %d in call to ffgsvj is out of range",
                 naxis);
        ffpmsg(msg);
        return (*status = BAD_DIMEN);
    }
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TULONG, blcll, trcll, inc, nullcheck, NULL,
                                 array, flagval, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (trc[ii] < blc[ii]) {
            snprintf(msg, FLEN_ERRMSG, "ffgsvj: illegal range specified for axis %ld",
                     ii + 1);
            ffpmsg(msg);
            return (*status = BAD_PIX_NUM);
        }
...
*/
{
    ...
    */
    ...
    ffgcluj(fptr, 1, row, firstelem, nelem, 1, 1, 0L, array, &cdummy, &idummy, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgcluj(fptr, colnum, firstrow, firstelem, nelem, 1, 1, nulval, array, &cdummy,
            anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgcluj(fptr, colnum, firstrow, firstelem, nelem, 1, 2, dummy, array, nularray,
            anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (nultyp == 2) memset(nularray, 0, (size_t)nelem); /* initialize nullarray */

    /*---------------------------------------------------*/
    /*  Check input and get parameters about the column: */
    /*---------------------------------------------------*/
    if (ffgcprll(fptr, colnum, firstrow, firstelem, nelem, 0, &scale, &zero, tform,
                 &twidth, &tcode, &maxelem2, &startpos, &elemnum, &incre, &repeat,
                 &rowlen, &hdutype, &tnull, snull, status) > 0)
        return (*status);
    maxelem = maxelem2;
    ...
    if (tcode == TSTRING) /* setup for ASCII tables */
    {
        ...
        for (ii = 0; ii < decimals; ii++) power *= 10.;
    }
    /*------------------------------------------------------------------*/
    ...
    while (remain) {
        ...
        */
        ...
        readptr = startpos + ((LONGLONG)rownum * rowlen) +
                  (elemnum * (incre / elemincre));
        ...
        switch (tcode) {
            case (TLONG):
                if (LONGSIZE == 32) {
                    ffgi4b(fptr, readptr, ntodo, incre, (INT32BIT*)&array[next],
                           status);
                    fffi4u4((INT32BIT*)&array[next], ntodo, scale, zero, nulcheck,
                            (INT32BIT)tnull, nulval, &nularray[next], anynul,
                            &array[next], status);
                } else { /* case where sizeof(long) = 8 */
                    ffgi4b(fptr, readptr, ntodo, incre, (INT32BIT*)buffer, status);
                    fffi4u4((INT32BIT*)buffer, ntodo, scale, zero, nulcheck,
                            (INT32BIT)tnull, nulval, &nularray[next], anynul,
                            &array[next], status);
                }
            ...
            case (TLONGLONG):
                ...
                fffi8u4((LONGLONG*)buffer, ntodo, scale, zero, nulcheck, tnull, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TBYTE):
                ffgi1b(fptr, readptr, ntodo, incre, (unsigned char*)buffer, status);
            ...
            case (TSHORT):
                ...
                fffi2u4((short*)buffer, ntodo, scale, zero, nulcheck, (short)tnull,
                        nulval, &nularray[next], anynul, &array[next], status);
                break;
            case (TFLOAT):
                ffgr4b(fptr, readptr, ntodo, incre, (float*)buffer, status);
                fffr4u4((float*)buffer, ntodo, scale, zero, nulcheck, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TDOUBLE):
                ffgr8b(fptr, readptr, ntodo, incre, (double*)buffer, status);
                fffr8u4((double*)buffer, ntodo, scale, zero, nulcheck, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TSTRING):
                ...
                fffstru4((char*)buffer, ntodo, scale, zero, twidth, power, nulcheck,
                         snull, nulval, &nularray[next], anynul, &array[next], status);
                break;
        ...
        if (*status > 0) /* test for error during previous read operation */
        {
            ...
            if (hdutype > 0)
                snprintf(message, FLEN_ERRMSG,
                         "Error reading elements %.0f thru %.0f from column %d "
                         "(ffgcluj).",
...
*/
{
    ...
    for (ii = 0; ii < ntodo; ii++) {
        ...
        *tpos = 0;
        ...
        if (snull[0] != ASCII_NULL_UNDEFINED && !strncmp(snull, cptr, nullen)) {
            if (nullcheck) {
                *anynull = 1;
                if (nullcheck == 1)
                    output[ii] = nullval;
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TULONGLONG, firstelem, nelem, nullcheck,
                                    &nullvalue, array, NULL, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    ffgclujj(fptr, 2, row, firstelem, nelem, 1, 1, nulval, array, &cdummy, anynul,
             status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TULONGLONG, firstelem, nelem, nullcheck, NULL,
                                    array, nularray, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    ffgclujj(fptr, 2, row, firstelem, nelem, 1, 2, dummy, array, nularray, anynul,
             status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffg3dujj(fptr, group, nulval, ncols, naxis2, naxis1, naxis2, 1, array, anynul,
             status);
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TULONGLONG, fpixel, lpixel, inc, nullcheck,
                                 &nullvalue, array, NULL, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    {
        /* all the image pixels are contiguous, so read all at once */
        ffgclujj(fptr, 2, tablerow, 1, naxis1 * naxis2 * naxis3, 1, 1, nulval, array,
                 &cdummy, anynul, status);
        return (*status);
    }
    ...
    for (jj = 0; jj < naxis3; jj++) {
        ...
        for (ii = 0; ii < naxis2; ii++) {
            if (ffgclujj(fptr, 2, tablerow, nfits, naxis1, 1, 1, nulval, &array[narray],
                         &cdummy, anynul, status) > 0)
                return (*status);
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TULONGLONG, blcll, trcll, inc, nullcheck,
                                 &nullvalue, array, NULL, anynul, status);
        return (*status);
    }
        ...
        if (trc[ii] < blc[ii]) {
            if (hdutype == IMAGE_HDU) {
                ...
                snprintf(msg, FLEN_ERRMSG,
                         "ffgsvj: illegal range specified for axis %ld", ii + 1);
                ffpmsg(msg);
                return (*status = BAD_PIX_NUM);
            }
        }
            ...
            for (i7 = str[7] * dir[7]; i7 <= stp[7] * dir[7]; i7 += incr[7]) {
                for (i6 = str[6] * dir[6]; i6 <= stp[6] * dir[6]; i6 += incr[6]) {
                    for (i5 = str[5] * dir[5]; i5 <= stp[5] * dir[5]; i5 += incr[5]) {
                        for (i4 = str[4] * dir[4]; i4 <= stp[4] * dir[4];
                             i4 += incr[4]) {
                            for (i3 = str[3] * dir[3]; i3 <= stp[3] * dir[3];
                                 i3 += incr[3]) {
                                for (i2 = str[2] * dir[2]; i2 <= stp[2] * dir[2];
                                     i2 += incr[2]) {
                                    for (i1 = str[1] * dir[1]; i1 <= stp[1] * dir[1];
                                         i1 += incr[1]) {
                                        felem = str[0] + (i1 - dir[1]) * dsize[1] +
                                                (i2 - dir[2]) * dsize[2] +
                                                (i3 - dir[3]) * dsize[3] +
                                                (i4 - dir[4]) * dsize[4] +
                                                (i5 - dir[5]) * dsize[5] +
                                        ...
                                        if (ffgclujj(fptr, numcol, row, felem, nelem,
                                                     ninc, nultyp, nulval, &array[i0],
                                                     &ldummy, &anyf, status) > 0)
                                            return (*status);
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TULONGLONG, blcll, trcll, inc, nullcheck, NULL,
                                 array, flagval, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (trc[ii] < blc[ii]) {
            snprintf(msg, FLEN_ERRMSG, "ffgsvujj: illegal range specified for axis %ld",
                     ii + 1);
            ffpmsg(msg);
            return (*status = BAD_PIX_NUM);
        }
...
*/
{
    ...
    */
    ...
    ffgclujj(fptr, 1, row, firstelem, nelem, 1, 1, dummy, array, &cdummy, &idummy,
             status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgclujj(fptr, colnum, firstrow, firstelem, nelem, 1, 1, nulval, array, &cdummy,
             anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgclujj(fptr, colnum, firstrow, firstelem, nelem, 1, 2, dummy, array, nularray,
             anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    /*  Check input and get parameters about the column: */
    /*---------------------------------------------------*/
    if (elemincre < 0) readcheck = -1; /* don't do range checking in this case */
    ...
    while (remain) {
        ...
        */
        ...
        switch (tcode) {
            case (TLONGLONG):
                ffgi8b(fptr, readptr, ntodo, incre, (long*)&array[next], status);
                fffi8u8((LONGLONG*)&array[next], ntodo, scale, zero, nulcheck, tnull,
                        nulval, &nularray[next], anynul, &array[next], status);
                break;
            case (TLONG):
                ffgi4b(fptr, readptr, ntodo, incre, (INT32BIT*)buffer, status);
                fffi4u8((INT32BIT*)buffer, ntodo, scale, zero, nulcheck,
                        (INT32BIT)tnull, nulval, &nularray[next], anynul, &array[next],
                        status);
                break;
            case (TBYTE):
                ffgi1b(fptr, readptr, ntodo, incre, (unsigned char*)buffer, status);
            ...
            case (TSHORT):
                ffgi2b(fptr, readptr, ntodo, incre, (short*)buffer, status);
                fffi2u8((short*)buffer, ntodo, scale, zero, nulcheck, (short)tnull,
                        nulval, &nularray[next], anynul, &array[next], status);
                break;
            case (TFLOAT):
                ffgr4b(fptr, readptr, ntodo, incre, (float*)buffer, status);
                fffr4u8((float*)buffer, ntodo, scale, zero, nulcheck, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TDOUBLE):
                ffgr8b(fptr, readptr, ntodo, incre, (double*)buffer, status);
                fffr8u8((double*)buffer, ntodo, scale, zero, nulcheck, nulval,
                        &nularray[next], anynul, &array[next], status);
                break;
            case (TSTRING):
                ffmbyt(fptr, readptr, REPORT_EOF, status);
                ...
                fffstru8((char*)buffer, ntodo, scale, zero, twidth, power, nulcheck,
                         snull, nulval, &nularray[next], anynul, &array[next], status);
                break;
            ...
            for (ii = 0; ii < ntodo; ii++) {
                if (input[ii] == tnull) {
                    *anynull = 1;
                    ...
                    output[ii] =
                            (ULONGLONG)(((LONGLONG)input[ii]) ^ 0x8000000000000000);
                }
            }
        } else if (scale == 1. && zero == 0.) /* no scaling */
        {
