...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TUINT, firstelem, nelem, nullcheck,
    ...
    */
    ...
    ffgcluk(fptr, 2, row, firstelem, nelem, 1, 1, nulval, array, &cdummy, anynul,
            status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_pixels(fptr, TUINT, firstelem, nelem, nullcheck, NULL,
    ...
    */
    ...
    ffgcluk(fptr, 2, row, firstelem, nelem, 1, 2, 0L, array, nularray, anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffg3duk(fptr, group, nulval, ncols, naxis2, naxis1, naxis2, 1, array, anynul,
            status);
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TUINT, fpixel, lpixel, inc, nullcheck,
                                 &nullvalue, array, NULL, anynul, status);
        return (*status);
    }
    ...
    */
    ...
    if (ncols == naxis1 && nrows == naxis2) /* arrays have same size? */
    {
        ...
        ffgcluk(fptr, 2, tablerow, 1, naxis1 * naxis2 * naxis3, 1, 1, nulval, array,
    ...
    for (jj = 0; jj < naxis3; jj++) {
        ...
        for (ii = 0; ii < naxis2; ii++) {
            if (ffgcluk(fptr, 2, tablerow, nfits, naxis1, 1, 1, nulval, &array[narray],
...
*/
{
    ...
    if (naxis < 1 || naxis > 9) {
        snprintf(msg, FLEN_ERRMSG, "NAXIS = %d in call to ffgsvuk is out of range",
                 naxis);
        ffpmsg(msg);
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TUINT, blcll, trcll, inc, nullcheck, &nullvalue,
    ...
    */
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (trc[ii] < blc[ii]) {
            snprintf(msg, FLEN_ERRMSG, "ffgsvuk: illegal range specified for axis %ld",
                     ii + 1);
            ffpmsg(msg);
                            ...
                            for (i3 = str[3]; i3 <= stp[3]; i3 += incr[3]) {
                                for (i2 = str[2]; i2 <= stp[2]; i2 += incr[2]) {
                                    for (i1 = str[1]; i1 <= stp[1]; i1 += incr[1]) {
                                        ...
                                        if (ffgcluk(fptr, numcol, row, felem, nelem,
...
*/
{
    ...
    if (fits_is_compressed_image(fptr, status)) {
        ...
        fits_read_compressed_img(fptr, TUINT, blcll, trcll, inc, nullcheck, NULL, array,
...
*/
{
    ...
    */
    ...
    ffgcluk(fptr, 1, row, firstelem, nelem, 1, 1, 0L, array, &cdummy, &idummy, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgcluk(fptr, colnum, firstrow, firstelem, nelem, 1, 1, nulval, array, &cdummy,
            anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    ffgcluk(fptr, colnum, firstrow, firstelem, nelem, 1, 2, dummy, array, nularray,
            anynul, status);
    return (*status);
}
/*--------------------------------------------------------------------------*/
...
*/
{
    ...
    if (sizeof(int) == sizeof(short))
        ...
        ffgcluj(fptr, colnum, firstrow, firstelem, nelem, elemincre, nultyp,
                (unsigned long)nulval, (unsigned long*)array, nularray, anynul, status);
    else {
        /*
        ...
        */
            ...
            */
            ...
            switch (tcode) {
                case (TLONG):
                    ffgi4b(fptr, readptr, ntodo, incre, (INT32BIT*)&array[next],
                           status);
                    fffi4uint((INT32BIT*)&array[next], ntodo, scale, zero, nulcheck,
                ...
                case (TLONGLONG):
                    ...
                    fffi8uint((LONGLONG*)buffer, ntodo, scale, zero, nulcheck, tnull,
                ...
                case (TFLOAT):
                    ...
                    fffr4uint((float*)buffer, ntodo, scale, zero, nulcheck, nulval,
                              &nularray[next], anynul, &array[next], status);
                    break;
                case (TDOUBLE):
                    ffgr8b(fptr, readptr, ntodo, incre, (double*)buffer, status);
                    fffr8uint((double*)buffer, ntodo, scale, zero, nulcheck, nulval,
                              &nularray[next], anynul, &array[next], status);
                    break;
                case (TSTRING):
            ...
            if (*status > 0) /* test for error during previous read operation */
            {
                ...
                if (hdutype > 0)
                    ...
                    snprintf(message, FLEN_ERRMSG,
                             "Error reading elements %.0f thru %.0f from image "
                             "(ffgcluk).",
                             dtemp + 1., dtemp + ntodo);
        ...
        if (*status == OVERFLOW_ERR) {
            ffpmsg("Numerical overflow during type conversion while reading FITS "
                   "data.");
            *status = NUM_OVERFLOW;
        }
