...
*/
{
    ...
    if (infptr == outfptr) return (*status = SAME_FILE);
...
*/
{
    ...
    if (!tmpbuff) return (*status = MEMORY_ALLOCATION);
...
*/
{
    ...
    for (ii = 0; (ii < naxis); ii++) tnaxes[ii] = naxes[ii];
...
*/
{
    ...
    if (bitpix == 8)
        ...
        snprintf(errmsg, FLEN_ERRMSG, "Illegal value for BITPIX keyword: %d", bitpix);
        ffpmsg(errmsg);
        return (*status = BAD_BITPIX); /* illegal bitpix value */
    }
    if (naxis < 0 || naxis > 999) {
        snprintf(errmsg, FLEN_ERRMSG, "Illegal value for NAXIS keyword: %d", naxis);
        ffpmsg(errmsg);
    ...
    for (ii = 0; ii < naxis; ii++) {
        if (naxes[ii] < 0) {
            snprintf(errmsg, FLEN_ERRMSG, "Illegal value for NAXIS%d keyword: %ld",
                     ii + 1, (long)naxes[ii]);
            ffpmsg(errmsg);
    ...
    for (ii = 1; ii < naxis; ii++) npixels = npixels * naxes[ii];
    ...
    for (ii = (fptr->Fptr)->maxhdu; ii > (fptr->Fptr)->curhdu; ii--)
        (fptr->Fptr)->headstart[ii + 1] =
                (fptr->Fptr)->headstart[ii]; /* incre start addr */

    if (nexthdu == 0)
        (fptr->Fptr)->headstart[1] =
                nblocks * 2880; /* start of the old Primary array */
...
*/
{
    ...
    if (((fptr->Fptr)->headend == (fptr->Fptr)->headstart[(fptr->Fptr)->curhdu])
        ...
        || ((((fptr->Fptr)->curhdu) == maxhdu) &&
            ((fptr->Fptr)->headstart[maxhdu + 1] >= (fptr->Fptr)->logfilesize))) {
        /* then simply append new image extension */
        ffcrtb(fptr, ASCII_TBL, naxis2, tfields, ttype, tform, tunit, extnm, status);
        return (*status);
    }
    ...
    if (naxis1 < 0)
        ...
        snprintf(errmsg, FLEN_ERRMSG, "Illegal value for TFIELDS keyword: %d", tfields);
        ffpmsg(errmsg);
    ...
    for (ii = 0; ii < tfields; ii++) {
        if (tunit && *tunit && *tunit[ii]) nunit++;
    }

    if (*extnm) nunit++; /* add one for the EXTNAME keyword */
    ...
    ffphtb(fptr, rowlen, naxis2, tfields, ttype, tbcol, tform, tunit, extnm, status);
...
*/
{
    ...
    if (((fptr->Fptr)->headend == (fptr->Fptr)->headstart[(fptr->Fptr)->curhdu])
        ...
        || ((((fptr->Fptr)->curhdu) == maxhdu) &&
            ((fptr->Fptr)->headstart[maxhdu + 1] >= (fptr->Fptr)->logfilesize))) {
        /* then simply append new image extension */
        ffcrtb(fptr, BINARY_TBL, naxis2, tfields, ttype, tform, tunit, extnm, status);
        return (*status);
    }
    ...
    ffphbn(fptr, naxis2, tfields, ttype, tform, tunit, extnm, pcount, status);
...
*/
{
    ...
    if ((fptr->Fptr)->curhdu == 0) /* replace primary array with null image */
    {
        ...
        nblocks = (long)(((fptr->Fptr)->headstart[(fptr->Fptr)->curhdu + 1] - 2880) /
                         2880);
